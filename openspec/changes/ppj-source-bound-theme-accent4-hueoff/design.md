...
token. This change applies the same ownership boundary to accent4 hue offset
...
- Accent4 transforms other than `hueOff`, transforms on accent4 through
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:hueOff`. The source writer patches `Accent4Color` only when the authored
   transform is the one existing accent4 hueOff and rejects all other transform
   flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent4HueOff` is accepted only with
   `accentTransforms.accent4.hueOff` in the source-issued capability. This
