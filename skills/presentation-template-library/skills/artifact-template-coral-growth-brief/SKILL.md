...
Use this style when the brief matches **commercial growth review; consulting presentation**. It is designed for
executives, strategy teams, decision makers. Its visual premise is One coral signal cuts through disciplined black-and-white analysis. The style is a
...
**question → hypothesis → framework → evidence → implication → recommendation → action**. This style is not a good fit for **photo essay**;
...
- Use **#FFFFFF** as the main field, **#181716** for primary type, **#E2534B** for the decisive signal, **#72787F** for secondary evidence, and **#ECEDEF** for rules or context.
...
- The recurring structure is **rail, variable page silhouettes, native data graphics**. Vary the silhouette between
  cover, evidence, data, process, comparison, and decision pages rather than
  repeating a component grid.
- Use a long rail or baseline to carry a sequence of evidence. Anchor annotations to the rail and reserve the focal color for the point that changes the decision.
...
- **Page skeleton.** Set a black rule and a white field, then let one coral rail or mark guide the reader from growth claim to proof. Use a conclusion column only when it adds a real implication rather than a second title.
- **Type rhythm.** Use a strong but restrained display face for assertion titles and a neutral sans for data. Coral is a signal, not a text color for long passages.
- **Evidence carriers.** Use growth bridges, funnels, cohort bars, contribution waterfalls, and ranked comparisons. Attach the growth rate, denominator, and period to the mark that proves it.
- **Images and layers.** Keep imagery rare and contextual. A product or customer photograph belongs to an opener or case, not to a page where it competes with a quantitative argument.
- **Pacing.** Run assertion → driver → proof → action. Repeat the coral rail only at chapter boundaries or on the single current focus; do not turn it into a stripe on every box.
- **Selection rule.** Choose this style when a growth story must cut through noise and end in a specific commercial action.

## Signature moves and variation limits

- Let **rail** recur as a quiet identity cue on selected pages; it must
