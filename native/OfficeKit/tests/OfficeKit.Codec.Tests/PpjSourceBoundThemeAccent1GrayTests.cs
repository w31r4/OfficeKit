...
{
    [Fact]
    public void PpjSourceBoundThemeAccent1GrayRemovesOnlyGrayAndReprojects()
    {
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            {
                SourceUri = "deck.assets/source/theme-accent1-gray.pptx",
            },
        });
        ...
        Assert.True(projectedTransform["gray"]!.GetValue<bool>());
        var nativeRef = Assert.IsType<JsonObject>(projectedTheme["nativeRef"]);
        Assert.Contains(nativeRef["capabilities"]!.AsArray(), capability =>
            capability!["operation"]!.GetValue<string>() == "setThemeAccent1Gray" &&
            capability["fields"]!.AsArray().Any(field => field!.GetValue<string>() == "accentTransforms.accent1.gray"));
        ...
        editedProgram["design"]!["theme"]!["accentTransforms"]!["accent1"]!["gray"] = false;
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            Assert.Empty(accent1.Elements<A.Gray>());
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/theme-accent1-gray.pptx",
            },
        });
        Assert.True(reprojected.Ok, Diagnostics(reprojected));
        var reprojectedProgram = JsonNode.Parse(reprojected.PresentationProgram.ProgramJson.ToByteArray())!.AsObject();
        Assert.False(reprojectedProgram["design"]!["theme"]!["accentTransforms"]?.AsObject().ContainsKey("accent1") ?? false);

        var unauthorizedProgram = JsonNode.Parse(projectedProgram.ToJsonString())!.AsObject();
        unauthorizedProgram["design"]!["theme"]!["accentTransforms"]!["accent1"]!["gray"] = false;
        unauthorizedProgram["design"]!["theme"]!["nativeRef"]!["capabilities"]!.AsArray()
            .Single(capability => capability!["operation"]!.GetValue<string>() == "setThemeAccent1Gray")!["fields"] = new JsonArray("accentTransforms.accent1.shade");
        ...
        deletionProgram["design"]!["theme"]!["accentTransforms"]!["accent1"]!.AsObject().Remove("gray");
        ...
        var invalidProgram = JsonNode.Parse(editedProgram.ToJsonString())!.AsObject();
        invalidProgram["design"]!["theme"]!["accentTransforms"]!["accent1"]!["gray"] = "yes";
