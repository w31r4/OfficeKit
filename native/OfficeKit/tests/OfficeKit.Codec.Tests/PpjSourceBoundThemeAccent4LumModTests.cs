...
{
    [Fact]
    public void PpjSourceBoundThemeAccent4LumModEditsOnlyLumModAndReprojects()
    {
        ...
        {
            Role = "accent4",
            LuminanceModulationThousandth = 25_000,
        });
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            {
                SourceUri = "deck.assets/source/theme-accent4-lummod.pptx",
            },
        });
        ...
        Assert.Equal(0.25, projectedTransform["lumMod"]!.GetValue<double>());
        var nativeRef = Assert.IsType<JsonObject>(projectedTheme["nativeRef"]);
        Assert.Contains(nativeRef["capabilities"]!.AsArray(), capability =>
            capability!["operation"]!.GetValue<string>() == "setThemeAccent4LumMod" &&
            capability["fields"]!.AsArray().Any(field => field!.GetValue<string>() == "accentTransforms.accent4.lumMod"));
        ...
        editedProgram["design"]!["theme"]!["accentTransforms"]!["accent4"]!["lumMod"] = 0.5;
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            Assert.Equal(50_000, Assert.Single(accent4.Elements<A.LuminanceModulation>()).Val!.Value);
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/theme-accent4-lummod.pptx",
            },
        });
        Assert.True(reprojected.Ok, Diagnostics(reprojected));
        var reprojectedProgram = JsonNode.Parse(reprojected.PresentationProgram.ProgramJson.ToByteArray())!.AsObject();
        Assert.Equal(0.5, reprojectedProgram["design"]!["theme"]!["accentTransforms"]!["accent4"]!["lumMod"]!.GetValue<double>());

        var unauthorizedProgram = JsonNode.Parse(projectedProgram.ToJsonString())!.AsObject();
        unauthorizedProgram["design"]!["theme"]!["accentTransforms"]!["accent4"]!["lumMod"] = 0.6;
        unauthorizedProgram["design"]!["theme"]!["nativeRef"]!["capabilities"]!.AsArray()
            .Single(capability => capability!["operation"]!.GetValue<string>() == "setThemeAccent4LumMod")!["fields"] = new JsonArray("accentTransforms.accent4.shade");
        ...
        deletionProgram["design"]!["theme"]!["accentTransforms"]!["accent4"]!.AsObject().Remove("lumMod");
        ...
        outOfRangeProgram["design"]!["theme"]!["accentTransforms"]!["accent4"]!["lumMod"] = 1.01;
