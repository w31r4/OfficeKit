...
{
    [Fact]
    public void PpjSourceBoundShape3dSceneBackdropAnchorZLeafEditsAndReprojects()
    {
        var authored = Invoke(ExportRequest());
        Assert.True(authored.Ok, Diagnostics(authored));
        var source = AddShape3dSceneBackdropAnchorZ(RemoveEmbeddedPpj(authored.File.ToByteArray()), "300");
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            {
                SourceUri = "deck.assets/source/shape-3d-scene-backdrop-anchor-z.pptx",
            },
        });
        ...
        Assert.Contains("<a:anchor x=\"100\" y=\"200\" z=\"600\"", editedXml, StringComparison.Ordinal);
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/shape-3d-scene-backdrop-anchor-z.pptx",
            },
        });
        ...
        var reprojectedTarget = Assert.Single(reprojectedShape["nativeRef"]!["leaves"]!.AsArray()
            .Select(leaf => leaf!.AsObject())
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "shape3dSceneBackdropAnchorZEmu"));
        Assert.Equal(600, reprojectedTarget["value"]!.GetValue<long>());
    }

    private static byte[] AddShape3dSceneBackdropAnchorZ(byte[] source, string anchorZ)
    {
