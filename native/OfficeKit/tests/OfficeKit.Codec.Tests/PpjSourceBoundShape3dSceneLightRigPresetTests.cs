...
{
    [Fact]
    public void PpjSourceBoundShape3dSceneLightRigPresetLeafEditsAndReprojects()
    {
        var authored = Invoke(ExportRequest());
        Assert.True(authored.Ok, Diagnostics(authored));
        var source = AddShape3dSceneLightRigPreset(RemoveEmbeddedPpj(authored.File.ToByteArray()), "threePt");
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            {
                SourceUri = "deck.assets/source/shape-3d-scene-light-rig.pptx",
            },
        });
        ...
        var target = Assert.Single(projectedShape["nativeRef"]!["leaves"]!.AsArray()
            ...
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "shape3dSceneLightRigPreset"));
        Assert.Equal("threePt", target["value"]!.GetValue<string>());
        target["value"] = "soft";
        ...
        Assert.Contains("<a:lightRig rig=\"soft\" dir=\"t\"", editedXml, StringComparison.Ordinal);
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/shape-3d-scene-light-rig.pptx",
            },
        });
        ...
        var reprojectedTarget = Assert.Single(reprojectedShape["nativeRef"]!["leaves"]!.AsArray()
            .Select(leaf => leaf!.AsObject())
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "shape3dSceneLightRigPreset"));
        Assert.Equal("soft", reprojectedTarget["value"]!.GetValue<string>());
    }

    private static byte[] AddShape3dSceneLightRigPreset(byte[] source, string preset)
    {
        ...
        using (var package = PresentationDocument.Open(stream, true))
        {
            ...
            lightRig.SetAttribute(new OpenXmlAttribute("rig", string.Empty, preset));
