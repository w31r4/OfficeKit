...
const postfixPath = path.join(repoRoot, "evals", "presentation-authoring-compiler", "postfix-c.v8.json");
const postfix = JSON.parse(await fs.readFile(postfixPath, "utf8"));
assert.equal(postfix.rollout.status, "switch-C");
assert.equal(postfix.rollout.shippedDefault, "C");
assert.equal(postfix.rollout.experimental, null);
assert.equal(postfix.rollout.explicitFallback, "A");
assert.match(postfix.rollout.reason, /historical frozen pilot remains preserved/i);
assert.equal(postfix.primaryMatrix.passed, postfix.primaryMatrix.uniqueRuns);
assert.equal(postfix.unseenHoldout.runs.C.passed, postfix.unseenHoldout.runs.C.completed);
