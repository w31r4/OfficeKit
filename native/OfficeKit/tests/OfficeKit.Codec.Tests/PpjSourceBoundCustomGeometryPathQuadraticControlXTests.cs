...
{
    [Fact]
    public void PpjSourceBoundCustomGeometryPathQuadraticControlXEditsAndReprojects()
    {
        ...
        path.Commands.Add(new PresentationCustomGeometryCommand
        {
            ...
            {
                Control = new PresentationCustomGeometryPoint { X = 300, Y = 200 },
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            {
                SourceUri = "deck.assets/source/custom-geometry-path-quadratic-control-x.pptx",
            },
        });
        ...
        var quadraticLeaves = projectedShape["nativeRef"]!["leaves"]!.AsArray()
            .Select(leaf => leaf!.AsObject())
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "customGeometryPathQuadraticControlX")
            .ToArray();
        var target = Assert.Single(quadraticLeaves);
        Assert.Equal(300, target["value"]!.GetValue<long>());
        target["value"] = 350;
        ...
        Assert.Contains("x=\"350\" y=\"200\"", editedXml, StringComparison.Ordinal);
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/custom-geometry-path-quadratic-control-x.pptx",
            },
        });
        ...
        var reprojectedLeaves = reprojectedShape["nativeRef"]!["leaves"]!.AsArray()
            ...
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "customGeometryPathQuadraticControlX")
            .ToArray();
        Assert.Equal(350, Assert.Single(reprojectedLeaves)["value"]!.GetValue<long>());
    }
}
