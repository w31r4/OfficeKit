...
Use this style when the brief matches **capital markets report; finance presentation**. It is designed for
investors, finance leaders, analysts. Its visual premise is A black-and-white ledger reserves gilt for the current value or threshold. The style is a
...
**stance → drivers → evidence → scenarios → risks → decision or allocation**. This style is not a good fit for **children training**;
...
- Use **#FBFBF8** as the main field, **#191A1B** for primary type, **#C79522** for the decisive signal, **#5E6266** for secondary evidence, and **#E7E5DE** for rules or context.
...
- The recurring structure is **rule, variable page silhouettes, native data graphics**. Vary the silhouette between
  cover, evidence, data, process, comparison, and decision pages rather than
  repeating a component grid.
- Use hairlines, baselines, and one decisive rule to make evidence feel accountable. Vary page silhouette while keeping alignment and direct labels stable.
...
- **Page skeleton.** Use white evidence pages with a thin black rule, a conclusion-led title, and a measured basis line. Reserve dark pages for a stance or close; the gold endpoint or threshold is the last visual stop.
- **Type rhythm.** Use a modern sans for data and source notes; a restrained serif may carry a cover thesis when gravitas helps. Keep numbers close to units and as-of dates.
- **Evidence carriers.** Use bar/line combinations, full-width time series, horizontal rankings, composition bars, sensitivity tables, and direct-labelled scenario bands. No chart should need an unexplained legend.
- **Images and layers.** Photography is rare and institutional. If a full-page image is justified, use a tested scrim and leave every axis, label, and decision mark on the foreground plane.
- **Pacing.** Move from thesis to drivers to evidence to scenario and risk. Alternate dense audit pages with one quiet judgment page; use one gold focus and mostly grayscale data per page.
- **Selection rule.** Choose this style for capital-markets research, allocation memos, and evidence-heavy financial outlooks.

## Signature moves and variation limits

- Let **rule** recur as a quiet identity cue on selected pages; it must
