...
- Owner: `p:pic/p:spPr/a:sp3d/a:extrusionClr/a:srgbClr/@val`.
- Native leaf: existing `shape3dExtrusionRgb`.
- Source model: use the additive `PresentationImage.shape_3d_extrusion_rgb`
...
- The picture owner must have exactly one direct `a:sp3d` with exactly one
  direct `a:extrusionClr` child and exactly one direct `a:srgbClr` child. The
