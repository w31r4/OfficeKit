...
internal static class OpenXmlChartSpaceCodec
{
        ...
        bool allowScatterStyleVariants = false)
    {
        ...
        editable &= PlotProfileEditable(plot, chart.Type, allowScatterStyleVariants);
    ...
    private static bool TryLegendPosition(XElement legend, out string position)
    {
        ...
        if (legend.Attributes().Any(attribute => !attribute.IsNamespaceDeclaration) ||
            legend.Elements().Any(element => element.Name != ChartNs + "legendPos" && element.Name != ChartNs + "layout" && element.Name != ChartNs + "overlay" && element.Name != ChartNs + "txPr" && element.Name != ChartNs + "spPr") ||
    ...
    private static void PatchPlotOptions(XElement plot, SpreadsheetChartArtifact chart)
    {
        ...
        if (chart.HasGapWidth || chart.HasOverlap || chart.Grouping.Length > 0 || chart.BarDirection.Length > 0 || chart.HasFirstSliceAngle || chart.HasDoughnutHoleSize || chart.HasBubbleScale || chart.BubbleSizeMode.Length > 0)
            throw new CodecException("invalid_chart_style", "The selected chart type cannot carry grouping, gap width, overlap, bar direction, or circular-plot geometry.");
    }
    ...
    private static bool TryOptionalInt(XElement owner, string name, int minimum, int maximum, out bool present, out int value)
    {
        present = false;
        value = 0;
        var matches = owner.Elements(ChartNs + name).Take(2).ToArray();
        ...
        if (text is null ||
            element.Attributes().Any(attribute => !attribute.IsNamespaceDeclaration && attribute.Name != "val") ||
            element.Nodes().Any(node => node is XText nativeText ? !string.IsNullOrWhiteSpace(nativeText.Value) : true) ||
            !int.TryParse(text, NumberStyles.Integer, CultureInfo.InvariantCulture, out value) || value < minimum || value > maximum)
            return false;
        present = true;
        return true;
    }
    ...
    private static void PatchOptionalInt(XElement owner, string name, bool present, int value)
    {
        var existing = owner.Element(ChartNs + name);
        if (!present)
        {
    ...
    private static bool PlotProfileEditable(XElement plot, SpreadsheetChartType type, bool allowScatterStyleVariants)
    {
    ...
    private static bool ScalarAllowed(XElement owner, string name, IReadOnlyCollection<string> allowed, bool required)
    {
        var matches = owner.Elements(ChartNs + name).Take(2).ToArray();
        if (matches.Length == 0) return !required;
        if (matches.Length != 1) return false;
        var element = matches[0];
        return (string?)element.Attribute("val") is { } value && allowed.Contains(value) && !element.Attributes().Any(attribute => !attribute.IsNamespaceDeclaration && attribute.Name != "val") && !element.Nodes().Any(node => node is XText text ? !string.IsNullOrWhiteSpace(text.Value) : true);
    }

    private static bool ScalarEquals(XElement owner, string name, string expected, bool required) =>
        ScalarAllowed(owner, name, new[] { expected }, required);

    private static string CanonicalScatterStyle(string value) => value switch
    {
        "lineMarker" => "lineWithMarkers",
        "smoothMarker" => "smoothWithMarkers",
        _ => value,
    };

    private static string ScatterStyleToken(string value) => value switch
    {
        "" or "marker" => "marker",
        "line" => "line",
