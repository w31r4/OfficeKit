...
transform and the direct accent6 tint, shade, luminance, and alpha leaves at a
time, then patches the existing DrawingML token. This change applies the same
ownership boundary to accent6 saturation modulation without broadening the
...
- Accent6 transforms other than `satMod`, transforms on accent6 through
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:satMod`. The source writer patches `Accent6Color` only when the authored
   transform is the one existing accent6 satMod and rejects all other
   transform flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent6SatMod` is accepted only with
   `accentTransforms.accent6.satMod` in the source-issued capability. This
