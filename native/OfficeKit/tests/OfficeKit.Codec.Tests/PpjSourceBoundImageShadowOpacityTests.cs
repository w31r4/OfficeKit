...
{
    [Fact]
    public void PpjSourceBoundImageShadowOpacityEditsLeafAndReprojects()
    {
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/source/image-shadow-opacity.pptx",
            },
        });
        ...
        var opacityLeaves = projectedImage["nativeRef"]!["leaves"]!.AsArray()
            .Select(leaf => leaf!.AsObject())
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "imageShadowOpacityThousandthPercent")
            .ToArray();
        Assert.Single(opacityLeaves);
        Assert.Equal(24_000, opacityLeaves[0]["value"]!.GetValue<long>());

        opacityLeaves[0]["value"] = 33_000;
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            var shadow = Assert.IsType<A.OuterShadow>(Assert.Single(picture.ShapeProperties!.GetFirstChild<A.EffectList>()!.ChildElements));
            Assert.True(shadow.RotateWithShape!.Value);
            Assert.Equal(33_000, shadow.Descendants<A.Alpha>().Single().Val!.Value);
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/image-shadow-opacity.pptx",
            },
        });
        ...
        Assert.Equal(0.33, reprojectedImage["shadow"]!["opacity"]!.GetValue<double>());
        Assert.Equal(33_000, reprojectedImage["nativeRef"]!["leaves"]!.AsArray().Single(leaf =>
            leaf!["kind"]!.GetValue<string>() == "imageShadowOpacityThousandthPercent")!["value"]!.GetValue<long>());
    }
}
