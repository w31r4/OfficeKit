...
- Owner: `p:sp/p:spPr/a:custGeom/a:pathLst/a:path/a:arcTo/@swAng`.
- PPJ surface: `shape.nativeRef.leaves[]`.
- Native leaf: `customGeometryPathArcSweepAngle60000`.
...
- The geometry must pass the existing bounded custom-geometry recognition
  ...
  no child content, and a non-zero canonical literal sweep in the signed
...
non-zero one-turn numeric `swAng`, and the requested changed bounded sweep. The
writer then replaces only the selected `swAng` value token.

## Non-goals

- No public path-command array or command insertion/deletion.
- No arc radii, start angle, point coordinates, formula/reference, path
...
and arc command, removes embedded PPJ, projects the arc sweep-angle leaf,
changes it, checks that only the owning SlidePart changed and the package
remains valid, then projects again to recover the new sweep and unchanged arc
attributes.
