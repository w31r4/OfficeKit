...
核对日期：2026-09-10。最新原生回归快照为 `c8b0d324561bc9d390f9f71aa108fad05bd9344b`，独立构建与实际结果见第 3.5 节。工作区另有持续更新的原生、测试和能力声明，HEAD 不能单独标识未提交修改。本文区分所读源码、留存运行结果与尚未验证的变更，不代表安装包或远端分支状态。

本文记录现有 PPT 静态功能的已知差距，不是完整字段覆盖率报告：目前缺少逐字段分母，不能据此声称已穷尽所有 OOXML 属性。最近实施已重建指定快照并运行内部集成及窄门禁；没有执行全仓测试、外部 Office 或人类验收。各历史运行仍按自己的版本和范围解释。

### 本次复核摘要

当前集成汇总采用 `tmp/officekit-native-scene-paint-bIwSP1/integration.json`，报告时间为北京时间 2026-09-10 12:53:46。它使用第 3.5 节的独立包与单独记录摘要的 JS，不是整个持续变化工作区的全量验收。本轮已将正式入口代码接到 scene；默认安装的旧包尚未更新，不能据此宣称开箱可用：

| 核对项 | 结果 | 对完成判断的含义 |
| --- | --- | --- |
| 正式预览入口 | 请求 `includePreviewScene: true` 并实际绘制 scene，旧组件/图表猜测分支已删除 | 匹配包下已验证；默认旧包报 `preview.scene.missing`，需配套构建交付 |
| 输入可靠性检查 | 正式 `renderPpjSceneSvg` 强制合并原始输入与场景检查，没有关闭选项 | 内部 paint-only 仍可独立测试；公共入口不能借此跳过检查 |
| 正式入口案例 | 10 项组件/样式/dataset/源候选通过内容区像素、身份、发布与输入保留检查；另有 CLI 子进程回归 | 证明接入链路，不代表所有事实映射、默认安装包或完整功能验收 |
...
| 文字近期进展 | 小字号基线、边距、直接垂直锚定；六项锚定/底边距源修改与删除通过 | 完整换行、字体度量、垂直布局和 AutoFit 仍缺 |
...
| 线性渐变 | 形状和表格单元格共享绘制，七项作者/源 no-op/源修改案例通过 | 仅验证直接色标的线性渐变；径向、背景和其他消费者仍缺 |
| 报告整体状态 | `failed`；4 项 `transformProfileFailures` 加 7 项 `spacingDeletionFailures`，其余失败数组为空 | 十一项失败不能被局部对照成功抵消 |
...
该报告使用冻结提交 `c8b0d324` 的独立构建，包摘要见第 3.5 节；之后的工作区变更不在该原生包内。性能证据按该报告的 `performance` 字段关联，不能沿用先前 GjnuAZ 的文件摘要。报告中的 SmartArt 源绘制是明确的 unavailable 反例，不因失败列表为空就变成视觉保真成功。历史成功报告保留为对应版本的证据，不能覆盖新增的失败断言。

## 1. 结论与阅读范围

**渲染器尚未整体完成。正式 CLI 代码已消费编译器场景，匹配原生包下有真实成功案例；默认安装包仍需更新。** 当前不能把“生成了 PNG”“测试通过”或“原生接口能保留某个字段”解释成现有功能已全面、正确地显示。

主要差距分为五组：

1. 场景绘制已接入正式 CLI 的检查和发布，但当前默认安装包不提供 scene；配套运行时更新与默认环境复验尚未完成。
...
| --- | --- | --- | --- |
| G-01 | 正式入口已接 scene，默认原生包配套未交付 | 旧包明确拒绝预览，需匹配版本 | 默认包更新复验、完整场景字段与剩余任务验收 |
...
| G-12 | 场景发布已接正式入口，匹配包的限定回归通过 | 旧包仍无法使用；发布完成不等于事实/保真通过 | 配套默认包、新错误分支与实际环境验收 |
...
| 当前意义 | 指定匹配包下通过；默认旧包明确报缺场景 | 隔离验证绘制行为，不证明默认环境和完整交付 |
...
DUtafR 的 `productionEntryCases` 有十项：plain/styled 嵌套组件与显式对照各两项，line/heatmap 的 encoded/explicit 各两项，源 no-op/文字候选两项。测试只注入内存加载，真实编译、强制合并检查和发布走正式函数；每页除顶部 24px 审查条外的完整像素与已验证内部结果一致，输入/源字节、scene/候选摘要、JSON 序列化及返回/落盘一致均通过。两个 encoded 案例仍为事实 failed，显式对照为 requires-review；dataset 旧事实规则尚未逐项解除，不能用画面相同自动清除错误。第一次 xmbPm5 在 Buffer/Uint8Array 容器类型比较处失败，改为等类型逐字节断言后完整重跑。

另有真实 CLI 子进程回归验证作者、去快照源 no-op 和重复输出目录拒绝，产物在 `tmp/officekit-ppj-preview-isytO3`。测试 preload 仅选择第 3.5 节真实包并传给子进程，没有模拟原生响应或替换安装包。直接使用默认 `packages/office-kit-codec-linux-x64` 已实际得到 `preview.scene.missing`。默认包更新与无 preload 的复验是交付前必须补完的工作。

更新 registry 对正式入口的描述后，最终 bIwSP1 再次完成同样十项入口断言和七项渐变回归，所有非删除失败数组为空；整套仍因四项变换/七项间距删除失败退出 1。plain/styled 组件和数据对照都不是额外累加案例。匹配包下 presentation 门禁 4/4（tliOkO），无 preload 的合成诊断门禁单独通过；后者仍覆盖新进程 root/内存 SVG 懒加载，不依赖预加载掩盖初始化。已查看正式 styled-component PNG（Agent 检查，非人类校准）。生成摘要、gate-policy、严格 OpenSpec 与 whitespace 检查通过。
...
这轮只增加 JS 回归并重建已存在的原生实现，没有修改生产 C#。nullable/trim/AOT 警告仍保留。没有为新包重做双构建一致性、全仓/完整字段、人类或宿主验收；第 3.3 节的双构建证据仍只归属旧快照。后续优先使用这份新包回归，旧包留作历史反例。

## 4. 按功能核对剩余差距

### G-01：共用场景与正式入口

**现状：10/15 个任务勾选，正式函数已接场景，整体绘制与默认包交付未完成。** 编译采集、传输、归属与适配已有基础，任务 5.2 固定版本构建、4.2 场景发布和 5.1 成对 fixture 验收通过；3.3/3.4 剩余绘制、4.1 未完成事实映射以及 5.3、5.4 仍开放。这些是任务编号，不是本文章节编号。

剩余工作：更新默认原生包并复验；按 renderer profile 逐条核对事实规则，特别是已绘制 dataset/vector 状态仍有旧事实失败的情况；补足当前字段与复杂组合的绘制/诊断覆盖。保留固定成对案例，不用“两边都编译成功”或“两边都显示占位”代替等价性。
...
内部形状和表格单元格现已共享线性渐变函数，消费 2～16 个有序直接 RGB 色标、逐色标透明度和角度。重复位置保留硬边，透明度 0 不回落为不透明；方向使用对象实际宽高和原生 `scaled=false` 的物理角度，非正方形的 45° 不按缩放后的坐标轴猜测。独立 SVG 定义 ID 避免多对象相互覆盖。径向渐变、非法角度/色标/透明度及冲突填充仍明确失败。
...
theme/master/layout、命名样式及继承状态仍未统一解析；径向渐变、背景/图表/文字渐变、图片填充、阴影、内阴影、glow、reflection、soft edge，以及效果缩放/斜切仍缺实际消费或验收。形状/单元格线性渐变的成功不能推导为这些消费者都已支持。
...
**不要再把裁切、有限遮罩和边框整体记为“未实现”。** 它们已有第 3 节的限定实现及真实证据，现由正式入口共用；默认包配套要求见第 2 节。
...
这些历史清单完成结论归属当时的旧路线。scene 发布随后完成任务 4.2：场景身份、作者/源候选、24 例操作失败及两例事实失败警示见第 3.4 节。正式接入的新增证据见第 2 节；它不补足全部 profile 规则或默认安装包交付。

当前声明为 16 类元素中 9 类 partial、7 类 opaque；16 种 chartType 均 partial，没有整个类型提升为 supported。这是保守声明分布，不是完成率。

正式节点检查树已强制合并原始输入，仍需完成其他事实映射、更多归属边界及默认包配套。场景身份、源 no-op/文字候选发布、操作失败与事实失败警示已有证据；新类型或新错误分支仍须扩充回归。旧事实规则只能在有对应修复断言后按 profile 调整，不能为了让新路线变绿全部删除。
...
1. **维护验证快照**：以第 3.5 节的 c8b0d324 独立构建作为当前回归基线；第 3.3 节及 G-14 保留旧构建和两项 AOT 修复证据。后续修改记录源码、dirty diff、二进制 manifest/hash 和测试脚本身份；新字段继续补视觉断言，原生语义改变时再更新精确包验证。当前基线中的四项删除失败仍须保留，不能称作全绿基线。
...
4. **完成默认包配套交付**：正式函数已接场景、profile 检查及发布保护；更新匹配原生包并在无测试 preload 下复验。未解决项继续明确失败或标注限制，接入不等于全面覆盖。
