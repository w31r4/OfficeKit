...
| `nativeObject.setDiagramNodeRunText` | api | Replace one existing direct a:r/a:t value by zero-based source-order run index across a proven SmartArt node's fixed direct paragraphs. Paragraph/run topology, a:pPr, a:rPr, and a:endParaRPr stay source-bound; fields, breaks, topology changes, and unsupported diagrams reject without fallback. |
...
Replace one existing direct a:r/a:t value by zero-based source-order run index across a proven SmartArt node's fixed direct paragraphs. Paragraph/run topology, a:pPr, a:rPr, and a:endParaRPr stay source-bound; fields, breaks, topology changes, and unsupported diagrams reject without fallback.
...
- `nativeObject` (NativePresentationObject) — Queues one source-ordered run-local update without changing a:pPr, a:rPr, a:endParaRPr, or guessing style ownership. Export re-proves the source digest, node IDs/order, fixed paragraph/run topology, and closed graph; only the bound DiagramDataPart may change. Paragraph changes, fields, breaks, and unsupported markup fail closed.
...
- `nativeObject` (NativePresentationObject) — Queues one text-only update for a one-run node in a source-bound top-level SmartArt frame. Export re-proves the closed dm/lo/qs/cs graph and fixed direct-paragraph/run profile, may rewrite only its bound DiagramDataPart, and preserves paragraph/node/run topology, formatting, frame, relationships, layout, quick-style, colors, and all non-data parts. Multi-run nodes require setDiagramNodeRunText; fields, breaks, connected, nested, or otherwise unrecognized graphs fail closed.
