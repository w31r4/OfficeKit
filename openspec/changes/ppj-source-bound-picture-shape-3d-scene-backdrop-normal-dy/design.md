...
- Expose one canonical signed `a:backdrop/a:norm/@dy` coordinate in EMU.
- Replace only `backdrop/a:norm/@dy` in the owning SlidePart.
...
- Reuse `TryReadSceneBackdropNormalDy` and its requested-value validator so
  picture and shape owners share the same complete-backdrop proof.
...
- Token-splice only the direct `backdrop/a:norm/@dy` attribute.

# Risks / Trade-offs

- [Risk] A normal Y token without a complete backdrop owner could be mistaken
  for a safe scene. → Require the shared complete-backdrop reader.
