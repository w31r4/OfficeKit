...
This capability gives PPJ a safe, field-qualified source-bound owner for one
 direct accent6 hue-modulation token while preserving the rest of an imported theme.

## ADDED Requirements

### Requirement: Strict source-bound accent6 hueMod projection

The PPTX projector MUST expose
`design.theme.accentTransforms.accent6.hueMod` and
`setThemeAccent6HueMod` only when the canonical shared ThemePart has one
direct accent6 RGB leaf with exactly one direct `a:hueMod/@val` child, no
...
#### Scenario: Project a direct accent6 hueMod leaf

- **GIVEN** a canonical imported ThemePart with accent6 `334455` and
  `a:hueMod/@val="35000"`
- **WHEN** the presentation is projected to PPJ
- **THEN** the theme contains `accentTransforms.accent6.hueMod` equal to
  `0.35`
- **AND** the native reference contains `setThemeAccent6HueMod` authorized
  for `accentTransforms.accent6.hueMod`

#### Scenario: Reject unsupported accent6 hueMod topology
...
- **THEN** the source-bound accent6 hueMod field and capability are omitted

### Requirement: Source-bound accent6 hueMod editing

The source-preserving compiler MUST accept one authorized change to the
existing `design.theme.accentTransforms.accent6.hueMod` field, convert a
...
#### Scenario: Edit and reproject one accent6 hueMod token

- **GIVEN** a projected direct accent6 hueMod owner at `0.35`
...
#### Scenario: Fail closed for unsafe accent6 hueMod edits

- **GIVEN** a source-bound accent6 hueMod owner
