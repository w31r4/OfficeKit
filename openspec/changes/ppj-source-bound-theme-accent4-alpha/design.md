...
The source-bound accent palette currently accepts only six-digit direct RGB leaves for accent4 through accent6. Authored PPJ already accepts six- or eight-digit theme colors and lowers the trailing byte to `a:alpha`; the imported profile does not yet preserve that child for accent4.

## Goals / Non-Goals

**Goals:**

- Read `accent4` as `#RRGGBBAA` when its direct `srgbClr` has exactly one canonical `a:alpha` child.
- Keep the existing six accent object and `setThemeAccent4Color` field authority.
...
- Adding alpha ownership to accent4 through accent6 or the six non-accent color roles in this increment.
...
4. **Patch one field owner.** The writer compares RGB and alpha separately, updates only those two attributes under accent4, and leaves the other five accents and all theme descendants source-owned.
