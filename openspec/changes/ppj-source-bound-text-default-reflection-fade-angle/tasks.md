...
- [x] 1.1 Add `textDefaultReflectionFadeAngleDegrees` to the PPJ schema, native
      leaf registry, capability reference, and generated manual; verify JSON
      and maintainer contract checks.
- [x] 1.2 Project an existing paragraph default reflection `fadeDir` as a
...
- [x] 2.1 Extend edit-plan validation and expected-value proof for the strict
      circular-angle token; verify bound, malformed, stale, and unsupported
      graphs fail closed.
- [x] 2.2 Splice only `a:reflection/@fadeDir` in the owning SlidePart; verify
      sibling effects, other reflection attributes, text topology, and
      non-target package parts remain unchanged.
