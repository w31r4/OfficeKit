...
- Owner: `p:sp/p:spPr/a:scene3d/a:backdrop/a:norm/@dy`.
- Native leaf: `shape3dSceneBackdropNormalDyEmu`.
- The owner must be the single direct `a:scene3d` child of `a:spPr`. The scene
  ...
  Y value is exposed; all other coordinates are validation context only.
- The coordinate uses DrawingML EMUs and the standard bounded signed
  `ST_Coordinate` range. Projection stores only normal `dy` in the native
  shape model. The edit plan rechecks the complete backdrop owner and
  token-splices only `norm/@dy`, preserving the anchor, normal `dx/dz`, up
...
No source-free authoring, anchor editing, normal `dx/dz` editing, up-vector
