...
- Owner: `p:sp/p:spPr/a:custGeom/a:pathLst/a:path/a:quadBezTo/a:pt[1]/@x`.
- PPJ surface: `shape.nativeRef.leaves[]`.
- Native leaf: `customGeometryPathQuadraticEndX`.
...
- The geometry must pass the existing bounded custom-geometry recognition
  ...
  attribute-only, and the end-point x must be a canonical signed DrawingML
  coordinate; formula/reference-backed x values are not issued.
...
one end point, the canonical numeric end-point x token, and the requested
changed bounded coordinate. The writer then replaces only the selected `x`
value token on the second point.

## Non-goals

- No public path-command array or command insertion/deletion.
- No line-to, move-to, control-point, quadratic end-point y, cubic/arc,
  formula/reference, path property, or multi-path topology edits.
...
quadratic command, removes embedded PPJ, projects the quadratic end-point x
leaf, changes it, checks that only the owning SlidePart changed and the
package remains valid, then projects again to recover the new x coordinate and
unchanged control/end-point y coordinates.
