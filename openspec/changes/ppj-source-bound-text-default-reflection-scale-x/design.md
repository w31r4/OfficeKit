...
- Native leaf: `textDefaultReflectionScaleX`.
- PPJ location: `paragraph.style.defaultText.reflection.scaleX`.
- Unit: signed ratio, represented by the native `sx` integer at 1/100000
  precision.
...
single strict full-span reflection owner and contains `sx`. The owner may also
contain the already modelled `fadeDir`; `sy`, `kx`, `ky`, `algn`,
`rotWithShape`, unknown attributes, children, duplicate lists/effects and DAGs
remain source-owned.

An edit proves the paragraph index and expected raw `sx` token, then replaces
only `a:reflection/@sx` in the owning SlidePart. The package compiler keeps all
