...
internal sealed class XlsxChartCodec
{
    ...
    internal void Apply(WorksheetPart worksheetPart, string worksheetId, IReadOnlyList<SpreadsheetChartArtifact> charts, bool sourceBound, string? originalDrawingXmlSha256 = null)
    {
        ...
        for (var index = 0; index < records.Count; index++)
        {
            ...
            var chartSpaceChanged = !ChartSpaceEditSemanticHash(target).Equals(ChartSpaceEditSemanticHash(record.Artifact), StringComparison.OrdinalIgnoreCase);
    ...
    internal static void Validate(IEnumerable<SpreadsheetChartArtifact> charts, string worksheetId, bool allowStandardRadar = false)
    {
        ...
        foreach (var chart in charts)
        {
            ...
            if (chart.TitlePlacement is not ("" or "none" or "aboveChart" or "centeredOverlay")) throw InvalidChart(worksheetId, chart.Id, "title placement must be none, aboveChart, or centeredOverlay.");
            if (chart.Title.Length == 0 && chart.TitlePlacement is ("aboveChart" or "centeredOverlay")) throw InvalidChart(worksheetId, chart.Id, "a visible title is required for aboveChart or centeredOverlay placement.");
            if (chart.Title.Length > 0 && chart.TitlePlacement == "none") throw InvalidChart(worksheetId, chart.Id, "title placement none requires an omitted title.");
            ...
            if (chart.HasLegendOverlay && !chart.HasLegend)
                throw InvalidChart(worksheetId, chart.Id, "legend overlay requires an enabled legend.");
            if (chart.LegendFill is not null && !chart.HasLegend)
                throw InvalidChart(worksheetId, chart.Id, "legend fill requires an enabled legend.");
            if (chart.LegendLine is not null && !chart.HasLegend)
                throw InvalidChart(worksheetId, chart.Id, "legend line requires an enabled legend.");
            ...
            foreach (var series in chart.Series)
            {
                ...
                XlsxChartSeriesExplosionCodec.Validate(series, chart.Type, worksheetId, chart.Id);
                XlsxChartSeriesValueFormatCodec.Validate(series, worksheetId, chart.Id);
                ...
                OpenXmlChartTrendlineCodec.Validate(series, chart.Type, worksheetId, chart.Id);
                OpenXmlChartErrorBarsCodec.Validate(series, chart.Type, worksheetId, chart.Id);
                XlsxChartSeriesDataLabelsCodec.Validate(series, chart.Type, worksheetId, chart.Id);
                XlsxChartPointStyleCodec.Validate(series, chart.Type, worksheetId, chart.Id);
                ...
                ValidateMissingValueIndexes(series, worksheetId, chart.Id);
    ...
    private static string SemanticHash(SpreadsheetChartArtifact chart) => Hash(string.Join('\0', chart.Name, XlsxNonVisualAccessibilityCodec.Semantics(AccessibilityTitle(chart), AccessibilityDescription(chart), AccessibilityDecorative(chart)), ChartSpaceSemanticHash(chart), chart.HasOverlap ? chart.Overlap.ToString(CultureInfo.InvariantCulture) : "absent", chart.HasVaryColors ? (chart.VaryColors ? "1" : "0") : "absent"));
    private static string ChartSpaceEditSemanticHash(SpreadsheetChartArtifact chart) => Hash(string.Join('\0', ChartSpaceSemanticHash(chart), chart.HasOverlap ? chart.Overlap.ToString(CultureInfo.InvariantCulture) : "absent", chart.HasVaryColors ? (chart.VaryColors ? "1" : "0") : "absent"));
    private static string ChartSpaceSemanticHash(SpreadsheetChartArtifact chart) => Hash(string.Join('\0', chart.Id, chart.Title, chart.HasTitlePlacement ? chart.TitlePlacement : "absent", chart.HasStyleIndex ? chart.StyleIndex.ToString(CultureInfo.InvariantCulture) : "absent", XlsxChartTextStyleCodec.Semantics(chart.TextStyle), XlsxChartTextStyleCodec.Semantics(chart.TitleTextStyle), XlsxChartTextStyleCodec.Semantics(chart.LegendTextStyle), XlsxChartLineOptionsCodec.Semantics(chart.LineOptions), XlsxChartDataLabelsCodec.Semantics(chart.DataLabels), ((int)chart.Type).ToString(CultureInfo.InvariantCulture), chart.ScatterStyle.Length == 0 ? "marker" : chart.ScatterStyle, chart.RadarStyle.Length == 0 ? "standard" : chart.RadarStyle, chart.HasLegend ? "1" : "0", chart.HasLegend ? (chart.LegendPosition.Length == 0 ? "right" : chart.LegendPosition) : "-", chart.HasLegendOverlay ? (chart.LegendOverlay ? "1" : "0") : "absent", XlsxChartSurfaceFillCodec.Semantics(chart.LegendFill), XlsxChartSeriesLineStyleCodec.Semantics(chart.LegendLine), chart.Grouping, chart.HasGapWidth ? chart.GapWidth.ToString(CultureInfo.InvariantCulture) : "absent", chart.BarDirection, chart.HasFirstSliceAngle ? chart.FirstSliceAngle.ToString(CultureInfo.InvariantCulture) : "absent", chart.HasDoughnutHoleSize ? chart.DoughnutHoleSize.ToString(CultureInfo.InvariantCulture) : "absent", chart.HasBubbleScale ? chart.BubbleScale.ToString(CultureInfo.InvariantCulture) : "absent", chart.BubbleSizeMode, chart.HasDisplayBlanksAs ? chart.DisplayBlanksAs : "absent", XlsxChartSurfaceFillCodec.Semantics(chart.ChartAreaFill), XlsxChartSurfaceFillCodec.Semantics(chart.PlotAreaFill), XlsxChartSeriesLineStyleCodec.Semantics(chart.PlotAreaLine), XlsxChartDataTableCodec.Semantics(chart.DataTable), AnchorSemantics(chart), XlsxChartAxisCodec.Semantics(chart), string.Join('\u001e', chart.Categories), string.Join('\u001d', chart.Series.Select(series => string.Join('\u001f', series.Name, series.CategoryFormula, series.XValueFormula, series.ValueFormula, series.BubbleSizeFormula, XlsxChartSeriesStyleCodec.Semantics(series), XlsxChartSeriesExplosionCodec.Semantics(series), XlsxChartSeriesValueFormatCodec.Semantics(series), XlsxChartSeriesLineStyleCodec.Semantics(series.Line), XlsxChartSeriesMarkerCodec.Semantics(series.Marker), XlsxChartSeriesDataLabelsCodec.Semantics(series.DataLabels), XlsxChartPointStyleCodec.Semantics(series.PointStyles), OpenXmlChartTrendlineCodec.Semantics(series.Trendlines), OpenXmlChartErrorBarsCodec.Semantics(series.ErrorBars), string.Join(',', series.XValues.Select(value => value.ToString("R", CultureInfo.InvariantCulture))), string.Join(',', series.Values.Select(value => value.ToString("R", CultureInfo.InvariantCulture))), string.Join(',', series.MissingValueIndexes), string.Join(',', series.BubbleSizes.Select(value => value.ToString("R", CultureInfo.InvariantCulture))))))));

    private static void ValidateMissingValueIndexes(SpreadsheetChartSeriesArtifact series, string worksheetId, string chartId)
    {
        uint? previous = null;
        foreach (var index in series.MissingValueIndexes)
        {
            if (index >= (uint)series.Values.Count || previous is not null && index <= previous.Value || series.Values[checked((int)index)] != 0d)
                throw InvalidChart(worksheetId, chartId, $"series {series.Name} missing-value indexes must be strictly increasing, in range, and backed by canonical zero placeholders.");
            previous = index;
        }
    }
    ...
    private static void ValidatePlotStyle(SpreadsheetChartArtifact chart, string worksheetId)
    {
        ...
        if (chart.HasVaryColors && chart.Type is not (SpreadsheetChartType.Bar or SpreadsheetChartType.Area or SpreadsheetChartType.Scatter))
            throw InvalidChart(worksheetId, chart.Id, "vary colors is supported only on bar, area, or scatter charts.");
        ...
        if (chart.HasFirstSliceAngle && (chart.Type is not (SpreadsheetChartType.Pie or SpreadsheetChartType.Doughnut) || chart.FirstSliceAngle > 360))
            throw InvalidChart(worksheetId, chart.Id, "first slice angle is supported only on pie and doughnut charts and must be 0 through 360.");
        if (chart.HasDoughnutHoleSize && (chart.Type != SpreadsheetChartType.Doughnut || chart.DoughnutHoleSize is < 10 or > 90))
            throw InvalidChart(worksheetId, chart.Id, "hole size is supported only on doughnut charts and must be 10 through 90.");
        if (chart.HasBubbleScale && (chart.Type != SpreadsheetChartType.Bubble || chart.BubbleScale > 300))
            throw InvalidChart(worksheetId, chart.Id, "bubble scale is supported only on bubble charts and must be 0 through 300.");
        if (chart.BubbleSizeMode is not ("" or "area" or "width") || chart.BubbleSizeMode.Length > 0 && chart.Type != SpreadsheetChartType.Bubble)
            throw InvalidChart(worksheetId, chart.Id, "bubble size mode must be area or width and is supported only on bubble charts.");
        if (chart.ScatterStyle is not ("" or "line" or "lineWithMarkers" or "marker" or "smooth" or "smoothWithMarkers") ||
            chart.ScatterStyle.Length > 0 && chart.Type != SpreadsheetChartType.Scatter)
            throw InvalidChart(worksheetId, chart.Id, "scatter style must be line, lineWithMarkers, marker, smooth, or smoothWithMarkers and is supported only on scatter charts.");
        if (chart.RadarStyle is not ("" or "standard" or "marker" or "filled") ||
            chart.RadarStyle.Length > 0 && chart.Type != SpreadsheetChartType.Radar)
            throw InvalidChart(worksheetId, chart.Id, "radar style must be standard, marker, or filled and is supported only on radar charts.");
        if (chart.HasDisplayBlanksAs && chart.DisplayBlanksAs is not ("zero" or "gap" or "span"))
            throw InvalidChart(worksheetId, chart.Id, "display blanks mode must be zero, gap, or span.");
        if (chart.HasStyleIndex && chart.StyleIndex is < 1 or > 48)
            throw InvalidChart(worksheetId, chart.Id, "style index must be 1 through 48.");
        ...
        XlsxChartSurfaceFillCodec.Validate(chart.LegendFill, $"Worksheet {worksheetId} chart {chart.Id} legend fill");
        XlsxChartSeriesLineStyleCodec.ValidateLine(chart.LegendLine, worksheetId, chart.Id, chart.Name, "legend line");
        XlsxChartSeriesLineStyleCodec.ValidateLine(chart.PlotAreaLine, worksheetId, chart.Id, chart.Name, "plot area line");
        XlsxChartDataTableCodec.Validate(chart.DataTable, worksheetId, chart.Id);
    }
