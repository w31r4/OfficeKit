...
accent1 plus accent6 tint, shade, luminance modulation, luminance offset, and
alpha modulation. A direct imported accent6 `a:alphaOff` remains opaque,
...
- Project a canonical shared ThemePart whose accent6 direct RGB leaf contains
  ...
  `design.theme.accentTransforms.accent6.alphaOff`, using a -1..1 fraction.
- Issue `setThemeAccent6AlphaOff` for that field and compile one authorized
...
- `presentation-theme-accent6-alphaoff`: Strict source-bound projection and
  source-preserving editing of one direct accent6 `a:alphaOff` leaf.
