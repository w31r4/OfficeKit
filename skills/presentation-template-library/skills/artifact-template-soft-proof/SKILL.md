...
Use this style when the brief matches **mathematical derivation; academic presentation**. It is designed for
researchers, students, technical reviewers. Its visual premise is A patient proof-board where color has mathematical meaning. The style is a
...
**question → method → observation → derivation or comparison → interpretation → limitations → conclusion**. This style is not a good fit for **dense board report**;
...
- Use **#FCFCFA** as the main field, **#18313A** for primary type, **#2278E8** for the decisive signal, **#B64A98** for secondary evidence, and **#E5ECE8** for rules or context.
...
- The recurring structure is **contour, variable page silhouettes, native data graphics**. Vary the silhouette between
  cover, evidence, data, process, comparison, and decision pages rather than
  repeating a component grid.
- Use contour lines or nested marks to explain levels, ranges, or a derivation. Each contour needs a legend-free label or variable definition.
...
The expected density is **sparse**: complexity should come from relationships, not tiny type. Alternate page
...
- **Page skeleton.** Use a whiteboard-like field where definitions lead to an equation, contour, or result. Keep pale chapter colors as local sandboxes and preserve an explicit reading path from input to transformation to conclusion.
- **Type rhythm.** Use a display sans for headings, a text face for explanation, and a math face only for equations or variables. Blue and magenta have fixed semantic roles and must not swap mid-proof.
- **Evidence carriers.** Use equations, trajectories, scatter plots, sample matrices, vector fields, and paired comparisons. An almond or contour frame is a mathematical workspace, not a generic card.
- **Images and layers.** Do not add photos to an abstract derivation. Use native vectors, line art, and a small number of samples; keep any source screenshot bounded and labelled.
- **Pacing.** Introduce one variable or operation at a time, show the transformation, then state the result. Split long derivations across pages rather than shrinking the notation.
- **Selection rule.** Choose this style for mathematical, analytical, or technical teaching where the derivation itself is the visual story.

## Signature moves and variation limits

- Let **contour** recur as a quiet identity cue on selected pages; it must
