...
internal static partial class PptxEditPlanCodec
{
        ...
        EffectiveCodecLimits limits)
    {
        ...
        foreach (var operation in request.Operations)
        {
            ...
            if (element is P.GraphicFrame tableField &&
                ...
                LeafKind(operation) == "tableTextFieldType" &&
    ...
    private static string ReadLeafValue(OpenXmlElement element, PresentationEditOperation operation)
    {
        ...
        if (kind == "tableTextFieldType")
        {
            if (element is not P.GraphicFrame table || !PptxTableCodec.TryRead(table, out _))
                throw new CodecException("presentation_edit_target_mismatch", $"PPTX edit operation {operation.OperationId} tableTextFieldType target is not a supported table.", operation.SlidePartPath);
