...
本轮后续增量（2026-09-06）：`textField.automatic` 已进入 PPJ/schema/wire，并在当前有界 profile 中支持 `type: "slidenum"` 与默认 `type: "datetime"`。`slidenum` 的 authoring/source-bound 缓存按所属 slide 序号刷新；`datetime` 写入并回读 host-refreshable 标记及缓存文本，不在 OfficeKit 内伪造本机时间。PPJ 二次投影恢复 `automatic: true`；`datetime1` 等日期格式、author 等未覆盖的自动字段和完整字段指令图继续 fail closed。
