...
const oracleSourcesByFamily = new Map([
  ["pdf", ["scripts/agent-eval-pdf-graders.mjs", "scripts/agent-eval-pdf-oracle.py", "scripts/agent-eval-pades-ltv-validator.py"]],
  ["documents", ["scripts/agent-eval-office-graders.mjs", "scripts/agent-eval-office-fixtures.mjs", "scripts/agent-eval-docx-graders.mjs", "scripts/agent-eval-docx-complex-table-grader.mjs", "scripts/agent-eval-docx-section-page-numbering-grader.mjs", "scripts/agent-eval-office-native-render.mjs"]],
  ["spreadsheets", ["scripts/agent-eval-office-graders.mjs", "scripts/agent-eval-office-fixtures.mjs", "scripts/agent-eval-spreadsheet-graders.mjs", "scripts/agent-eval-xlsx-nested-reply-grader.mjs", "scripts/agent-eval-office-native-render.mjs"]],
  ["presentations", ["scripts/agent-eval-office-graders.mjs", "scripts/agent-eval-office-fixtures.mjs", "scripts/agent-eval-presentation-graders.mjs", "scripts/agent-eval-pptx-smartart-boundary-grader.mjs", "scripts/agent-eval-office-native-render.mjs"]],
]);
...
function oracleFingerprint(item) {
  const sources = [...commonOracleSources, ...(oracleSourcesByFamily.get(item.family) || [])];
  const implementation = sources.map((relative) => ({
    path: relative,
    sha256: sha256(readFileSync(path.join(repoRoot, relative))),
  }));
  return sha256(Buffer.from(JSON.stringify({ expectedOutcome: item.expectedOutcome, grade: item.grade, implementation }), "utf8"));
}
...
export function providerRuntimePath(item, environment = process.env) {
  if (item?.family !== "pdf") return null;
  const configured = environment.OFFICE_KIT_AGENT_EVAL_PROVIDER_PYTHON
    || environment.OFFICE_KIT_PDF_PROVIDER_PYTHON
    || environment.OFFICE_KIT_AGENT_EVAL_PYTHON;
...
function help() {
  return [
    ...
    "",
    "The Agent receives only PROMPT.md, declared inputs, the selected Skill, and an installed candidate tarball. Prepared PDF trials declare one authoritative provider interpreter in PROMPT.md from OFFICE_KIT_AGENT_EVAL_PROVIDER_PYTHON, OFFICE_KIT_PDF_PROVIDER_PYTHON, or OFFICE_KIT_AGENT_EVAL_PYTHON, in that order. OFFICE_KIT_AGENT_EVAL_PYTHON remains the separate fixture/oracle interpreter, so a small evaluator runtime can grade a managed specialist provider without pretending that they are the same environment. Fixture specifications and graders are never copied into its workspace; run.json records integrity evidence and only a fingerprint of the hidden oracle, never the grading specification. The default run root is outside the repository in the OS temp directory. A production benchmark must additionally mount only the trial workspace into a no-network container, because a CLI sandbox alone is not an oracle confidentiality boundary. The 21 ready PDF cases include fourteen locked corpus signature/boundary/repair/redaction/table fixtures with independent source-structure, typed table, audit, render, OCR, and trace grading; the bounded PAdES-LTA case additionally carries disclosed disposable test PKCS#12 material and evaluator-only explicit-root/CRL cryptographic validation. They do not pass from generic no-artifact gates alone. The five ready XLSX cases cover direct threaded-comment success, nested-reply refusal, growth-assumption update, connection refresh-on-open, and Pivot refresh-on-open; six DOCX cases (classic-comment, nested-modern-comment refusal, complex-table-topology refusal, source-bound DOCX header text, source-bound DOCX footer text, and source-bound DOCX section page-numbering), and five PPTX cases (title plus fixed-topology rich-notes run edit, source-bound slide-name edit, source-bound complete section-boundary edit, closed-leaf slide clone, and connected-SmartArt compound refusal) also have independent semantic, native-render where applicable, security, and provider-trace graders. PDF must remain an absolute majority of the full suite; the remaining 4 asset-required cases never claim full success before pinned corpus or PKI fixtures exist.",
    "",
  ].join("\n");
}
