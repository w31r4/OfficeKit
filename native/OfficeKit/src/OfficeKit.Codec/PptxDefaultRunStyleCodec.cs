...
internal static class PptxDefaultRunStyleCodec
{
    ...
    internal static void Read(PresentationTextParagraph target, A.TextParagraphPropertiesType? source)
    {
        ...
        if (TryReadEffect<A.OuterShadow, PresentationShadow>(properties, TryReadShadow, out var shadow) && shadow is not null) style.Shadow = shadow;
    ...
    internal static void Apply(A.TextParagraphPropertiesType target, PresentationTextParagraph source)
    {
        ...
        if (source.DefaultRunStyleCase == PresentationTextParagraph.DefaultRunStyleOneofCase.NoDefaultRunProperties)
        {
            ...
            if (clearing.DefaultRunProperties is { ColorCase: not PresentationTextStyle.ColorOneofCase.None })
                ValidateDirectFillEdit(properties);
        ...
        if (beforeWithoutScalars.Equals(afterWithoutScalars))
        {
            ...
            if (before.HasFontBaselinePercent != after.HasFontBaselinePercent || before.FontBaselinePercent != after.FontBaselinePercent)
            {
                if (properties.Baseline is not null && !PptxTextDecoration.TryBaseline(properties, out _))
            ...
            if (before.HasStrike != after.HasStrike || before.Strike != after.Strike)
            {
                if (properties.Strike is not null && !PptxTextDecoration.TryStrike(properties, out _))
            ...
            if (before.ColorCase != after.ColorCase || before.ColorRgb != after.ColorRgb || before.ColorScheme != after.ColorScheme ||
                ...
                !Equals(before.GradientFill, after.GradientFill))
            {
                ValidateDirectFillEdit(properties);
            ...
            if (!Equals(before.InnerShadow, after.InnerShadow) ||
                ...
                before.InnerShadow?.HasDirectionAngle60000 != after.InnerShadow?.HasDirectionAngle60000 ||
                before.InnerShadow?.HasOpacityThousandthPercent != after.InnerShadow?.HasOpacityThousandthPercent)
                ApplyDirectInnerShadow(properties, after.InnerShadow);
    ...
    private static void ValidateStyle(PresentationTextStyle style)
    {
        ...
        switch (style.HighlightCase)
        {
            case PresentationTextStyle.HighlightOneofCase.None:
                break;
            case PresentationTextStyle.HighlightOneofCase.HighlightRgb:
                _ = PptxColor.Normalize(style.HighlightRgb);
                break;
            case PresentationTextStyle.HighlightOneofCase.HighlightScheme:
                _ = PptxColor.NormalizeScheme(style.HighlightScheme);
                break;
            default:
                throw Invalid("Presentation default run properties contain an unknown highlight color case.");
        }
    ...
    private delegate bool EffectReader<TValue>(OpenXmlCompositeElement? source, out TValue? value) where TValue : class;

    private static bool TryReadEffect<TNative, TValue>(
        A.DefaultRunProperties? source, EffectReader<TValue> reader, out TValue? value)
        where TNative : OpenXmlElement
        where TValue : class
    {
        value = null;
        if (source?.ChildElements.Any(child => child.LocalName == "effectDag") == true) return false;
        var lists = source?.Elements<A.EffectList>().ToArray() ?? [];
        if (lists.Length > 1) return false;
        var effects = lists.SingleOrDefault()?.Elements<TNative>().ToArray() ?? [];
        if (effects.Length == 0) return true;
        if (effects.Length != 1) return false;
        var raw = XElement.Parse(effects[0].OuterXml);
        XNamespace drawing = "http://schemas.openxmlformats.org/drawingml/2006/main";
        if (raw.DescendantNodes().Any(node => node is not XElement && (node is not XText text || !string.IsNullOrWhiteSpace(text.Value))) ||
            raw.Descendants(drawing + "alpha").Any(alpha => alpha.HasElements) ||
            effects[0] is A.Reflection or A.SoftEdge && raw.HasElements) return false;
        try
        {
            var isolated = new A.RunProperties(new A.EffectList(effects[0].CloneNode(true)));
            return reader(isolated, out value) && value is not null;
        }
    ...
    private static bool TryReadInnerShadow(OpenXmlCompositeElement? source, out PresentationInnerShadow? shadow)
    {
        if (!PptxInnerShadowCodec.TryRead(source, out shadow) ||
            shadow is not null && (shadow.HasBlurRadiusEmu && shadow.BlurRadiusEmu > 12_700_000 ||
    ...
    private static void ApplyDirectGlow(A.DefaultRunProperties target, PresentationGlow? source) =>
        ApplyDirectEffect<A.Glow, PresentationGlow>(target, source, PptxGlowCodec.TryRead, (owner, value) => PptxGlowCodec.Apply(owner, value));
    ...
    private static bool TryReadShadow(OpenXmlCompositeElement? source, out PresentationShadow? shadow)
    {
        shadow = null;
        var native = source?.GetFirstChild<A.EffectList>()?.GetFirstChild<A.OuterShadow>();
        if (native is null) return true;
        if (!PptxShadowCodec.TryReadOuterShadow(native, out shadow, allowTransforms: true) ||
            shadow is not null && (shadow.HasBlurRadiusEmu && shadow.BlurRadiusEmu > 12_700_000 ||
                shadow.HasDistanceEmu && shadow.DistanceEmu > 1_270_000_000))
        {
            shadow = null;
    ...
    private static void ApplyDirectEffect<TNative, TValue>(
        ...
        where TValue : class
    {
        if (!TryReadEffect<TNative, TValue>(target, read, out _))
            throw Unsupported("Source-preserving PPTX export cannot replace or delete an unmodeled paragraph default effect owner.");
        var list = target.GetFirstChild<A.EffectList>();
        var existing = list?.GetFirstChild<TNative>();
        if (source is null)
        {
            existing?.Remove();
        ...
        var built = new A.RunProperties();
        write(built, source);
        var effect = built.GetFirstChild<A.EffectList>()!.GetFirstChild<TNative>()!.CloneNode(true);
    ...
    private static void ValidateDirectFillEdit(A.DefaultRunProperties target)
    {
        var colors = ColorChoices(target).ToArray();
        if (colors.Length > 1 || colors.Any(color => !ModeledFill(color) ||
            color is A.SolidFill fill && fill.ChildElements[0].ChildElements.Any(child => child is not A.Alpha)))
            throw Unsupported("Source-preserving PPTX export cannot replace or delete default-run fill with unmodeled paint or transforms.");
    }
    ...
    private static void ClearModeled(A.DefaultRunProperties target)
    {
        ...
        if (TryReadEffect<A.OuterShadow, PresentationShadow>(target, TryReadShadow, out var shadow) && shadow is not null)
            ApplyDirectShadow(target, null);
        if (TryReadEffect<A.Glow, PresentationGlow>(target, PptxGlowCodec.TryRead, out var glow) && glow is not null)
