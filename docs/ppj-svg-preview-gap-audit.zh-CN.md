...
审计起始日期：2026-09-09；最新实施日期：2026-09-10。当前基线：HEAD `1fe503cd` 与普通堆叠柱形/条形工作区修改。最新实施、前次文档复核和更早实施分别记录在第 1.3 节，避免混用结果。
...
| 内部 column / bar | 普通分组及普通堆叠、正负值、真实零、方向/反向轴、间距/重叠与直接样式有局部证据；新增两种方向的堆叠源数值修改通过 | 百分比、对数基线、混合/副轴、完整标签/继承尚缺；堆叠缺失分类仅保留值与未知位置提示；仍未接入正式 CLI |
...
本节先记录最新普通堆叠实施，再保留前次文档复核和普通分组实施结果。后面的折叠区和[历史验证记录](ppj-svg-preview-gap-audit-history.zh-CN.md) 保留更早轮次。历史通过不自动延续到新代码，各表的执行范围不可混用。

#### 最新实施：普通堆叠 column / bar（2026-09-10）
...
| 原生 chart | 普通分类 line；单系列 pie/doughnut；普通分组及普通堆叠 column/bar 均有局部回归，见第 2.5～2.8 节 | 其他 native 枚举仍占位；百分比、折线/面积堆叠、平滑、混合/副轴、多环尚缺；explosion 仅径向 review 布局；轴、标签、图例、主题和完整显示策略未完成 |
...
| 行为 | 已实现及验证 | 尚未完成 |
| --- | --- | --- |
| 普通分组 | native grouping 空值/none；多个系列各有固定位置，缺失点留空而不挤占/移动其他系列 | 普通 stacked 后续已单独实现，见第 2.8 节；percent-stacked 仍明确失败，不得作为普通分组绘制 |
...
正负混合图额外显示虚线零参考线及 0 刻度，明确标记为 review guide；它不是作者指定的 gridline/axis crossing。纯零点也只有审阅刻线，不画伪造高度/长度的柱。所有内部图保留 `chart-layout` 和每页 `integration-pending`；标签、图例、效果、百分比堆叠、对数、混合轴及生产接入未完成，G-01 3.4 与 G-08～G-10 均不关闭。

### 2.8 内部普通堆叠：累计坐标和缺失分类

当前仅新增普通 `stacked`，不包含 `percent-stacked`，也没有把 line/area 的堆叠一起实现。painter 仍读取已编译的 native 分类/系列/缺失索引，不计算 PPJ dataset 或解析 OOXML。
...
仍缺百分比分母与负值策略、缺失显示策略、系列/对象增删、完整标签与主题，以及生产检查/发布接入。新增绘制与源编辑证据不关闭 G-01/G-08～G-10。
...
| 百分比堆叠，G-08～G-10 | 明确分母、零总量、负值及缺失策略，保留原始值与显示比例的对应 | 完整正值 1、3 显示 25%/75%；有未知项时不得声称已知整体百分比；全零不能除零或造出面积 | 先确认现有 codec 语义；未确定的负值/缺失规则保留为待决，不自行取绝对值或补零 |
...
| 文字和形状，G-02～G-04 | 字体度量、换行、段落/列表、AutoFit、溢出、剩余 preset/adjustments、路径 arc 与效果 | 同段多 run 不拆段；长中文、字号变化、旋转文字与非矩形各验证实际边界 | 场景文字/几何/变换；明确字体环境与允许的栅格差异 |
