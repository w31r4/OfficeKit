...
Use this style when the brief matches **experimental report; academic presentation**. It is designed for
researchers, students, technical reviewers. Its visual premise is Wine-red chapter marks focus attention like an analytical lens. The style is a
...
**question → method → observation → derivation or comparison → interpretation → limitations → conclusion**. This style is not a good fit for **playful workshop**;
...
- Use **#FFFFFF** as the main field, **#231C21** for primary type, **#8E244D** for the decisive signal, **#6550A5** for secondary evidence, and **#E6DDE2** for rules or context.
...
- The recurring structure is **lens, variable page silhouettes, native data graphics**. Vary the silhouette between
  cover, evidence, data, process, comparison, and decision pages rather than
  repeating a component grid.
- Use a lens, frame, or focused crop to narrow attention to the observed phenomenon. The frame should clarify scope, not become a repeated decorative container.
...
- **Page skeleton.** Use white evidence pages with a wine-red chapter cue and occasional cream interludes. Place the conclusion in the title, the figure or definition in the body, and a short source line at the edge.
- **Type rhythm.** Allow the type relationship to change only when the section changes: bold sans for ordinary analysis, a measured serif or display treatment for a special subsection. Keep code and formulas visibly distinct.
- **Evidence carriers.** Use bar/line/scatter evidence, closed-loop method diagrams, comparison tables, and carefully scoped screenshots. A radar or unusual chart is acceptable only if the axes carry a real analytical dimension.
- **Images and layers.** Screenshots and illustrations are evidence when they explain the case. Preserve their native colors inside a bounded frame; do not let them redefine the deck palette.
- **Pacing.** Move between a clear claim page, a method page, and a dense result page. Wine-red full-bleed slides are chapter pauses, not body backgrounds.
- **Selection rule.** Choose this style for research, evaluation, or data-heavy narratives with visible method and limitations.

## Signature moves and variation limits

- Let **lens** recur as a quiet identity cue on selected pages; it must
