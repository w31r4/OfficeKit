...
The native leaf `shape3dSceneBackdropUpDyEmu` already covers strict imported
shapes, but the same complete backdrop owner on a picture is still opaque. Its
up-vector Y coordinate is an independently bounded literal that can be edited
...
- Project and source-bound-edit only a strict
  `p:pic/p:spPr/a:scene3d/a:backdrop/a:up/@dy` token.
...
- `ppj-source-bound-picture-shape-3d-scene-backdrop-up-dy`:
  Bounded projection and source-bound editing of a picture backdrop up-vector Y.
