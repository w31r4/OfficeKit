# Design: source-bound accent3 lumMod

## Context

The existing source-preserving theme path recognizes one direct accent1
transform and the direct accent3 tint/shade leaves at a time, then patches the
existing DrawingML token. This change applies the same ownership boundary to
accent3 luminance modulation without broadening the theme graph or changing
...
- Accent3 transforms other than `lumMod`, transforms on accent4 through
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:lumMod`. The source writer patches `Accent3Color` only when the authored
   transform is the one existing accent3 lumMod and rejects all other
   transform flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent3LumMod` is accepted only with
   `accentTransforms.accent3.lumMod` in the source-issued capability. This
