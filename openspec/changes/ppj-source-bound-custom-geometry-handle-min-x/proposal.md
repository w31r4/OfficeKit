...
F-04 now exposes the direct x and y positions of an XY adjustment handle, but
its literal minimum-x range bound remains opaque. A bounded `minX` leaf lets an
agent tune an existing handle constraint while keeping the controlled guide
identity, current geometry, and the rest of the range graph source-faithful.

## What Changes

- Add `customGeometryAdjustmentHandleMinXEmu` as a bounded PPJ native leaf for
  an ordered `a:ahXY` handle with a direct literal `@minX`.
...
- Token-splice only the selected `a:ahXY/@minX` value in the owning SlidePart;
  preserve `gdRefX/gdRefY`, `maxX`, min/max y, position, other handles, and
...
- `ppj-source-bound-geometry-handle-min-x`: source-bound editing of one direct
  custom-geometry XY adjustment-handle minimum-x bound.
