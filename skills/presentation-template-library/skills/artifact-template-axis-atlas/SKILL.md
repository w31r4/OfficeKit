...
Use this style when the brief matches **systems research; academic presentation**. It is designed for
researchers, students, technical reviewers. Its visual premise is Large indexes and orthogonal routes turn research structure into a map. The style is a
clean-room OfficeKit grammar: the included images teach a range of decisions,
not pages or coordinates to copy.

The deck should help the audience lead a reader from a question through method and evidence to an interpretable, bounded conclusion. The natural story arc is
**question → method → observation → derivation or comparison → interpretation → limitations → conclusion**. This style is not a good fit for **soft lifestyle story**;
...
- Use **#FFFFFF** as the main field, **#102B55** for primary type, **#15A6D8** for the decisive signal, **#F06B4F** for secondary evidence, and **#D8DEE8** for rules or context.
...
- Academic / Research guidance: Define terms before relying on them, distinguish observation from interpretation, cite the source and sample, show uncertainty and limitations, and preserve the order of a derivation or experiment.
...
- The recurring structure is **route, variable page silhouettes, native data graphics**. Vary the silhouette between
  cover, evidence, data, process, comparison, and decision pages rather than
  repeating a component grid.
- Use orthogonal routes, nodes, and index marks to make dependencies visible. Every line must connect two meaningful objects; remove orphan arrows and decorative crossings.
...
For this category, favor **research question, concept map, method/process, evidence chart or table, derivation/proof, limitations and conclusion**. The style's content shapes are
claim, method, evidence; select one primary carrier per page and explain why it earns the
space.

### Information carriers

- traceable axes, annotated distributions, comparison tables, timelines, dependency graphs, proof boards, and stepwise diagrams; name variables and units at the point of use
- use diagrams, specimen/context photography, or restrained chapter imagery only when it teaches the subject. Never let a photograph replace a method or evidence explanation.
...
The expected density is **dense**: complexity should come from relationships, not tiny type. Alternate page
...
- **Page skeleton.** Use a white map-like canvas with asymmetric zones. Let a single orthogonal route, index, or dependency contour carry the page; connect evidence to the route with thin lines rather than enclosing it in cards.
- **Type rhythm.** Use a modern sans throughout, with semi-bold indexed labels and a regular explanatory layer. Two-digit indexes and one cyan focus point provide wayfinding without becoming decoration.
- **Evidence carriers.** Choose dependency graphs, decision trees, annotated line plots, method sequences, and ruled comparison tables. Keep one primary relationship per page and name every node or variable at its point of use.
- **Images and layers.** Do not add photos or textures. Diagrams and native vectors are the visual material; an illustration is acceptable only when it explains a system or specimen.
- **Pacing.** Give the reader a clear Z path: assertion, main relation, explanation, then conclusion. Alternate high-detail relationship pages with short theorem or finding pages.
- **Selection rule.** Choose this style when research or engineering evidence needs to be navigated as a system rather than presented as a gallery of charts.

## Signature moves and variation limits

- Let **route** recur as a quiet identity cue on selected pages; it must
...
- Avoid marketing claims, unexplained decorative diagrams, equations without a reading path, tiny footnotes used as the only caveat, and visual hierarchy that hides the research question.
...
- Check that the question is answered, methods and units are visible, every figure has a reading cue, citations are attached to claims, and limitations are not visually buried.
