...
const TRENDLINE_TYPES_TO_WIRE = new Map([
...
function seriesLineToWire(line) {
...
function trendlineFromWire(value, name, chart) {
  const type = TRENDLINE_TYPES_FROM_WIRE.get(value.type);
  if (!type) fail(chart, `${name} has unsupported type ${value.type}.`, "unsupported_spreadsheet_chart");
  return {
    type,
    ...(value.name ? { name: value.name } : {}),
    ...(value.polynomialOrder == null ? {} : { order: value.polynomialOrder }),
    ...(value.period == null ? {} : { period: value.period }),
    ...(value.forward == null ? {} : { forward: value.forward }),
    ...(value.backward == null ? {} : { backward: value.backward }),
    ...(value.intercept == null ? {} : { intercept: value.intercept }),
    displayEquation: value.displayEquation === true,
    displayRSquared: value.displayRSquared === true,
...
export function spreadsheetChartSnapshot(chart, options = {}) {
  ...
  return {
    ...
    series: chartSeries(chart).map((series) => {
      const values = options.resolveSourceData ? resolvedWorksheetChartSeriesValues(chart, series) : [...(series?.values || [])].map(Number);
      return {
...
function wireChart(chart, original) {
  ...
  const output = {
    ...
    dataLabels: snapshot.dataLabels == null ? undefined : {
      ...
      showPercent: snapshot.dataLabels.showPercent,
    },
    type,
...
export function wireWorksheetCharts(sheet, state) {
  ...
  for (const slot of state?.slots || []) {
    if (!remaining.delete(slot.chart)) {
      throw new OfficeKitCodecError(`Worksheet ${sheet.name} cannot remove imported chart ${slot.chart?.name || slot.wire.id} in the bounded OfficeKit slice.`, [], { code: "invalid_spreadsheet_chart_topology" });
    }
    ...
    if (JSON.stringify(current.position) !== JSON.stringify(slot.publicSnapshot.position)) {
      fail(slot.chart, "cannot change imported anchor geometry in the bounded OfficeKit slice.", "unsupported_spreadsheet_chart_edit");
    }
  ...
  if (state && remaining.size) {
    ...
    fail(chart, "cannot be added to an imported source package in the bounded OfficeKit slice.", "invalid_spreadsheet_chart_topology");
  }
...
export function spreadsheetChartFromWire(sheet, source) {
  ...
  const importedTrendlines = sourceSeries.map((series, index) => trendlinesSnapshot(
    (series.trendlines || []).map((trendline, trendlineIndex) => trendlineFromWire(trendline, `series ${index + 1} trendline ${trendlineIndex + 1}`, source)),
  ...
  if (source.dataLabels != null) {
    ...
    if (source.dataLabels.showPercent != null) dataLabelsInput.showPercent = source.dataLabels.showPercent;
