...
This change applies the same ownership boundary to accent6 hue modulation
...
- Accent6 transforms other than `hueMod`, transforms on other accent roles, inherited or effect theme state, or arbitrary theme XML editing.
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:hueMod`. The source writer patches `Accent6Color` only when the authored
   transform is the one existing accent6 hueMod and rejects all other transform
   flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent6HueMod` is accepted only with
   `accentTransforms.accent6.hueMod` in the source-issued capability. This
