...
transform and direct accent5 tint, shade, luminance, alpha, saturation, red,
blue, and hue modulation leaves at a time, then patches the existing DrawingML
token. This change applies the same ownership boundary to accent5 hue offset
...
- Accent5 transforms other than `hueOff`, transforms on accent5 through
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:hueOff`. The source writer patches `Accent5Color` only when the authored
   transform is the one existing accent5 hueOff and rejects all other transform
   flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent5HueOff` is accepted only with
   `accentTransforms.accent5.hueOff` in the source-issued capability. This
