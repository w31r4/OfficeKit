# Custom geometry cubic first-control-point y coordinate

## ADDED Requirements

### Requirement: expose one bounded cubic first-control-point y coordinate

The system MUST expose a `customGeometryPathCubicControl1Y` native leaf only
...
- **WHEN** an imported custom geometry contains a recognized path with a
  literal cubic first-control-point y coordinate
- **THEN** the projected shape exposes that coordinate with path order in
  ...
  control-point x, second control point, end point, and rest of the path graph
  remain source-owned

#### Scenario: edit only the selected control-point y token

- **WHEN** a source-bound PPJ edit changes an issued cubic first-control-point y
