...
{
    [Fact]
    public void PpjSourceBoundTextShadowAlignmentEditsDirectRunOwnerAndReprojects()
    {
        var program = ShadowProgram(alignment: "ctr");
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            Assert.Equal(A.RectangleAlignmentValues.Center, shadow.Alignment!.Value);
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/source/text-shadow-alignment.pptx",
            },
        });
        ...
        Assert.Equal("ctr", shadowStyle["alignment"]!.GetValue<string>());
        var alignmentLeaf = Assert.Single(projectedElement["nativeRef"]!["leaves"]!.AsArray(),
            leaf => leaf!["kind"]!.GetValue<string>() == "textShadowAlignment");
        Assert.Equal("ctr", alignmentLeaf!["value"]!.GetValue<string>());
        ...
        using (var themeStream = new MemoryStream())
        {
            ...
            var themeProjected = Invoke(new CodecRequest
            {
                ...
                PresentationProgram = new PresentationProgramRequest
                {
                    SourceUri = "deck.assets/source/text-shadow-alignment-theme.pptx",
                },
            });
            ...
            Assert.Equal("ctr", themeElement["text"]!["paragraphs"]![0]!["runs"]![0]!["style"]!["shadow"]!["alignment"]!.GetValue<string>());
        ...
        using (var staleStream = new MemoryStream())
        {
            ...
            using (var stalePackage = PresentationDocument.Open(staleStream, true))
            {
                ...
                staleShadow.Alignment = A.RectangleAlignmentValues.BottomRight;
            }
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            Assert.Equal(19_050, shadow.Distance!.Value);
            Assert.Equal(5_400_000, shadow.Direction!.Value);
            Assert.Equal(A.RectangleAlignmentValues.BottomRight, shadow.Alignment!.Value);
            Assert.Equal("16324F", shadow.GetFirstChild<A.RgbColorModelHex>()!.Val!.Value);
            Assert.Equal(72_000, shadow.Descendants<A.Alpha>().Single().Val!.Value);
        }
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/text-shadow-alignment.pptx",
            },
        });
        ...
        Assert.Equal("br", reprojectedShadow["alignment"]!.GetValue<string>());
        Assert.Equal("br", reprojectedElement["nativeRef"]!["leaves"]!.AsArray()
            .Select(leaf => leaf!.AsObject())
            .Single(leaf => leaf["kind"]!.GetValue<string>() == "textShadowAlignment")["value"]!.GetValue<string>());
        ...
        invalidProgram["pages"]![0]!["elements"]![0]!["nativeRef"]!["leaves"]!.AsArray()
            .Single(leaf => leaf!["kind"]!.GetValue<string>() == "textShadowAlignment")!["value"] = "invalid";
    ...
    [Fact]
    public void PpjSourceBoundTextShadowAlignmentLeavesStayOpaqueForUnsupportedGraphs()
    {
        var authored = CompileShadow(ShadowProgram(alignment: "ctr"));
        Assert.True(authored.Ok, Diagnostics(authored));
        var source = RemoveEmbeddedPpj(authored.File.ToByteArray());
        ...
        foreach (var (label, mutate) in new[]
        {
            ...
            ("missing-alignment", (Action<A.OuterShadow>)(shadow => shadow.Alignment = null)),
            ("invalid-alignment", (Action<A.OuterShadow>)(shadow => shadow.SetAttribute(new OpenXmlAttribute("algn", "", "invalid")))),
        ...
        })
        {
            ...
            var projected = Invoke(new CodecRequest
            {
                ...
                PresentationProgram = new PresentationProgramRequest
                {
                    SourceUri = $"deck.assets/source/text-shadow-alignment-{label}.pptx",
                },
            });
            if (label == "invalid-alignment")
            {
        ...
        var siblingAuthored = CompileShadow(ShadowProgram(alignment: "ctr", siblingGlow: true));
        ...
        var siblingProjected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/source/text-shadow-alignment-sibling.pptx",
            },
        });
    ...
    private static JsonObject ShadowProgram(string alignment, bool siblingGlow = false)
    {
    ...
    private static CodecResponse CompileShadow(JsonObject program) => Invoke(new CodecRequest
    {
        ProtocolVersion = CodecProtocol.ProtocolVersion,
        Operation = CodecOperation.CompilePpjToPptx,
        Family = ArtifactFamily.Presentation,
