...
{
    [Fact]
    public void PpjSourceBoundPictureShape3dSceneBackdropNormalDyLeafEditsAndReprojects()
    {
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest { SourceUri = "deck.assets/source/picture-backdrop-normal-dy.pptx" },
        });
        ...
        var target = Assert.Single(image["nativeRef"]!["leaves"]!.AsArray().Select(leaf => leaf!.AsObject())
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "shape3dSceneBackdropNormalDyEmu"));
        Assert.Equal(2, target["value"]!.GetValue<long>());
        target["value"] = 3;
        ...
        Assert.Contains("<a:norm dx=\"1\" dy=\"3\" dz=\"3\"", xml, StringComparison.Ordinal);
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest { SourceUri = "deck.assets/edited/picture-backdrop-normal-dy.pptx" },
        });
        ...
        var reprojectedTarget = Assert.Single(reprojectedImage["nativeRef"]!["leaves"]!.AsArray().Select(leaf => leaf!.AsObject())
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "shape3dSceneBackdropNormalDyEmu"));
        Assert.Equal(3, reprojectedTarget["value"]!.GetValue<long>());
    }
}
