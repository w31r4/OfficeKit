...
The PPJ native leaf `shape3dSceneCameraRotationRevolution60000` already covers
strict imported shapes, but the same complete camera rotation on a picture is
still opaque. Revolution is one bounded token that can be edited while
preserving the required latitude/longitude context and the rest of the scene.
...
- Project and source-bound-edit only a strict
  `p:pic/p:spPr/a:scene3d/a:camera/a:rot/@rev` token.
...
- `ppj-source-bound-picture-shape-3d-scene-camera-rotation-revolution`: Bounded
  projection and source-bound editing of picture camera rotation revolution.
