...
Use this style when the brief matches **industry research; consulting presentation**. It is designed for
executives, strategy teams, decision makers. Its visual premise is Cyan evidence and editorial breadth support industry research. The style is a
...
**question → hypothesis → framework → evidence → implication → recommendation → action**. This style is not a good fit for **minimal stage keynote**;
...
- Use **#FFFFFF** as the main field, **#17324B** for primary type, **#0C9ED0** for the decisive signal, **#DE784B** for secondary evidence, and **#DCE8EE** for rules or context.
...
- The recurring structure is **grid, variable page silhouettes, documentary imagery**. Vary the silhouette between
  cover, evidence, data, process, comparison, and decision pages rather than
  repeating a component grid.
- Use the grid motif only when it carries the page's argument; keep all labels and evidence legible.
...
- **Page skeleton.** Use a broad research spread: a conclusion-led title, a narrow navigation cue, then an asymmetric evidence area that can switch between two columns and a wide chart. A chapter image may sit above the body, never behind dense labels.
- **Type rhythm.** Use an editorial sans with a restrained display weight; keep chart labels and methodology smaller but fully readable. Make the central thesis more prominent than the chapter ornament.
- **Evidence carriers.** Use market maps, benchmark lines, dual charts, comparison tables, and a short synthesis rail. Match scales before comparing series and label the selected comparison directly.
- **Images and layers.** Use documentary industry imagery only for context or a chapter break. External images need a clear role and source; do not add a gallery to fill the wide page.
- **Pacing.** Let a wide survey page introduce the field, then move through narrower evidence pages and end with a concise implication. Vary chart and text silhouettes while keeping the evidence baseline stable.
- **Selection rule.** Choose this style for industry research, market scans, and consulting analysis that must feel expansive but still evidence-led.

## Signature moves and variation limits

- Let **grid** recur as a quiet identity cue on selected pages; it must
