# Design: source-bound accent2 shade

## Context

The existing source-preserving theme path recognizes one direct accent1
transform and the direct accent2 tint leaf at a time, then patches the existing
DrawingML token. This change applies the same ownership boundary to accent2
shade without broadening the theme graph or changing authored six-role
lowering.
...
- Accent2 transforms other than `shade`, transforms on accent3 through
  accent6, inherited or effect theme state, or arbitrary theme XML editing.
- Creating or deleting a missing shade node, changing multiple theme fields,
...
1. **Use the existing native transform field.**
   ...
   DrawingML 0..100000 integer and the authored compiler already emits it.
   The importer records a role of `accent2`; the projector converts by
   dividing by 100000, and the source-bound compiler rounds by
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:shade`. The source writer patches `Accent2Color` only when the authored
   transform is the one existing accent2 shade and rejects all other transform
   flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent2Shade` is accepted only with
   `accentTransforms.accent2.shade` in the source-issued capability. This
