...
The source-bound accent palette currently accepts only six-digit direct RGB leaves for accent3 through accent6. Authored PPJ already accepts six- or eight-digit theme colors and lowers the trailing byte to `a:alpha`; the imported profile does not yet preserve that child for accent3.

## Goals / Non-Goals

**Goals:**

- Read `accent3` as `#RRGGBBAA` when its direct `srgbClr` has exactly one canonical `a:alpha` child.
- Keep the existing six accent object and `setThemeAccent3Color` field authority.
...
4. **Patch one field owner.** The writer compares RGB and alpha separately, updates only those two attributes under accent3, and leaves the other five accents and all theme descendants source-owned.
