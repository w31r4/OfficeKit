...
Expose one complete, field-qualified source-bound PPJ owner for an existing presentation theme dark2 color while preserving the imported theme graph and the existing dark1/light1 owners.
...
A source-bound PPJ compile SHALL accept a changed existing `design.theme.colorRoles.dark2` only with the freshly projected theme native reference and its `setThemeColorRoleDark2` capability. The export SHALL replace only the owning ThemePart's `a:dk2/a:srgbClr/@val`, report that ThemePart as changed, and preserve all other theme roles, accents, descendants, relationships, and package members.
