...
assert.ok(HELP_CATALOG.some((item) => item.name === "shape.setAccessibilityMetadata"));
assert.match(HELP_CATALOG.find((item) => item.name === "slide.shapes.add")?.schema?.parameters?.accessibility?.description || "", /1 through 1024.*p:nvSpPr\/p:cNvPr\/@title.*@descr.*visible shape text.*layout/i);
assert.match(HELP_CATALOG.find((item) => item.name === "shape.setAccessibilityMetadata")?.summary || "", /ordinary shape.*visible text.*layout.*drawing order.*relationships.*irregular.*fail closed/i);
assert.match(HELP_CATALOG.find((item) => item.name === "shape.setAccessibilityMetadata")?.schema?.returns?.shape?.description || "", /ordinary p:sp.*frame.*fill.*line.*child-free.*canonical.*unknown attributes.*fail closed/i);
assert.equal(HELP_CATALOG.find((item) => item.name === "shape.setAccessibilityMetadata")?.schema?.parameters?.title?.type, "string | null");
...
assert.equal(formulaCatalog.length, 113);
...
assert.equal(presentationCatalog.length, 76);
