...
This change applies the same ownership boundary to accent4 green-channel offset
...
- Accent4 transforms other than `greenOff`, transforms on other accent roles, inherited or effect theme state, or arbitrary theme XML editing.
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:greenOff`. The source writer patches `Accent4Color` only when the authored
   transform is the one existing accent4 greenOff and rejects all other transform
   flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent4GreenOff` is accepted only with
   `accentTransforms.accent4.greenOff` in the source-issued capability. This
