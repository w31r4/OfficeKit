...
- Give a direct literal `a:ahXY/@maxY` bound a stable native index and explicit
  EMU value.
- Keep guide identity, x bounds, the paired minimum bound, position, handle
...
- Editing x bounds, minimum y bounds, handle position, guide identity,
...
3. **Keep the paired range graph intact.** The writer proves the direct
   `ahXY` attribute structure and changes only `maxY`; it does not add/remove
   the range or edit `minY`, x bounds, current adjustment, or other bounds. The
