...
- Owner: `p:sp/p:spPr/a:custGeom/a:pathLst/a:path/a:cubicBezTo/a:pt[1]/@y`.
- PPJ surface: `shape.nativeRef.leaves[]`.
- Native leaf: `customGeometryPathCubicControl2Y`.
...
- The geometry must pass the existing bounded custom-geometry recognition
  ...
  attribute-only, and the second control-point y must be a canonical signed
...
one end point, the canonical numeric second-control-point y token, and the
requested changed bounded coordinate. The writer then replaces only the
selected `y` value token on the second point.
...
- No line-to, move-to, quadratic, cubic first-control-point or end-point,
  second-control-point x, formula/reference, path property, or multi-path
  topology edits.
...
cubic command, removes embedded PPJ, projects the cubic second-control-point y
