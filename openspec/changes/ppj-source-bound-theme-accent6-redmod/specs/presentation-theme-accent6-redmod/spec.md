...
direct accent6 red-channel modulation token while preserving the rest of an
imported theme.

## ADDED Requirements

### Requirement: Strict source-bound accent6 redMod projection

The PPTX projector MUST expose
`design.theme.accentTransforms.accent6.redMod` and
`setThemeAccent6RedMod` only when the canonical shared ThemePart has one
direct accent6 RGB leaf with exactly one direct `a:redMod/@val` child, no
...
#### Scenario: Project a direct accent6 redMod leaf

- **GIVEN** a canonical imported ThemePart with accent6 `334455` and
  `a:redMod/@val="25000"`
- **WHEN** the presentation is projected to PPJ
- **THEN** the theme contains `accentTransforms.accent6.redMod` equal to
  `0.25`
- **AND** the native reference contains `setThemeAccent6RedMod` authorized
  for `accentTransforms.accent6.redMod`

#### Scenario: Reject unsupported accent6 redMod topology
...
- **THEN** the source-bound accent6 redMod field and capability are omitted

### Requirement: Source-bound accent6 redMod editing

The source-preserving compiler MUST accept one authorized change to the
existing `design.theme.accentTransforms.accent6.redMod` field, convert a
...
#### Scenario: Edit and reproject one accent6 redMod token

- **GIVEN** a projected direct accent6 redMod owner at `0.25`
...
#### Scenario: Fail closed for unsafe accent6 redMod edits

- **GIVEN** a source-bound accent6 redMod owner
