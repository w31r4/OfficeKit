# Design: source-bound accent5 lumOff
...
accent5 luminance offset without broadening the theme graph or changing
...
- Accent5 transforms other than `lumOff`, transforms on accent5 through
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:lumOff`. The source writer patches `Accent5Color` only when the authored
   transform is the one existing accent5 lumOff and rejects all other
   transform flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent5LumOff` is accepted only with
   `accentTransforms.accent5.lumOff` in the source-issued capability. This
