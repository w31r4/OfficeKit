# Proposal: expose one custom-geometry cubic second-control-point x coordinate

F-04 still keeps custom path command coordinates source-owned. Add one narrow
native leaf for the direct literal `a:cubicBezTo/a:pt[1]/@x` so an agent can
move the second control point of one cubic segment horizontally without
rewriting the rest of the path graph.

## What Changes

- Expose each direct literal cubic second-control-point x coordinate as
  `customGeometryPathCubicControl2X` in the owning path/command order.
- Allow a source-bound PPJ edit to replace only that point's `x` attribute in
  the owning SlidePart.
- Keep its y, the first control point, end point, path properties, other
...
- `ppj-source-bound-custom-geometry-path-cubic-control2-x`: a bounded native
  leaf for editing one direct cubic second-control-point x coordinate.
