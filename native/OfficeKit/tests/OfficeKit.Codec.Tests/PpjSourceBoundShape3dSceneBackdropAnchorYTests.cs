...
{
    [Fact]
    public void PpjSourceBoundShape3dSceneBackdropAnchorYLeafEditsAndReprojects()
    {
        var authored = Invoke(ExportRequest());
        Assert.True(authored.Ok, Diagnostics(authored));
        var source = AddShape3dSceneBackdropAnchorY(RemoveEmbeddedPpj(authored.File.ToByteArray()), "200");
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            {
                SourceUri = "deck.assets/source/shape-3d-scene-backdrop-anchor-y.pptx",
            },
        });
        ...
        Assert.Contains("<a:anchor x=\"100\" y=\"400\" z=\"300\"", editedXml, StringComparison.Ordinal);
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/shape-3d-scene-backdrop-anchor-y.pptx",
            },
        });
        ...
        var reprojectedTarget = Assert.Single(reprojectedShape["nativeRef"]!["leaves"]!.AsArray()
            .Select(leaf => leaf!.AsObject())
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "shape3dSceneBackdropAnchorYEmu"));
        Assert.Equal(400, reprojectedTarget["value"]!.GetValue<long>());
    }

    private static byte[] AddShape3dSceneBackdropAnchorY(byte[] source, string anchorY)
    {
