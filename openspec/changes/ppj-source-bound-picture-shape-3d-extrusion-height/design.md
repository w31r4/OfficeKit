...
- Owner: `p:pic/p:spPr/a:sp3d/@extrusionH`.
- Native leaf: existing `shape3dExtrusionHeightEmu`.
- Source model: add an additive `PresentationImage.shape_3d_extrusion_height_emu`
...
- The picture owner must have exactly one direct child-free `a:sp3d` with a
  canonical non-negative signed 32-bit `extrusionH` token. Other direct scalar
  ...
  picture profile.
- Picture projection stores the extrusion height, issues the existing native
