# ppj-source-bound-shape-3d-backdrop-anchor-z Specification

## ADDED Requirements

### Requirement: Strict direct backdrop anchor Z projection

The presentation projection MUST expose
`shape3dSceneBackdropAnchorZEmu` only when a shape has one direct `a:scene3d`
...
standard bounded signed EMU range. The leaf value MUST be the anchor's `z`
coordinate. Existing sibling 3-D owners MAY remain and MUST be preserved.

#### Scenario: Project a backdrop anchor Z coordinate
...
- **THEN** exactly one `shape3dSceneBackdropAnchorZEmu` native leaf is emitted
  with value `300`
...
- **WHEN** the PPTX is projected to PPJ
- **THEN** no `shape3dSceneBackdropAnchorZEmu` leaf is emitted

### Requirement: Source-bound backdrop anchor Z token splice

The PPJ edit plan MUST permit `shape3dSceneBackdropAnchorZEmu` only when the
...
replace only `a:backdrop/a:anchor/@z` in the owning SlidePart, preserve all
other scene and package content, and allow the new coordinate to be recovered
on a second projection.

#### Scenario: Edit and reproject backdrop anchor Z

- **GIVEN** a projected strict backdrop owner with anchor `z="300"`
- **WHEN** PPJ changes the leaf to `600`
- **THEN** only the owning SlidePart changes, the package remains Open XML
  valid, the anchor token becomes `600`, and a second projection returns `600`
...
- **WHEN** an edit targets `shape3dSceneBackdropAnchorZEmu`
