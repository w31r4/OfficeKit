...
| `slide.continuationCapability` | api | Report the current authoring profile for this slide. Source-free slides use full-authoring. A newly duplicated imported slide is pending-clone and must be exported and reimported before further edits. A reopened imported or source-derived slide uses bounded-overlay: one clean SlidePart mutation class per export may append top-layer textbox, rect, roundRect, ellipse, or embedded rectangular image elements while preserving the existing raw shape tree; export/reimport before combining that overlay with native-leaf or other SlidePart edits. |
...
| `slide.images.add` | api | Add an inspectable image facade with non-visible title/description/decorative metadata, the compatible alt description alias, embedded data, contain/cover/stretch fitting, explicit crop, frame, direct rotation/flips, layout JSON, crop-aware SVG preview, and PPTX output. Source-free slides support the full documented profile. An imported or source-derived slide may append one or more embedded rectangular images when slide.continuationCapability reports ready bounded-overlay; the raw source shape tree is otherwise preserved, and the overlay must be the only SlidePart mutation class in that export. OfficeKit maps the bounded rectangular profile to native DrawingML p:cNvPr, the Office 2019+ decorative extension, and a:srcRect. |
...
| `slide.shapes.add` | api | Add a shape/textbox, free-positioned p:sp line, or exact-site p:cxnSp connector with optional non-visible title/description/decorative metadata. Source-free slides support the full documented profile. An imported or source-derived slide may append only textbox, rect, roundRect, or ellipse when slide.continuationCapability reports ready bounded-overlay; the raw source shape tree is otherwise preserved, and the overlay must be the only SlidePart mutation class in that export. Lines support bounded dash/line-end/cap/join styling; custom geometry supports ordered adjustment/guide formulas, XY/polar adjustment handles, and connection sites. A free line is defined by its start-plus-delta frame; only a connector retains target-plus-site identity. |
...
Report the current authoring profile for this slide. Source-free slides use full-authoring. A newly duplicated imported slide is pending-clone and must be exported and reimported before further edits. A reopened imported or source-derived slide uses bounded-overlay: one clean SlidePart mutation class per export may append top-layer textbox, rect, roundRect, ellipse, or embedded rectangular image elements while preserving the existing raw shape tree; export/reimport before combining that overlay with native-leaf or other SlidePart edits.

**Schema returns:**

- `capability` (object) — Defensive { sourceBound, ready, profile, requiresExportReopen, oneSlideMutationPerExport?, shapeGeometries?, embeddedImage?, sourceRevisionSha256? } authoring state. full-authoring applies to source-free slides. pending-clone requires export/reimport before any edit. bounded-overlay permits top-layer textbox/rect/roundRect/ellipse and embedded rectangular image additions while token-preserving the existing source shape tree; it does not authorize connector, group, table, chart, placeholder, custom-geometry, or arbitrary native additions. Native-leaf and other SlidePart edits require a separate reviewed revision.
