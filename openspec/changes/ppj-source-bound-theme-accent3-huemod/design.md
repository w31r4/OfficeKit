...
transform and direct accent3 tint, shade, luminance, alpha, saturation, red,
and blue transform leaves at a time, then patches the existing DrawingML token.
This change applies the same ownership boundary to accent3 hue modulation
...
- Accent3 transforms other than `hueMod`, transforms on other accent roles, inherited or effect theme state, or arbitrary theme XML editing.
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:hueMod`. The source writer patches `Accent3Color` only when the authored
   transform is the one existing accent3 hueMod and rejects all other transform
   flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent3HueMod` is accepted only with
   `accentTransforms.accent3.hueMod` in the source-issued capability. This
