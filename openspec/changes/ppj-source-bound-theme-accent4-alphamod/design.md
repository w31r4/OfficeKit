...
transform and the direct accent4 tint, shade, luminance modulation, and
luminance offset leaves at a time, then patches the existing DrawingML token.
This change applies the same ownership boundary to accent4 alpha modulation
...
- Accent4 transforms other than `alphaMod`, transforms on accent4 through
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:alphaMod`. The source writer patches `Accent4Color` only when the authored
   transform is the one existing accent4 alphaMod and rejects all other
   transform flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent4AlphaMod` is accepted only with
   `accentTransforms.accent4.alphaMod` in the source-issued capability. This
