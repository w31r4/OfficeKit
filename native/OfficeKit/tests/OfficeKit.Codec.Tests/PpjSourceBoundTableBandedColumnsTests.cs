...
{
    [Fact]
    public void PpjSourceBoundTableBandedColumnsEditsLeafAndReprojects()
    {
        ...
        {
            Id = "presentation/slide/1/table/native-banded-columns",
            Name = "Native banded columns table",
            Table = table,
        });
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            Assert.True(nativeTable.TableProperties!.BandColumn!.Value);
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            {
                SourceUri = "table-banded-columns/source.pptx",
            },
        });
        ...
        Assert.True(projectedTable["style"]!["bandedColumns"]!.GetValue<bool>());
        var leaves = projectedTable["nativeRef"]!["leaves"]!.AsArray()
            .Select(leaf => leaf!.AsObject())
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "tableBandedColumns")
            .ToArray();
        ...
        {
            var nativeTable = Assert.Single(package.PresentationPart!.SlideParts.Single().Slide!.Descendants<A.Table>());
            Assert.False(nativeTable.TableProperties!.BandColumn!.Value);
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "table-banded-columns/edited.pptx",
            },
        });
        ...
        var reprojectedTable = reprojectedProgram["pages"]!.AsArray()
            ...
            .Select(item => item!.AsObject())
            .Single(item => item["type"]!.GetValue<string>() == "table");
        Assert.False(reprojectedTable["style"]!["bandedColumns"]!.GetValue<bool>());
        Assert.False(reprojectedTable["nativeRef"]!["leaves"]!.AsArray().Single(leaf =>
            leaf!["kind"]!.GetValue<string>() == "tableBandedColumns")["value"]!.GetValue<bool>());
    }
}
