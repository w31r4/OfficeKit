...
- Expose one canonical signed `a:backdrop/a:norm/@dz` coordinate in EMU.
- Replace only `backdrop/a:norm/@dz` in the owning SlidePart.
...
- Reuse `TryReadSceneBackdropNormalDz` and its requested-value validator so
...
- Token-splice only the direct `backdrop/a:norm/@dz` attribute.

# Risks / Trade-offs

- [Risk] A normal Z token without a complete backdrop owner could be mistaken
