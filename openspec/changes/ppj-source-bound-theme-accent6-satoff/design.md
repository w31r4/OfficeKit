...
transform and the direct accent6 tint, shade, luminance, alpha, and saturation
modulation leaves at a time, then patches the existing DrawingML token. This
change applies the same ownership boundary to accent6 saturation offset without
...
- Accent6 transforms other than `satOff`, transforms on accent6 through
...
1. **Use the existing native transform field.**
   ...
   compiler already emits it. The importer records a role of `accent6`; the
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:satOff`. The source writer patches `Accent6Color` only when the authored
   transform is the one existing accent6 satOff and rejects all other transform
   flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent6SatOff` is accepted only with
   `accentTransforms.accent6.satOff` in the source-issued capability. This
