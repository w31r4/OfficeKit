# ppj-source-bound-shape-3d-camera-rotation-longitude Specification

## ADDED Requirements

### Requirement: Strict direct camera rotation longitude projection

The presentation projection MUST expose
`shape3dSceneCameraRotationLongitude60000` only when a shape has one direct
...
#### Scenario: Project a camera rotation longitude
...
- **THEN** exactly one `shape3dSceneCameraRotationLongitude60000` native leaf
...
- **WHEN** the PPTX is projected to PPJ
- **THEN** no `shape3dSceneCameraRotationLongitude60000` leaf is emitted

### Requirement: Source-bound camera rotation longitude token splice

The PPJ edit plan MUST permit
`shape3dSceneCameraRotationLongitude60000` only when the source-bound shape
...
#### Scenario: Edit and reproject camera rotation longitude

- **GIVEN** a projected strict scene owner with camera `lon="0"`
...
- **WHEN** an edit targets `shape3dSceneCameraRotationLongitude60000`
