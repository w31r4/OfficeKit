# Proposal: add source-bound paragraph default-text outer-shadow distance to PPJ

Expose a bounded `paragraph.style.defaultText.shadow.distance` owner for an
...
change an existing direct `a:outerShdw/@dist` without rewriting the
surrounding slide.

This slice does not infer omitted geometry or rebuild an effect list. Only a
strict direct `a:outerShdw` with an existing bounded `dist` is editable;
