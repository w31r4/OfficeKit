...
Use this style when the brief matches **technical courseware; academic presentation**. It is designed for
researchers, students, technical reviewers. Its visual premise is Blue rules make each step of the lesson traceable. The style is a
...
**question → method → observation → derivation or comparison → interpretation → limitations → conclusion**. This style is not a good fit for **luxury launch**;
...
- Use **#F8FAFC** as the main field, **#132A3A** for primary type, **#1769D2** for the decisive signal, **#78B9E8** for secondary evidence, and **#DCE6EF** for rules or context.
...
- The recurring structure is **axis, variable page silhouettes, native data graphics**. Vary the silhouette between
  cover, evidence, data, process, comparison, and decision pages rather than
  repeating a component grid.
- Use axes and short ticks as a teaching scaffold. Align explanations to the same reference line and reserve color for the variable or step being discussed.
...
The expected density is **medium**: complexity should come from relationships, not tiny type. Alternate page
...
- **Page skeleton.** Construct each lesson as a traceable board: title and small navigation at the top, a definition or starting point on the left, and the main diagram, equation, or example on the right. Use a blue rule to keep the page anchored.
- **Type rhythm.** Use a clear sans for teaching copy and a monospaced or math face only for code, equations, and symbols. Increase the scale of the rule or result, not the amount of decoration.
- **Evidence carriers.** Use process diagrams, document/database pipelines, small multiples, comparisons, error bars, and short tables. Directly label axes, units, and the step that changes the conclusion.
- **Images and layers.** Line-art icons, a restrained screenshot, or one contextual photograph can ground an abstract lesson. Do not use a photo as a text background.
- **Pacing.** Teach one transformation at a time. Start with a question, reveal method, show evidence, and close with a check or takeaway before moving to the next section.
- **Selection rule.** Choose this style for technical training, engineering walkthroughs, and research presentations that need a reproducible reading path.

## Signature moves and variation limits

- Let **axis** recur as a quiet identity cue on selected pages; it must
