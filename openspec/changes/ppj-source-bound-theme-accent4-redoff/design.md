...
This change applies the same ownership boundary to accent4 red-channel offset
...
- Accent4 transforms other than `redOff`, transforms on accent4 through
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:redOff`. The source writer patches `Accent4Color` only when the authored
   transform is the one existing accent4 redOff and rejects all other transform
   flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent4RedOff` is accepted only with
   `accentTransforms.accent4.redOff` in the source-issued capability. This
