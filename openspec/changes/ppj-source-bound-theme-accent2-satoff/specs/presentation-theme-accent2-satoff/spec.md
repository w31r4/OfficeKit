...
direct accent2 saturation offset token while preserving the rest of an
imported theme.

## ADDED Requirements

### Requirement: Strict source-bound accent2 satOff projection

The PPTX projector MUST expose
`design.theme.accentTransforms.accent2.satOff` and
`setThemeAccent2SatOff` only when the canonical shared ThemePart has one
direct accent2 RGB leaf with exactly one direct `a:satOff/@val` child, no
...
#### Scenario: Project a direct accent2 satOff leaf

- **GIVEN** a canonical imported ThemePart with accent2 `223344` and
  `a:satOff/@val="-25000"`
- **WHEN** the presentation is projected to PPJ
- **THEN** the theme contains `accentTransforms.accent2.satOff` equal to
  `-0.25`
- **AND** the native reference contains `setThemeAccent2SatOff` authorized
  for `accentTransforms.accent2.satOff`

#### Scenario: Reject unsupported accent2 satOff topology
...
- **THEN** the source-bound accent2 satOff field and capability are omitted

### Requirement: Source-bound accent2 satOff editing

The source-preserving compiler MUST accept one authorized change to the
existing `design.theme.accentTransforms.accent2.satOff` field, convert a
...
#### Scenario: Edit and reproject one accent2 satOff token

- **GIVEN** a projected direct accent2 satOff owner at `-0.25`
...
#### Scenario: Fail closed for unsafe accent2 satOff edits

- **GIVEN** a source-bound accent2 satOff owner
