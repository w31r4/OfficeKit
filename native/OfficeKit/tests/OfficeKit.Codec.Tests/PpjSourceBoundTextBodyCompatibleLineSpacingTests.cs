...
{
    [Fact]
    public void PpjSourceBoundTextBodyCompatibleLineSpacingEditsLeafAndReprojects()
    {
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            Assert.True(bodyProperties.CompatibleLineSpacing!.Value);
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/source/text-body-compatible-line-spacing.pptx",
            },
        });
        Assert.True(projected.Ok, Diagnostics(projected));
        var projectedProgram = JsonNode.Parse(projected.PresentationProgram.ProgramJson.ToByteArray())!.AsObject();
        var projectedShape = projectedProgram["pages"]![0]!["elements"]!.AsArray().Single()!.AsObject();
        Assert.True(projectedShape["textStyle"]!["compatibleLineSpacing"]!.GetValue<bool>());
        var leaves = projectedShape["nativeRef"]!["leaves"]!.AsArray()
            .Select(leaf => leaf!.AsObject())
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "textBodyCompatibleLineSpacing")
            .ToArray();
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            var nativeShape = package.PresentationPart!.SlideParts.Single().Slide!.Descendants<P.Shape>().Single();
            var bodyProperties = nativeShape.TextBody!.GetFirstChild<A.BodyProperties>()!;
            Assert.False(bodyProperties.CompatibleLineSpacing!.Value);
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/text-body-compatible-line-spacing.pptx",
            },
        });
        ...
        Assert.False(reprojectedShape["textStyle"]!["compatibleLineSpacing"]!.GetValue<bool>());
        Assert.False(reprojectedShape["nativeRef"]!["leaves"]!.AsArray().Single(leaf =>
            leaf!["kind"]!.GetValue<string>() == "textBodyCompatibleLineSpacing")["value"]!.GetValue<bool>());

        var absentSource = ReplaceZipText(source, "ppt/slides/slide1.xml", xml =>
            xml.Replace("compatLnSpc=\"1\"", string.Empty, StringComparison.Ordinal));
        var absent = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/source/text-body-compatible-line-spacing-absent.pptx",
            },
        });
        ...
        Assert.DoesNotContain(absentShape["nativeRef"]!["leaves"]!.AsArray(), leaf =>
            leaf!["kind"]!.GetValue<string>() == "textBodyCompatibleLineSpacing");
    }
}
