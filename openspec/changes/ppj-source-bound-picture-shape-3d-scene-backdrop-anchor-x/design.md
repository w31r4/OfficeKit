...
- Expose one canonical signed `a:backdrop/a:anchor/@x` coordinate in EMU.
- Replace only `backdrop/a:anchor/@x` in the owning SlidePart.
...
- Reuse `TryReadSceneBackdropAnchorX` and its requested-value validator so
...
- Token-splice only the direct `backdrop/a:anchor/@x` attribute.

# Risks / Trade-offs

- [Risk] An anchor X token without a complete backdrop owner could be mistaken
