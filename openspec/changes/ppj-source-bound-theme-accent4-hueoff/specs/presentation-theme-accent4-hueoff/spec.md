...
direct accent4 hue-offset token while preserving the rest of an imported theme.

## ADDED Requirements

### Requirement: Strict source-bound accent4 hueOff projection

The PPTX projector MUST expose
`design.theme.accentTransforms.accent4.hueOff` and
`setThemeAccent4HueOff` only when the canonical shared ThemePart has one
direct accent4 RGB leaf with exactly one direct `a:hueOff/@val` child, no
...
#### Scenario: Project a direct accent4 hueOff leaf
...
- **THEN** the theme contains `accentTransforms.accent4.hueOff` equal to
  `210`
- **AND** the native reference contains `setThemeAccent4HueOff` authorized
  for `accentTransforms.accent4.hueOff`

#### Scenario: Reject unsupported accent4 hueOff topology
...
- **THEN** the source-bound accent4 hueOff field and capability are omitted

### Requirement: Source-bound accent4 hueOff editing

The source-preserving compiler MUST accept one authorized change to the
existing `design.theme.accentTransforms.accent4.hueOff` field, convert a
...
#### Scenario: Edit and reproject one accent4 hueOff token

- **GIVEN** a projected direct accent4 hueOff owner at `210`
...
#### Scenario: Fail closed for unsafe accent4 hueOff edits

- **GIVEN** a source-bound accent4 hueOff owner
