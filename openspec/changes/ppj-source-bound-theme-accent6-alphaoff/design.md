...
transform and the direct accent6 tint, shade, luminance modulation, luminance
offset, and alpha modulation leaves at a time, then patches the existing
DrawingML token. This change applies the same ownership boundary to accent6
...
- Accent6 transforms other than `alphaOff`, transforms on accent6 through
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:alphaOff`. The source writer patches `Accent6Color` only when the
   authored transform is the one existing accent6 alphaOff and rejects all
   other transform flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent6AlphaOff` is accepted only with
   `accentTransforms.accent6.alphaOff` in the source-issued capability. This
