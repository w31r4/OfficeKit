...
direct accent4 alpha offset token while preserving the rest of an imported
theme.

## ADDED Requirements

### Requirement: Strict source-bound accent4 alphaOff projection

The PPTX projector MUST expose
`design.theme.accentTransforms.accent4.alphaOff` and
`setThemeAccent4AlphaOff` only when the canonical shared ThemePart has one
direct accent4 RGB leaf with exactly one direct `a:alphaOff/@val` child, no
...
#### Scenario: Project a direct accent4 alphaOff leaf
...
- **THEN** the theme contains `accentTransforms.accent4.alphaOff` equal to
  `-0.25`
- **AND** the native reference contains `setThemeAccent4AlphaOff` authorized
  for `accentTransforms.accent4.alphaOff`

#### Scenario: Reject unsupported accent4 alphaOff topology
...
- **THEN** the source-bound accent4 alphaOff field and capability are omitted

### Requirement: Source-bound accent4 alphaOff editing

The source-preserving compiler MUST accept one authorized change to the
existing `design.theme.accentTransforms.accent4.alphaOff` field, convert a
...
#### Scenario: Edit and reproject one accent4 alphaOff token

- **GIVEN** a projected direct accent4 alphaOff owner at `-0.25`
...
#### Scenario: Fail closed for unsafe accent4 alphaOff edits

- **GIVEN** a source-bound accent4 alphaOff owner
