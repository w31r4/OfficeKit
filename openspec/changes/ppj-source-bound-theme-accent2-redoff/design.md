...
transform and the direct accent2 tint, shade, luminance, alpha, saturation, and
red modulation leaves at a time, then patches the existing DrawingML token.
This change applies the same ownership boundary to accent2 red-channel offset
...
- Recognize only a canonical shared ThemePart whose accent2 color is a direct
  six-digit RGB leaf with one direct `a:redOff/@val` child.
...
- Accent2 transforms other than `redOff`, transforms on accent3 through
  accent6, inherited or effect theme state, or arbitrary theme XML editing.
- Creating or deleting a missing redOff node, changing multiple theme fields,
  or claiming PowerPoint host color rendering.

## Decisions

1. **Use the existing native transform field.**
   `PresentationThemeColorTransform.RedOffsetThousandth` already represents
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:redOff`. The source writer patches `Accent2Color` only when the authored
   transform is the one existing accent2 redOff and rejects all other transform
   flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent2RedOff` is accepted only with
   `accentTransforms.accent2.redOff` in the source-issued capability. This
