...
{
    [Fact]
    public void PpjSourceBoundLiteralCustomGeometryPathArcSweepAngleLeafEditsAndReprojects()
    {
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            {
                SourceUri = "deck.assets/source/custom-geometry-path-arc-sweep-angle.pptx",
            },
        });
        ...
        var arcLeaves = projectedShape["nativeRef"]!["leaves"]!.AsArray()
            .Select(leaf => leaf!.AsObject())
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "customGeometryPathArcSweepAngle60000")
            .ToArray();
        var target = Assert.Single(arcLeaves);
        Assert.Equal(5_400_000, target["value"]!.GetValue<long>());
        ...
        Assert.Contains("wR=\"200\" hR=\"150\" stAng=\"0\" swAng=\"7200000\"", editedXml, StringComparison.Ordinal);
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/custom-geometry-path-arc-sweep-angle.pptx",
            },
        });
        ...
        var reprojectedLeaves = reprojectedShape["nativeRef"]!["leaves"]!.AsArray()
            ...
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "customGeometryPathArcSweepAngle60000")
            .ToArray();
        Assert.Equal(7_200_000, Assert.Single(reprojectedLeaves)["value"]!.GetValue<long>());
    }
}
