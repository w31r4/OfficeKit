...
The native leaf `shape3dSceneLightRigDirection` already covers strict imported
shapes, but the same complete scene on a picture is still opaque. The
light-rig direction is one bounded token that can be edited while preserving
...
- Project and source-bound-edit only a strict
  `p:pic/p:spPr/a:scene3d/a:lightRig/@dir` token.
...
- `ppj-source-bound-picture-shape-3d-scene-light-rig-direction`: Bounded
  projection and source-bound editing of a picture light-rig direction.
