...
{
    [Fact]
    public void PpjSourceBoundTableFirstColumnEmphasisEditsLeafAndReprojects()
    {
        ...
        {
            Id = "presentation/slide/1/table/native-first-column",
            Name = "Native first-column table",
            Table = table,
        });
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            Assert.True(nativeTable.TableProperties!.FirstColumn!.Value);
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            {
                SourceUri = "table-first-column/source.pptx",
            },
        });
        ...
        Assert.True(projectedTable["style"]!["firstColumnEmphasis"]!.GetValue<bool>());
        var leaves = projectedTable["nativeRef"]!["leaves"]!.AsArray()
            .Select(leaf => leaf!.AsObject())
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "tableFirstColumnEmphasis")
            .ToArray();
        ...
        {
            var nativeTable = Assert.Single(package.PresentationPart!.SlideParts.Single().Slide!.Descendants<A.Table>());
            Assert.False(nativeTable.TableProperties!.FirstColumn!.Value);
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "table-first-column/edited.pptx",
            },
        });
        ...
        var reprojectedTable = reprojectedProgram["pages"]!.AsArray()
            ...
            .Select(item => item!.AsObject())
            .Single(item => item["type"]!.GetValue<string>() == "table");
        Assert.False(reprojectedTable["style"]!["firstColumnEmphasis"]!.GetValue<bool>());
        Assert.False(reprojectedTable["nativeRef"]!["leaves"]!.AsArray().Single(leaf =>
            leaf!["kind"]!.GetValue<string>() == "tableFirstColumnEmphasis")["value"]!.GetValue<bool>());
    }
}
