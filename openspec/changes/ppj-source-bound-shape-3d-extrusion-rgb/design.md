...
- Owner: `p:sp/p:spPr/a:sp3d/a:extrusionClr/a:srgbClr/@val`.
- Native leaf: `shape3dExtrusionRgb`.
- The owner must be the single direct `a:sp3d` child of `a:spPr`, with no
  ...
  `z`, `extrusionH`, `contourW`, and `prstMaterial`; `a:extrusionClr` has no
...
- Missing, malformed, duplicate, extension-bearing, transformed, alternate
  color-model, or otherwise ambiguous extrusion color remains source-owned.
