...
transform and the direct accent5 tint, shade, luminance, alpha, and saturation
leaves at a time, then patches the existing DrawingML token. This change applies
the same ownership boundary to accent5 green-channel offset without
...
- Accent5 transforms other than `greenOff`, transforms on accent5 through
...
1. **Use the existing native transform field.**
   `PresentationThemeColorTransform.GreenOffsetThousandth` already
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:greenOff`. The source writer patches `Accent5Color` only when the authored
   transform is the one existing accent5 greenOff and rejects all other transform
   flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent5GreenOff` is accepted only with
   `accentTransforms.accent5.greenOff` in the source-issued capability. This
