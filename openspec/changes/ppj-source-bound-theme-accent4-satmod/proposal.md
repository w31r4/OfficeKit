...
accent1 plus accent4 tint, shade, luminance, and alpha transforms. A direct
imported accent4 `a:satMod` remains opaque, leaving one concrete F-15
...
- Project a canonical shared ThemePart whose accent4 direct RGB leaf contains
  ...
  `design.theme.accentTransforms.accent4.satMod`, using a 0..1 fraction.
- Issue `setThemeAccent4SatMod` for that field and compile one authorized
...
- `presentation-theme-accent4-satmod`: Strict source-bound projection and
  source-preserving editing of one direct accent4 `a:satMod` leaf.
