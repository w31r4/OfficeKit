...
- `customPaths` (object[]) — For geometry custom, 1-64 literal DrawingML paths with positive integer width/height and bounded moveTo, lineTo, quadraticBezTo, cubicBezTo, arcTo, and close commands. arcTo uses positive widthRadius/heightRadius plus integer startAngle/sweepAngle in 1/60000-degree units, requires a current point, and limits non-zero sweep to one full turn. Formula geometry, guides, handles, connection sites, text rectangles, and path-specific paint overrides are not authored.
- `position` (object) — Pixel left/top/width/height frame.
