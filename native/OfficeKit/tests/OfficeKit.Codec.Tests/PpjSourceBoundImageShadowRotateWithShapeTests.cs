...
{
    [Fact]
    public void PpjSourceBoundImageShadowRotateWithShapeEditsLeafAndReprojects()
    {
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/source/image-shadow-rotate-with-shape.pptx",
            },
        });
        ...
        var rotateLeaves = projectedImage["nativeRef"]!["leaves"]!.AsArray()
            .Select(leaf => leaf!.AsObject())
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "imageShadowRotateWithShape")
            .ToArray();
        Assert.Single(rotateLeaves);
        Assert.True(rotateLeaves[0]["value"]!.GetValue<bool>());

        rotateLeaves[0]["value"] = false;
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/image-shadow-rotate-with-shape.pptx",
            },
        });
        ...
        var reprojectedImage = reprojectedProgram["pages"]![0]!["elements"]!.AsArray()
            .Select(item => item!.AsObject())
            .Single(item => item["name"]!.GetValue<string>() == "evidence identity");
        Assert.False(reprojectedImage["shadow"]!["rotateWithShape"]!.GetValue<bool>());
        Assert.False(reprojectedImage["nativeRef"]!["leaves"]!.AsArray().Single(leaf =>
            leaf!["kind"]!.GetValue<string>() == "imageShadowRotateWithShape")!["value"]!.GetValue<bool>());
    }
}
