...
Use this style when the brief matches **cultural pictorial; promotion presentation**. It is designed for
public audiences, communities, brand teams. Its visual premise is Full-scale documentary imagery carries the narrative; color appears as a pulse. The style is a
...
**big idea → audience truth → creative system → waves or experiences → measured outcome → next move**. This style is not a good fit for **dense compliance**;
...
- Use **#111214** as the main field, **#F2F0EA** for primary type, **#D53A45** for the decisive signal, **#C21E8B** for secondary evidence, and **#494A50** for rules or context.
...
- The recurring structure is **photo, variable page silhouettes, documentary imagery**. Vary the silhouette between
  cover, evidence, data, process, comparison, and decision pages rather than
  repeating a component grid.
- Let one documentary image carry the narrative when it is genuinely informative. Put claims on a tested, editable text plane and vary the crop rather than adding more images.
...
- **Page skeleton.** Split a black field from a hard-cropped documentary photograph, optionally mirroring the split across chapters. Put the title and explanation only on the black plane or tested darkened negative space.
- **Type rhythm.** Use large, tight sans titles with small micro-navigation and a lower-left page number. Keep labels short; white type must never fight bright photographic texture.
- **Evidence carriers.** This is image-led rather than chart-led: use a single photograph, an unequal image matrix for facets of one subject, and attribute labels. Do not force quantitative data into an ornamental image wall.
- **Images and layers.** Cinematic documentary photography is essential. Source it, crop it hard, and use a local darkening layer only where natural negative space does not protect the text.
- **Pacing.** Let image-led chapters create pauses around a small number of explanatory pages. Use red or magenta only for a cover burst or explicit identity cue, never as body decoration.
- **Selection rule.** Choose this style for cultural, editorial, or brand stories where place and material evidence are the message.

## Signature moves and variation limits

- Let **photo** recur as a quiet identity cue on selected pages; it must
