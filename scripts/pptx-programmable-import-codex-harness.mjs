...
1. Start with \`officekit repl --new <goal>\`. In that session call \`ctx.input\` for the immutable source, import it with public \`PresentationFile.importPptx\`, duplicate source slide ${task.sourceSlide}, move the duplicate to the final page, cross an export/import boundary, perform only phase-1's typed edit, call \`reviewArtifact\`, then \`ctx.commit\`. End the REPL process and retain its opaque task id.
2. Start a new process with \`officekit repl <task-id>\`. Use the restored reviewed full-file revision, not old JavaScript heap state. Reimport it, perform only phase-2's typed edit, call \`reviewArtifact\`, then \`ctx.commit\`. End that REPL process.
3. Start a third process with \`officekit repl <task-id>\`. Reimport the restored current revision, verify the source slide count plus one and both exact values, then publish the current reviewed commit as \`${path.basename(task.output)}\` with \`ctx.publish\`.

Drive every REPL process with finite JSONL stdin (for example one prepared request line piped into the command) so it receives EOF and exits after its cell. Never leave an unattended interactive REPL waiting for input. Parse the first process's \`session.ready\` record to retain the opaque task id, and parse each terminal response before starting the next process.
