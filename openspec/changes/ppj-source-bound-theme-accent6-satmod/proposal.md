...
accent1 plus accent6 tint, shade, luminance, and alpha transforms. A direct
imported accent6 `a:satMod` remains opaque, leaving one concrete F-15
...
- Project a canonical shared ThemePart whose accent6 direct RGB leaf contains
  ...
  `design.theme.accentTransforms.accent6.satMod`, using a 0..1 fraction.
- Issue `setThemeAccent6SatMod` for that field and compile one authorized
...
- `presentation-theme-accent6-satmod`: Strict source-bound projection and
  source-preserving editing of one direct accent6 `a:satMod` leaf.
