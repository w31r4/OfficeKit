...
- Give a direct literal polar-handle position `x` a stable native index and
  explicit shape-local EMU value.
- Keep guide identity, radial/angular bounds, y, position topology, handle
...
- Editing polar position `y`, guide identity, adjustment values, formulas,
...
2. **Require a canonical shape-local coordinate.** The direct `@x` token must
   be a non-negative integer no greater than the custom geometry width;
...
3. **Keep the entire handle graph intact.** The writer proves all direct
   `ahPolar` attributes and the direct position child, then changes only `x`.
   It does not add/remove the position or edit y, range state, current
