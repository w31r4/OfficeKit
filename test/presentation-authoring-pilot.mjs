...
assert.equal(holdout.runs.length, 6);
...
assert.equal(holdout.runs.filter((run) => run.taskSet === "unseen").length, 3);
assert.equal(holdout.runs.filter((run) => run.taskSet === "frozen-pilot-rerun").length, 3);
...
assert.match(buildPilotPrompt({ manifest, ...buildPilotMatrix(manifest, { taskId: "business-review-01", arm: "C", trial: 1 })[0] }), /minimumBodyFontSize[\s\S]*22[\s\S]*minimumCaptionFontSize[\s\S]*20[\s\S]*four distinct composition silhouettes[\s\S]*cardWallPattern[\s\S]*no unrecorded design warnings[\s\S]*intentionalWarnings[\s\S]*dominant reading anchor[\s\S]*contrast[\s\S]*pale card surface[\s\S]*palette\.strict false[\s\S]*closed brand palette[\s\S]*every allowed emitted color[\s\S]*presentation\.validateLayout[\s\S]*quantitative claim[\s\S]*fewer, stronger elements[\s\S]*plain text mainly[\s\S]*contact-sheet scale[\s\S]*Compose nodes[\s\S]*slide\.compose[\s\S]*do not pass a compose node[\s\S]*head[\s\S]*from[\s\S]*tail[\s\S]*bringToFront/i);
