...
`inspect({ includeNativeLeaves: true })` issues revision-bound leaf IDs only for codec-proven safe fields. `editNativeLeaf` accepts one such ID, expected hash, and value. It never accepts a part path, XPath, QName, arbitrary attribute, relationship ID, namespace, identity, or topology mutation. Initial leaf kinds are text, color, local geometry scalars, direct rich-title text runs from a uniquely bound internal ChartPart, direct numeric category-cache points and scatter/bubble X/Y/size cache points whose matching cells are proven in one uniquely bound embedded XLSX, and direct SmartArt text runs in one canonical closed DiagramDataPart with a unique inbound owner. Dependent bindings stay internal to the compiler and codec; unsupported nodes remain inspectable only through summaries or opaque preservation.

Repeated visual candidates may expose an occurrence-level `editCapability` containing only the native leaf IDs already issued for that closed source graph. `presentation.editComponentOccurrence` accepts a bounded list of those exact leaf records and validates all hashes, values, ownership, and occurrence membership before applying any change. It is a batching convenience for component continuation, not a component JSON schema or a new raw-XML surface; opaque, relationship-bound, and topology-bearing candidates remain blocked.
