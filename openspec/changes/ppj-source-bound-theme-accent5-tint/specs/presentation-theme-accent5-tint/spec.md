...
direct accent5 tint token while preserving the rest of an imported theme.

## ADDED Requirements

### Requirement: Strict source-bound accent5 tint projection

The PPTX projector MUST expose
`design.theme.accentTransforms.accent5.tint` and
`setThemeAccent5Tint` only when the canonical shared ThemePart has one direct
accent5 RGB leaf with exactly one direct `a:tint/@val` child, no extra
attributes or descendants, and an integer value in `[0, 100000]`. The
projected value MUST be the native value divided by 100000.

#### Scenario: Project a direct accent5 tint leaf

- **GIVEN** a canonical imported ThemePart with accent5 `445566` and
  `a:tint/@val="35000"`
- **WHEN** the presentation is projected to PPJ
- **THEN** the theme contains `accentTransforms.accent5.tint` equal to `0.35`
- **AND** the native reference contains `setThemeAccent5Tint` authorized for
  `accentTransforms.accent5.tint`

#### Scenario: Reject unsupported accent5 tint topology
...
- **THEN** the source-bound accent5 tint field and capability are omitted

### Requirement: Source-bound accent5 tint editing

The source-preserving compiler MUST accept one authorized change to the
existing `design.theme.accentTransforms.accent5.tint` field, convert a finite
...
#### Scenario: Edit and reproject one accent5 tint token

- **GIVEN** a projected direct accent5 tint owner at `0.35`
...
#### Scenario: Fail closed for unsafe accent5 tint edits

- **GIVEN** a source-bound accent5 tint owner
