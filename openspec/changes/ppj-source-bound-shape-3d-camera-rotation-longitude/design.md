...
- Owner: `p:sp/p:spPr/a:scene3d/a:camera/a:rot/@lon`.
- Native leaf: `shape3dSceneCameraRotationLongitude60000`.
- The owner must be the single direct `a:scene3d` child of `a:spPr`. The
  ...
  are not modeled.
- Projection stores only the camera rotation longitude in the native shape
  model. The edit plan rechecks the complete camera-rotation owner and
  token-splices only `rot/@lon`, preserving latitude, revolution, light-rig
