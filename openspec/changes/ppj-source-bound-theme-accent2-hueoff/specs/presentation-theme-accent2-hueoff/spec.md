...
direct accent2 hue-offset token while preserving the rest of an imported theme.

## ADDED Requirements

### Requirement: Strict source-bound accent2 hueOff projection

The PPTX projector MUST expose
`design.theme.accentTransforms.accent2.hueOff` and
`setThemeAccent2HueOff` only when the canonical shared ThemePart has one
direct accent2 RGB leaf with exactly one direct `a:hueOff/@val` child, no
...
#### Scenario: Project a direct accent2 hueOff leaf
...
- **THEN** the theme contains `accentTransforms.accent2.hueOff` equal to
  `210`
- **AND** the native reference contains `setThemeAccent2HueOff` authorized
  for `accentTransforms.accent2.hueOff`

#### Scenario: Reject unsupported accent2 hueOff topology
...
- **THEN** the source-bound accent2 hueOff field and capability are omitted

### Requirement: Source-bound accent2 hueOff editing

The source-preserving compiler MUST accept one authorized change to the
existing `design.theme.accentTransforms.accent2.hueOff` field, convert a
...
#### Scenario: Edit and reproject one accent2 hueOff token

- **GIVEN** a projected direct accent2 hueOff owner at `210`
...
#### Scenario: Fail closed for unsafe accent2 hueOff edits

- **GIVEN** a source-bound accent2 hueOff owner
