...
### Requirement: project a bounded picture backdrop normal Z

The presentation codec MUST project
`p:pic/p:spPr/a:scene3d/a:backdrop/a:norm/@dz` through the existing
`shape3dSceneBackdropNormalDzEmu` native leaf when the picture owner has
...
#### Scenario: project a source-bound picture backdrop normal Z

- **WHEN** a source PPTX contains a picture with a strict complete backdrop
  owner
- **THEN** the projected picture contains exactly one
  `shape3dSceneBackdropNormalDzEmu` native leaf with the normal Z coordinate

### Requirement: edit only the picture backdrop normal Z token

For a projected picture backdrop-normal-Z leaf, the presentation codec MUST
...
`a:scene3d/a:backdrop/a:norm/@dz` in the owning SlidePart. It MUST preserve the
anchor, normal X/Y, up vector, camera/light-rig state, picture
...
- **WHEN** a projected picture backdrop normal Z changes from `3` to `4`
- **THEN** only the owning SlidePart changes, the edited `dz` is `4`, and the
  picture's other XML and package parts remain intact

### Requirement: keep unsupported picture backdrops opaque

The codec MUST keep a picture backdrop-normal-Z owner source-bound when the
...
normalize such markup merely to expose or edit normal Z.
...
- **THEN** projection does not expose `shape3dSceneBackdropNormalDzEmu` for
