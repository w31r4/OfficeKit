...
export const HELP_CATALOG = [
  ...
  { artifactKind: "presentation", kind: "api", name: "presentation.editNativeLeaf", summary: "Change one native leaf issued by presentation.inspect({ includeNativeLeaves: true }) using its targetId, leafId, expectedHash, and a typed value. Leaf IDs are bound to the exact imported revision and target. The current profile changes one existing text leaf, including a group child or a shape with source-owned outer styling, one shape RGB/local-geometry scalar, or one picture local-geometry scalar through the token-preserving Edit Plan compiler. The compiler binds an ownership-tree snapshot. Stale hashes, concurrent non-leaf changes, foreign IDs, raw XML, XPath, part paths, arbitrary attributes, relationship fields, namespaces, and topology changes reject." },
