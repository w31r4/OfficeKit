...
最新热力图事实检查复验（2026-09-10 20:30:02）：`YqCTgw` 使用冻结副本 `heatmap-fact-snapshot-a3dTPY` 和同一 035472e9 原生包，741 项正式入口通过限定内容/发布检查，均保留 requires-review。两个热力图数值变体解除旧通道忽略误报；原候选、场景及警示条以下全部 RGBA 不变。原 12 项删除、3 项语言及 1 项表格文字样式丢失仍使整套 `failed/exit1`；完整参考门禁在显式临时 Poppler PATH 下通过，系统默认 PATH 仍缺该工具。详见[热力图映射与验证边界](ppj-preview-current-gaps.zh-CN.md#作者热力图的数据通道误报)。

最新非对称圆角复验（2026-09-10 20:05:18）：`RAAEyl` 使用冻结副本 `rounded-corner-snapshot-QzUx5q` 和同一 035472e9 原生包，739 项正式入口通过限定内容/发布检查，可靠性为 738 项 requires-review、1 项 failed。新增四种圆角的 81 项轮廓/源生命周期、24 项文字区域和 9 项非法输入拒绝通过；外部 19 个轮廓和文字对照均通过。原 12 项删除、3 项语言及 1 项表格文字样式丢失仍使整套 `failed/exit1`，不计为本轮修复。详见[非对称圆角的实现、参考精度与验收](ppj-preview-current-gaps.zh-CN.md#四种非对称圆角轮廓图片和文字使用同一几何)。

此前切角复验（2026-09-10 19:39:56）：`Td0XOT` 使用冻结副本 `cut-corner-snapshot-rhWueu` 和同一 035472e9 原生包，634 项正式入口通过限定内容/发布检查，可靠性为 633 项 requires-review、1 项 failed。新增四种切角的 78 项轮廓/源生命周期、24 项文字区域及 6 项非法输入拒绝通过；外部 18 个轮廓和文字对照均通过。原 12 项删除、3 项语言及 1 项表格文字样式丢失仍使整套 `failed/exit1`，原生修复尚未实施。详见[四种切角的实现、测试与限制](ppj-preview-current-gaps.zh-CN.md#四种切角轮廓共享几何与调整值生命周期)。

此前小型大写复验（2026-09-10）：`d5XctG` 有 532 个正式入口通过限定内容/发布检查，可靠性为 531 项 requires-review、1 项 failed。新增小型大写 26/27 通过，表格源导入仍丢失文字样式；原 12 项删除及 3 项语言写回失败保留，完整集成 failed/exit1。外部小型大写对照 6/8 通过，两项字宽差异未通过。原 12 项已另立 [原生删除修复提案](../openspec/changes/ppj-source-deletion-lifecycle/proposal.md)，尚未实施，新文字问题不并入该授权；详见[小型大写与源导入记录](ppj-preview-current-gaps.zh-CN.md#小型大写本地映射与表格源导入失败)。
...
`datasetLine` 映射仅解除普通折线图 `data.dataset` 的旧通道忽略错误：全部 owner 绑定必须指向有分类与系列的原生 LINE 节点，完成实际折线构造，且自身与全部祖先的外部变换已成功。JS 不重新解释数据集或编码。缺记录、隐藏、混合归属、通道错误和不支持的平滑折线保留失败；其他图表没有借用这条规则。真实输入的 1→2 修改已有像素移动、缺失断段/真实零及显式数据对照；文字、轴、样式、源编辑和其他事实限制继续单独检查，解除误报不等于整体通过。

`datasetHeatmap` 使用独立映射：仅对 authored-lowering 中同一原图表 owner 的完整生成矩形/文本框子树，要求每个正面积节点完成基础绘制、几何及整条祖先变换。源导入、隐藏/退化/未映射节点、未知原生字段或缺一条记录仍保留通道错误。首值 1/2、缺失绿色/真实零黑色及独立显式数据的整页像素对照通过，旧例候选和内容像素保持；可靠性仍为 requires-review，文字布局等限制不解除。详细证据见[热力图检查](ppj-preview-current-gaps.zh-CN.md#作者热力图的数据通道误报)。
