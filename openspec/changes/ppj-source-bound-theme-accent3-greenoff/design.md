...
This change applies the same ownership boundary to accent3 green-channel offset
...
- Recognize only a canonical shared ThemePart whose accent3 color is a direct
  six-digit RGB leaf with one direct `a:greenOff/@val` child.
...
- Accent3 transforms other than `greenOff`, transforms on other accent roles, inherited or effect theme state, or arbitrary theme XML editing.
- Creating or deleting a missing greenOff node, changing multiple theme fields,
  or claiming PowerPoint host color rendering.

## Decisions

1. **Use the existing native transform field.**
  `PresentationThemeColorTransform.GreenOffsetThousandth` already represents
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:greenOff`. The source writer patches `Accent3Color` only when the authored
   transform is the one existing accent3 greenOff and rejects all other transform
   flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent3GreenOff` is accepted only with
   `accentTransforms.accent3.greenOff` in the source-issued capability. This
