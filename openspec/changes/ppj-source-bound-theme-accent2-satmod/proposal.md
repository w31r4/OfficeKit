...
accent1 plus accent2 tint, shade, luminance, and alpha transforms. A direct
imported accent2 `a:satMod` remains opaque, leaving one concrete F-15
...
- Project a canonical shared ThemePart whose accent2 direct RGB leaf contains
  ...
  `design.theme.accentTransforms.accent2.satMod`, using a 0..1 fraction.
- Issue `setThemeAccent2SatMod` for that field and compile one authorized
...
- `presentation-theme-accent2-satmod`: Strict source-bound projection and
  source-preserving editing of one direct accent2 `a:satMod` leaf.
