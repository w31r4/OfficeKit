...
public sealed class DocxCodecTests
{
    ...
    [Fact]
    public void ParagraphWidowControlAuthorsImportsEditsAndRejectsIrregularMarkup()
    {
        var request = ExportRequest(includeSecondParagraph: true);
        request.Artifact.Document.Blocks[0].Paragraph.Formatting = new DocumentParagraphFormatting
        {
        ...
        using (var package = WordprocessingDocument.Open(stream, false))
        {
            var paragraphs = package.MainDocumentPart!.Document!.Body!.Elements<W.Paragraph>().ToArray();
            var firstProperties = paragraphs[0].ParagraphProperties!;
            Assert.True(firstProperties.GetFirstChild<W.WidowControl>()!.Val!.Value);
            var propertyChildren = firstProperties.ChildElements.ToList();
            Assert.True(propertyChildren.IndexOf(firstProperties.GetFirstChild<W.PageBreakBefore>()!) <
                        propertyChildren.IndexOf(firstProperties.GetFirstChild<W.WidowControl>()!));
            Assert.False(paragraphs[1].ParagraphProperties!.GetFirstChild<W.WidowControl>()!.Val!.Value);
            Assert.Empty(new OpenXmlValidator(FileFormatVersions.Office2021).Validate(package));
        }
        ...
        var importedSecond = imported.Artifact.Document.Blocks[1];
        Assert.True(importedFirst.Source.Editable);
        Assert.True(importedFirst.Paragraph.Formatting.HasWidowControl);
        Assert.True(importedFirst.Paragraph.Formatting.WidowControl);
        Assert.True(importedSecond.Paragraph.Formatting.HasWidowControl);
        Assert.False(importedSecond.Paragraph.Formatting.WidowControl);
        ...
        importedFirst.Paragraph.Formatting.WidowControl = false;
        importedSecond.Paragraph.Formatting.WidowControl = true;
        var edited = Invoke(new CodecRequest
        {
            ProtocolVersion = CodecProtocol.ProtocolVersion,
            Operation = CodecOperation.ExportDocx,
        ...
        Assert.True(roundTrip.Ok, Diagnostics(roundTrip));
        Assert.False(roundTrip.Artifact.Document.Blocks[0].Paragraph.Formatting.WidowControl);
        Assert.True(roundTrip.Artifact.Document.Blocks[1].Paragraph.Formatting.WidowControl);

        foreach (var mode in new[] { "duplicate", "extension", "invalid" })
        {
            var irregularBytes = MakeWidowControlIrregular(authored.File.ToByteArray(), mode);
            var irregular = Invoke(new CodecRequest
            {
                ProtocolVersion = CodecProtocol.ProtocolVersion,
                Operation = CodecOperation.ImportDocx,
            ...
            var irregularBlock = irregular.Artifact.Document.Blocks[0];
            Assert.False(irregularBlock.Source.Editable);
            Assert.False(irregularBlock.Paragraph.Formatting?.HasWidowControl ?? false);
            var preserved = Invoke(new CodecRequest
            {
                ProtocolVersion = CodecProtocol.ProtocolVersion,
                Operation = CodecOperation.ExportDocx,
            ...
            irregularBlock.Paragraph.Formatting = new DocumentParagraphFormatting { WidowControl = false };
            var rejected = Invoke(new CodecRequest
            {
                ProtocolVersion = CodecProtocol.ProtocolVersion,
                Operation = CodecOperation.ExportDocx,
    ...
    [Fact]
    public void ParagraphOutlineLevelAuthorsImportsEditsAndRejectsIrregularMarkup()
    {
        var request = ExportRequest(includeSecondParagraph: true);
        request.Artifact.Document.Blocks[0].Paragraph.Formatting = new DocumentParagraphFormatting
        {
        ...
        // 9 is the native explicit "no outline level" override, distinct from
        // omitting the property and inheriting a named style's level.
        request.Artifact.Document.Blocks[1].Paragraph.Formatting = new DocumentParagraphFormatting
        {
            OutlineLevel = 9,
        };
        ...
        using (var package = WordprocessingDocument.Open(stream, false))
        {
            var paragraphs = package.MainDocumentPart!.Document!.Body!.Elements<W.Paragraph>().ToArray();
            var firstProperties = paragraphs[0].ParagraphProperties!;
            Assert.Equal(0, firstProperties.GetFirstChild<W.OutlineLevel>()!.Val!.Value);
            var propertyChildren = firstProperties.ChildElements.ToList();
            Assert.True(propertyChildren.IndexOf(firstProperties.Justification!) <
                        propertyChildren.IndexOf(firstProperties.GetFirstChild<W.OutlineLevel>()!));
            Assert.Equal(9, paragraphs[1].ParagraphProperties!.GetFirstChild<W.OutlineLevel>()!.Val!.Value);
            Assert.Empty(new OpenXmlValidator(FileFormatVersions.Office2021).Validate(package));
        }
        ...
        var importedSecond = imported.Artifact.Document.Blocks[1];
        Assert.True(importedFirst.Source.Editable);
        Assert.True(importedFirst.Paragraph.Formatting.HasOutlineLevel);
        Assert.Equal(0u, importedFirst.Paragraph.Formatting.OutlineLevel);
        Assert.True(importedSecond.Paragraph.Formatting.HasOutlineLevel);
        Assert.Equal(9u, importedSecond.Paragraph.Formatting.OutlineLevel);
        ...
        importedFirst.Paragraph.Formatting.OutlineLevel = 4;
        importedSecond.Paragraph.Formatting.OutlineLevel = 0;
        var edited = Invoke(new CodecRequest
        {
            ProtocolVersion = CodecProtocol.ProtocolVersion,
            Operation = CodecOperation.ExportDocx,
        ...
        Assert.True(roundTrip.Ok, Diagnostics(roundTrip));
        Assert.Equal(4u, roundTrip.Artifact.Document.Blocks[0].Paragraph.Formatting.OutlineLevel);
        Assert.Equal(0u, roundTrip.Artifact.Document.Blocks[1].Paragraph.Formatting.OutlineLevel);

        foreach (var mode in new[] { "duplicate", "extension", "invalid" })
        {
            var irregularBytes = MakeOutlineLevelIrregular(authored.File.ToByteArray(), mode);
            var irregular = Invoke(new CodecRequest
            {
                ProtocolVersion = CodecProtocol.ProtocolVersion,
                Operation = CodecOperation.ImportDocx,
            ...
            var irregularBlock = irregular.Artifact.Document.Blocks[0];
            Assert.False(irregularBlock.Source.Editable);
            Assert.False(irregularBlock.Paragraph.Formatting?.HasOutlineLevel ?? false);
            var preserved = Invoke(new CodecRequest
            {
                ProtocolVersion = CodecProtocol.ProtocolVersion,
                Operation = CodecOperation.ExportDocx,
            ...
            irregularBlock.Paragraph.Formatting = new DocumentParagraphFormatting { OutlineLevel = 0 };
            var rejected = Invoke(new CodecRequest
            {
                ProtocolVersion = CodecProtocol.ProtocolVersion,
                Operation = CodecOperation.ExportDocx,
    ...
    [Fact]
    public void ParagraphShadingAndBordersAuthorImportEditClearAndRejectIrregularMarkup()
    {
        var request = ExportRequest(includeSecondParagraph: true);
        request.Artifact.Document.Blocks[0].Paragraph.Formatting = new DocumentParagraphFormatting
        {
        ...
        var unformatted = Invoke(ExportRequest(includeSecondParagraph: false));
        Assert.True(unformatted.Ok, Diagnostics(unformatted));
        var unformattedImport = Invoke(new CodecRequest
        {
            ProtocolVersion = CodecProtocol.ProtocolVersion,
            Operation = CodecOperation.ImportDocx,
            Family = ArtifactFamily.Document,
        ...
        Assert.True(unformattedImport.Ok, Diagnostics(unformattedImport));
        var unformattedBlock = Assert.Single(unformattedImport.Artifact.Document.Blocks,
            block => block.ContentCase == DocumentBlock.ContentOneofCase.Paragraph);
        Assert.True(unformattedBlock.Source.Editable);
        Assert.False(unformattedBlock.Paragraph.Formatting?.HasShadingFill ?? false);
        Assert.Null(unformattedBlock.Paragraph.Formatting?.Borders);
        unformattedBlock.Paragraph.Formatting = new DocumentParagraphFormatting
        {
            ShadingFill = "FDE68A",
            Borders = new DocumentParagraphBorders
            {
        ...
        var added = Invoke(new CodecRequest
        {
            ...
            Operation = CodecOperation.ExportDocx,
            Family = ArtifactFamily.Document,
            Artifact = unformattedImport.Artifact,
        });
        ...
        var addedBlock = Assert.Single(addedRoundTrip.Artifact.Document.Blocks,
            block => block.ContentCase == DocumentBlock.ContentOneofCase.Paragraph);
        Assert.Equal("FDE68A", addedBlock.Paragraph.Formatting.ShadingFill);
        Assert.Equal("7C3AED", Assert.IsType<DocumentParagraphBorders>(addedBlock.Paragraph.Formatting.Borders).Left!.ColorRgb);

        foreach (var makeIrregular in new Func<byte[], string, byte[]>[]
        {
            MakeParagraphShadingIrregular,
            MakeParagraphBordersIrregular,
        })
        {
            foreach (var mode in new[] { "duplicate", "extension", "invalid" })
            {
                var irregularBytes = makeIrregular(authored.File.ToByteArray(), mode);
                var irregular = Invoke(new CodecRequest
                {
                    ProtocolVersion = CodecProtocol.ProtocolVersion,
                    Operation = CodecOperation.ImportDocx,
    ...
    private static CodecRequest BibliographyExportRequest(bool includeOutputField = false)
    {
        ...
        document.Bookmarks.Add(new DocumentBookmark
        {
            ...
            Name = "OfficeKitCitation_AgentSource",
        ...
        using (var document = WordprocessingDocument.Open(stream, true, new OpenSettings { AutoSave = true }))
        {
            ...
            switch (mode)
            {
                case "duplicate":
                    ...
                    break;
                case "extension":
                    suppression.SetAttribute(new OpenXmlAttribute("oat", "probe", "urn:office-kit:test", "1"));
                    break;
                case "invalid":
    ...
    private static byte[] MakeWidowControlIrregular(byte[] bytes, string mode)
    {
        using var stream = new MemoryStream();
        stream.Write(bytes);
        stream.Position = 0;
        using (var document = WordprocessingDocument.Open(stream, true, new OpenSettings { AutoSave = true }))
        {
            var paragraph = document.MainDocumentPart!.Document!.Body!.Elements<W.Paragraph>().First();
            var properties = paragraph.ParagraphProperties!;
            var widowControl = properties.GetFirstChild<W.WidowControl>()!;
            switch (mode)
            {
                case "duplicate":
                    properties.InsertAfter(new W.WidowControl(), widowControl);
                    break;
                case "extension":
                    widowControl.SetAttribute(new OpenXmlAttribute("oat", "probe", "urn:office-kit:test", "1"));
                    break;
                case "invalid":
                    widowControl.SetAttribute(new OpenXmlAttribute("w", "val", "http://schemas.openxmlformats.org/wordprocessingml/2006/main", "invalid"));
                    break;
                default:
                    throw new ArgumentOutOfRangeException(nameof(mode), mode, "Unknown widow-control irregularity.");
            }
            document.MainDocumentPart.Document.Save();
        }
        return stream.ToArray();
    ...
    private static byte[] MakeOutlineLevelIrregular(byte[] bytes, string mode)
    {
        using var stream = new MemoryStream();
        stream.Write(bytes);
        stream.Position = 0;
        using (var document = WordprocessingDocument.Open(stream, true, new OpenSettings { AutoSave = true }))
        {
            var paragraph = document.MainDocumentPart!.Document!.Body!.Elements<W.Paragraph>().First();
            var properties = paragraph.ParagraphProperties!;
            var outlineLevel = properties.GetFirstChild<W.OutlineLevel>()!;
            switch (mode)
            {
                case "duplicate":
                    properties.InsertAfter(new W.OutlineLevel { Val = 1 }, outlineLevel);
                    break;
                case "extension":
                    outlineLevel.SetAttribute(new OpenXmlAttribute("oat", "probe", "urn:office-kit:test", "1"));
                    break;
                case "invalid":
                    outlineLevel.SetAttribute(new OpenXmlAttribute("w", "val", "http://schemas.openxmlformats.org/wordprocessingml/2006/main", "10"));
                    break;
                default:
                    throw new ArgumentOutOfRangeException(nameof(mode), mode, "Unknown outline-level irregularity.");
            }
            document.MainDocumentPart.Document.Save();
        }
        return stream.ToArray();
