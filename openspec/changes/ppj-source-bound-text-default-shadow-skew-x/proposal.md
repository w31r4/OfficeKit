...
Paragraph default-text shadows already round-trip authored `skewX`, but an
imported default-run shadow still lacks a native field for its direct
horizontal skew token. A caller therefore cannot make a narrow source-bound
edit without treating the whole effect graph as source-owned; this closes the
next F-03 default-text effect leaf beside `scaleX` and `scaleY`.

## What Changes

- expose `text.paragraphs[].style.defaultText.shadow.skewX` as a source-bound
  native field for a strict direct paragraph default outer shadow;
- validate the strict `-90 < skewX < 90` range after native 1/60000-degree
  rounding and preserve the signed Int32 token;
- patch only `a:outerShdw/@kx` in the owning slide while preserving all sibling
...
- `ppj-source-bound-text-default-shadow-skew-x`: source-preserving editing of
  one paragraph default-text outer-shadow horizontal skew token.
