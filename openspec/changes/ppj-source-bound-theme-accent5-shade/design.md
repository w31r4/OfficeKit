...
This change applies the same ownership boundary to accent5 shade without
...
- Accent5 transforms other than `shade`, transforms on accent5 through accent6,
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:shade`. The source writer patches `Accent5Color` only when the authored
   transform is the one existing accent5 shade and rejects all other transform
   flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent5Shade` is accepted only with
   `accentTransforms.accent5.shade` in the source-issued capability. This keeps
