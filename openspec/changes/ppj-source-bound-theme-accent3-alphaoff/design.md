...
transform and the direct accent3 tint, shade, luminance modulation, luminance
offset, and alpha modulation leaves at a time, then patches the existing
DrawingML token. This change applies the same ownership boundary to accent3
...
- Accent3 transforms other than `alphaOff`, transforms on accent4 through
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:alphaOff`. The source writer patches `Accent3Color` only when the
   authored transform is the one existing accent3 alphaOff and rejects all
   other transform flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent3AlphaOff` is accepted only with
   `accentTransforms.accent3.alphaOff` in the source-issued capability. This
