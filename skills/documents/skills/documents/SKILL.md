...
- `examples/officekit-table-formatting-edit-workflow.mjs` — one fixed-layout direct-formatting transaction with an optional paired `minimumRowHeightsDxa` non-clipping row-minimum profile, raw residual proof, second import, model/native-render QA, and byte-bound audit
...
- For one imported flat rectangular fixed-layout table whose inspected block index and complete direct profile are known, `examples/officekit-table-formatting-edit-workflow.mjs` changes only table indentation, four cell margins, six uniform borders, the uniformly shaded first-row fill, and one optional uniform physical-cell `w:vAlign` value. An optional `minimumRowHeightsDxa` must appear in both source and replacement profiles, contain one value per physical row, and maps only to canonical non-clipping `w:trHeight hRule="atLeast"` leaves; it can add, change, or clear those row minima but never writes exact heights. Mixed, missing, duplicate, exact, or irregular alignment/row-property leaves fail closed. It retains the table width, grid/cell widths, text, style, and topology; permits only `word/document.xml`; compares a raw OPC residual outside those bound leaves; reimports the full table projection; and writes a no-overwrite byte-bound audit. It is not a generic table-style editor and never infers wrapping or page flow → `tasks/tables_spreadsheets.md`
