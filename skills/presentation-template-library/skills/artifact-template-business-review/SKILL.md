...
description: "Use the Business Review visual grammar when the user selects it for a new presentation. Read the guide and inspect the examples before composing."
---
...
A measured executive brief: evidence leads, decisions land visibly, and page rhythm moves from signal to implication to action.

## Rules

- Use an open paper surface with one ink color and one restrained accent. Separate evidence with rules and whitespace rather than repeated panels.
- Use a compact overline, a sentence-case conclusion title, and a readable body. Let the conclusion be larger than its decoration.
- Favor one dominant chart or comparison per page, with a narrow read-through column only when it adds a decision. Use baselines, labels, and annotated marks.
- Use documentary or product imagery only when it explains context; crop it deliberately and never use a stock image as filler.
- A full-bleed image may sit behind editable text only when contrast is tested. Keep axes, labels, and decision lines unobstructed.
- Avoid dashboard mosaics, equal card grids, decorative rings, and generic metric boxes.
