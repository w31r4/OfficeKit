...
{
    [Fact]
    public void PpjSourceBoundThemeColorRoleLight2EditsOnlyLight2AndReprojects()
    {
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            {
                SourceUri = "deck.assets/source/theme-color-role-light2.pptx",
            },
        });
        ...
        Assert.Equal("#404142", projectedColorRoles["light2"]!.GetValue<string>());
        var nativeRef = Assert.IsType<JsonObject>(projectedTheme["nativeRef"]);
        Assert.Contains(nativeRef["capabilities"]!.AsArray(), capability =>
            capability!["operation"]!.GetValue<string>() == "setThemeColorRoleLight2" &&
            capability["fields"]!.AsArray().Any(field => field!.GetValue<string>() == "colorRoles.light2"));
        Assert.Contains(nativeRef["capabilities"]!.AsArray(), capability =>
        ...
        editedProgram["design"]!["theme"]!["colorRoles"]!["light2"] = "#ABCDEF";
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            Assert.Equal("ABCDEF", scheme.Light2Color!.RgbColorModelHex!.Val!.Value);
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/theme-color-role-light2.pptx",
            },
        });
        Assert.True(reprojected.Ok, Diagnostics(reprojected));
        var reprojectedProgram = JsonNode.Parse(reprojected.PresentationProgram.ProgramJson.ToByteArray())!.AsObject();
        Assert.Equal("#ABCDEF", reprojectedProgram["design"]!["theme"]!["colorRoles"]!["light2"]!.GetValue<string>());

        var unauthorizedProgram = JsonNode.Parse(projectedProgram.ToJsonString())!.AsObject();
        unauthorizedProgram["design"]!["theme"]!["colorRoles"]!.AsObject().Remove("light2");
        ...
        unownedProgram["design"]!["theme"]!["colorRoles"]!["hyperlink"] = "#202122";
        ...
        tamperedProgram["design"]!["theme"]!["nativeRef"]!["capabilities"]!.AsArray()
            .Single(capability => capability!["operation"]!.GetValue<string>() == "setThemeColorRoleLight2")!["fields"] = new JsonArray("colorRoles.hyperlink");
