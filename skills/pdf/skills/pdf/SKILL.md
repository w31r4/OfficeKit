...
- `add_text_annotation`, `add_free_text_annotation`, `add_area_annotation`, `add_text_markup` (Highlight/Underline/StrikeOut/Squiggly), compatibility `add_text_highlight`, `add_link`, `delete_annotation`,
  ...
  fixed-Helvetica visible text box and refuses an appearance that omits text; `add_area_annotation` adds one unfilled solid rectangle/ellipse whose full native appearance stays visible. All require rewrite. Recognized FreeText permits content/reviewer updates, and a recognized solid/no-fill area mark permits content/reviewer/RGB-stroke updates, each with a complete snapshot; text markup requires one unique native text selection.
