...
The native leaf `shape3dSceneBackdropUpDxEmu` already covers strict imported
shapes, but the same complete backdrop owner on a picture is still opaque. Its
up-vector X coordinate is an independently bounded literal that can be edited
...
- Project and source-bound-edit only a strict
  `p:pic/p:spPr/a:scene3d/a:backdrop/a:up/@dx` token.
...
- `ppj-source-bound-picture-shape-3d-scene-backdrop-up-dx`:
  Bounded projection and source-bound editing of a picture backdrop up-vector X.
