...
- Delete only through the typed facade. Imported top-level ordinary shapes and embedded pictures expose `deletionCapability`; `shape.delete()` requires a relationship-free subtree, while `image.delete()` owns its exact SlidePart relationship and garbage-collects only media without another package parent. Export re-proves both contracts, and raw collection mutation is rejected.
