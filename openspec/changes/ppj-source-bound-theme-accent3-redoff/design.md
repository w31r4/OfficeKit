...
This change applies the same ownership boundary to accent3 red-channel offset
...
- Accent3 transforms other than `redOff`, transforms on accent3 through
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:redOff`. The source writer patches `Accent3Color` only when the authored
   transform is the one existing accent3 redOff and rejects all other transform
   flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent3RedOff` is accepted only with
   `accentTransforms.accent3.redOff` in the source-issued capability. This
