...
{
    [Fact]
    public void PpjChartTitlePlacementAuthorAndEditSourceChart()
    {
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest { SourceUri = "chart-title-placement/source.pptx" },
        });
        ...
        var projectedChart = projectedProgram["pages"]!.AsArray()
            ...
            .Select(item => item!.AsObject())
            .Single(item => item["type"]!.GetValue<string>() == "chart");
        Assert.Equal("Evidence chart", projectedChart["title"]!.GetValue<string>());
        Assert.Equal("centeredOverlay", projectedChart["titlePlacement"]!.GetValue<string>());

        projectedChart["titlePlacement"] = "aboveChart";
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            Family = ArtifactFamily.Presentation,
            File = edited.File,
            PresentationProgram = new PresentationProgramRequest { SourceUri = "chart-title-placement/edited.pptx" },
        });
        ...
        var reprojectedChart = reprojectedProgram["pages"]!.AsArray()
            ...
            .Select(item => item!.AsObject())
            .Single(item => item["type"]!.GetValue<string>() == "chart");
        Assert.Equal("aboveChart", reprojectedChart["titlePlacement"]!.GetValue<string>());
    }
}
