...
{
    [Fact]
    public void PpjSourceBoundImageShadowBlurEditsLeafAndReprojects()
    {
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/source/image-shadow-blur.pptx",
            },
        });
        ...
        var blurLeaves = projectedImage["nativeRef"]!["leaves"]!.AsArray()
            .Select(leaf => leaf!.AsObject())
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "imageShadowBlurRadiusEmu")
            .ToArray();
        Assert.Single(blurLeaves);
        Assert.Equal(76_200, blurLeaves[0]["value"]!.GetValue<long>());

        blurLeaves[0]["value"] = 152_400;
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            var shadow = Assert.IsType<A.OuterShadow>(Assert.Single(picture.ShapeProperties!.GetFirstChild<A.EffectList>()!.ChildElements));
            Assert.True(shadow.RotateWithShape!.Value);
            Assert.Equal(152_400U, shadow.BlurRadius!.Value);
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/image-shadow-blur.pptx",
            },
        });
        ...
        var reprojectedImage = reprojectedProgram["pages"]![0]!["elements"]!.AsArray()
            .Select(item => item!.AsObject())
            .Single(item => item["name"]!.GetValue<string>() == "evidence identity");
        Assert.Equal(12d, reprojectedImage["shadow"]!["blur"]!.GetValue<double>());
        ...
        Assert.Equal(152_400, reprojectedImage["nativeRef"]!["leaves"]!.AsArray().Single(leaf =>
            leaf!["kind"]!.GetValue<string>() == "imageShadowBlurRadiusEmu")!["value"]!.GetValue<long>());
    }
}
