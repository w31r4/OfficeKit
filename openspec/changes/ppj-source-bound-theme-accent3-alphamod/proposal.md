...
accent1 plus accent3 tint, shade, luminance modulation, and luminance offset.
A direct imported accent3 `a:alphaMod` remains opaque, leaving one concrete F-15
...
- Project a canonical shared ThemePart whose accent3 direct RGB leaf contains
  ...
  `design.theme.accentTransforms.accent3.alphaMod`, using a 0..1 fraction.
- Issue `setThemeAccent3AlphaMod` for that field and compile one authorized
...
- `presentation-theme-accent3-alphamod`: Strict source-bound projection and
  source-preserving editing of one direct accent3 `a:alphaMod` leaf.
