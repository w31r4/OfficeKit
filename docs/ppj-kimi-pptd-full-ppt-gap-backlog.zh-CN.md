...
**本轮后续增量（2026-09-13）：** `design.theme.name` 现在作为一个 source-bound 元数据字段闭合：当所有 master 共享一个已有、非空名称的 ThemePart 时，投影器颁发 `setThemeName` capability；编辑只改对应 ThemePart 的 `a:theme/@name`，并由二次投影恢复。随后同一 owner 的已有 `a:majorFont/a:latin/@typeface` 与 `a:minorFont/a:latin/@typeface` 分别作为 `design.theme.fontScheme.major`、`.minor` 闭合，`setThemeFontScheme` capability 按字段授权；现有 `a:majorFont/a:ea/@typeface` 与 `a:minorFont/a:ea/@typeface` 也分别作为 `design.theme.fontScheme.majorEastAsia`、`.minorEastAsia` 闭合，`a:majorFont/a:cs/@typeface` 作为 `.majorComplexScript` 闭合，均颁发独立字段 capability。每次编译只改一个字体槽位。缺失或多 ThemePart、缺少所需字体槽位、字段删除及其余 theme graph 仍保持 source-owned；focused native 回归覆盖 no-op 字节、changed-part、Open XML、非目标 ZIP 和 capability authority。

**仍缺：** 完整 `theme1.xml` color/font/effect scheme、更多颜色变换、继承后的 effect style、更完整的逐脚本字体回退、字体嵌入/替换、复杂语言字体映射的其余组合、WordArt 和宿主字体回退差异；当前 color roles、alpha、tint/shade、minor complex-script slot 与复杂脚本 direct owner 都只覆盖 PPJ 的有限 authored/native lowering，不会改写任意 imported theme XML。
