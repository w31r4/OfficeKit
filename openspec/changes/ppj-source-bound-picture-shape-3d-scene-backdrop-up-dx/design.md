...
- Expose one canonical signed `a:backdrop/a:up/@dx` coordinate in EMU.
- Replace only `backdrop/a:up/@dx` in the owning SlidePart.
...
- Reuse `TryReadSceneBackdropUpDx` and its requested-value validator so picture
...
- Token-splice only the direct `backdrop/a:up/@dx` attribute.

# Risks / Trade-offs

- [Risk] An up-vector X token without a complete backdrop owner could be
