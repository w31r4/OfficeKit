...
- Give a direct literal `a:ahPolar/@minR` bound a stable native index and
  explicit shape-local EMU value.
- Keep radial/angle guide identity, `maxR`, angle bounds, position, handle
...
- Editing `maxR`, angle bounds, polar position, guide identity, adjustment
...
3. **Keep the paired range graph intact.** The writer proves all direct
   `ahPolar` attributes and changes only `minR`; it does not add/remove the
   range or edit `maxR`, angle state, current adjustment, or position. The
