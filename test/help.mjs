...
assert.match(areaHelp?.summary || "", /source- and page-snapshot-bound.*unfilled.*Square or Circle.*rewrite-only.*painted bounds outside.*fail closed/i);
...
assert.match(areaHelp?.schema?.returns?.operation?.description || "", /solid\/no-fill.*appearanceBboxVerified=true.*complete-snapshot deletion/i);
