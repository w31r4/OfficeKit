...
internal static partial class PptxEditPlanCodec
{
    ...
    private static void ValidateRequest(byte[] sourceBytes, PresentationEditPlanRequest request, EffectiveCodecLimits limits)
    {
        ...
        foreach (var operation in request.Operations)
        {
            ...
            if (leafKind == "textShadowBlurRadiusEmu")
            {
                if (!ValidShadowGeometryToken(leafKind, operation.ExpectedValue) ||
                    !ValidShadowGeometryToken(leafKind, operation.Value) ||
                    operation.ExpectedValue == operation.Value)
                    throw new CodecException("invalid_presentation_edit_operation", $"PPTX edit operation {operation.OperationId} textShadowBlurRadiusEmu must use a changed bounded native shadow blur radius.");
            }
    ...
    private static PptxXmlPatch[] CompileXmlPatches(byte[] partBytes, IReadOnlyList<PptxEditPlanProof> proofs)
    {
        ...
        foreach (var proof in proofs)
        {
            ...
            {
                if (range.LocalName != "sp")
                    throw new CodecException("presentation_edit_target_mismatch", $"PPTX edit operation {operation.OperationId} textShadowBlurRadiusEmu target has the wrong native element type.", operation.SlidePartPath);
    ...
    private static bool HasSafeNativeTextShadow(OpenXmlCompositeElement properties) =>
        ...
        shadow is { HasBlurRadiusEmu: true } &&
    ...
    private static string ReadLeafValue(OpenXmlElement element, PresentationEditOperation operation)
    {
        ...
        if (kind == "textShadowBlurRadiusEmu")
        {
            if (element is not P.Shape shape)
                throw new CodecException("presentation_edit_target_mismatch", $"PPTX edit operation {operation.OperationId} textShadowBlurRadiusEmu target is not a shape.", operation.SlidePartPath);
            var leaves = shape.Descendants<A.Text>().ToArray();
            if (operation.TextLeafIndex >= (uint)leaves.Length || leaves[operation.TextLeafIndex].Parent is not A.Run run ||
            ...
            if (!PptxShadowCodec.TryRead(shadowProperties, out var shadow) || shadow is not { HasBlurRadiusEmu: true })
                throw new CodecException("presentation_edit_target_missing", $"PPTX edit operation {operation.OperationId} target has no bounded explicit text outer shadow.", operation.SlidePartPath);
            return shadow.BlurRadiusEmu.ToString(System.Globalization.CultureInfo.InvariantCulture);
        }
        ...
        IReadOnlySet<string> drawingPrefixes)
    {
        ...
        var run = TextRunPattern().Matches(elementXml).Cast<Match>()
            ...
            .SingleOrDefault(match => match.Index <= leaf.Index && leaf.Index < match.Index + match.Length);
        if (run is null)
            throw new CodecException("presentation_edit_target_mismatch", $"PPTX edit operation {operation.OperationId} textShadowBlurRadiusEmu leaf is not owned by an editable text run.", operation.SlidePartPath);
        ...
        var attributes = XmlAttributePattern().Matches(startTag.Value).Cast<Match>()
            .Where(match => LocalAttributeName(match.Groups["name"].Value) == "blurRad")
            .ToArray();
        if (attributes.Length != 1)
            throw new CodecException("presentation_edit_target_mismatch", $"PPTX edit operation {operation.OperationId} outer-shadow blurRad attribute is missing or ambiguous.", operation.SlidePartPath);
        var valueGroup = attributes[0].Groups["value"];
        if (System.Net.WebUtility.HtmlDecode(valueGroup.Value) != operation.ExpectedValue)
            throw new CodecException("presentation_leaf_precondition_failed", $"PPTX edit operation {operation.OperationId} raw text outer-shadow blurRad does not match the expected value.", operation.SlidePartPath);
    ...
    {
        if (kind is "shadowBlurRadiusEmu" or "imageShadowBlurRadiusEmu" or "textShadowBlurRadiusEmu" or "shadowDistanceEmu" or "imageShadowDistanceEmu" or "textDefaultShadowBlurRadiusEmu" or "textDefaultShadowDistanceEmu")
        {
            return long.TryParse(value, System.Globalization.NumberStyles.None, System.Globalization.CultureInfo.InvariantCulture, out var coordinate) &&
                coordinate is >= 0 and <= 2_147_483_647 &&
                coordinate.ToString(System.Globalization.CultureInfo.InvariantCulture) == value;
