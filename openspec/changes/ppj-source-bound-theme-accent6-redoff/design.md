...
the same ownership boundary to accent6 red-channel offset without
...
- Accent6 transforms other than `redOff`, transforms on accent6 through
...
1. **Use the existing native transform field.**
   `PresentationThemeColorTransform.RedOffsetThousandth` already
   represents the DrawingML -1..100000 integer and the authored compiler
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:redOff`. The source writer patches `Accent6Color` only when the authored
   transform is the one existing accent6 redOff and rejects all other transform
   flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent6RedOff` is accepted only with
   `accentTransforms.accent6.redOff` in the source-issued capability. This
