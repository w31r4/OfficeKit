...
{
    [Fact]
    public void PpjSourceBoundTextShadowSkewYEditsDirectRunOwnerAndReprojects()
    {
        var program = ShadowSkewYProgram(skewY: -1.25);
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            Assert.Equal(-75_000, shadow.VerticalSkew!.Value);
        ...
        Assert.Equal(-1.25, shadowStyle["skewY"]!.GetValue<double>(), precision: 6);
        var skewLeaf = Assert.Single(projectedElement["nativeRef"]!["leaves"]!.AsArray(),
            leaf => leaf!["kind"]!.GetValue<string>() == "textShadowSkewY");
        ...
        using (var themeStream = new MemoryStream())
        {
            ...
            Assert.Equal(-1.25, themeElement["text"]!["paragraphs"]![0]!["runs"]![0]!["style"]!["shadow"]!["skewY"]!.GetValue<double>(), precision: 6);
        ...
        using (var staleStream = new MemoryStream())
        {
            ...
            using (var stalePackage = PresentationDocument.Open(staleStream, true))
            {
                ...
                staleShadow.VerticalSkew = 125_000;
            }
        ...
        Assert.Equal(1.25, reprojectedShadow["skewY"]!.GetValue<double>(), precision: 6);
        Assert.Equal(1.25, reprojectedElement["nativeRef"]!["leaves"]!.AsArray()
            .Select(leaf => leaf!.AsObject())
            .Single(leaf => leaf["kind"]!.GetValue<string>() == "textShadowSkewY")["value"]!.GetValue<double>(), precision: 6);
        ...
        invalidProgram["pages"]![0]!["elements"]![0]!["nativeRef"]!["leaves"]!.AsArray()
            .Single(leaf => leaf!["kind"]!.GetValue<string>() == "textShadowSkewY")!["value"] = 90;
    ...
    [Fact]
    public void PpjSourceBoundTextShadowSkewYLeavesStayOpaqueForUnsupportedGraphs()
    {
        var authored = CompileSkewYShadow(ShadowSkewYProgram(skewY: -1.25));
        Assert.True(authored.Ok, Diagnostics(authored));
        var source = RemoveEmbeddedPpj(authored.File.ToByteArray());
        ...
        foreach (var (label, mutate) in new[]
        {
            ...
            ("missing-skew", (Action<A.OuterShadow>)(shadow => shadow.VerticalSkew = null)),
            ("invalid-skew", (Action<A.OuterShadow>)(shadow => shadow.SetAttribute(new OpenXmlAttribute("ky", "", "invalid")))),
        ...
        var siblingAuthored = CompileSkewYShadow(ShadowSkewYProgram(skewY: -1.25, siblingGlow: true));
    ...
    private static JsonObject ShadowSkewYProgram(double skewY, bool siblingGlow = false)
    {
    ...
    private static CodecResponse CompileSkewYShadow(JsonObject program) => Invoke(new CodecRequest
    {
        ProtocolVersion = CodecProtocol.ProtocolVersion,
        Operation = CodecOperation.CompilePpjToPptx,
        Family = ArtifactFamily.Presentation,
