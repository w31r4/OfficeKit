...
- Expose one canonical signed `a:rot/@lon` angle in 60000ths of a degree.
- Replace only `lightRig/a:rot/@lon` in the owning SlidePart.
- Preserve latitude, revolution, camera state, light-rig state, picture
...
- Reuse `TryReadSceneLightRigRotationLongitude` and its requested-value
...
- Token-splice only the direct `lightRig/a:rot/@lon` attribute.

# Risks / Trade-offs

- [Risk] A longitude token without a complete rotation owner could be mistaken
