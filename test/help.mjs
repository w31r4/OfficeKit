...
assert.match(HELP_CATALOG.find((item) => item.name === "PdfFile.inspectPdf")?.summary || "", /source-bound Document Info records.*complete XMP stream identity\/profile\/mutable-field evidence/i);
...
assert.match(HELP_CATALOG.find((item) => item.name === "PdfFile.editPdf")?.schema?.parameters?.operations?.description || "", /set_metadata.*source hash.*mupdf-document-info.*complete inspect-returned snapshot.*eight standard keys.*non-target Info entries.*canonical-simple.*xmpMutableFields.*fail closed/i);
...
assert.match(HELP_CATALOG.find((item) => item.name === "PdfFile.inspectPdf")?.schema?.returns?.inspection?.description || "", /mupdfDocumentMetadata.*fingerprints raw Info entries.*XMP.*stream object\/hash\/length.*canonical-simple.*fail-closed/i);
