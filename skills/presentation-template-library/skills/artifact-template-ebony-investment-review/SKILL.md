...
Use this style when the brief matches **investment benchmark; finance presentation**. It is designed for
investors, finance leaders, analysts. Its visual premise is Ebony structure, teal evidence, and terracotta judgment create institutional calm. The style is a
...
**stance → drivers → evidence → scenarios → risks → decision or allocation**. This style is not a good fit for **product keynote**;
...
- Use **#FCFBF8** as the main field, **#1C1A19** for primary type, **#31836F** for the decisive signal, **#A65C42** for secondary evidence, and **#DDD7CF** for rules or context.
...
- The recurring structure is **terracotta, variable page silhouettes, native data graphics**. Vary the silhouette between
  cover, evidence, data, process, comparison, and decision pages rather than
  repeating a component grid.
- Use a warm judgment accent against a quiet institutional skeleton. Apply it to conclusions and exceptions, not to every data series.
...
- **Page skeleton.** Use a charcoal-and-paper institutional chassis: a dark judgment surface for the stance, a light evidence surface for tables and charts, and terracotta only for a decision or exception. Keep the split intentional.
- **Type rhythm.** Use a sober serif or humanist display for thesis lines and a sans for numerals, tables, and source notes. Figures should be aligned by column and never float without a basis.
- **Evidence carriers.** Use valuation tables, scenario bridges, risk/mitigation pairs, benchmark bars, and sensitivity grids. Put the anchor or comparable set on the same page as the conclusion.
- **Images and layers.** Avoid lifestyle photography. If context is needed, use a single documentary image on a transition page and preserve the editable evidence plane above it.
- **Pacing.** Dense analysis pages should earn their weight; insert a dark decision page only for an actual stance or approval. Terracotta should appear as a judgment, not as a series palette.
- **Selection rule.** Choose this style for institutional investment review, benchmarking, valuation, and risk committees.

## Signature moves and variation limits

- Let **terracotta** recur as a quiet identity cue on selected pages; it must
