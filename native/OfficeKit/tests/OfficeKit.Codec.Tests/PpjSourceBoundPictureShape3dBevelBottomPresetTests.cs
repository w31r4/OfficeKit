...
{
    [Fact]
    public void PpjSourceBoundPictureShape3dBevelBottomPresetLeafEditsAndReprojects()
    {
        ...
        var source = AddPictureShape3dBevelBottomPreset(RemoveEmbeddedPpj(authored.File.ToByteArray()));
        var projected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/source/picture-shape-3d-bevel-bottom-preset.pptx",
            },
        });
        ...
        var target = Assert.Single(projectedImage["nativeRef"]!["leaves"]!.AsArray()
            ...
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "shape3dBevelBottomPreset"));
        Assert.Equal("angle", target["value"]!.GetValue<string>());
        target["value"] = "softRound";
        ...
        Assert.Contains("prst=\"softRound\"", editedXml, StringComparison.Ordinal);
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            Assert.Equal("softRound", bevel!.GetAttributes().Single(attribute => attribute.LocalName == "prst").Value);
        }
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/picture-shape-3d-bevel-bottom-preset.pptx",
            },
        });
        ...
        var reprojectedImage = reprojectedProgram["pages"]!.AsArray()
            ...
            .Select(item => item!.AsObject())
            .Single(item => item["name"]!.GetValue<string>() == "evidence identity");
        Assert.Equal("softRound", reprojectedImage["nativeRef"]!["leaves"]!.AsArray()
            .Select(leaf => leaf!.AsObject())
            .Single(leaf => leaf["kind"]!.GetValue<string>() == "shape3dBevelBottomPreset")["value"]!.GetValue<string>());
    }

    private static byte[] AddPictureShape3dBevelBottomPreset(byte[] source)
    {
        ...
        using (var package = PresentationDocument.Open(stream, true))
        {
            ...
            bevel.SetAttribute(new OpenXmlAttribute("w", string.Empty, "1200"));
