...
transform and the direct accent3 tint, shade, luminance, and alpha leaves at a
time, then patches the existing DrawingML token. This change applies the same
ownership boundary to accent3 saturation modulation without broadening the
...
- Accent3 transforms other than `satMod`, transforms on accent4 through
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:satMod`. The source writer patches `Accent3Color` only when the authored
   transform is the one existing accent3 satMod and rejects all other
   transform flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent3SatMod` is accepted only with
   `accentTransforms.accent3.satMod` in the source-issued capability. This
