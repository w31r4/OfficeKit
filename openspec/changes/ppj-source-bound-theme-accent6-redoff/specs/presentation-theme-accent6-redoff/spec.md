...
direct accent6 red-channel offset token while preserving the rest of an
imported theme.

## ADDED Requirements

### Requirement: Strict source-bound accent6 redOff projection

The PPTX projector MUST expose
`design.theme.accentTransforms.accent6.redOff` and
`setThemeAccent6RedOff` only when the canonical shared ThemePart has one
direct accent6 RGB leaf with exactly one direct `a:redOff/@val` child, no
...
#### Scenario: Project a direct accent6 redOff leaf

- **GIVEN** a canonical imported ThemePart with accent6 `334455` and
  `a:redOff/@val="-25000"`
- **WHEN** the presentation is projected to PPJ
- **THEN** the theme contains `accentTransforms.accent6.redOff` equal to
  `-0.25`
- **AND** the native reference contains `setThemeAccent6RedOff` authorized
  for `accentTransforms.accent6.redOff`

#### Scenario: Reject unsupported accent6 redOff topology
...
- **THEN** the source-bound accent6 redOff field and capability are omitted

### Requirement: Source-bound accent6 redOff editing

The source-preserving compiler MUST accept one authorized change to the
existing `design.theme.accentTransforms.accent6.redOff` field, convert a
...
#### Scenario: Edit and reproject one accent6 redOff token

- **GIVEN** a projected direct accent6 redOff owner at `-0.25`
...
#### Scenario: Fail closed for unsafe accent6 redOff edits

- **GIVEN** a source-bound accent6 redOff owner
