...
`upright`, `rotation` and `columnDirection` belong to the text body: `text.style` on structured table-cell text,
...
`upright`, `rotation` and/or `columnDirection`. If a table cell then projects as plain text, add the
