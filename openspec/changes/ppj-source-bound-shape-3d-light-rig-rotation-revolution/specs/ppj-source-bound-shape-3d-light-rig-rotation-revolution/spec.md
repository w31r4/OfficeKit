# ppj-source-bound-shape-3d-light-rig-rotation-revolution Specification

## ADDED Requirements

### Requirement: Strict direct light-rig rotation revolution projection

The presentation projection MUST expose
`shape3dSceneLightRigRotationRevolution60000` only when a shape has one direct
...
#### Scenario: Project a light-rig rotation revolution

- **GIVEN** an ordinary shape with a canonical scene whose light rig has
  `rig="threePt"`, `dir="t"`, and `rot lat="0" lon="0" rev="0"`
- **WHEN** the PPTX is projected to PPJ
- **THEN** exactly one `shape3dSceneLightRigRotationRevolution60000` native
  leaf is emitted with value `0`
...
- **WHEN** the PPTX is projected to PPJ
- **THEN** no `shape3dSceneLightRigRotationRevolution60000` leaf is emitted

### Requirement: Source-bound light-rig rotation revolution token splice

The PPJ edit plan MUST permit
`shape3dSceneLightRigRotationRevolution60000` only when the source-bound shape
...
replace only the `a:rot/@rev` token in the owning SlidePart, preserve the
scene/camera/light-rig topology and unrelated package parts, and allow the new
revolution to be recovered on a second projection.

#### Scenario: Edit and reproject light-rig rotation revolution

- **GIVEN** a projected strict scene owner with `rev="0"`
...
- **WHEN** an edit targets `shape3dSceneLightRigRotationRevolution60000`
