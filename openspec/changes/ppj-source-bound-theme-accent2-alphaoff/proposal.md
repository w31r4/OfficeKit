...
accent1 plus accent2 tint, shade, luminance modulation, luminance offset, and
alpha modulation. A direct imported accent2 `a:alphaOff` remains opaque,
...
- Project a canonical shared ThemePart whose accent2 direct RGB leaf contains
  ...
  `design.theme.accentTransforms.accent2.alphaOff`, using a -1..1 fraction.
- Issue `setThemeAccent2AlphaOff` for that field and compile one authorized
...
- `presentation-theme-accent2-alphaoff`: Strict source-bound projection and
  source-preserving editing of one direct accent2 `a:alphaOff` leaf.
