# Design: source-bound paragraph default-text reflection end opacity
...
`endA`, optionally preceded by one valid direct `a:outerShdw`. The reflection
keeps its blur, start opacity, distance, direction and full-span positions
...
The leaf kind is `textDefaultReflectionEndOpacityThousandthPercent` and its
PPJ location is `paragraph.style.defaultText.reflection.endOpacity`. The edit
plan re-proves the selected `a:p`, direct `a:pPr`, `a:defRPr`, strict full-span
effect list, and direct `a:reflection` before token-splicing only `endA` in the
owning SlidePart. No protobuf or wire-version change is needed. The focused
fixture removes the embedded PPJ, changes the paragraph default end opacity
