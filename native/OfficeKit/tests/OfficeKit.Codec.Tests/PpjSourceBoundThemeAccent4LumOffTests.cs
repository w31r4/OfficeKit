...
{
    [Fact]
    public void PpjSourceBoundThemeAccent4LumOffEditsOnlyLumOffAndReprojects()
    {
        ...
        {
            Role = "accent4",
            LuminanceOffsetThousandth = -25_000,
        });
        ...
        Assert.Equal(-0.25, projectedTransform["lumOff"]!.GetValue<double>());
        var nativeRef = Assert.IsType<JsonObject>(projectedTheme["nativeRef"]);
        Assert.Contains(nativeRef["capabilities"]!.AsArray(), capability =>
            capability!["operation"]!.GetValue<string>() == "setThemeAccent4LumOff" &&
            capability["fields"]!.AsArray().Any(field => field!.GetValue<string>() == "accentTransforms.accent4.lumOff"));
        ...
        editedProgram["design"]!["theme"]!["accentTransforms"]!["accent4"]!["lumOff"] = -0.5;
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            Assert.Equal(-50_000, Assert.Single(accent4.Elements<A.LuminanceOffset>()).Val!.Value);
        ...
        Assert.Equal(-0.5, reprojectedProgram["design"]!["theme"]!["accentTransforms"]!["accent4"]!["lumOff"]!.GetValue<double>());

        var unauthorizedProgram = JsonNode.Parse(projectedProgram.ToJsonString())!.AsObject();
        unauthorizedProgram["design"]!["theme"]!["accentTransforms"]!["accent4"]!["lumOff"] = 0.6;
        unauthorizedProgram["design"]!["theme"]!["nativeRef"]!["capabilities"]!.AsArray()
            .Single(capability => capability!["operation"]!.GetValue<string>() == "setThemeAccent4LumOff")!["fields"] = new JsonArray("accentTransforms.accent4.shade");
        ...
        deletionProgram["design"]!["theme"]!["accentTransforms"]!["accent4"]!.AsObject().Remove("lumOff");
        ...
        outOfRangeProgram["design"]!["theme"]!["accentTransforms"]!["accent4"]!["lumOff"] = -1.01;
