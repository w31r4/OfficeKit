...
transform and the direct accent6 tint, shade, luminance, alpha, saturation, and
red modulation leaves at a time, then patches the existing DrawingML token.
This change applies the same ownership boundary to accent6 blue-channel modulation
...
- Accent6 transforms other than `blueMod`, transforms on other accent roles, inherited or effect theme state, or arbitrary theme XML editing.
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:blueMod`. The source writer patches `Accent6Color` only when the authored
   transform is the one existing accent6 blueMod and rejects all other transform
   flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent6BlueMod` is accepted only with
   `accentTransforms.accent6.blueMod` in the source-issued capability. This
