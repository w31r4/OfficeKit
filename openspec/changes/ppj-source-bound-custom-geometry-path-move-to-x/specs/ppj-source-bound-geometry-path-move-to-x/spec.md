# Custom geometry move-to x coordinate

## ADDED Requirements

### Requirement: expose one bounded move-to x coordinate

The system MUST expose a `customGeometryPathMoveToX` native leaf only for a
direct ordered `a:path/a:moveTo/a:pt` whose `x` is a canonical signed integer
...
- **WHEN** an imported custom geometry contains a recognized path with a
  literal move-to point x coordinate
...
- **WHEN** a source-bound PPJ edit changes an issued move-to x leaf to another
