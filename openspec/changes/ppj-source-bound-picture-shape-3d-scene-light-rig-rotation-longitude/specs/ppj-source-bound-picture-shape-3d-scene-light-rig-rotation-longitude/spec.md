...
This capability preserves and minimally edits the light-rig rotation longitude
of a strict 3-D scene on an imported picture without owning the scene graph.

## ADDED Requirements

### Requirement: project a bounded picture light-rig rotation longitude

The presentation codec MUST project
`p:pic/p:spPr/a:scene3d/a:lightRig/a:rot/@lon` through the existing
`shape3dSceneLightRigRotationLongitude60000` native leaf when the picture owner
...
#### Scenario: project a source-bound picture light-rig longitude

- **WHEN** a source PPTX contains a picture with a strict complete light-rig
  rotation owner
- **THEN** the projected picture contains exactly one
  `shape3dSceneLightRigRotationLongitude60000` native leaf with the longitude
  in 60000ths-of-a-degree units

### Requirement: edit only the picture light-rig longitude token

For a projected picture light-rig-longitude leaf, the presentation codec MUST
accept a source-bound edit only when the requested value is a changed canonical
angle token, and MUST replace only `a:scene3d/a:lightRig/a:rot/@lon` in the
owning SlidePart. It MUST preserve latitude, revolution, camera/light-rig
...
- **WHEN** a projected picture light-rig longitude changes from `0` to
  `1800000`
...
The codec MUST keep a picture light-rig-longitude owner source-bound when the
...
- **THEN** projection does not expose
  `shape3dSceneLightRigRotationLongitude60000` for that owner and a
