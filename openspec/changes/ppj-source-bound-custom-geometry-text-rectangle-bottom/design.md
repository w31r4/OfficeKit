...
- Give a fully literal, in-frame text-rectangle bottom edge a stable native
  leaf and shape-local EMU value.
...
- Editing left/top/right edges, guide references, guide formulas, handles,
...
1. **Use one native index for the bottom edge.** The leaf represents the one
...
3. **Preserve representation.** A direct source keeps `a:rect/@b` numeric. An
   authored/private source keeps `b="officeKitTextBottom"` and changes only the
...
- A bottom-edge change can alter text layout inside a custom shape. Restrict
  the regression to an ordered in-frame change; reflow behavior remains
  host-owned.
