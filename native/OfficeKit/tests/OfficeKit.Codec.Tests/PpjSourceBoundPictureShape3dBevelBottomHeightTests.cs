...
{
    [Fact]
    public void PpjSourceBoundPictureShape3dBevelBottomHeightLeafEditsAndReprojects()
    {
        ...
        var source = AddPictureShape3dBevelBottomHeight(RemoveEmbeddedPpj(authored.File.ToByteArray()), 800);
        var projected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/source/picture-shape-3d-bevel-bottom-height.pptx",
            },
        });
        ...
        var target = Assert.Single(projectedImage["nativeRef"]!["leaves"]!.AsArray()
            ...
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "shape3dBevelBottomHeightEmu"));
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            Assert.Equal("1600", bevel!.GetAttributes().Single(attribute => attribute.LocalName == "h").Value);
        }
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/picture-shape-3d-bevel-bottom-height.pptx",
            },
        });
        ...
        var reprojectedImage = reprojectedProgram["pages"]!.AsArray()
            ...
            .Select(item => item!.AsObject())
            .Single(item => item["name"]!.GetValue<string>() == "evidence identity");
        Assert.Equal(1_600, reprojectedImage["nativeRef"]!["leaves"]!.AsArray()
            .Select(leaf => leaf!.AsObject())
            .Single(leaf => leaf["kind"]!.GetValue<string>() == "shape3dBevelBottomHeightEmu")["value"]!.GetValue<long>());
    }

    private static byte[] AddPictureShape3dBevelBottomHeight(byte[] source, long height)
    {
        ...
        using (var package = PresentationDocument.Open(stream, true))
        {
            ...
            bevel.SetAttribute(new OpenXmlAttribute("h", string.Empty, height.ToString(System.Globalization.CultureInfo.InvariantCulture)));
