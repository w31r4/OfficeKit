...
{
    [Fact]
    public void PpjSourceBoundCustomGeometryPathLineToYEditsAndReprojects()
    {
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            {
                SourceUri = "deck.assets/source/custom-geometry-path-line-to-y.pptx",
            },
        });
        ...
        var lineLeaves = projectedShape["nativeRef"]!["leaves"]!.AsArray()
            ...
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "customGeometryPathLineToY")
            .ToArray();
        Assert.Equal(2, lineLeaves.Length);
        Assert.Contains(lineLeaves, leaf => leaf["value"]!.GetValue<long>() == 100);
        ...
        Assert.Contains("x=\"500\" y=\"850\"", editedXml, StringComparison.Ordinal);
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/custom-geometry-path-line-to-y.pptx",
            },
        });
        ...
        Assert.Contains(reprojectedLeaves, leaf => leaf["value"]!.GetValue<long>() == 100);
    }
}
