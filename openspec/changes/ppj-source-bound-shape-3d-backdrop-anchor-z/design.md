...
- Owner: `p:sp/p:spPr/a:scene3d/a:backdrop/a:anchor/@z`.
- Native leaf: `shape3dSceneBackdropAnchorZEmu`.
- The owner must be the single direct `a:scene3d` child of `a:spPr`. The scene
  ...
  Z value is exposed; X/Y and both vectors are validation context only.
- The coordinate uses DrawingML EMUs and the standard bounded signed
  `ST_Coordinate` range. Projection stores only anchor Z in the native shape
  model. The edit plan rechecks the complete backdrop owner and token-splices
  only `anchor/@z`, preserving the plane topology and all other package bytes.
...
No source-free authoring, anchor X/Y editing, normal/up vector editing,
