# Proposal: add source-bound paragraph default-text glow theme color to PPJ

Expose a bounded `paragraph.style.defaultText.glow.color` theme-token owner
...
can change an existing direct `a:schemeClr/@val` without rewriting the
surrounding slide.

This slice does not change RGB-color ownership or infer arbitrary paragraph
effect graphs. Only a strict direct `a:glow` with one direct theme color,
