...
{
    [Fact]
    public void PpjChartLegendOverlayAuthorAndEditSourceChart()
    {
        ...
        {
            ["legendOverlay"] = new JsonObject { ["kind"] = "boolean", ["value"] = true },
        };
        ...
        {
            ["legend"] = "right",
            ["legendOverlay"] = new JsonObject { ["token"] = "legendOverlay" },
        };
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest { SourceUri = "legend-overlay/source.pptx" },
        });
        ...
        var projectedChart = projectedProgram["pages"]!.AsArray()
            ...
            .Select(item => item!.AsObject())
            .Single(item => item["type"]!.GetValue<string>() == "chart");
        Assert.True(projectedChart["style"]!["legendOverlay"]!.GetValue<bool>());

        projectedProgram["design"]!["grammar"]!["tokens"] = new JsonObject
        {
            ["legendOverlayEdited"] = new JsonObject { ["kind"] = "boolean", ["value"] = false },
        };
        projectedChart["style"]!["legendOverlay"] = new JsonObject { ["token"] = "legendOverlayEdited" };
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            Family = ArtifactFamily.Presentation,
            File = edited.File,
            PresentationProgram = new PresentationProgramRequest { SourceUri = "legend-overlay/edited.pptx" },
        });
        ...
        var reprojectedChart = reprojectedProgram["pages"]!.AsArray()
            ...
            .Select(item => item!.AsObject())
            .Single(item => item["type"]!.GetValue<string>() == "chart");
        Assert.False(reprojectedChart["style"]!["legendOverlay"]!.GetValue<bool>());
    }
}
