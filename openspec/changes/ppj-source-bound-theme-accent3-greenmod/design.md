...
This change applies the same ownership boundary to accent3 green-channel modulation
...
- Accent3 transforms other than `greenMod`, transforms on accent3 through
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:greenMod`. The source writer patches `Accent3Color` only when the authored
   transform is the one existing accent3 greenMod and rejects all other transform
   flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent3GreenMod` is accepted only with
   `accentTransforms.accent3.greenMod` in the source-issued capability. This
