...
- `nativeObject` (NativePresentationObject) — Queues one source-ordered run-local update without changing a:pPr, a:rPr, canonical fixed a:br, a:endParaRPr, or guessing style ownership. Export re-proves the source digest, node IDs/order, fixed paragraph/run/break topology, and closed graph; only the bound DiagramDataPart may change. Paragraph or break changes, fields, noncanonical breaks, and unsupported markup fail closed.
