...
accent1 plus accent6 tint, shade, luminance, alpha, and saturation modulation
transforms. A direct imported accent6 `a:satOff` remains opaque, leaving one
...
- Project a canonical shared ThemePart whose accent6 direct RGB leaf contains
  ...
  `design.theme.accentTransforms.accent6.satOff`, using a -1..1 fraction.
- Issue `setThemeAccent6SatOff` for that field and compile one authorized
...
- `presentation-theme-accent6-satoff`: Strict source-bound projection and
  source-preserving editing of one direct accent6 `a:satOff` leaf.
