...
internal static partial class PpjAuthoredPresentationCompiler
{
    ...
    private static SpreadsheetChartDataLabelOverrideArtifact BuildChartLabelOverride(JsonElement source, Catalog catalog)
    {
        ...
        if (source.TryGetProperty("line", out var line))
            output.Line = BuildChartLine(line, catalog);
        return output;
    }
    ...
    private static SpreadsheetChartAxisArtifact BuildChartAxis(JsonElement source, Catalog catalog)
    {
        ...
        if (source.TryGetProperty("majorTickMark", out var majorTickMark))
            axis.MajorTickMark = ChartEnumToken(majorTickMark, catalog, "chart axis majorTickMark", "cross", "in", "out", "none");
        if (source.TryGetProperty("minorTickMark", out var minorTickMark))
            axis.MinorTickMark = ChartEnumToken(minorTickMark, catalog, "chart axis minorTickMark", "cross", "in", "out", "none");
