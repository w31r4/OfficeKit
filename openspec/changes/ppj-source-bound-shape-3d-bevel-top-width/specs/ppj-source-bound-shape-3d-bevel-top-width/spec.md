# Shape 3-D top-bevel width

## ADDED Requirements

### Requirement: Project one direct top-bevel width

The system MUST expose one `shape3dBevelTopWidthEmu` native leaf only for an
ordinary shape with exactly one direct `a:sp3d` whose only child is a direct
`a:bevelT` and whose `w` value is a canonical non-negative 32-bit coordinate.

#### Scenario: Project a recognized top bevel

- **WHEN** an imported shape contains a direct `a:sp3d/a:bevelT/@w="1200"`
...
### Requirement: Edit only the top-bevel width token

The system MUST allow a source-bound edit to replace only the proven
`a:bevelT/@w` attribute in the owning SlidePart.

#### Scenario: Reproject an edited width

- **WHEN** the `shape3dBevelTopWidthEmu` leaf changes from `1200` to `2400`
...
- **THEN** the projection emits no `shape3dBevelTopWidthEmu` leaf
