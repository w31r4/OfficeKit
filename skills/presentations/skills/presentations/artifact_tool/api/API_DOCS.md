...
- Delete only through the typed facade. Imported top-level ordinary shapes, embedded pictures, canonical connectors, bounded tables, and charts expose `deletionCapability`. Shapes/connectors/tables must be relationship-free; pictures/charts own one exact SlidePart relationship and garbage-collect only descendants without another package parent. Export re-proves the concrete source graph, shared media/ChartParts survive, and raw collection mutation is rejected.
