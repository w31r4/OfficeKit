...
accent1 plus accent3 tint, shade, luminance, alpha, and saturation modulation
transforms. A direct imported accent3 `a:satOff` remains opaque, leaving one
...
- Project a canonical shared ThemePart whose accent3 direct RGB leaf contains
  ...
  `design.theme.accentTransforms.accent3.satOff`, using a -1..1 fraction.
- Issue `setThemeAccent3SatOff` for that field and compile one authorized
...
- `presentation-theme-accent3-satoff`: Strict source-bound projection and
  source-preserving editing of one direct accent3 `a:satOff` leaf.
