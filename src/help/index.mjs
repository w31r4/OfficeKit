...
export const HELP_CATALOG = [
  ...
  { artifactKind: "presentation", kind: "api", name: "shape.replaceImageFill", summary: "Replace the embedded image in one imported source-bound custom-geometry shape while preserving its native path, crop, effects, and relationship topology. The replacement must use the issued shape's existing content type and is re-proven as one bounded imageAsset edit; unsupported image-fill graphs and arbitrary custom-path changes fail closed." },
...
const PRESENTATION_HELP_SCHEMAS = {
  ...
  "shape.replaceImageFill": helpSchema({
    blob: { type: "FileBlob", required: true, description: "Embedded replacement bytes. The content type must match the imported shape's existing image fill; prefer FileBlob.load(path, { type }) over constructing base64 in task code." },
    contentType: { type: "string", description: "Optional MIME override for the FileBlob; it must resolve to the existing image-fill type." },
  }, "receipt", "object", "Immutable imageFillReplacement receipt. The target must be an imported source-bound custom-geometry shape with one strict direct a:blipFill. Export changes only the owning slide's relationship/media closure, preserves crop, effects, custom paths, IDs, and topology, and re-proves the new asset after re-import. Unsupported image-fill graphs, cross-format replacements, and custom-path edits fail closed."),
