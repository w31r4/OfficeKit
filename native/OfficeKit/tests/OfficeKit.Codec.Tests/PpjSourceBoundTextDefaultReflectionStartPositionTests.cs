...
{
    [Fact]
    public void PpjSourceBoundTextDefaultReflectionStartPositionEditsLeafAndReprojects()
    {
        var authored = CompileDefaultReflectionProgram();
        var source = RemoveEmbeddedPpj(authored.File.ToByteArray());
        source = RewriteReflectionPosition(source, 20_000, 100_000);
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            {
                SourceUri = "deck.assets/source/text-default-reflection-start-position.pptx",
            },
        });
        ...
        Assert.Equal(0.2, reflectionJson["startPosition"]!.GetValue<double>(), precision: 6);
        Assert.Equal(1, reflectionJson["endPosition"]!.GetValue<double>(), precision: 6);
        var leaf = projectedElement["nativeRef"]!["leaves"]!.AsArray()
            .Select(item => item!.AsObject())
            .Single(item => item["kind"]!.GetValue<string>() == "textDefaultReflectionStartPosition");
        Assert.Equal(0.2, leaf["value"]!.GetValue<double>(), precision: 6);
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            Assert.Equal(35_000, reflection.StartPosition!.Value);
            Assert.Equal(100_000, reflection.EndPosition!.Value);
        }
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/text-default-reflection-start-position.pptx",
            },
        });
        ...
        Assert.Equal(0.35, reprojectedElement["text"]!["paragraphs"]![0]!["style"]!["defaultText"]!["reflection"]!["startPosition"]!.GetValue<double>(), precision: 6);
        Assert.Equal(0.35, reprojectedElement["nativeRef"]!["leaves"]!.AsArray()
            .Select(item => item!.AsObject())
            .Single(item => item["kind"]!.GetValue<string>() == "textDefaultReflectionStartPosition")["value"]!.GetValue<double>(), precision: 6);
    }

    [Fact]
    public void PpjSourceBoundTextDefaultReflectionStartPositionStaysOpaqueForPartialSpan()
    {
        var authored = CompileDefaultReflectionProgram();
        var source = RemoveEmbeddedPpj(authored.File.ToByteArray());
        source = RewriteReflectionPosition(source, 20_000, 80_000);
        ...
        Assert.DoesNotContain(projectedElement["nativeRef"]!["leaves"]!.AsArray(), item =>
            item!["kind"]!.GetValue<string>() == "textDefaultReflectionStartPosition");
    }

    private static CodecResponse CompileDefaultReflectionProgram()
    {
        ...
        program["pages"]![0]!["elements"]![0]!["text"] = new JsonObject
        {
            ["paragraphs"] = new JsonArray
            {
                new JsonObject
                {
                    ...
                    ["runs"] = new JsonArray
                    {
                        new JsonObject { ["text"] = "Default reflection start position" },
                    },
                },
            },
        };
    ...
    private static byte[] RewriteReflectionPosition(byte[] source, int startPosition, int endPosition)
    {
