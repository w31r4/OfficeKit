...
{
    [Fact]
    public void PpjSourceBoundImageShadowDistanceEditsLeafAndReprojects()
    {
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/source/image-shadow-distance.pptx",
            },
        });
        ...
        Assert.Equal(0.24, projectedImage["shadow"]!["opacity"]!.GetValue<double>());
        Assert.True(projectedImage["shadow"]!["rotateWithShape"]!.GetValue<bool>());
        var distanceLeaves = projectedImage["nativeRef"]!["leaves"]!.AsArray()
            .Select(leaf => leaf!.AsObject())
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "imageShadowDistanceEmu")
            .ToArray();
        Assert.Single(distanceLeaves);
        Assert.Equal(38_100, distanceLeaves[0]["value"]!.GetValue<long>());

        distanceLeaves[0]["value"] = 76_200;
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            var shadow = Assert.IsType<A.OuterShadow>(Assert.Single(picture.ShapeProperties!.GetFirstChild<A.EffectList>()!.ChildElements));
            Assert.True(shadow.RotateWithShape!.Value);
            Assert.Equal(76_200U, shadow.Distance!.Value);
            Assert.Equal(76_200U, shadow.BlurRadius!.Value);
            Assert.Equal(24_000, shadow.Descendants<A.Alpha>().Single().Val!.Value);
        }
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/image-shadow-distance.pptx",
            },
        });
        ...
        var reprojectedImage = reprojectedProgram["pages"]![0]!["elements"]!.AsArray()
            .Select(item => item!.AsObject())
            .Single(item => item["name"]!.GetValue<string>() == "evidence identity");
        Assert.Equal(6d, reprojectedImage["shadow"]!["distance"]!.GetValue<double>());
        Assert.Equal(6d, reprojectedImage["shadow"]!["blur"]!.GetValue<double>());
        Assert.Equal(0.24, reprojectedImage["shadow"]!["opacity"]!.GetValue<double>());
        Assert.True(reprojectedImage["shadow"]!["rotateWithShape"]!.GetValue<bool>());
        Assert.Equal(76_200, reprojectedImage["nativeRef"]!["leaves"]!.AsArray().Single(leaf =>
            leaf!["kind"]!.GetValue<string>() == "imageShadowDistanceEmu")!["value"]!.GetValue<long>());
    }
}
