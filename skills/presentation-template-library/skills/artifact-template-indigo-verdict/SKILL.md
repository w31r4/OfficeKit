...
Use this style when the brief matches **investment diligence; consulting presentation**. It is designed for
executives, strategy teams, decision makers. Its visual premise is A cool evidence field resolves into one explicit verdict and its conditions. The style is a
clean-room OfficeKit grammar: the included images teach a range of decisions,
not pages or coordinates to copy.

The deck should help the audience make a complex business choice legible by ordering evidence around a clear recommendation and its conditions. The natural story arc is
**question → hypothesis → framework → evidence → implication → recommendation → action**. This style is not a good fit for **creative manifesto**;
...
- Use **#FAFBFD** as the main field, **#15253C** for primary type, **#234E9C** for the decisive signal, **#25A6D9** for secondary evidence, and **#DCE3EA** for rules or context.
...
- Consulting guidance: Use assertion titles, one analytical framework per page, visible synthesis or so-what lines, and comparable fields for parallel cases. A two- or three-column layout must reflect a real content relationship rather than a default grid.
...
- The recurring structure is **verdict, variable page silhouettes, native data graphics**. Vary the silhouette between
  cover, evidence, data, process, comparison, and decision pages rather than
  repeating a component grid.
- Use a visible verdict field or end rail only after the evidence. Conditions and caveats must sit beside the verdict rather than being hidden in footnotes.
...
For this category, favor **executive overview, driver tree or pressure chart, comparison/ranking, bridge or waterfall, phased roadmap, implications by stakeholder**. The style's content shapes are
finding, comparison, recommendation; select one primary carrier per page and explain why it earns the
space.

### Information carriers

- frameworks, driver trees, waterfalls, scenario tables, timelines, matrices, horizontal rankings, and annotated before/after bars; label the argument on the marks rather than hiding it in a legend
- use documentary images only for context, a chapter break, or a real customer/operating setting; keep evidence pages independent from decorative photography
...
The expected density is **dense**: vary a decisive statement page with evidence-rich pages so the deck has pace without losing substance. Alternate page
...
- **Page skeleton.** Give the left two-thirds to evidence and the right third to a judgment panel. Begin with a serif assertion and scope line, then use a blue outline or target row to show what the evidence resolves.
- **Type rhythm.** Use high-contrast serif titles with neutral sans body and data. Mark actual versus estimate and keep every figure tied to a period and unit.
- **Evidence carriers.** Use stacked columns, waterfall bridges, heat tables, scatter plots, rankings, and target annotations. The right panel states the implication and condition; it is not a second chart.
- **Images and layers.** Use a profile or operating photograph only on a context page. Keep evidence pages clean and never place a label on top of an untested image texture.
- **Pacing.** Each page should progress evidence → judgment → condition. Use the blue focus as a target or selected row, not as a background wash.
- **Selection rule.** Choose this style when diligence or benchmarking must end in an explicit verdict with conditions.

## Signature moves and variation limits

- Let **verdict** recur as a quiet identity cue on selected pages; it must
...
- Avoid loose collages, unlabelled frameworks, equal columns with no comparison logic, generic quote slides, and recommendation pages that omit implementation conditions.
...
- Read titles alone, test the evidence order, verify every framework has a named question, check synthesis lines, and confirm each recommendation has an owner or next step.
