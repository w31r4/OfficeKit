...
- Expose one canonical signed `a:backdrop/a:up/@dy` coordinate in EMU.
- Replace only `backdrop/a:up/@dy` in the owning SlidePart.
...
- Reuse `TryReadSceneBackdropUpDy` and its requested-value validator so picture
...
- Token-splice only the direct `backdrop/a:up/@dy` attribute.

# Risks / Trade-offs

- [Risk] An up-vector Y token without a complete backdrop owner could be
