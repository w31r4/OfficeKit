...
internal static partial class PptxEditPlanCodec
{
        ...
        foreach (var operation in request.Operations)
        {
            ...
            if (element is P.Shape shape &&
                projectedElement.ContentCase == PresentationElement.ContentOneofCase.Shape &&
                 ((projectedElement.Source.Editable && (LeafKind(operation) is "fillRgb" or "fillOpacityThousandthPercent" or "shadowOpacityThousandthPercent" or "shadowColorRgb" or "shadowColorScheme" or "lineRgb" or "lineScheme" or "lineWidthEmu" or "leftEmu" or "topEmu" or "widthEmu" or "heightEmu" or "rotationDegrees" or "flipHorizontal" or "flipVertical")) ||
                 (!projectedElement.Source.Editable && LeafKind(operation) is ("fillRgb" or "fillOpacityThousandthPercent" or "shadowOpacityThousandthPercent" or "shadowColorRgb" or "shadowColorScheme" or "fillScheme" or "lineRgb" or "lineScheme" or "lineWidthEmu") && HasSafeNativeShapeStyle(shape, LeafKind(operation))) ||
    ...
    private static bool HasSafeNativeShadowOpacity(OpenXmlCompositeElement properties)
    {
        ...
        if (lists.Length != 1 || lists[0].ChildElements.Count != 1 || lists[0].FirstChild is not A.OuterShadow outer ||
            outer.ChildElements.Count != 1)
            return false;
        if (outer.FirstChild is not { } color)
            return false;
        var alphas = color.Elements<A.Alpha>().ToArray();
        return alphas.Length == 1 && color.ChildElements.Count == 1 &&
    ...
    private static PptxXmlPatch CompileScalarXmlPatch(string xml, XmlRange elementRange, PptxEditPlanProof proof)
    {
        ...
        switch (LeafKind(operation))
        {
            ...
            case "shadowOpacityThousandthPercent":
                if (owner != "sp") throw new CodecException("invalid_presentation_edit_target", $"PPTX edit operation {operation.OperationId} target does not expose shadowOpacityThousandthPercent.", operation.SlidePartPath);
                var effectList = DirectChildRange(xml, properties, "spPr", "effectLst", operation);
                var outerShadow = DirectChildRange(xml, effectList, "effectLst", "outerShdw", operation);
                var shadowColors = DirectChildRanges(xml, outerShadow)
                ...
                leaf = DirectChildRange(xml, shadowColors[0], shadowColors[0].LocalName, "alpha", operation);
                attribute = "val";
                break;
            case "shadowColorRgb":
            case "shadowColorScheme":
                ...
                var shadowOuter = DirectChildRange(xml, shadowEffectList, "effectLst", "outerShdw", operation);
            ...
            case "lineStyle":
                ...
                break;
            case "lineCap":
                if (owner != "cxnSp") throw new CodecException("invalid_presentation_edit_target", $"PPTX edit operation {operation.OperationId} target does not expose lineCap.", operation.SlidePartPath);
                leaf = DirectChildRange(xml, properties, "spPr", "ln", operation);
                attribute = "cap";
                break;
            case "lineJoin":
                if (owner != "cxnSp") throw new CodecException("invalid_presentation_edit_target", $"PPTX edit operation {operation.OperationId} target does not expose lineJoin.", operation.SlidePartPath);
                var joinOutline = DirectChildRange(xml, properties, "spPr", "ln", operation);
                return CompileNativeJoinXmlPatch(xml, joinOutline, proof);
            case "lineStartArrow":
                if (owner != "cxnSp") throw new CodecException("invalid_presentation_edit_target", $"PPTX edit operation {operation.OperationId} target does not expose lineStartArrow.", operation.SlidePartPath);
            ...
            case "lineEndArrow":
                if (owner != "cxnSp") throw new CodecException("invalid_presentation_edit_target", $"PPTX edit operation {operation.OperationId} target does not expose lineEndArrow.", operation.SlidePartPath);
