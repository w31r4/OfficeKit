...
- Owner: `p:sp/p:spPr/a:custGeom/a:pathLst/a:path/a:quadBezTo/a:pt[0]/@x`.
- PPJ surface: `shape.nativeRef.leaves[]`.
- Native leaf: `customGeometryPathQuadraticControlX`.
...
- The geometry must pass the existing bounded custom-geometry recognition
  ...
  attribute-only, and the control-point x must be a canonical signed
...
one end point, the canonical numeric control-point x token, and the requested
...
- No line-to, move-to, quadratic end-point, quadratic control-point y,
...
x leaf, changes it, checks that only the owning SlidePart changed and the
package remains valid, then projects again to recover the new x coordinate and
unchanged control-point y and end-point coordinates.
