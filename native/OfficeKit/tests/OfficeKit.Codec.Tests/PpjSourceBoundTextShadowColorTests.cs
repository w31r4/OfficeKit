...
{
    [Fact]
    public void PpjSourceBoundTextShadowColorEditsDirectRunOwnerAndReprojects()
    {
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/source/text-shadow-color.pptx",
            },
        });
        ...
        var leaves = projectedElement["nativeRef"]!["leaves"]!.AsArray()
            ...
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "textShadowColorRgb")
            .ToArray();
        var colorLeaf = Assert.Single(leaves);
        Assert.Equal("#16324f", colorLeaf["value"]!.GetValue<string>());
        ...
        using (var staleStream = new MemoryStream())
        {
            ...
            using (var stalePackage = PresentationDocument.Open(staleStream, true))
            {
                ...
                staleShadow.GetFirstChild<A.RgbColorModelHex>()!.Val = "A64B2A";
            }
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            Assert.Equal(19_050, shadow.Distance!.Value);
            Assert.Equal(5_400_000, shadow.Direction!.Value);
            Assert.Equal("A64B2A", shadow.GetFirstChild<A.RgbColorModelHex>()!.Val!.Value);
            Assert.Equal(72_000, shadow.Descendants<A.Alpha>().Single().Val!.Value);
        }
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/text-shadow-color.pptx",
            },
        });
        ...
        Assert.Equal("#A64B2A", reprojectedShadow["color"]!.GetValue<string>());
        ...
        Assert.Equal("#a64b2a", reprojectedElement["nativeRef"]!["leaves"]!.AsArray()
            .Select(leaf => leaf!.AsObject())
            .Single(leaf => leaf["kind"]!.GetValue<string>() == "textShadowColorRgb")["value"]!.GetValue<string>());
        ...
        var invalidLeaf = invalidProgram["pages"]![0]!["elements"]![0]!["nativeRef"]!["leaves"]!.AsArray()
            .Single(leaf => leaf!["kind"]!.GetValue<string>() == "textShadowColorRgb")!.AsObject();
        invalidLeaf["value"] = "#A64B2";
    ...
    [Fact]
    public void PpjSourceBoundTextShadowColorLeavesStayOpaqueForSiblingEffects()
    {
        var root = new DirectoryInfo(AppContext.BaseDirectory);
        while (root is not null && !File.Exists(Path.Combine(root.FullName, "package.json")))
            root = root.Parent;
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/source/text-shadow-color-complex.pptx",
            },
        });
        Assert.True(projected.Ok, Diagnostics(projected));
        var projectedProgram = JsonNode.Parse(projected.PresentationProgram.ProgramJson.ToByteArray())!.AsObject();
        ...
        foreach (var (label, mutate) in new[]
        {
            ...
            ("theme-color", (Action<A.OuterShadow>)(shadow => { shadow.GetFirstChild<A.RgbColorModelHex>()!.Remove(); shadow.AppendChild(new A.SchemeColor { Val = A.SchemeColorValues.Accent1 }); })),
        })
        {
            ...
            var unsupportedProjected = Invoke(new CodecRequest
            {
                ...
                PresentationProgram = new PresentationProgramRequest
                {
                    SourceUri = $"deck.assets/source/text-shadow-color-{label}.pptx",
                },
            });
