...
recognizes ordered `a:ahXY` and `a:ahPolar` handles, preserves their controlled
adjustment-guide identities, and validates their positions against the shape
frame. Generic PPJ native leaves are source-bound scalar operations proved
...
- Give a direct literal x position on an XY handle a stable native index and
  explicit EMU value.
- Keep `gdRefX/gdRefY`, bounds, handle kind/order, and all custom geometry
  topology source-bound.
- Prove a SlidePart-only x edit and second projection with a focused fixture.

**Non-Goals:**

- Editing handle guide identity, min/max bounds, y position, polar handles,
...
1. **Use the ordered handle index as identity.** The direct `ahLst` order is
   already retained by the native model, and the index remains stable even
   when a polar handle precedes an XY target. Name or coordinate matching was
   rejected because it would be ambiguous and could alter identity.

2. **Limit the leaf to direct XY literal x positions.** XY and polar handles
   use different native semantics; this leaf authorizes only `ahXY/pos/@x`.
   Literal canonicality and the shape-width bound are required because the
   resolved model cannot prove whether the source used a reference.
...
3. **Splice the nested position attribute.** The writer proves one direct
   ...
   one direct `a:pos` under the selected `ahXY`; it then replaces only `@x`.
   Guide references, range attributes, the y token, and package relationships
   remain untouched.
