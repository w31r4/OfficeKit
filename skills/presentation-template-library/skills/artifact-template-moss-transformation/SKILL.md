...
Use this style when the brief matches **transformation roadmap; consulting presentation**. It is designed for
executives, strategy teams, decision makers. Its visual premise is A dark transformation field makes progress, friction, and gates visible. The style is a
...
**question → hypothesis → framework → evidence → implication → recommendation → action**. This style is not a good fit for **heritage editorial**;
...
- Use **#202522** as the main field, **#EEF4EE** for primary type, **#58B88B** for the decisive signal, **#A5E26D** for secondary evidence, and **#414B45** for rules or context.
...
- The recurring structure is **wedge, variable page silhouettes, native data graphics**. Vary the silhouette between
  cover, evidence, data, process, comparison, and decision pages rather than
  repeating a component grid.
- Use a wedge or narrowing path to show concentration, bottleneck, or transformation. The narrowing must correspond to a measurable choice or constraint.
...
- **Page skeleton.** Use a charcoal transformation field with white type, a green phase cue, and square evidence blocks. Keep the title/subtitle/divider sequence stable while the body switches between a maturity view, pilot proof, and roadmap.
- **Type rhythm.** Use bold sans titles and compact body copy; all-caps section labels are navigation, not a second headline. Keep burgundy or other warning colors local to a real gap.
- **Evidence carriers.** Use horizontal stacked bars, maturity grids, pilot-versus-value figures, and roadmap tables. Put the interpretation beside the data and identify the gate that follows.
- **Images and layers.** A darkened context strip can establish transformation scope. Keep it separate from evidence and never use green blocks as a full-page decorative texture.
- **Pacing.** Baseline → gap → pilot → value → gate. Dense pages are expected, but each page must expose one operating question and one next move.
- **Selection rule.** Choose this style for transformation programs, capability maturity, and executive progress reviews.

## Signature moves and variation limits

- Let **wedge** recur as a quiet identity cue on selected pages; it must
