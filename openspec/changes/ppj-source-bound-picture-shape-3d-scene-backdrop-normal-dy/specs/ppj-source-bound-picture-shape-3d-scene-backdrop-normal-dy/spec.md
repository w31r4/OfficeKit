...
This capability preserves and minimally edits the backdrop normal Y coordinate
of a strict 3-D scene on an imported picture without owning the scene graph.

## ADDED Requirements

### Requirement: project a bounded picture backdrop normal Y

The presentation codec MUST project
`p:pic/p:spPr/a:scene3d/a:backdrop/a:norm/@dy` through the existing
`shape3dSceneBackdropNormalDyEmu` native leaf when the picture owner has
...
#### Scenario: project a source-bound picture backdrop normal Y

- **WHEN** a source PPTX contains a picture with a strict complete backdrop
  owner
- **THEN** the projected picture contains exactly one
  `shape3dSceneBackdropNormalDyEmu` native leaf with the normal Y coordinate

### Requirement: edit only the picture backdrop normal Y token

For a projected picture backdrop-normal-Y leaf, the presentation codec MUST
...
`a:scene3d/a:backdrop/a:norm/@dy` in the owning SlidePart. It MUST preserve
the anchor, normal X/Z, up vector, camera/light-rig state, picture
...
- **WHEN** a projected picture backdrop normal Y changes from `2` to `3`
- **THEN** only the owning SlidePart changes, the edited `dy` is `3`, and the
  picture's other XML and package parts remain intact

### Requirement: keep unsupported picture backdrops opaque

The codec MUST keep a picture backdrop-normal-Y owner source-bound when the
...
normalize such markup merely to expose or edit normal Y.
...
- **THEN** projection does not expose `shape3dSceneBackdropNormalDyEmu` for
