...
- Owner: `p:sp/p:spPr/a:scene3d/a:lightRig/@rig`.
- Native leaf: `shape3dSceneLightRigPreset`.
...
- Projection stores only the bounded light-rig preset in the native shape
  model and emits one leaf. The edit plan rechecks the full owner topology and
  token-splices only `lightRig/@rig`, preserving the camera, light direction,
...
No source-free authoring, camera editing, light rotation/direction editing,
