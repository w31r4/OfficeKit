...
token. This change applies the same ownership boundary to accent3 hue offset
...
- Accent3 transforms other than `hueOff`, transforms on accent3 through
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:hueOff`. The source writer patches `Accent3Color` only when the authored
   transform is the one existing accent3 hueOff and rejects all other transform
   flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent3HueOff` is accepted only with
   `accentTransforms.accent3.hueOff` in the source-issued capability. This
