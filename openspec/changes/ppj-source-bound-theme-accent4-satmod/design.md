...
transform and the direct accent4 tint, shade, luminance, and alpha leaves at a
time, then patches the existing DrawingML token. This change applies the same
ownership boundary to accent4 saturation modulation without broadening the
...
- Accent4 transforms other than `satMod`, transforms on accent4 through
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:satMod`. The source writer patches `Accent4Color` only when the authored
   transform is the one existing accent4 satMod and rejects all other
   transform flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent4SatMod` is accepted only with
   `accentTransforms.accent4.satMod` in the source-issued capability. This
