...
| F-03 | fields/WordArt/完整文本 | 部分完成（rich text 子集、静态 typed field display source-bound 编辑、bounded automatic `slidenum` cache refresh、复杂脚本 direct `a:cs` owner、formal `text.language` owner、source-free authored text glow/inner shadow/reflection/soft edge、direct rich-text run 及 paragraph `defaultText` glow-radius/color/theme-color/opacity/shadow-blur/distance/direction/alignment/color/theme-color/opacity/rotate-with-shape/inner-shadow-blur-distance-direction-color-theme-color-alpha/reflection-blur-distance-start-opacity-end-opacity-direction/soft-edge radius source-bound leaves 已交付） | 未覆盖的自动字段求值（date/author 等）、WordArt、defaultText 其它 effect leaf、复杂 effect 与完整语言/脚本字体回退组合 | P0 |
...
本轮后续增量（2026-09-06）：`textField.automatic` 已进入 PPJ/schema/wire，并在当前有界 profile 中支持 `type: "slidenum"`。authoring 与 source-bound 的形状、表格字段会按所属 slide 序号刷新 native 缓存；PPJ 二次投影恢复 `automatic: true` 和实际页码。静态字段缓存仍按原值写回，date/author 等未覆盖的自动字段和完整字段指令图继续 fail closed。
