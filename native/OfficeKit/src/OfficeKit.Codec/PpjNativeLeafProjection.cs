...
internal static class PpjNativeLeafProjection
{
    ...
    private static readonly HashSet<string> StringKinds = new(StringComparer.Ordinal)
    {
        ...
        "textBodyWarpPreset", "customGeometryGuideFormula", "customGeometryAdjustmentFormula", "textFieldType", "textFieldId", "tableTextFieldType", "tableTextFieldId",
    ...
    private static readonly HashSet<string> IntegerKinds = new(StringComparer.Ordinal)
    {
        ...
        "textInnerShadowBlurRadiusEmu", "textInnerShadowDistanceEmu", "textInnerShadowOpacityThousandthPercent", "textShadowBlurRadiusEmu", "textShadowDistanceEmu", "textShadowOpacityThousandthPercent", "textDefaultInnerShadowBlurRadiusEmu", "textDefaultInnerShadowDistanceEmu", "textDefaultInnerShadowDirectionDegrees", "textDefaultInnerShadowOpacityThousandthPercent",
    ...
    private static readonly IReadOnlyDictionary<string, int> ScaledNumberKinds =
        new Dictionary<string, int>(StringComparer.Ordinal)
        {
            ...
            ["textDefaultReflectionScaleX"] = 100_000,
            ...
            ["textDefaultReflectionEndPosition"] = 100_000,
    ...
    internal static string NormalizeValue(string kind, JsonElement value, string path)
    {
        ...
        if (kind is "textFieldId" or "tableTextFieldId")
        {
            var token = RequireString(value, kind, path);
            ...
            if (cell.TextBody is not null && PptxTableCodec.IsBoundedMixedRunTextBody(cell.TextBody))
            {
                ...
                foreach (var run in cell.TextBody.Paragraphs.SelectMany(paragraph => paragraph.Runs))
                {
                    if (run.ContentCase == PresentationTextRun.ContentOneofCase.Field)
                    {
                        ...
                        {
                            add("tableTextFieldId", run.Field.Id, JsonValue.Create(run.Field.Id), fieldIndex, cellIndex);
        ...
        Action<string, string, JsonNode?, uint, uint> add)
    {
        ...
        for (var paragraphIndex = 0; paragraphIndex < body.Paragraphs.Count; paragraphIndex++)
        {
            ...
            if (paragraph.DefaultRunStyleCase == PresentationTextParagraph.DefaultRunStyleOneofCase.DefaultRunProperties &&
                paragraph.DefaultRunProperties.Shadow is { HasScaleXThousandthPercent: true } shadowScaleX)
                AddScaled(add, "textDefaultShadowScaleX",
                    shadowScaleX.ScaleXThousandthPercent / 100_000d, 100_000, nativeIndex);
            if (paragraph.DefaultRunStyleCase == PresentationTextParagraph.DefaultRunStyleOneofCase.DefaultRunProperties &&
                paragraph.DefaultRunProperties.Shadow is { HasScaleYThousandthPercent: true } shadowScaleY)
                AddScaled(add, "textDefaultShadowScaleY",
                    shadowScaleY.ScaleYThousandthPercent / 100_000d, 100_000, nativeIndex);
            if (paragraph.DefaultRunStyleCase == PresentationTextParagraph.DefaultRunStyleOneofCase.DefaultRunProperties &&
            ...
            if (paragraph.DefaultRunStyleCase == PresentationTextParagraph.DefaultRunStyleOneofCase.DefaultRunProperties &&
                paragraph.DefaultRunProperties.Reflection is { HasEndPositionThousandthPercent: true } reflectionEndPosition &&
                (!reflectionEndPosition.HasStartPositionThousandthPercent || reflectionEndPosition.StartPositionThousandthPercent == 0))
                AddScaled(add, "textDefaultReflectionEndPosition",
                    reflectionEndPosition.EndPositionThousandthPercent / 100_000d, 100_000, nativeIndex);
            if (paragraph.DefaultRunStyleCase == PresentationTextParagraph.DefaultRunStyleOneofCase.DefaultRunProperties &&
            ...
            if (paragraph.DefaultRunStyleCase == PresentationTextParagraph.DefaultRunStyleOneofCase.DefaultRunProperties &&
                paragraph.DefaultRunProperties.Reflection is { HasScaleXThousandthPercent: true } reflectionScaleX)
                AddScaled(add, "textDefaultReflectionScaleX",
                    reflectionScaleX.ScaleXThousandthPercent / 100_000d, 100_000, nativeIndex);
            if (paragraph.DefaultRunStyleCase == PresentationTextParagraph.DefaultRunStyleOneofCase.DefaultRunProperties &&
                paragraph.DefaultRunProperties.Reflection is { HasScaleYThousandthPercent: true } reflectionScaleY)
                AddScaled(add, "textDefaultReflectionScaleY",
                    reflectionScaleY.ScaleYThousandthPercent / 100_000d, 100_000, nativeIndex);
            if (paragraph.DefaultRunStyleCase == PresentationTextParagraph.DefaultRunStyleOneofCase.DefaultRunProperties &&
                paragraph.DefaultRunProperties.Reflection is { HasSkewXAngle60000: true } reflectionSkewX)
                AddScaled(add, "textDefaultReflectionSkewX",
                    reflectionSkewX.SkewXAngle60000 / 60_000d, 60_000, nativeIndex);
            if (paragraph.DefaultRunStyleCase == PresentationTextParagraph.DefaultRunStyleOneofCase.DefaultRunProperties &&
