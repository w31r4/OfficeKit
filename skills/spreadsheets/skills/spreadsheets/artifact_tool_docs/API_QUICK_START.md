...
- `SLN` accepts `cost, salvage, life` for straight-line expense. `DB` accepts `cost, salvage, life, period, [month]` and preserves its native three-decimal fixed-declining rate with optional first/final partial-year proration. `DDB` accepts `cost, salvage, life, period, [factor]`, defaults factor to `2`, and caps expense at the remaining amount above salvage. `DB`/`DDB` require nonnegative cost/salvage, salvage no greater than cost, integer useful life/periods through 9,999, and valid month/factor values; invalid inputs fail as `#VALUE!` or `#NUM!` rather than being coerced.
