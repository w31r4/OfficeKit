...
{
    [Fact]
    public void PpjSourceBoundLiteralCustomGeometryPathArcStartAngleLeafEditsAndReprojects()
    {
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            {
                SourceUri = "deck.assets/source/custom-geometry-path-arc-start-angle.pptx",
            },
        });
        ...
        var arcLeaves = projectedShape["nativeRef"]!["leaves"]!.AsArray()
            ...
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "customGeometryPathArcStartAngle60000")
            .ToArray();
        ...
        Assert.Contains("wR=\"200\" hR=\"150\" stAng=\"1800000\" swAng=\"5400000\"", editedXml, StringComparison.Ordinal);
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/custom-geometry-path-arc-start-angle.pptx",
            },
        });
        ...
        var reprojectedLeaves = reprojectedShape["nativeRef"]!["leaves"]!.AsArray()
            ...
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "customGeometryPathArcStartAngle60000")
            .ToArray();
        Assert.Equal(1_800_000, Assert.Single(reprojectedLeaves)["value"]!.GetValue<long>());
    }
}
