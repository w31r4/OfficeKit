...
This capability gives PPJ a safe, field-qualified source-bound owner for one
 direct accent4 hue-modulation token while preserving the rest of an imported theme.

## ADDED Requirements

### Requirement: Strict source-bound accent4 hueMod projection

The PPTX projector MUST expose
`design.theme.accentTransforms.accent4.hueMod` and
`setThemeAccent4HueMod` only when the canonical shared ThemePart has one
direct accent4 RGB leaf with exactly one direct `a:hueMod/@val` child, no
...
#### Scenario: Project a direct accent4 hueMod leaf

- **GIVEN** a canonical imported ThemePart with accent4 `334455` and
  `a:hueMod/@val="35000"`
- **WHEN** the presentation is projected to PPJ
- **THEN** the theme contains `accentTransforms.accent4.hueMod` equal to
  `0.35`
- **AND** the native reference contains `setThemeAccent4HueMod` authorized
  for `accentTransforms.accent4.hueMod`

#### Scenario: Reject unsupported accent4 hueMod topology
...
- **THEN** the source-bound accent4 hueMod field and capability are omitted

### Requirement: Source-bound accent4 hueMod editing

The source-preserving compiler MUST accept one authorized change to the
existing `design.theme.accentTransforms.accent4.hueMod` field, convert a
...
#### Scenario: Edit and reproject one accent4 hueMod token

- **GIVEN** a projected direct accent4 hueMod owner at `0.35`
...
#### Scenario: Fail closed for unsafe accent4 hueMod edits

- **GIVEN** a source-bound accent4 hueMod owner
