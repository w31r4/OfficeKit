...
const evidence = JSON.parse(await readFile(path.resolve("evals/pptx-lossless/source-agent-continuation.v2.json"), "utf8"));
assert.equal(evidence.schema, "office-kit/pptx-source-agent-continuation-rehearsal/v2");
assert.deepEqual(evidence.protocol, { repl: 2, visualReview: "unavailable", package: "public-office-kit", workflow: "bounded-overlay-resume" });
...
for (const source of evidence.sources) {
  ...
  assert.equal(source.finalVerification.kind, "bounded-overlay");
  assert.equal(source.finalVerification.result.foundResumed, true);
  assert.equal(source.finalVerification.result.accentMoved, true);
  assert.equal(source.finalVerification.result.imagePresent, true);
  assert.equal(source.finalVerification.result.capabilityReady, true);
  assert.equal(source.finalVerification.result.capabilityProfile, "bounded-overlay");
  ...
  assert.equal(source.publishedSha256, source.commits[1].revisionSha256);
