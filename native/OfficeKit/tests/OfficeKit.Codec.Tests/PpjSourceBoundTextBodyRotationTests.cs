...
{
    [Fact]
    public void PpjSourceBoundTextBodyRotationEditsLeafAndReprojects()
    {
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            Assert.Equal(90 * 60_000, bodyProperties.Rotation!.Value);
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/source/text-body-rotation.pptx",
            },
        });
        Assert.True(projected.Ok, Diagnostics(projected));
        var projectedProgram = JsonNode.Parse(projected.PresentationProgram.ProgramJson.ToByteArray())!.AsObject();
        var projectedShape = projectedProgram["pages"]![0]!["elements"]!.AsArray().Single()!.AsObject();
        Assert.Equal(90, projectedShape["textStyle"]!["rotation"]!.GetValue<double>(), 3);
        var leaves = projectedShape["nativeRef"]!["leaves"]!.AsArray()
            .Select(leaf => leaf!.AsObject())
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "textBodyRotationDegrees")
            .ToArray();
        Assert.Single(leaves);
        Assert.Equal(90, leaves[0]["value"]!.GetValue<double>(), 3);
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            var nativeShape = package.PresentationPart!.SlideParts.Single().Slide!.Descendants<P.Shape>().Single();
            var bodyProperties = nativeShape.TextBody!.GetFirstChild<A.BodyProperties>()!;
            Assert.Equal(135 * 60_000, bodyProperties.Rotation!.Value);
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/text-body-rotation.pptx",
            },
        });
        ...
        Assert.Equal(135, reprojectedShape["textStyle"]!["rotation"]!.GetValue<double>(), 3);
        Assert.Equal(135, reprojectedShape["nativeRef"]!["leaves"]!.AsArray().Single(leaf =>
            leaf!["kind"]!.GetValue<string>() == "textBodyRotationDegrees")["value"]!.GetValue<double>(), 3);
    }
}
