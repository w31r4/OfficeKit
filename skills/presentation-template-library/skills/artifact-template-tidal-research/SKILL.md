...
Use this style when the brief matches **field study; academic presentation**. It is designed for
researchers, students, technical reviewers. Its visual premise is Field evidence is organized by tidal lines and observation windows. The style is a
...
**question → method → observation → derivation or comparison → interpretation → limitations → conclusion**. This style is not a good fit for **consumer launch**;
...
- Use **#FBFCFA** as the main field, **#17352F** for primary type, **#188A78** for the decisive signal, **#58AEE6** for secondary evidence, and **#E1B84D** for rules or context.
...
- The recurring structure is **tide, variable page silhouettes, documentary imagery**. Vary the silhouette between
  cover, evidence, data, process, comparison, and decision pages rather than
  repeating a component grid.
- Use observation windows, repeated horizontal traces, and measured intervals to show field change. Keep the time basis visible and do not decorate the trace.
...
- **Page skeleton.** Use a field-notebook grammar: observation window, measured trace, and a calm explanatory margin. Start with the research question, then expose method and evidence before interpretation; use dark green/teal as a measured focus.
- **Type rhythm.** Use a readable type pairing and reserve strong weight for question, result, and labels. Keep variables, sample sizes, units, and limitations adjacent to the figure they qualify.
- **Evidence carriers.** Use measurement lines, annotated distributions, maps, timelines, specimen/context diagrams, and comparison tables. A line is meaningful only when its time or sampling basis is visible.
- **Images and layers.** Prefer authentic field photographs or specimen images when they teach context. Crop and credit them; never use a generic landscape to imply evidence.
- **Pacing.** Question → method → observation → interpretation → limitation → conclusion. Give a complex method page enough local whitespace without leaving the body empty.
- **Selection rule.** Choose this style for field studies, experiments, academic defenses, and evidence with explicit uncertainty.

## Signature moves and variation limits

- Let **tide** recur as a quiet identity cue on selected pages; it must
