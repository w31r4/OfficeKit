...
function isArtifactToolPackage(packageDir) {
  return isNamedPackage(packageDir, ARTIFACT_TOOL_PACKAGE);
}

function artifactToolEntrypointPath(packageDir) {
  const packageJson = readPackageJson(packageDir);
  const rootExport = typeof packageJson?.exports?.["."] === "string"
    ? path.resolve(packageDir, packageJson.exports["."])
    : undefined;
  const candidates = [
    rootExport,
...
function owningArtifactToolPackage(startDir = import.meta.dirname) {
  let current = path.resolve(startDir);
  while (true) {
    if (isArtifactToolPackage(current)) return current;
...
function validateArtifactToolPackage(packageDir, context) {
  ...
  if (!isArtifactToolPackage(packageDir)) {
    ...
    throw new Error(
      [
        `Expected ${context} to point to ${ARTIFACT_TOOL_PACKAGE}.`,
  ...
  if (compareSemver(packageJson.version, MIN_ARTIFACT_TOOL_VERSION) < 0) {
    throw new Error(
      [
        `Expected ${context} to point to ${ARTIFACT_TOOL_PACKAGE} ${MIN_ARTIFACT_TOOL_VERSION} or newer.`,
        `Checked ${packageDir}; found ${ARTIFACT_TOOL_PACKAGE}@${packageJson.version || "unknown"}.`,
      ].join("\n"),
    );
  }
  ...
  if (!entrypoint) {
    throw new Error(
      [
        ...
        `Checked the package root export plus ${path.join(packageDir, "src", "index.mjs")} and legacy dist entrypoints.`,
        "Install or build the public package before retrying.",
      ].join("\n"),
    );
  }
...
export async function ensureArtifactToolWorkspace(workspaceDir) {
  ...
  await ensureWorkspacePackage(resolvedWorkspace, ARTIFACT_TOOL_PACKAGE, sourcePackage);
