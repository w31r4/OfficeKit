...
direct accent4 green-channel modulation token while preserving the rest of an
imported theme.

## ADDED Requirements

### Requirement: Strict source-bound accent4 greenMod projection

The PPTX projector MUST expose
`design.theme.accentTransforms.accent4.greenMod` and
`setThemeAccent4GreenMod` only when the canonical shared ThemePart has one
direct accent4 RGB leaf with exactly one direct `a:greenMod/@val` child, no
...
#### Scenario: Project a direct accent4 greenMod leaf
...
- **THEN** the theme contains `accentTransforms.accent4.greenMod` equal to
  `0.25`
- **AND** the native reference contains `setThemeAccent4GreenMod` authorized
  for `accentTransforms.accent4.greenMod`

#### Scenario: Reject unsupported accent4 greenMod topology
...
- **THEN** the source-bound accent4 greenMod field and capability are omitted

### Requirement: Source-bound accent4 greenMod editing

The source-preserving compiler MUST accept one authorized change to the
existing `design.theme.accentTransforms.accent4.greenMod` field, convert a
...
#### Scenario: Edit and reproject one accent4 greenMod token

- **GIVEN** a projected direct accent4 greenMod owner at `0.25`
...
#### Scenario: Fail closed for unsafe accent4 greenMod edits

- **GIVEN** a source-bound accent4 greenMod owner
