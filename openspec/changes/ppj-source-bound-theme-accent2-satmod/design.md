...
transform and the direct accent2 tint, shade, luminance, and alpha leaves at a
time, then patches the existing DrawingML token. This change applies the same
ownership boundary to accent2 saturation modulation without broadening the
...
- Accent2 transforms other than `satMod`, transforms on accent3 through
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:satMod`. The source writer patches `Accent2Color` only when the authored
   transform is the one existing accent2 satMod and rejects all other
   transform flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent2SatMod` is accepted only with
   `accentTransforms.accent2.satMod` in the source-issued capability. This
