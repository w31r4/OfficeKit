...
当前集成汇总采用 `tmp/officekit-native-scene-paint-GjnuAZ/integration.json`，报告时间为北京时间 2026-09-10 07:29:52。它使用第 3.5 节的独立新包与单独记录摘要的 JS，不是整个持续变化工作区的全量验收。正式入口和检查开关仍未切换：
...
该报告使用冻结提交 `c8b0d324` 的独立构建，包摘要见第 3.5 节；之后的工作区变更不在该原生包内。性能文件摘要记录为 `a9b3daace6a12a8a9226d1e45d17e79cd83e68cfec12f5cb1288885884ee9719`。报告中的 SmartArt 源绘制是明确的 unavailable 反例，不因失败列表为空就变成视觉保真成功。历史成功报告保留为对应版本的证据，不能覆盖新增的失败断言。
