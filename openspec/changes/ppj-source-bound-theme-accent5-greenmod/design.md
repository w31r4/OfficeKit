...
the same ownership boundary to accent5 green-channel modulation without
...
- Accent5 transforms other than `greenMod`, transforms on accent5 through
...
1. **Use the existing native transform field.**
   `PresentationThemeColorTransform.GreenModulationThousandth` already
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:greenMod`. The source writer patches `Accent5Color` only when the authored
   transform is the one existing accent5 greenMod and rejects all other transform
   flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent5GreenMod` is accepted only with
   `accentTransforms.accent5.greenMod` in the source-issued capability. This
