...
This change applies the same ownership boundary to accent2 blue-channel modulation
...
- Recognize only a canonical shared ThemePart whose accent2 color is a direct
  six-digit RGB leaf with one direct `a:blueMod/@val` child.
...
- Accent2 transforms other than `blueMod`, transforms on accent3 through
  accent6, inherited or effect theme state, or arbitrary theme XML editing.
- Creating or deleting a missing blueMod node, changing multiple theme fields,
  or claiming PowerPoint host color rendering.

## Decisions

1. **Use the existing native transform field.**
  `PresentationThemeColorTransform.BlueModulationThousandth` already represents
  the DrawingML 0..100000 modulation integer and the authored compiler
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:blueMod`. The source writer patches `Accent2Color` only when the authored
   transform is the one existing accent2 blueMod and rejects all other transform
   flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent2BlueMod` is accepted only with
   `accentTransforms.accent2.blueMod` in the source-issued capability. This
