...
{
    [Fact]
    public void PpjSourceBoundPictureShape3dExtrusionHeightLeafEditsAndReprojects()
    {
        ...
        var source = AddPictureShape3dExtrusionHeight(RemoveEmbeddedPpj(authored.File.ToByteArray()), 1_200);
        var projected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/source/picture-shape-3d-extrusion-height.pptx",
            },
        });
        ...
        var target = Assert.Single(projectedImage["nativeRef"]!["leaves"]!.AsArray()
            ...
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "shape3dExtrusionHeightEmu"));
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            Assert.Equal(2_400, picture.ShapeProperties!.GetFirstChild<A.Shape3DType>()!.ExtrusionHeight!.Value);
        }
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/picture-shape-3d-extrusion-height.pptx",
            },
        });
        ...
        Assert.Equal(2_400, reprojectedImage["nativeRef"]!["leaves"]!.AsArray()
            .Select(leaf => leaf!.AsObject())
            .Single(leaf => leaf["kind"]!.GetValue<string>() == "shape3dExtrusionHeightEmu")["value"]!.GetValue<long>());
    }

    private static byte[] AddPictureShape3dExtrusionHeight(byte[] source, long height)
    {
        ...
        using (var package = PresentationDocument.Open(stream, true))
        {
            ...
            picture.ShapeProperties!.AppendChild(new A.Shape3DType
            {
