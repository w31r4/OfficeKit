...
| `slide.shapes.add` | api | Add a shape/textbox with preset or bounded DrawingML custom geometry using ordered adjustment/guide formulas, or create a direct connector only with explicit from/to target and connection-site indexes. |
...
Add a shape/textbox with preset or bounded DrawingML custom geometry using ordered adjustment/guide formulas, or create a direct connector only with explicit from/to target and connection-site indexes.

**Schema parameters:**

- `name` (string) — Inspectable shape name.
- `geometry` (string) — rect, ellipse, roundRect, textbox, custom, or connector. Custom requires customPaths; connector requires from, to, fromIdx, and toIdx.
...
- `shape` (Shape|ConnectorElement) — Appended editable shape/textbox or exact-site connector. Unsupported site tables and incomplete connector endpoints fail closed.
