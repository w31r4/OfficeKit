...
{
    [Fact]
    public void PpjSourceBoundTableBandedRowsEditsLeafAndReprojects()
    {
        ...
        {
            Id = "presentation/slide/1/table/native-banded-rows",
            Name = "Native banded rows table",
            Table = table,
        });
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            Assert.True(nativeTable.TableProperties!.BandRow!.Value);
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            {
                SourceUri = "table-banded-rows/source.pptx",
            },
        });
        ...
        Assert.True(projectedTable["style"]!["bandedRows"]!.GetValue<bool>());
        var leaves = projectedTable["nativeRef"]!["leaves"]!.AsArray()
            .Select(leaf => leaf!.AsObject())
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "tableBandedRows")
            .ToArray();
        ...
        {
            var nativeTable = Assert.Single(package.PresentationPart!.SlideParts.Single().Slide!.Descendants<A.Table>());
            Assert.False(nativeTable.TableProperties!.BandRow!.Value);
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "table-banded-rows/edited.pptx",
            },
        });
        ...
        var reprojectedTable = reprojectedProgram["pages"]!.AsArray()
            ...
            .Select(item => item!.AsObject())
            .Single(item => item["type"]!.GetValue<string>() == "table");
        Assert.False(reprojectedTable["style"]!["bandedRows"]!.GetValue<bool>());
        Assert.False(reprojectedTable["nativeRef"]!["leaves"]!.AsArray().Single(leaf =>
            leaf!["kind"]!.GetValue<string>() == "tableBandedRows")["value"]!.GetValue<bool>());
    }
}
