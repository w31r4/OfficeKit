...
direct accent3 red-channel modulation token while preserving the rest of an
imported theme.

## ADDED Requirements

### Requirement: Strict source-bound accent3 redMod projection

The PPTX projector MUST expose
`design.theme.accentTransforms.accent3.redMod` and
`setThemeAccent3RedMod` only when the canonical shared ThemePart has one
direct accent3 RGB leaf with exactly one direct `a:redMod/@val` child, no
...
#### Scenario: Project a direct accent3 redMod leaf

- **GIVEN** a canonical imported ThemePart with accent3 `334455` and
  `a:redMod/@val="25000"`
- **WHEN** the presentation is projected to PPJ
- **THEN** the theme contains `accentTransforms.accent3.redMod` equal to
  `0.25`
- **AND** the native reference contains `setThemeAccent3RedMod` authorized
  for `accentTransforms.accent3.redMod`

#### Scenario: Reject unsupported accent3 redMod topology
...
- **THEN** the source-bound accent3 redMod field and capability are omitted

### Requirement: Source-bound accent3 redMod editing

The source-preserving compiler MUST accept one authorized change to the
existing `design.theme.accentTransforms.accent3.redMod` field, convert a
...
#### Scenario: Edit and reproject one accent3 redMod token

- **GIVEN** a projected direct accent3 redMod owner at `0.25`
...
#### Scenario: Fail closed for unsafe accent3 redMod edits

- **GIVEN** a source-bound accent3 redMod owner
