...
transform and the direct accent2 tint, shade, luminance modulation, luminance
offset, and alpha modulation leaves at a time, then patches the existing
DrawingML token. This change applies the same ownership boundary to accent2
alpha offset without broadening the theme graph or changing authored six-role
lowering.
...
- Recognize only a canonical shared ThemePart whose accent2 color is a direct
  six-digit RGB leaf with one direct `a:alphaOff/@val` child.
...
- Accent2 transforms other than `alphaOff`, transforms on accent3 through
  accent6, inherited or effect theme state, or arbitrary theme XML editing.
- Creating or deleting a missing alphaOff node, changing multiple theme fields,
  or claiming PowerPoint host color rendering.

## Decisions

1. **Use the existing native transform field.**
   `PresentationThemeColorTransform.AlphaOffsetThousandth` already represents
   the DrawingML signed -100000..100000 integer and the authored compiler
   already emits it. The importer records a role of `accent2`; the projector
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:alphaOff`. The source writer patches `Accent2Color` only when the
   authored transform is the one existing accent2 alphaOff and rejects all
   other transform flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent2AlphaOff` is accepted only with
   `accentTransforms.accent2.alphaOff` in the source-issued capability. This
