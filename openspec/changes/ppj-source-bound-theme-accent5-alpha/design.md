...
The source-bound accent palette currently accepts only six-digit direct RGB leaves for accent5 through accent6. Authored PPJ already accepts six- or eight-digit theme colors and lowers the trailing byte to `a:alpha`; the imported profile does not yet preserve that child for accent5.

## Goals / Non-Goals

**Goals:**

- Read `accent5` as `#RRGGBBAA` when its direct `srgbClr` has exactly one canonical `a:alpha` child.
- Keep the existing six accent object and `setThemeAccent5Color` field authority.
...
- Adding alpha ownership to accent6 or the six non-accent color roles in this increment.
...
1. **Use the existing string field.** `PresentationThemeArtifact.accent_rgb[4]` already carries authored RGB/RGBA and avoids a new protobuf field or wire version.
...
4. **Patch one field owner.** The writer compares RGB and alpha separately, updates only those two attributes under accent5, and leaves the other five accents and all theme descendants source-owned.
