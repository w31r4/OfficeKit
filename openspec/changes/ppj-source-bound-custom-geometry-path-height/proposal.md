...
explicit `a:path/@h` viewport height is still trapped inside the opaque path
...
- Add `customGeometryPathHeight` as a source-bound PPJ native leaf for each
  recognized custom-geometry path with a positive literal height.
...
- Token-splice only the selected direct `a:path/@h`, preserving width,
  fill/stroke/extrusion attributes, commands, sibling paths, and package parts.
...
- `ppj-source-bound-geometry-path-height`: source-bound editing of one explicit
  custom-geometry path viewport height.
