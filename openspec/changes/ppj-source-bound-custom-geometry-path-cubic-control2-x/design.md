...
- Owner: `p:sp/p:spPr/a:custGeom/a:pathLst/a:path/a:cubicBezTo/a:pt[1]/@x`.
- PPJ surface: `shape.nativeRef.leaves[]`.
- Native leaf: `customGeometryPathCubicControl2X`.
...
- The geometry must pass the existing bounded custom-geometry recognition
  ...
  attribute-only, and the second control-point x must be a canonical signed
  DrawingML coordinate; formula/reference-backed x values are not issued.
...
one end point, the canonical numeric second-control-point x token, and the
requested changed bounded coordinate. The writer then replaces only the
selected `x` value token on the second point.

## Non-goals

- No public path-command array or command insertion/deletion.
- No line-to, move-to, quadratic, cubic first-control-point or end-point,
  second-control-point y, formula/reference, path property, or multi-path
  topology edits.
...
cubic command, removes embedded PPJ, projects the cubic second-control-point x
...
unchanged control-point y and remaining point coordinates.
