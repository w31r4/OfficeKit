...
and blue transform leaves at a time, then patches the existing DrawingML token.
This change applies the same ownership boundary to accent2 hue modulation
...
- Recognize only a canonical shared ThemePart whose accent2 color is a direct
  six-digit RGB leaf with one direct `a:hueMod/@val` child.
...
- Accent2 transforms other than `hueMod`, transforms on accent3 through
  accent6, inherited or effect theme state, or arbitrary theme XML editing.
- Creating or deleting a missing hueMod node, changing multiple theme fields,
  or claiming PowerPoint host color rendering.

## Decisions

1. **Use the existing native transform field.**
   `PresentationThemeColorTransform.HueModulationThousandth` already represents
   the DrawingML 0..100000 modulation integer and the authored compiler already
   emits it. The importer records a role of `accent2`; the projector converts
   the native value by dividing by 100000, and the source-bound compiler rounds
   by `Math.Round(value * 100000d, MidpointRounding.AwayFromZero)`.
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:hueMod`. The source writer patches `Accent2Color` only when the authored
   transform is the one existing accent2 hueMod and rejects all other transform
   flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent2HueMod` is accepted only with
   `accentTransforms.accent2.hueMod` in the source-issued capability. This
