...
the same ownership boundary to accent5 red-channel offset without
...
- Accent5 transforms other than `redOff`, transforms on accent5 through
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:redOff`. The source writer patches `Accent5Color` only when the authored
   transform is the one existing accent5 redOff and rejects all other transform
   flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent5RedOff` is accepted only with
   `accentTransforms.accent5.redOff` in the source-issued capability. This
