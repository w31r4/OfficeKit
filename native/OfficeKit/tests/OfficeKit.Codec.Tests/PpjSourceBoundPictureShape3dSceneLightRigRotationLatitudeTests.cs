...
{
    [Fact]
    public void PpjSourceBoundPictureShape3dSceneLightRigRotationLatitudeLeafEditsAndReprojects()
    {
        ...
        var source = AddPictureShape3dSceneLightRigRotationLatitude(RemoveEmbeddedPpj(authored.File.ToByteArray()), "0");
        var projected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/source/picture-shape-3d-scene-light-rig-rotation-latitude.pptx",
            },
        });
        ...
        var target = Assert.Single(projectedImage["nativeRef"]!["leaves"]!.AsArray()
            ...
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "shape3dSceneLightRigRotationLatitude60000"));
        ...
        Assert.Contains("lat=\"1800000\"", editedXml, StringComparison.Ordinal);
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            Assert.Equal("1800000", rotation.GetAttribute("lat", string.Empty).Value);
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/picture-shape-3d-scene-light-rig-rotation-latitude.pptx",
            },
        });
    ...
    private static byte[] AddPictureShape3dSceneLightRigRotationLatitude(byte[] source, string latitude)
    {
        ...
        using (var package = PresentationDocument.Open(stream, true))
        {
            ...
            rotation.SetAttribute(new OpenXmlAttribute("lat", string.Empty, latitude));
