...
accent1 plus accent5 tint, shade, luminance, alpha, saturation, and red
modulation transforms. A direct imported accent5 `a:blueMod` remains opaque,
...
- Project a canonical shared ThemePart whose accent5 direct RGB leaf contains
  ...
  `design.theme.accentTransforms.accent5.blueMod`, using a 0..1 fraction.
- Issue `setThemeAccent5BlueMod` for that field and compile one authorized
...
- `presentation-theme-accent5-bluemod`: Strict source-bound projection and
  source-preserving editing of one direct accent5 `a:blueMod` leaf.
