...
Use this style when the brief matches **community campaign; promotion presentation**. It is designed for
public audiences, communities, brand teams. Its visual premise is Hard red and blue paper cuts turn information into invitation. The style is a
...
**big idea → audience truth → creative system → waves or experiences → measured outcome → next move**. This style is not a good fit for **audit report**;
...
- Use **#F2E6CE** as the main field, **#171717** for primary type, **#F0523B** for the decisive signal, **#2E5ED7** for secondary evidence, and **#C9DA43** for rules or context.
...
- The recurring structure is **collage, variable page silhouettes, native data graphics**. Vary the silhouette between
  cover, evidence, data, process, comparison, and decision pages rather than
  repeating a component grid.
- Use hard-edged fields, cropped media, and one controlled overlap to create invitation and energy. Keep measurement labels on a clean plane and avoid sticker-like clutter.
...
- **Page skeleton.** Lay a cream paper ground under hard-edged red and cobalt planes. Use an invitation or audience truth as the headline, then let a collage field, timeline, or outcome chart carry the story; never make every page a cut-paper poster.
- **Type rhythm.** Use a bold display face for the proposition and plain, readable body text. Keep the yellow-green action signal short and local, and avoid playful lettering in evidence notes.
- **Evidence carriers.** Use audience ladders, activation timelines, channel mixes, before/after bars, and a compact outcome score. A geometric cut is meaningful only when it marks sequence, contrast, or identity.
- **Images and layers.** Use user-provided or properly sourced images when they carry community identity. Crop them as hard-edged fields, attach alt text, and do not use generic stock people as emotional filler.
- **Pacing.** Alternate a bold proposition, a human insight, a system or activation page, and measurable results. Keep one visual gesture per page so the collage remains legible.
- **Selection rule.** Choose this style for civic campaigns, public programs, and community-facing proposals that need energy and evidence together.

## Signature moves and variation limits

- Let **collage** recur as a quiet identity cue on selected pages; it must
