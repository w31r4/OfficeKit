...
{
    [Fact]
    public void PpjSourceBoundTextBodyHorizontalOverflowEditsLeafAndReprojects()
    {
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/source/text-body-horizontal-overflow.pptx",
            },
        });
        Assert.True(projected.Ok, Diagnostics(projected));
        var projectedProgram = JsonNode.Parse(projected.PresentationProgram.ProgramJson.ToByteArray())!.AsObject();
        var projectedShape = projectedProgram["pages"]![0]!["elements"]!.AsArray().Single()!.AsObject();
        Assert.Equal("overflow", projectedShape["textStyle"]!["horizontalOverflow"]!.GetValue<string>());
        var leaves = projectedShape["nativeRef"]!["leaves"]!.AsArray()
            .Select(leaf => leaf!.AsObject())
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "textBodyHorizontalOverflow")
            .ToArray();
        Assert.Single(leaves);
        Assert.Equal("overflow", leaves[0]["value"]!.GetValue<string>());
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/text-body-horizontal-overflow.pptx",
            },
        });
        ...
        Assert.Equal("clip", reprojectedShape["textStyle"]!["horizontalOverflow"]!.GetValue<string>());
        Assert.Equal("clip", reprojectedShape["nativeRef"]!["leaves"]!.AsArray().Single(leaf =>
            leaf!["kind"]!.GetValue<string>() == "textBodyHorizontalOverflow")["value"]!.GetValue<string>());
    }
}
