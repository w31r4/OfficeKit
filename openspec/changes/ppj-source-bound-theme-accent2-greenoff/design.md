...
This change applies the same ownership boundary to accent2 green-channel offset
...
- Accent2 transforms other than `greenOff`, transforms on accent3 through
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:greenOff`. The source writer patches `Accent2Color` only when the authored
   transform is the one existing accent2 greenOff and rejects all other transform
   flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent2GreenOff` is accepted only with
   `accentTransforms.accent2.greenOff` in the source-issued capability. This
