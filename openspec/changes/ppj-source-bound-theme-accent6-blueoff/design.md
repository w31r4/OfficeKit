...
This change applies the same ownership boundary to accent6 blue-channel offset
...
- Accent6 transforms other than `blueOff`, transforms on other accent roles, inherited or effect theme state, or arbitrary theme XML editing.
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:blueOff`. The source writer patches `Accent6Color` only when the authored
   transform is the one existing accent6 blueOff and rejects all other transform
   flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent6BlueOff` is accepted only with
   `accentTransforms.accent6.blueOff` in the source-issued capability. This
