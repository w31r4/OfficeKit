...
- Owner: `p:sp/p:spPr/a:custGeom/a:pathLst/a:path/a:cubicBezTo/a:pt[2]/@x`.
- PPJ surface: `shape.nativeRef.leaves[]`.
- Native leaf: `customGeometryPathCubicEndX`.
...
- No line-to, move-to, quadratic, cubic control-point, cubic end-point y,
...
cubic command, removes embedded PPJ, projects the cubic end-point x leaf,
changes it, checks that only the owning SlidePart changed and the package
remains valid, then projects again to recover the new x coordinate and
unchanged control-point and end-point y coordinates.
