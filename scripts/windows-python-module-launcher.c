/*
 * Minimal Windows launcher for the capability-pack-owned OCRmyPDF module.
 *
 * It starts root/python.exe directly with -I -m ocrmypdf, chooses no shell or
 * ambient Python interpreter, puts only the pack's native sidecar directory
 * ahead of Windows system DLL lookup, and clears Python import/startup
 * injection variables before the child begins.
 */
...
/* Quote argv according to the same CreateProcessW/CommandLineToArgvW rules
 * used by the child process when it reconstructs its arguments. */
...
static void report_failure(const wchar_t *message) {
  fwprintf(stderr, L"office-kit Python launcher: %ls (Windows error %lu)\n", message, GetLastError());
}
...
int wmain(int argc, wchar_t **argv) {
  ...
  wchar_t *python = join_path(root, L"python.exe");
  ...
  if (!python || !bin || !dlls || !system_root || !is_regular_non_reparse_file(python)) {
    report_failure(L"capability-pack Python runtime is missing or unsafe");
    free(root);
    free(python);
    free(bin);
    free(dlls);
  ...
  int assembled = append_quoted(&command, python) && append(&command, L" -I -m ocrmypdf");
  ...
  if (!system32 || !runtime_path) {
    report_failure(L"could not allocate native sidecar PATH");
    free(command.value);
    free(root);
    free(python);
    free(bin);
  ...
  /* -I is the primary import isolation boundary. Clear the remaining knobs
   * too so a caller cannot change startup, home, search paths, or DLL lookup
  ...
  SetEnvironmentVariableW(L"PYTHONINSPECT", NULL);
  ...
  if (!CreateProcessW(python, command.value, NULL, NULL, FALSE, 0, NULL, root, &startup, &process)) {
    report_failure(L"could not start capability-pack Python runtime");
    free(command.value);
    free(root);
    free(python);
    free(bin);
  ...
  DWORD exit_code = 2;
  if (!GetExitCodeProcess(process.hProcess, &exit_code)) report_failure(L"could not read Python process exit status");
