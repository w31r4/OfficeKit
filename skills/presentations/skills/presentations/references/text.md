...
For a direct static DrawingML field, `run.field.id` is a brace-wrapped UUID and
`run.field.type` is its printable type token. Imported ordinary text/shape
fields expose `textFieldId` and `textFieldType` only for the existing
non-automatic direct field profile. A source-bound identity edit replaces only
`a:fld/@id`, preserving type, cached display text, automatic state and inline
...
insets remain independent. Explicit zero retains a direct override.
Ordinary imported text/shape paragraphs support add, set, remove and restore
under exact rightIndent authority. Delete the field or its rightIndent-only
style wrapper to clear the direct right margin. Other properties, numeric
spelling, runs, neighbors and package content remain. Invalid source right
margins stay preserved and reject replacement. Preview reports the unresolved
right-indent layout; review actual host line wrapping separately.
...
For an imported direct rich-text run, `run.style.reflection.scaleX` is exposed as
`textReflectionScaleX` only when the direct `a:reflection` has full-span positions,
one explicit canonical `sx`, and no fade, scaleY, skew, alignment, or
rotate-with-shape transform. A source-bound edit token-splices only
`reflection/@sx` in that run's owning SlidePart, keeps all other effects and
...
For an imported direct rich-text run, `run.style.reflection.skewX` is exposed as
`textReflectionSkewX` only when the direct `a:reflection` has full-span positions,
one explicit canonical `kx`, and no fade, scale, `skewY`, alignment, or
rotate-with-shape transform. A source-bound edit token-splices only
`reflection/@kx` in that run's owning SlidePart, keeps all other effects and
package parts byte-stable, and reprojects the signed degree at native 1/60000
precision. Unsupported transform or effect-list topologies remain source-owned;
this field does not claim host reflection rendering.

For an imported direct rich-text run, `run.style.reflection.skewY` is exposed as
`textReflectionSkewY` only when the direct `a:reflection` has full-span positions,
one explicit canonical `ky`, and no fade, scale, `skewX`, alignment, or
rotate-with-shape transform. A source-bound edit token-splices only
`reflection/@ky` in that run's owning SlidePart, keeps all other effects and
package parts byte-stable, and reprojects the signed degree at native 1/60000
precision. Unsupported transform or effect-list topologies remain source-owned;
this field does not claim host reflection rendering.

For an imported direct rich-text run, `run.style.reflection.alignment` is exposed
as `textReflectionAlignment` only when the direct `a:reflection` has full-span
positions, one explicit canonical `algn`, and no fade, scale, skew, or
rotate-with-shape transform. A source-bound edit token-splices only
`reflection/@algn` in that run's owning SlidePart, keeps all other effects and
package parts byte-stable, and reprojects the rectangle-alignment token. The
allowed values are `tl`, `t`, `tr`, `l`, `ctr`, `r`, `bl`, `b`, and `br`.
...
For an imported direct rich-text run, `run.style.shadow.angle` is exposed as
`textShadowDirectionDegrees` only for one direct `a:outerShdw` with an existing
canonical `dir` from 0 through 21,599,999 (1/60000-degree units), one direct
RGB or theme color child, no transform attributes and no sibling effects or
unsupported descendants. A source-bound edit token-splices only
`outerShdw/@dir` in that run's owning SlidePart, keeps blur, distance, color,
opacity, other effects, run topology and non-target package parts byte-stable,
and reprojects the degree value. Missing/invalid direction, transform
...
For an imported direct rich-text run, `run.style.shadow.scaleY` is exposed as
`textShadowScaleY` for the same strict direct `a:outerShdw` profile when its
existing signed `sy` token is present. The profile keeps bounded blur, distance
or direction, one direct RGB or theme color child, and rejects `sx`, `kx`, `ky`,
`rotWithShape`, sibling effects, and unknown descendants. A source-bound edit
token-splices only `outerShdw/@sy` in the owning SlidePart, preserves the other
shadow fields and non-target package parts, and reprojects the ratio at 0.00001
...
For an imported direct rich-text run, `run.style.shadow.skewY` is exposed as
`textShadowSkewY` for the same strict direct `a:outerShdw` profile when its
existing signed `ky` token is strictly between -5400000 and 5400000. The profile
keeps bounded blur, distance or direction, one direct RGB or theme color child,
and rejects `sx`, `sy`, `kx`, `rotWithShape`, sibling effects, and unknown
descendants. A source-bound edit token-splices only `outerShdw/@ky` in the owning
SlidePart, preserves the other shadow fields and non-target package parts, and
reprojects the degree at 1/60000 precision. Missing or malformed `ky` and
unsupported shadow graphs remain source-owned; this field does not claim host
shadow rendering.

For an imported direct rich-text run, `run.style.shadow.rotateWithShape` is
exposed as `textShadowRotateWithShape` for the same strict direct
`a:outerShdw` profile when its existing `rotWithShape` token is canonical `0`
or `1`. The profile keeps bounded blur, distance or direction, one direct RGB
or theme color child, and rejects `sx`, `sy`, `kx`, `ky`, sibling effects, and
unknown descendants. A source-bound edit token-splices only
`outerShdw/@rotWithShape` in the owning SlidePart, preserves the other shadow
fields and non-target package parts, and reprojects the boolean; explicit
`false` remains present. Missing, non-canonical, or unsupported shadow graphs
remain source-owned; this field does not claim host shadow rendering.
