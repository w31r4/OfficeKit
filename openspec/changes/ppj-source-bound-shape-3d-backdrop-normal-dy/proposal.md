...
The strict source-bound backdrop profile now exposes the normal vector's X
coordinate, but its Y coordinate is still opaque despite sharing the same
complete literal vector owner and EMU unit.

# What Changes

- Expose one strict source-bound `shape3dSceneBackdropNormalDyEmu` native leaf
  for `a:scene3d/a:backdrop/a:norm/@dy`.
...
- Keep the backdrop anchor, other normal coordinate, up vector, and wider
...
- `ppj-source-bound-shape-3d-backdrop-normal-dy`: Project and token-splice one
  strict backdrop normal Y coordinate.
