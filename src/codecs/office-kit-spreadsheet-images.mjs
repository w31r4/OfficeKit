...
function fail(image, message, code = "invalid_spreadsheet_image") {
  throw new OfficeKitCodecError(`Worksheet image ${image?.name || image?.id || "(unnamed)"} ${message}`, [], { code });
}

function assetFromImage(image) {
  if (image.uri || image.prompt) fail(image, "must use embedded dataUrl bytes; external URIs and prompts are outside the bounded OfficeKit slice.", "unsupported_spreadsheet_image");
...
export function wireWorksheetImages(sheet, state, assets) {
  ...
  for (const slot of state?.slots || []) {
    if (!remaining.delete(slot.image)) {
      throw new OfficeKitCodecError(`Worksheet ${sheet.name} cannot remove imported image ${slot.image?.name || slot.wire.id} in the bounded OfficeKit slice.`, [], { code: "invalid_spreadsheet_image_topology" });
    }
  ...
  if (state && remaining.size) {
    ...
    throw new OfficeKitCodecError(`Worksheet ${sheet.name} cannot add image ${image.name} to an imported source package in the bounded OfficeKit slice.`, [], { code: "invalid_spreadsheet_image_topology" });
  }
