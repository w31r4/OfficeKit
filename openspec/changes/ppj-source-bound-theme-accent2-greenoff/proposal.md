...
modulation transforms. A direct imported accent2 `a:greenOff` remains opaque,
...
- Project a canonical shared ThemePart whose accent2 direct RGB leaf contains
  ...
  `design.theme.accentTransforms.accent2.greenOff`, using a -1..1 fraction.
- Issue `setThemeAccent2GreenOff` for that field and compile one authorized
...
- `presentation-theme-accent2-greenoff`: Strict source-bound projection and
  source-preserving editing of one direct accent2 `a:greenOff` leaf.
