# Design: source-bound accent4 lumMod
...
accent4 luminance modulation without broadening the theme graph or changing
...
- Accent4 transforms other than `lumMod`, transforms on accent4 through
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:lumMod`. The source writer patches `Accent4Color` only when the authored
   transform is the one existing accent4 lumMod and rejects all other
   transform flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent4LumMod` is accepted only with
   `accentTransforms.accent4.lumMod` in the source-issued capability. This
