...
transform and the direct accent3 tint, shade, luminance modulation, and
luminance offset leaves at a time, then patches the existing DrawingML token.
This change applies the same ownership boundary to accent3 alpha modulation
...
- Accent3 transforms other than `alphaMod`, transforms on accent4 through
...
1. **Use the existing native transform field.**
   ...
   emits it. The importer records a role of `accent3`; the projector converts
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:alphaMod`. The source writer patches `Accent3Color` only when the authored
   transform is the one existing accent3 alphaMod and rejects all other
   transform flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent3AlphaMod` is accepted only with
   `accentTransforms.accent3.alphaMod` in the source-issued capability. This
