...
{
    [Fact]
    public void PpjSourceBoundImageShadowColorEditsLeafAndReprojects()
    {
        ...
        image["shadow"]!.AsObject()["color"] = "#17324D";
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            var color = Assert.IsType<A.RgbColorModelHex>(shadow.FirstChild);
            Assert.Equal("17324D", color.Val!.Value);
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/source/image-shadow-color.pptx",
            },
        });
        ...
        Assert.Equal("#17324D", projectedImage["shadow"]!["color"]!.GetValue<string>());
        ...
        var colorLeaves = projectedImage["nativeRef"]!["leaves"]!.AsArray()
            ...
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "imageShadowColorRgb")
            .ToArray();
        Assert.Single(colorLeaves);
        Assert.Equal("#17324d", colorLeaves[0]["value"]!.GetValue<string>());

        colorLeaves[0]["value"] = "#0f6b5b";
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            var shadow = Assert.IsType<A.OuterShadow>(Assert.Single(picture.ShapeProperties!.GetFirstChild<A.EffectList>()!.ChildElements));
            var color = Assert.IsType<A.RgbColorModelHex>(shadow.FirstChild);
            Assert.Equal("0F6B5B", color.Val!.Value);
            Assert.True(shadow.RotateWithShape!.Value);
            Assert.Equal(76_200U, shadow.BlurRadius!.Value);
            Assert.Equal(38_100U, shadow.Distance!.Value);
            Assert.Equal(24_000, color.Descendants<A.Alpha>().Single().Val!.Value);
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/image-shadow-color.pptx",
            },
        });
        ...
        var reprojectedImage = reprojectedProgram["pages"]![0]!["elements"]!.AsArray()
            .Select(item => item!.AsObject())
            .Single(item => item["name"]!.GetValue<string>() == "evidence identity");
        Assert.Equal("#0F6B5B", reprojectedImage["shadow"]!["color"]!.GetValue<string>());
        Assert.True(reprojectedImage["shadow"]!["rotateWithShape"]!.GetValue<bool>());
        Assert.Equal("#0f6b5b", reprojectedImage["nativeRef"]!["leaves"]!.AsArray().Single(leaf =>
            leaf!["kind"]!.GetValue<string>() == "imageShadowColorRgb")["value"]!.GetValue<string>());
    }
}
