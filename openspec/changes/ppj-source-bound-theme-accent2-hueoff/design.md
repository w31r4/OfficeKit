...
token. This change applies the same ownership boundary to accent2 hue offset
...
- Accent2 transforms other than `hueOff`, transforms on accent3 through
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:hueOff`. The source writer patches `Accent2Color` only when the authored
   transform is the one existing accent2 hueOff and rejects all other transform
   flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent2HueOff` is accepted only with
   `accentTransforms.accent2.hueOff` in the source-issued capability. This
