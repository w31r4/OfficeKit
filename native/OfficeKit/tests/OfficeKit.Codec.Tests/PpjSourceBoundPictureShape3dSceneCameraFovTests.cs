...
{
    [Fact]
    public void PpjSourceBoundPictureShape3dSceneCameraFovLeafEditsAndReprojects()
    {
        ...
        var source = AddPictureShape3dSceneCameraFov(RemoveEmbeddedPpj(authored.File.ToByteArray()), "2700000");
        var projected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/source/picture-shape-3d-scene-camera-fov.pptx",
            },
        });
        ...
        var target = Assert.Single(projectedImage["nativeRef"]!["leaves"]!.AsArray()
            ...
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "shape3dSceneCameraFov60000"));
        Assert.Equal(2700000, target["value"]!.GetValue<long>());
        target["value"] = 5400000;
        ...
        Assert.Contains("<a:scene3d>", editedXml, StringComparison.Ordinal);
        Assert.Contains("prst=\"perspectiveFront\"", editedXml, StringComparison.Ordinal);
        Assert.Contains("zoom=\"100000\"", editedXml, StringComparison.Ordinal);
        Assert.Contains("fov=\"5400000\"", editedXml, StringComparison.Ordinal);
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            Assert.Equal("100000", camera.GetAttribute("zoom", string.Empty).Value);
            Assert.Equal("5400000", camera.GetAttribute("fov", string.Empty).Value);
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/picture-shape-3d-scene-camera-fov.pptx",
            },
        });
        ...
        var reprojectedTarget = Assert.Single(reprojectedImage["nativeRef"]!["leaves"]!.AsArray()
            .Select(leaf => leaf!.AsObject())
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "shape3dSceneCameraFov60000"));
        Assert.Equal(5400000, reprojectedTarget["value"]!.GetValue<long>());
    }

    private static byte[] AddPictureShape3dSceneCameraFov(byte[] source, string fov)
    {
        ...
        using (var package = PresentationDocument.Open(stream, true))
        {
            ...
            camera.SetAttribute(new OpenXmlAttribute("zoom", string.Empty, "100000"));
            camera.SetAttribute(new OpenXmlAttribute("fov", string.Empty, fov));
