...
export function buildPilotPrompt({ manifest, task, arm, trial, armOrder }) {
  ...
  const compilerGuardrails = arm === "C"
    ? " For this experimental authoring-compiler route, make the design grammar explicit before composing: set minimumBodyFontSize to at least 18 and minimumCaptionFontSize to at least 14 in the model's font-size units, and never shrink text to make it fit. Shorten, split, or restructure content instead. Across a deck of six or more pages, use at least four distinct composition silhouettes; repair repeatedComposition, densityRhythmJump, and cardWallPattern warnings unless the plan records a concrete reason. Treat those warnings as actions: the final plan-bound review must have no unrecorded design warnings. If a warning is intentional, add designGrammar.intentionalWarnings with its exact type, affected pageIds, and a concrete reason before commit; a prose invariant is not enough. Give each page one dominant reading anchor, use clear foreground/background contrast, and encode every quantitative claim as a meaningful relationship (chart, axis, connector, direct label, or spatial comparison) rather than a decorative list. Prefer direct labels and meaningful diagrams over tiny legends or decorative boxes."
    : "";
