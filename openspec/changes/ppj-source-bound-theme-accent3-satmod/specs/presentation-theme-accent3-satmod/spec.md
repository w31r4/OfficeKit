...
direct accent3 saturation modulation token while preserving the rest of an
imported theme.

## ADDED Requirements

### Requirement: Strict source-bound accent3 satMod projection

The PPTX projector MUST expose
`design.theme.accentTransforms.accent3.satMod` and
`setThemeAccent3SatMod` only when the canonical shared ThemePart has one
direct accent3 RGB leaf with exactly one direct `a:satMod/@val` child, no
...
#### Scenario: Project a direct accent3 satMod leaf

- **GIVEN** a canonical imported ThemePart with accent3 `223344` and
  `a:satMod/@val="25000"`
- **WHEN** the presentation is projected to PPJ
- **THEN** the theme contains `accentTransforms.accent3.satMod` equal to
  `0.25`
- **AND** the native reference contains `setThemeAccent3SatMod` authorized
  for `accentTransforms.accent3.satMod`

#### Scenario: Reject unsupported accent3 satMod topology
...
- **THEN** the source-bound accent3 satMod field and capability are omitted

### Requirement: Source-bound accent3 satMod editing

The source-preserving compiler MUST accept one authorized change to the
existing `design.theme.accentTransforms.accent3.satMod` field, convert a
...
#### Scenario: Edit and reproject one accent3 satMod token

- **GIVEN** a projected direct accent3 satMod owner at `0.25`
...
#### Scenario: Fail closed for unsafe accent3 satMod edits

- **GIVEN** a source-bound accent3 satMod owner
