...
This change applies the same ownership boundary to accent3 shade without
...
- Accent3 transforms other than `shade`, transforms on accent4 through accent6,
...
1. **Use the existing native transform field.**
   ...
   The importer records a role of `accent3`; the projector converts the native
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:shade`. The source writer patches `Accent3Color` only when the authored
   transform is the one existing accent3 shade and rejects all other transform
   flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent3Shade` is accepted only with
   `accentTransforms.accent3.shade` in the source-issued capability. This keeps
