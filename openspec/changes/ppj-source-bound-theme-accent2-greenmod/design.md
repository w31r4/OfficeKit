...
This change applies the same ownership boundary to accent2 green-channel modulation
...
- Accent2 transforms other than `greenMod`, transforms on accent3 through
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:greenMod`. The source writer patches `Accent2Color` only when the authored
   transform is the one existing accent2 greenMod and rejects all other transform
   flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent2GreenMod` is accepted only with
   `accentTransforms.accent2.greenMod` in the source-issued capability. This
