...
- Owner: `p:sp/p:spPr/a:scene3d/a:backdrop/a:up/@dz`.
- Native leaf: `shape3dSceneBackdropUpDzEmu`.
- The owner must be the single direct `a:scene3d` child of `a:spPr`. The scene
  ...
  three coordinates present as canonical `ST_Coordinate` literals. The up Z
...
- The coordinate uses DrawingML EMUs and the standard bounded signed
  `ST_Coordinate` range. Projection stores only up `dz` in the native shape
  model. The edit plan rechecks the complete backdrop owner and token-splices
  only `up/@dz`, preserving the anchor, normal vector, up `dx/dy`, and all
  other package bytes.
...
No source-free authoring, anchor editing, normal editing, up `dx/dy` editing,
