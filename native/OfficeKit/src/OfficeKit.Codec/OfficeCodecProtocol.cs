...
public static class OfficeCodecProtocol
{
    ...
    private static void ValidateRequest(CodecRequest request)
    {
        ...
        var expectedFamily = request.Operation switch
        {
            ...
            _ => throw Unsupported(request.Operation),
        };
        ...
        if (request.Operation is CodecOperation.ImportXlsx or CodecOperation.ImportDocx or CodecOperation.FinalizeDocxRevisions or CodecOperation.AddDocxTrackedReplacement && request.File.IsEmpty)
        {
    ...
    private static CodecException Unsupported(CodecOperation operation) =>
        new("unsupported_operation", $"Codec operation {operation} is not implemented by the Office profile.");
