...
- `add_text_annotation`, `add_free_text_annotation`, `add_text_markup` (Highlight/Underline/StrikeOut/Squiggly), compatibility `add_text_highlight`, `add_link`, `delete_annotation`,
  ...
  fixed-Helvetica visible text box and refuses an appearance that omits text; both require rewrite. Text markup requires a unique native text selection and rewrite; updates use the complete snapshot.
