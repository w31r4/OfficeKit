...
assert.equal(layoutBlob.type, "application/vnd.office-kit-artifact.layout+json");
...
const arbitraryPdfBytes = Buffer.from(ownedPdfText.replace(/%OFFICE_KIT_ARTIFACT [A-Za-z0-9+/=]+/, (match) => `%${"X".repeat(match.length - 1)}`), "latin1");
...
const arbitraryLinkBytes = Buffer.from(Buffer.from(accessibleLinkBlob.bytes).toString("latin1").replace(/%OFFICE_KIT_ARTIFACT [A-Za-z0-9+/=]+/, (match) => `%${"X".repeat(match.length - 1)}`), "latin1");
...
const arbitraryLinkBudgetPdf = new FileBlob(Buffer.from(Buffer.from(linkBudgetBlob.bytes).toString("latin1").replace(/%OFFICE_KIT_ARTIFACT [A-Za-z0-9+/=]+/, (match) => `%${"X".repeat(match.length - 1)}`), "latin1"), { type: "application/pdf" });
...
const out = path.join(os.tmpdir(), `office-kit-artifact-${process.pid}.pdf`);
await blob.save(out);
