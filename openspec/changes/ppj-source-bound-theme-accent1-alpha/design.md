...
The source-bound accent palette currently accepts only six-digit direct RGB leaves. Authored PPJ already accepts six- or eight-digit theme colors and lowers the trailing byte to `a:alpha`; the imported profile does not yet preserve that child.

## Goals / Non-Goals

**Goals:**

- Read `accent1` as `#RRGGBBAA` when its direct `srgbClr` has exactly one canonical `a:alpha` child.
- Keep the existing six accent object and `setThemeAccent1Color` field authority.
...
- Adding alpha ownership to accent2 through accent6 or the six non-accent color roles in this increment.
...
1. **Use the existing string field.** `PresentationThemeArtifact.accent_rgb[0]` already carries authored RGB/RGBA and avoids a new protobuf field or wire version.
...
4. **Patch one field owner.** The writer compares RGB and alpha separately, updates only those two attributes under accent1, and leaves the other five accents and all theme descendants source-owned.
