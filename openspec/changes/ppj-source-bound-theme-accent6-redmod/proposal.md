...
A direct imported accent6 `a:redMod` remains opaque, leaving one concrete F-15
...
- Project a canonical shared ThemePart whose accent6 direct RGB leaf contains
  ...
  `design.theme.accentTransforms.accent6.redMod`, using a 0..1 fraction.
- Issue `setThemeAccent6RedMod` for that field and compile one authorized
...
- `presentation-theme-accent6-redmod`: Strict source-bound projection and
  source-preserving editing of one direct accent6 `a:redMod` leaf.
