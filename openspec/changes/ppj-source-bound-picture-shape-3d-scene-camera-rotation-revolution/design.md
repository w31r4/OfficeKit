...
and a recognized light-rig pair. Picture scene preset, zoom, FOV, latitude,
and longitude now use the same source-bound approach.

## Goals / Non-Goals

**Goals:**

- Expose one canonical non-negative revolution in 60000ths-of-a-degree.
- Replace only the direct rotation `rev` token in the owning SlidePart.
- Preserve latitude, longitude, camera state, light rig, picture topology,
  and unrelated package parts.

**Non-Goals:**

- Partial rotation editing, rotation-node creation, or source-free authoring.
- Editing latitude/longitude, light-rig rotation, backdrop, or other scene
...
- Add picture dispatch to native proof/read and splice only `camera/rot/@rev`;
  broad scene serialization would risk changing source-owned state.

## Risks / Trade-offs

- [Risk] A rotation node with only revolution could be misinterpreted as a
