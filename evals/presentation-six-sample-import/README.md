...
within the target slide XML.  When a rich DrawingML table or grouped shape is
outside the semantic profile, the runner exposes safe existing text tokens as
bounded native leaves and proves one token-splice edit without rebuilding the
object.
