...
Use this style when the brief matches **business documentary; work presentation**. It is designed for
operators, managers, cross-functional teams. Its visual premise is Electric violet punctuates authentic work and measurable progress. The style is a
...
**current state → signal → cause → trade-off → intervention → owner/date → follow-through**. This style is not a good fit for **academic proof**;
...
- Use **#101115** as the main field, **#F8F8F7** for primary type, **#7047FF** for the decisive signal, **#E6E8EA** for secondary evidence, and **#373740** for rules or context.
...
- The recurring structure is **slash, variable page silhouettes, documentary imagery**. Vary the silhouette between
  cover, evidence, data, process, comparison, and decision pages rather than
  repeating a component grid.
- Use one energetic slash as a transition or emphasis, then return to a disciplined field. Do not repeat slashes as a substitute for hierarchy.
...
- **Page skeleton.** Use white data slides and black or photographic narrative slides with one Electric Violet focus. Keep the narrative split hard and the data pages spare; do not let violet become a field everywhere.
- **Type rhythm.** Use a neutral sans for body, charts, and labels with a bold section display only on special pages. Keep numbers close to the comparison they explain and avoid decorative all-caps.
- **Evidence carriers.** Use single bars, horizontal progress bars, dot-matrix ratings, number bubbles, and compact tables. Area must never imply quantity in a bubble; the value is written at the end.
- **Images and layers.** Documentary photographs of people and work environments are a major carrier on selected pages. Separate text from texture, crop with square corners, and use a white data slide when no suitable photo exists.
- **Pacing.** Alternate human context, operating evidence, and a short action or decision. Electric Violet marks the current subject or section, not every chart element.
- **Selection rule.** Choose this style for operational stories that need authentic work context alongside measurable progress.

## Signature moves and variation limits

- Let **slash** recur as a quiet identity cue on selected pages; it must
