...
- `examples/officekit-asset-depreciation-workflow.mjs`: Run or adapt this three-sheet `Inputs` / `Depreciation` / `Checks` example for visible fixed-asset assumptions, SLN/SYD/DB/DDB schedules, salvage-floor and depreciable-basis checks, finance color conventions, SVG review, and canonical OfficeKit roundtrip.
...
- `examples/officekit-exponential-growth-workflow.mjs`: Run or adapt this `Data` / `Analysis` / `Checks` example for a bounded positive-y exponential model, the single-variable `LOGEST` 5-by-2 coefficient/log-space diagnostics matrix, a three-point `GROWTH` forecast sequence, independent `LN`/`EXP` reconciliations, SVG review, and two canonical OfficeKit roundtrips. It does not claim seasonal, multivariable, or arbitrary nonlinear forecasting.
- `examples/officekit-robust-statistics-workflow.mjs`: Run or adapt this `Data` / `Analysis` / `Checks` example for tied average rank, exclusive percentiles/quartiles, multi-mode spill results, and a symmetric trimmed mean with independent formula reconciliations, SVG review, and two canonical OfficeKit roundtrips. Write formulas, including bounded `LET(...)`, exactly as Excel displays them; OfficeKit Codec owns the `_xlfn`/`_xlws`/scoped `_xlpm`/`ANCHORARRAY` package syntax.
