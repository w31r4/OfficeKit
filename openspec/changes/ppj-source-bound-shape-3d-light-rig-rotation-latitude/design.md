...
- Owner: `p:sp/p:spPr/a:scene3d/a:lightRig/a:rot/@lat`.
- Native leaf: `shape3dSceneLightRigRotationLatitude60000`.
- The owner must be the single direct `a:scene3d` child of `a:spPr`. The
  ...
  modeled.
- Projection stores only the rotation latitude in the native shape model. The
  edit plan rechecks the complete rotation owner and token-splices only
  `rot/@lat`, preserving longitude, revolution, camera/light-rig identity,
