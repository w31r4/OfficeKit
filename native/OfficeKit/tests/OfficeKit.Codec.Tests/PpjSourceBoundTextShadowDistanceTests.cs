...
{
    [Fact]
    public void PpjSourceBoundTextShadowDistanceEditsDirectRunOwnerAndReprojects()
    {
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/source/text-shadow-distance.pptx",
            },
        });
        ...
        var leaves = projectedElement["nativeRef"]!["leaves"]!.AsArray()
            ...
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "textShadowDistanceEmu")
            .ToArray();
        var distanceLeaf = Assert.Single(leaves);
        Assert.Equal(19_050, distanceLeaf["value"]!.GetValue<long>());
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            Assert.Equal(38_100, shadow.BlurRadius!.Value);
            Assert.Equal(50_800, shadow.Distance!.Value);
            Assert.Equal(5_400_000, shadow.Direction!.Value);
            Assert.Equal("16324F", shadow.GetFirstChild<A.RgbColorModelHex>()!.Val!.Value);
            Assert.Equal(72_000, shadow.Descendants<A.Alpha>().Single().Val!.Value);
        }
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/text-shadow-distance.pptx",
            },
        });
        ...
        Assert.Equal(4, reprojectedShadow["distance"]!.GetValue<double>(), precision: 6);
        ...
        Assert.Equal(50_800, reprojectedElement["nativeRef"]!["leaves"]!.AsArray()
            .Select(leaf => leaf!.AsObject())
            .Single(leaf => leaf["kind"]!.GetValue<string>() == "textShadowDistanceEmu")["value"]!.GetValue<long>());
        ...
        var invalidLeaf = invalidProgram["pages"]![0]!["elements"]![0]!["nativeRef"]!["leaves"]!.AsArray()
            .Single(leaf => leaf!["kind"]!.GetValue<string>() == "textShadowDistanceEmu")!.AsObject();
        invalidLeaf["value"] = 1_270_000_001;
    ...
    [Fact]
    public void PpjSourceBoundTextShadowDistanceLeavesStayOpaqueForSiblingEffects()
    {
        var root = new DirectoryInfo(AppContext.BaseDirectory);
        while (root is not null && !File.Exists(Path.Combine(root.FullName, "package.json")))
            root = root.Parent;
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/source/text-shadow-distance-complex.pptx",
            },
        });
        Assert.True(projected.Ok, Diagnostics(projected));
        var projectedProgram = JsonNode.Parse(projected.PresentationProgram.ProgramJson.ToByteArray())!.AsObject();
        ...
        Assert.DoesNotContain(
            ...
            leaf => leaf!["kind"]!.GetValue<string>() == "textShadowDistanceEmu");

        foreach (var (label, mutate) in new[]
        {
            ("transform", (Action<A.OuterShadow>)(shadow => shadow.HorizontalRatio = 120_000)),
            ("missing-distance", (Action<A.OuterShadow>)(shadow => shadow.Distance = null)),
        ...
        })
        {
            ...
            var unsupportedProjected = Invoke(new CodecRequest
            {
                ...
                PresentationProgram = new PresentationProgramRequest
                {
                    SourceUri = $"deck.assets/source/text-shadow-distance-{label}.pptx",
                },
            });
