...
- [x] 1.1 Add `textDefaultReflectionSkewY` to the PPJ schema, native leaf
      registry, capability reference, and generated manual; verify JSON and
      maintainer contract checks.
- [x] 1.2 Project an existing paragraph default reflection `ky` as a signed
...
- [x] 2.1 Extend edit-plan validation and expected-value proof for canonical
      ...
      graphs fail closed.
- [x] 2.2 Splice only `a:reflection/@ky` in the owning SlidePart; verify `sx`,
      `sy`, `fadeDir`, `kx`, sibling effects, text topology, and non-target
      package parts remain unchanged.
