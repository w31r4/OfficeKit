...
description: "Use the Project Kickoff visual grammar when the user selects it for a new presentation. Read the guide and inspect the examples before composing."
---
...
A route into action: the page gives a team a shared destination, sequence, and set of commitments.

## Rules

- Use an open light surface with a confident accent line. Distinguish phases by position, not a row of colored cards.
- Write active titles, short labels, and explicit verbs. Use a large outcome statement followed by the smallest useful supporting detail.
- Use routes, timelines, dependency lines, and role bands. Reserve the strongest shape for the destination or decision.
- Use a team, product, or place image only when it makes the change concrete; pair it with a caption or milestone.
- Keep connector routes visible and unobstructed. If a background image is used, add a scrim before placing editable commitments.
- Avoid generic success badges, decorative arrows without endpoints, and equal panels for every workstream.
