...
Use this style when the brief matches **growth strategy; consulting presentation**. It is designed for
executives, strategy teams, decision makers. Its visual premise is Forest-green steps connect real context to strategic action. The style is a
...
**question → hypothesis → framework → evidence → implication → recommendation → action**. This style is not a good fit for **luxury portfolio**;
...
- Use **#F6F8F5** as the main field, **#183A2A** for primary type, **#2E8C5C** for the decisive signal, **#8CDC76** for secondary evidence, and **#D6E3DB** for rules or context.
...
- The recurring structure is **staircase, variable page silhouettes, documentary imagery**. Vary the silhouette between
  cover, evidence, data, process, comparison, and decision pages rather than
  repeating a component grid.
- Use stepped levels to show progress, maturity, or an ordered intervention. Each step needs a named condition or metric; do not use stairs as empty decoration.
...
- **Page skeleton.** Start body pages with a darkened contextual photo band, a forest-green section cue, and a white assertion area below. Use a 2:1 body split or a three-column relationship only when the strategy has matching parts.
- **Type rhythm.** Use sans titles and widely tracked section labels; keep body and chart labels calm and readable against the white field. Mint is for the strategic focus, not for every heading.
- **Evidence carriers.** Use strategy staircases, capability matrices, roadmaps with gates, maturity bars, and before/after evidence. Each step needs an outcome or owner, not just a verb.
- **Images and layers.** Use authentic technology, workplace, or data-context photographs in the header band. Darken locally, not globally, and keep all claims on editable content layers.
- **Pacing.** Context → diagnosis → strategic moves → gate. Let the photo introduce the world, then let the evidence and action carry the rest of the page.
- **Selection rule.** Choose this style for transformation strategy, operating-model work, and technology programs that connect context to action.

## Signature moves and variation limits

- Let **staircase** recur as a quiet identity cue on selected pages; it must
