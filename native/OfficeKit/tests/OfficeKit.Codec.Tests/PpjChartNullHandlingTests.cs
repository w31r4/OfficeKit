...
{
    [Fact]
    public void PpjChartNullHandlingAuthorProjectAndEditOnlyChartPart()
    {
        ...
        chart["style"] = new JsonObject { ["legend"] = "none" };
        chart["data"] = new JsonObject
        {
            ...
            ["series"] = new JsonArray(new JsonObject
            {
                ["id"] = "line-series",
                ["name"] = "Reach",
                ["values"] = new JsonArray(10, null, 30),
        ...
        Assert.Equal("span", authoredChart.Descendants(chartNs + "dispBlanksAs").Single().Attribute("val")!.Value);
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest { SourceUri = "chart-null-handling/source.pptx" },
        });
        ...
        Assert.Null(projectedChart["displayBlanksAs"]);
        var projectedSeries = projectedChart["data"]!["series"]!.AsArray().Single()!.AsObject();
        Assert.Equal("connect", projectedSeries["nullHandling"]!.GetValue<string>());
        ...
        projectedSeries["nullHandling"] = "gap";
        ...
        Assert.Equal("gap", editedChart.Descendants(chartNs + "dispBlanksAs").Single().Attribute("val")!.Value);
        var reprojected = Invoke(new CodecRequest
        {
            ...
            Family = ArtifactFamily.Presentation,
            File = edited.File,
            PresentationProgram = new PresentationProgramRequest { SourceUri = "chart-null-handling/edited.pptx" },
        });
        ...
        var outputChart = output["pages"]!.AsArray()
            ...
            .Select(item => item!.AsObject())
            .Single(item => item["type"]!.GetValue<string>() == "chart");
        Assert.Null(outputChart["displayBlanksAs"]);
        Assert.Equal("gap", outputChart["data"]!["series"]!.AsArray().Single()!["nullHandling"]!.GetValue<string>());
    }
}
