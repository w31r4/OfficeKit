...
{
    [Fact]
    public void PpjSourceBoundShape3dContourRgbLeafEditsAndReprojects()
    {
        var authored = Invoke(ExportRequest());
        Assert.True(authored.Ok, Diagnostics(authored));
        var source = AddShape3dContourRgb(RemoveEmbeddedPpj(authored.File.ToByteArray()), "336699");
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            {
                SourceUri = "deck.assets/source/shape-3d-contour-rgb.pptx",
            },
        });
        ...
        var target = Assert.Single(projectedShape["nativeRef"]!["leaves"]!.AsArray()
            ...
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "shape3dContourRgb"));
        Assert.Equal("#336699", target["value"]!.GetValue<string>());
        target["value"] = "#6699CC";
        ...
        Assert.Contains("<a:contourClr><a:srgbClr val=\"6699CC\"", editedXml, StringComparison.Ordinal);
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/shape-3d-contour-rgb.pptx",
            },
        });
        ...
        var reprojectedTarget = Assert.Single(reprojectedShape["nativeRef"]!["leaves"]!.AsArray()
            .Select(leaf => leaf!.AsObject())
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "shape3dContourRgb"));
        Assert.Equal("#6699cc", reprojectedTarget["value"]!.GetValue<string>());
    }

    private static byte[] AddShape3dContourRgb(byte[] source, string rgb)
    {
        ...
        using (var package = PresentationDocument.Open(stream, true))
        {
            ...
            var shape3d = new A.Shape3DType(
                new A.ContourColor(new A.RgbColorModelHex { Val = rgb }));
