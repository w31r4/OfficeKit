...
modulation transforms. A direct imported accent3 `a:blueOff` remains opaque,
...
- Project a canonical shared ThemePart whose accent3 direct RGB leaf contains
  ...
  `design.theme.accentTransforms.accent3.blueOff`, using a -1..1 signed fraction.
- Issue `setThemeAccent3BlueOff` for that field and compile one authorized
...
- `presentation-theme-accent3-blueoff`: Strict source-bound projection and
  source-preserving editing of one direct accent3 `a:blueOff` leaf.
