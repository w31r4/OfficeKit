...
{
    [Fact]
    public void PpjSourceBoundPictureShape3dSceneCameraZoomLeafEditsAndReprojects()
    {
        ...
        var source = AddPictureShape3dSceneCameraZoom(RemoveEmbeddedPpj(authored.File.ToByteArray()), "100000");
        var projected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/source/picture-shape-3d-scene-camera-zoom.pptx",
            },
        });
        ...
        var target = Assert.Single(projectedImage["nativeRef"]!["leaves"]!.AsArray()
            ...
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "shape3dSceneCameraZoomThousandthPercent"));
        Assert.Equal(100000, target["value"]!.GetValue<long>());
        target["value"] = 50000;
        ...
        Assert.Contains("<a:scene3d><a:camera prst=\"perspectiveFront\" zoom=\"50000\"", editedXml, StringComparison.Ordinal);
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            Assert.Equal("50000", camera.GetAttribute("zoom", string.Empty).Value);
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/picture-shape-3d-scene-camera-zoom.pptx",
            },
        });
        ...
        var reprojectedTarget = Assert.Single(reprojectedImage["nativeRef"]!["leaves"]!.AsArray()
            .Select(leaf => leaf!.AsObject())
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "shape3dSceneCameraZoomThousandthPercent"));
        Assert.Equal(50000, reprojectedTarget["value"]!.GetValue<long>());
    }

    private static byte[] AddPictureShape3dSceneCameraZoom(byte[] source, string zoom)
    {
        ...
        using (var package = PresentationDocument.Open(stream, true))
        {
            ...
            camera.SetAttribute(new OpenXmlAttribute("zoom", string.Empty, zoom));
