# Custom geometry line-to y coordinate

## ADDED Requirements

### Requirement: expose one bounded line-to y coordinate

The system MUST expose a `customGeometryPathLineToY` native leaf only for a
direct ordered `a:path/a:lnTo/a:pt` whose `y` is a canonical signed integer
...
- **WHEN** an imported custom geometry contains a recognized path with a
  literal line-to point y coordinate
...
- **WHEN** a source-bound PPJ edit changes an issued line-to y leaf to another
