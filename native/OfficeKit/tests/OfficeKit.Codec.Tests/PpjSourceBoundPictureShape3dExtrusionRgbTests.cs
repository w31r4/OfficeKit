...
{
    [Fact]
    public void PpjSourceBoundPictureShape3dExtrusionRgbLeafEditsAndReprojects()
    {
        ...
        var source = AddPictureShape3dExtrusionRgb(RemoveEmbeddedPpj(authored.File.ToByteArray()), "663399");
        var projected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/source/picture-shape-3d-extrusion-rgb.pptx",
            },
        });
        ...
        var target = Assert.Single(projectedImage["nativeRef"]!["leaves"]!.AsArray()
            ...
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "shape3dExtrusionRgb"));
        Assert.Equal("#663399", target["value"]!.GetValue<string>());
        target["value"] = "#9966CC";
        ...
        Assert.Contains("<a:extrusionClr><a:srgbClr val=\"9966CC\"", editedXml, StringComparison.Ordinal);
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            Assert.Equal("9966CC", shape3d.GetFirstChild<A.ExtrusionColor>()!.GetFirstChild<A.RgbColorModelHex>()!.Val!.Value);
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/picture-shape-3d-extrusion-rgb.pptx",
            },
        });
        ...
        var reprojectedTarget = Assert.Single(reprojectedImage["nativeRef"]!["leaves"]!.AsArray()
            .Select(leaf => leaf!.AsObject())
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "shape3dExtrusionRgb"));
        Assert.Equal("#9966cc", reprojectedTarget["value"]!.GetValue<string>());
    }

    private static byte[] AddPictureShape3dExtrusionRgb(byte[] source, string rgb)
    {
        ...
        using (var package = PresentationDocument.Open(stream, true))
        {
            ...
            var shape3d = new A.Shape3DType(
                new A.ExtrusionColor(new A.RgbColorModelHex { Val = rgb }));
