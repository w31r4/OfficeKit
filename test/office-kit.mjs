...
const xlsx = await exportXlsxWithOfficeKit(workbook);
assert.equal(xlsx.metadata.codec, "office-kit");
...
const importedWorkbook = await importXlsxWithOfficeKit(xlsx);
...
await assert.rejects(exportXlsxWithOfficeKit(importedWorkbook), /nested or branched reply graph/i);
...
const xlsx2 = await exportXlsxWithOfficeKit(importedWorkbook, { recalculate: false });
assert.equal((await importXlsxWithOfficeKit(xlsx2)).worksheets.getItem("Core").getRange("C3").values[0][0], 12);
await assert.rejects(exportXlsxWithOfficeKit(workbook, { allowLossy: true }), /does not accept option/i);
...
const bodyParagraph = document.addParagraph("Canonical OfficeKit document.");
...
const docx = await exportDocxWithOfficeKit(document);
assert.equal(docx.metadata.codec, "office-kit");
...
const importedDocument = await importDocxWithOfficeKit(docx);
...
const unchangedDocx = await exportDocxWithOfficeKit(importedDocument);
...
const gifPictureRoundtrip = await importDocxWithOfficeKit(await exportDocxWithOfficeKit(gifPictureDocument));
...
await assert.rejects(
  exportDocxWithOfficeKit(conflictingPictureDocument),
...
await assert.rejects(
  exportDocxWithOfficeKit(invalidPictureDocument),
...
await assert.rejects(
  exportDocxWithOfficeKit(ambiguousPictureDocument),
...
const unsupportedAnchorDocument = await importDocxWithOfficeKit(unsupportedAnchorDocx);
...
const preservedUnsupportedAnchor = await exportDocxWithOfficeKit(unsupportedAnchorDocument);
...
await assert.rejects(
  exportDocxWithOfficeKit(unsupportedAnchorDocument),
...
importedDocument.blocks[2].text = "Edited through OfficeKit.";
...
const docx2 = await exportDocxWithOfficeKit(importedDocument);
const importedDocument2 = await importDocxWithOfficeKit(docx2);
assert.equal(importedDocument2.blocks[2].text, "Edited through OfficeKit.");
