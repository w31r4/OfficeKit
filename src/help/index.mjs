...
export const HELP_CATALOG = [
  ...
  { artifactKind: "presentation", kind: "api", name: "presentation.slides.add", summary: "Append an editable core slide with an optional bounded source-free layout, direct fade/push/wipe transition, solid/style-reference background, and plain-text speaker notes. A supplied layout is resolved and materialized transactionally; effective imported Layout/Master inheritance is never flattened." },
  ...
  { artifactKind: "presentation", kind: "api", name: "slide.setTransition", summary: "Set a direct p:transition to bounded fade or directional push/wipe behavior with slow/medium/fast speed plus click/timer advancement. Source-free slides may author it; imported slides may replace one canonical existing direct transition or add one only when transition.capability.addable is true. Timing, sound, extension, opaque-effect, and every other source graph fail closed." },
  ...
  { artifactKind: "presentation", kind: "api", name: "slide.shapes.add", summary: "Add a shape/textbox, a free-positioned p:sp line, bounded DrawingML custom geometry with ordered adjustment/guide formulas, or an exact-site p:cxnSp connector. A free line is defined only by its start-plus-delta frame; a connector retains target-plus-site identity." },
  { artifactKind: "presentation", kind: "api", name: "slide.shapes.connect", summary: "Connect two modeled shapes in the same slide/group tree by side or exact DrawingML connection-site index. The target-plus-site pair survives import, edit, clone, and second import; moved modeled targets reroute before render/export." },
  { artifactKind: "presentation", kind: "api", name: "slide.shapes.getConnectionSiteIndex", summary: "Resolve top/left/bottom/right to a stable bounded preset connection-site index for rect, roundRect, textbox, or ellipse. Unmodeled geometries fail closed." },
...
const PRESENTATION_HELP_SCHEMAS = {
  ...
  "presentation.slides.add": helpSchema({
    ...
    transition: { type: "object", description: "Optional direct transition: { effect: 'fade'|'push'|'wipe', direction?: 'left'|'up'|'right'|'down', speed?: 'slow'|'medium'|'fast', advanceOnClick?: boolean, advanceAfterMs?: integer 0..86400000 }. Fade rejects direction; push and wipe default to left; click defaults to true." },
  ...
  "presentation.slides.insert": helpSchema({
    ...
    transition: { type: "object", description: "Optional direct transition with the same bounded fade/push/wipe, speed, click, and timer profile as presentation.slides.add." },
    notes: { type: "string|PresentationParagraph[]", description: "Optional speaker notes authored into the canonical PresentationML notes graph. A paragraph has runs plus ordinary direct paragraph/run styling; note-local links, fields, picture bullets, list styles, and body layout are rejected." },
  ...
  "slide.setTransition": helpSchema({
    transition: { type: "object", required: true, description: "{ effect: 'fade'|'push'|'wipe', direction?: 'left'|'up'|'right'|'down', speed?: 'slow'|'medium'|'fast', advanceOnClick?: boolean, advanceAfterMs?: integer 0..86400000 }. Fade rejects direction; push and wipe default to left; speed defaults to medium; click defaults to true." },
  ...
  "slide.shapes.add": helpSchema({
    ...
    line: { type: "object", description: "For shapes and free lines: { style: solid|dashed|dotted|dash-dot|dash-dot-dot|none, fill, width }; dash/dot/dashDot/longDashDotDot aliases normalize canonically. Arrowheads, caps, and joins belong to connectors, not free lines." },
  ...
  "slide.shapes.connect": helpSchema({
    ...
    fromSide: { type: "string", description: "top, left, bottom, or right. Mutually exclusive with fromIdx." },
    toSide: { type: "string", description: "top, left, bottom, or right. Mutually exclusive with toIdx." },
    fromIdx: { type: "number", description: "Exact unsigned DrawingML start connection-site index." },
    toIdx: { type: "number", description: "Exact unsigned DrawingML end connection-site index." },
