...
the same ownership boundary to accent6 green-channel modulation without
...
- Accent6 transforms other than `greenMod`, transforms on accent6 through
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:greenMod`. The source writer patches `Accent6Color` only when the authored
   transform is the one existing accent6 greenMod and rejects all other transform
   flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent6GreenMod` is accepted only with
   `accentTransforms.accent6.greenMod` in the source-issued capability. This
