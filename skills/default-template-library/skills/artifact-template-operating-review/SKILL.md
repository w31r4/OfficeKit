...
description: "Use the Operating Review visual grammar when the user selects it for a new presentation. Read the guide and inspect the examples before composing."
---
...
A working control surface: the reader can see the cadence, the exception, its owner, and the next move without decorative ceremony.

## Rules

- Keep surfaces quiet and use a single alert accent for exceptions. Rules and aligned columns carry more structure than containers.
- Use terse conclusion titles, compact labels, and a visible source/date line. Do not shrink type to rescue an overfilled page.
- Lead with a score or trend, then route attention through exception markers, causes, owners, and actions. Tables are allowed when the comparison is real.
- Use imagery rarely; a process photo or system screenshot must explain an operating constraint.
- Keep lines, thresholds, and exception markers above surfaces. Background fields must never obscure the metric or its label.
- Avoid decorative dashboards, repeated rounded cards, unlabeled status colors, and dense text walls.
...
2. Inspect all five example images to learn the range, then write a deck-specific
