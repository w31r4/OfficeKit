...
- 35 `ready` cases: 21 PDF cases, five bounded XLSX workflows (direct threaded-comment reply success, nested-reply refusal, formula-assumption update, source-bound connection refresh-on-open, and source-bound Pivot refresh-on-open), five DOCX workflows (classic-comment, nested-modern-comment refusal, source-bound header/footer text, and source-bound section page numbering), and four PPTX workflows (title plus fixed-topology rich-speaker-notes run edit, source-bound slide-name edit, source-bound complete section-boundary edit, and closed-leaf slide clone).
...
- 6 `asset-required` cases still await their pinned corpus files. They remain excluded from readiness and repeat-matrix claims.
