import { SpreadsheetWorksheetProtectionOperation } from "../generated/office_kit/artifact/v1/office_artifact_pb.js";
...
export function publicWorksheetProtectionFromWire(wire) {
  ...
  const allow = (wire.allowedOperations || []).map((operation) => {
    ...
    if (!name) throw new Error(`OfficeKit returned unsupported worksheet protection operation ${operation}.`);
    return name;
  });
