...
accent1 plus accent5 tint, shade, luminance, and alpha transforms. A direct
imported accent5 `a:satMod` remains opaque, leaving one concrete F-15
...
- Project a canonical shared ThemePart whose accent5 direct RGB leaf contains
  ...
  `design.theme.accentTransforms.accent5.satMod`, using a 0..1 fraction.
- Issue `setThemeAccent5SatMod` for that field and compile one authorized
...
- `presentation-theme-accent5-satmod`: Strict source-bound projection and
  source-preserving editing of one direct accent5 `a:satMod` leaf.
