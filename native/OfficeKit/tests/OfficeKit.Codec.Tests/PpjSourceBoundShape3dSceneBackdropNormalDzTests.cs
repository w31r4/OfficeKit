...
{
    [Fact]
    public void PpjSourceBoundShape3dSceneBackdropNormalDzLeafEditsAndReprojects()
    {
        var authored = Invoke(ExportRequest());
        Assert.True(authored.Ok, Diagnostics(authored));
        var source = AddShape3dSceneBackdropNormalDz(RemoveEmbeddedPpj(authored.File.ToByteArray()), "1");
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            {
                SourceUri = "deck.assets/source/shape-3d-scene-backdrop-normal-dz.pptx",
            },
        });
        ...
        var target = Assert.Single(projectedShape["nativeRef"]!["leaves"]!.AsArray()
            ...
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "shape3dSceneBackdropNormalDzEmu"));
        Assert.Equal(1, target["value"]!.GetValue<long>());
        target["value"] = 2;
        ...
        Assert.Contains("<a:norm dx=\"0\" dy=\"0\" dz=\"2\"", editedXml, StringComparison.Ordinal);
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/shape-3d-scene-backdrop-normal-dz.pptx",
            },
        });
        ...
        var reprojectedTarget = Assert.Single(reprojectedShape["nativeRef"]!["leaves"]!.AsArray()
            .Select(leaf => leaf!.AsObject())
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "shape3dSceneBackdropNormalDzEmu"));
        Assert.Equal(2, reprojectedTarget["value"]!.GetValue<long>());
    }

    private static byte[] AddShape3dSceneBackdropNormalDz(byte[] source, string normalDz)
    {
        ...
        using (var package = PresentationDocument.Open(stream, true))
        {
            ...
            anchor.SetAttribute(new OpenXmlAttribute("x", string.Empty, "100"));
            anchor.SetAttribute(new OpenXmlAttribute("y", string.Empty, "200"));
            anchor.SetAttribute(new OpenXmlAttribute("z", string.Empty, "300"));
            var normal = new A.Normal();
            normal.SetAttribute(new OpenXmlAttribute("dx", string.Empty, "0"));
            normal.SetAttribute(new OpenXmlAttribute("dy", string.Empty, "0"));
            normal.SetAttribute(new OpenXmlAttribute("dz", string.Empty, normalDz));
            var up = new A.UpVector();
            up.SetAttribute(new OpenXmlAttribute("dx", string.Empty, "0"));
            up.SetAttribute(new OpenXmlAttribute("dy", string.Empty, "1"));
            up.SetAttribute(new OpenXmlAttribute("dz", string.Empty, "0"));
            var backdrop = new A.Backdrop(anchor, normal, up);
            var scene = new A.Scene3DType(camera, lightRig, backdrop);
