# Design: source-bound accent6 lumOff
...
accent6 luminance offset without broadening the theme graph or changing
...
- Accent6 transforms other than `lumOff`, transforms on other accent roles,
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:lumOff`. The source writer patches `Accent6Color` only when the authored
   transform is the one existing accent6 lumOff and rejects all other
   transform flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent6LumOff` is accepted only with
   `accentTransforms.accent6.lumOff` in the source-issued capability. This
