...
{
    [Fact]
    public void StaticTextFieldIdLifecyclePatchesOnlyFieldIdToken()
    {
        ...
        var idLeaf = element["nativeRef"]!["leaves"]!.AsArray()
            .Single(leaf => leaf!["kind"]!.GetValue<string>() == "textFieldId")!.AsObject();
        ...
        Assert.Contains("text.paragraphs[].runs[].field.id",
        ...
        var reprojectedElement = reprojected["pages"]![0]!["elements"]![0]!.AsObject();
        Assert.Equal(replacementId,
            reprojectedElement["text"]!["paragraphs"]![0]!["runs"]![0]!["field"]!["id"]!.GetValue<string>());
        Assert.Equal(replacementId, reprojectedElement["nativeRef"]!["leaves"]!.AsArray()
            .Single(leaf => leaf!["kind"]!.GetValue<string>() == "textFieldId")!["value"]!.GetValue<string>());

        var stale = Project(source);
        stale["pages"]![0]!["elements"]![0]!["text"]!["paragraphs"]![0]!["runs"]![0]!["field"]!["id"] = replacementId;
        stale["pages"]![0]!["elements"]![0]!["nativeRef"]!["leaves"]!.AsArray()
            .Single(leaf => leaf!["kind"]!.GetValue<string>() == "textFieldId")!["expectedHash"] = new string('0', 64);
        Assert.False(Compile(stale, source, success: false).Ok);

        var invalid = Project(source);
        invalid["pages"]![0]!["elements"]![0]!["text"]!["paragraphs"]![0]!["runs"]![0]!["field"]!["id"] = "not-a-guid";
        ...
        automatic["pages"]![0]!["elements"]![0]!["text"]!["paragraphs"]![0]!["runs"]![0]!["field"]!["id"] = replacementId;
        Assert.False(Compile(automatic, source, success: false).Ok);

        var combined = Project(source);
        var combinedField = combined["pages"]![0]!["elements"]![0]!["text"]!["paragraphs"]![0]!["runs"]![0]!["field"]!.AsObject();
    ...
    private static IEnumerable<string> ZipPaths(byte[] bytes)
    {
    ...
    {
        using var archive = new ZipArchive(new MemoryStream(bytes), ZipArchiveMode.Read);
        using var stream = archive.GetEntry(path)?.Open() ?? throw new InvalidOperationException($"Missing ZIP entry {path}.");
