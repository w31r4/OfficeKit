...
const temporary = fs.mkdtempSync(path.join(os.tmpdir(), "office-kit-reproducibility-"));
...
try {
  ...
  if (JSON.stringify(firstSnapshot) !== JSON.stringify(secondSnapshot)) {
    ...
    const changed = [...new Set([...firstByPath.keys(), ...secondByPath.keys()])]
      ...
      .slice(0, 20);
    throw new Error(`OfficeKit builds differ on the same host: ${changed.join(", ") || "file inventory changed"}.`);
  }
  console.log(`OfficeKit reproducibility ok: ${firstSnapshot.length} files.`);
} finally {
  fs.rmSync(temporary, { force: true, recursive: true });
}

function build(output) {
  const result = spawnSync(process.execPath, ["scripts/build-office-kit.mjs"], {
    cwd: repoRoot,
    env: { ...process.env, OFFICE_KIT_OUTPUT: output },
    stdio: "inherit",
  });
