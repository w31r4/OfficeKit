import { CellFormulaKind } from "../generated/office_kit/artifact/v1/office_artifact_pb.js";
...
export function wireWorksheetDataTables(sheet, state, cells) {
  ...
  for (const slot of state?.slots || []) {
    if (!slot.definition || !remaining.delete(slot.definition)) {
      throw new OfficeKitCodecError(`Worksheet ${sheet.name} cannot remove or reorder imported data table ${slot.wire.reference}.`, [], { code: "invalid_spreadsheet_data_table_topology" });
    }
    if (!sameSnapshot(slot.definition, slot.publicSnapshot)) {
      throw new OfficeKitCodecError(`Imported data table ${sheet.name}!${slot.wire.reference} is source-bound and read-only.`, [], { code: "unsupported_spreadsheet_data_table_edit" });
    }
    ordered.push({ definition: slot.definition, wire: slot.wire });
  }
  if (state && remaining.size) {
    const added = [...remaining][0];
    throw new OfficeKitCodecError(`Worksheet ${sheet.name} cannot add data table ${added.formulaRef} to a source-bound package.`, [], { code: "invalid_spreadsheet_data_table_topology" });
  }
  ...
  for (const item of ordered) {
    ...
    if (cell.formula || (cell.formulaMetadata && cell.formulaMetadata.kind !== CellFormulaKind.DATA_TABLE)) {
      throw new OfficeKitCodecError(`Data table ${sheet.name}!${item.definition.formulaRef} conflicts with another formula at ${item.definition.anchor.address}.`, [], { code: "invalid_spreadsheet_data_table_topology" });
    }
    cell.formula = "";
