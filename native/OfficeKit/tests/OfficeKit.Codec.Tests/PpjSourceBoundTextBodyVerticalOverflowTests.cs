...
{
    [Fact]
    public void PpjSourceBoundTextBodyVerticalOverflowEditsLeafAndReprojects()
    {
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/source/text-body-vertical-overflow.pptx",
            },
        });
        Assert.True(projected.Ok, Diagnostics(projected));
        var projectedProgram = JsonNode.Parse(projected.PresentationProgram.ProgramJson.ToByteArray())!.AsObject();
        var projectedShape = projectedProgram["pages"]![0]!["elements"]!.AsArray().Single()!.AsObject();
        Assert.Equal("ellipsis", projectedShape["textStyle"]!["verticalOverflow"]!.GetValue<string>());
        var leaves = projectedShape["nativeRef"]!["leaves"]!.AsArray()
            .Select(leaf => leaf!.AsObject())
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "textBodyVerticalOverflow")
            .ToArray();
        Assert.Single(leaves);
        Assert.Equal("ellipsis", leaves[0]["value"]!.GetValue<string>());
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            var nativeShape = package.PresentationPart!.SlideParts.Single().Slide!.Descendants<P.Shape>().Single();
            var bodyProperties = nativeShape.TextBody!.GetFirstChild<A.BodyProperties>()!;
            Assert.Equal(A.TextVerticalOverflowValues.Clip, bodyProperties.VerticalOverflow!.Value);
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/text-body-vertical-overflow.pptx",
            },
        });
        ...
        Assert.Equal("clip", reprojectedShape["textStyle"]!["verticalOverflow"]!.GetValue<string>());
        Assert.Equal("clip", reprojectedShape["nativeRef"]!["leaves"]!.AsArray().Single(leaf =>
            leaf!["kind"]!.GetValue<string>() == "textBodyVerticalOverflow")["value"]!.GetValue<string>());
    }
}
