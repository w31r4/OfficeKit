# Design: source-bound accent2 lumMod

## Context

The existing source-preserving theme path recognizes one direct accent1
transform and the direct accent2 tint/shade leaves at a time, then patches the
existing DrawingML token. This change applies the same ownership boundary to
accent2 luminance modulation without broadening the theme graph or changing
authored six-role lowering.
...
- Recognize only a canonical shared ThemePart whose accent2 color is a direct
  six-digit RGB leaf with one direct `a:lumMod/@val` child.
...
- Accent2 transforms other than `lumMod`, transforms on accent3 through
  accent6, inherited or effect theme state, or arbitrary theme XML editing.
- Creating or deleting a missing lumMod node, changing multiple theme fields,
  or claiming PowerPoint host color rendering.

## Decisions

1. **Use the existing native transform field.**
   `PresentationThemeColorTransform.LuminanceModulationThousandth` already
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:lumMod`. The source writer patches `Accent2Color` only when the authored
   transform is the one existing accent2 lumMod and rejects all other
   transform flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent2LumMod` is accepted only with
   `accentTransforms.accent2.lumMod` in the source-issued capability. This
...
- **Risk:** Fraction-to-integer conversion could drift at half-thousandths. →
  **Mitigation:** use explicit away-from-zero rounding and reproject the native
  integer in the focused regression.
