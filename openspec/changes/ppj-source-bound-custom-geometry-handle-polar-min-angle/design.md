...
- Give a direct literal `a:ahPolar/@minAng` bound a stable native index and
  explicit 1/60000-degree value.
- Keep radial/angle guide identity, radial bounds, `maxAng`, position, handle
...
- Editing `maxAng`, radial bounds, polar position, guide identity, adjustment
...
3. **Keep the paired range graph intact.** The writer proves all direct
   `ahPolar` attributes and changes only `minAng`; it does not add/remove the
   range or edit `maxAng`, radial state, current adjustment, or position. The
