...
Use this style when the brief matches **quarterly update; work presentation**. It is designed for
operators, managers, cross-functional teams. Its visual premise is Moon-white space and violet orientation keep management reporting bright. The style is a
...
**current state → signal → cause → trade-off → intervention → owner/date → follow-through**. This style is not a good fit for **cinematic launch**;
...
- Use **#FFFFFF** as the main field, **#1D2330** for primary type, **#8B6BE8** for the decisive signal, **#4E9B74** for secondary evidence, and **#E6E5EB** for rules or context.
...
- The recurring structure is **moon, variable page silhouettes, native data graphics**. Vary the silhouette between
  cover, evidence, data, process, comparison, and decision pages rather than
  repeating a component grid.
- Use a quiet light field and one cool orientation mark. Let calm whitespace frame an operating conclusion, while every visible metric remains grounded.
...
The expected density is **medium**: concentrate it around the title-to-body transition, chart
edges, column gutters, and the final implication.

## Style-specific playbook

- **Page skeleton.** Use a bright white management-report field with a quiet header, green current signal, and violet orientation cue. Let a photo or a dark closing page appear only when the narrative needs a change of temperature.
- **Type rhythm.** Use a neutral sans with a roomy title and restrained notes. Make current figures larger than support text, but keep the body at a comfortable reading size.
- **Evidence carriers.** Use bar/line trends, horizontal comparisons, sparse matrices, responsibility tracks, and a compact KPI band. Label the current subject directly and recede the baseline into gray.
- **Images and layers.** Allow one documentary workplace or product photo on a selected page; a multi-thumbnail collage is not part of this style. Text over photography requires a tested darkening field.
- **Pacing.** Open with the operating judgment, alternate readable headline pages and compact evidence, and close with owner/date or next action. Keep violet for orientation.
- **Selection rule.** Choose this style for quarterly updates, management reviews, and bright internal operating reports.

## Signature moves and variation limits

- Let **moon** recur as a quiet identity cue on selected pages; it must
