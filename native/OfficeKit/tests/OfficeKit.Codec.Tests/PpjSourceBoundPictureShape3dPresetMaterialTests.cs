...
{
    [Fact]
    public void PpjSourceBoundPictureShape3dPresetMaterialLeafEditsAndReprojects()
    {
        ...
        var source = AddPictureShape3dPresetMaterial(RemoveEmbeddedPpj(authored.File.ToByteArray()), "metal");
        var projected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/source/picture-shape-3d-preset-material.pptx",
            },
        });
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            Assert.Equal("matte", picture.ShapeProperties!.GetFirstChild<A.Shape3DType>()!
                .GetAttributes().Single(attribute => attribute.LocalName == "prstMaterial").Value);
        }
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/picture-shape-3d-preset-material.pptx",
            },
        });
        ...
        var reprojectedImage = reprojectedProgram["pages"]!.AsArray()
            ...
            .Select(item => item!.AsObject())
            .Single(item => item["name"]!.GetValue<string>() == "evidence identity");
        Assert.Equal("matte", reprojectedImage["nativeRef"]!["leaves"]!.AsArray()
            .Select(leaf => leaf!.AsObject())
            .Single(leaf => leaf["kind"]!.GetValue<string>() == "shape3dPresetMaterial")["value"]!.GetValue<string>());
    }

    private static byte[] AddPictureShape3dPresetMaterial(byte[] source, string material)
    {
