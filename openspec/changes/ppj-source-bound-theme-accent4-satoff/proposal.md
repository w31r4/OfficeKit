...
accent1 plus accent4 tint, shade, luminance, alpha, and saturation modulation
transforms. A direct imported accent4 `a:satOff` remains opaque, leaving one
...
- Project a canonical shared ThemePart whose accent4 direct RGB leaf contains
  ...
  `design.theme.accentTransforms.accent4.satOff`, using a -1..1 fraction.
- Issue `setThemeAccent4SatOff` for that field and compile one authorized
...
- `presentation-theme-accent4-satoff`: Strict source-bound projection and
  source-preserving editing of one direct accent4 `a:satOff` leaf.
