## 1. Language and compiler

- [x] 1.1 Add explicit authored diagram style, connector, and picture-node fields.
- [x] 1.2 Validate bounded layout-specific topology and asset references.
- [x] 1.3 Lower all eight authored layouts to stable native editable groups.
- [x] 1.4 Keep source-bound SmartArt projection and diff behavior unchanged.

## 2. Agent surface and evidence

- [x] 2.1 Add one focused diagram example and usage boundaries to PPJ guidance.
- [x] 2.2 Extend the existing comprehensive PPJ contract with authored diagrams.
- [x] 2.3 Run the owning contract, maintainer check, and strict OpenSpec validation.
- [x] 2.4 Commit atomically and fast-forward the verified branch into main.
