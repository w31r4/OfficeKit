...
recognizes `a:path` command topology and carries its optional width as a
...
- Expose each recognized custom path's existing positive direct `@w` as a
  path-indexed native leaf.
- Preserve the path's other attributes, commands, sibling paths, and source
  ownership boundaries while editing only `@w`.
- Demonstrate authored export, embedded-PPJ removal, import projection,
  source-bound edit, and second projection.

**Non-Goals:**

- Editing `@h`, fill/stroke/extrusion flags, commands, path order, or path-list
  topology.
- Materializing an omitted or zero width, changing formula-driven coordinates,
...
2. **Expose only positive canonical widths.** A direct numeric width must be a
   positive integer no larger than `int.MaxValue`; omission and zero retain the
   codec's existing semantics and are not made editable. The requested value
   must use the same canonical decimal form and remain positive and bounded.

3. **Keep the unit explicit.** The leaf is named without an `Emu` suffix and
   its registry unit is DrawingML path-coordinate units. `a:path/@w` defines
...
4. **Prove the minimal XML owner.** The proof checks a single direct custom
   geometry/path list, an in-range recognized path index, a positive width, and
   known path command children. Compilation targets that path's `w` attribute
   and lets the existing expected-value and element-hash checks reject stale or
   structurally changed sources.

## Risks / Trade-offs

- [Risk] Changing a path viewport width can alter the rendered geometry because
  command coordinates are interpreted in that viewport. → Mitigation: expose
  only an explicitly existing width, label the unit accurately, and preserve
