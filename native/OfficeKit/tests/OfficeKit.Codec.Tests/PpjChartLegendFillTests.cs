...
{
    [Fact]
    public void PpjChartLegendFillAuthorAndEditSourceChart()
    {
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest { SourceUri = "legend-fill/source.pptx" },
        });
        ...
        var projectedChart = projectedProgram["pages"]!.AsArray()
            ...
            .Select(item => item!.AsObject())
            .Single(item => item["type"]!.GetValue<string>() == "chart");
        Assert.Equal("solid", projectedChart["style"]!["legendFill"]!["type"]!.GetValue<string>());
        Assert.Equal("#F2C14E", projectedChart["style"]!["legendFill"]!["color"]!.GetValue<string>());

        projectedChart["style"]!["legendFill"] = new JsonObject
        {
            ["type"] = "solid",
            ["color"] = "#0B8F8F",
        };
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            Family = ArtifactFamily.Presentation,
            File = edited.File,
            PresentationProgram = new PresentationProgramRequest { SourceUri = "legend-fill/edited.pptx" },
        });
        ...
        var reprojectedChart = reprojectedProgram["pages"]!.AsArray()
            ...
            .Select(item => item!.AsObject())
            .Single(item => item["type"]!.GetValue<string>() == "chart");
        Assert.Equal("#0B8F8F", reprojectedChart["style"]!["legendFill"]!["color"]!.GetValue<string>());
    }
}
