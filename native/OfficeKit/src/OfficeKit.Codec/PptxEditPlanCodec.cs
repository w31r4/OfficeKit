...
internal static partial class PptxEditPlanCodec
{
    ...
    [GeneratedRegex("<(?<prefix>[A-Za-z_][\\w.-]*):fld\\b[^>]*>.*?</\\k<prefix>:fld\\s*>", RegexOptions.CultureInvariant | RegexOptions.Singleline)]
    ...
    private static void ValidateRequest(byte[] sourceBytes, PresentationEditPlanRequest request, EffectiveCodecLimits limits)
    {
        ...
        foreach (var operation in request.Operations)
        {
            ...
            if (leafKind is ("textFieldType" or "tableTextFieldType") &&
                (!PptxTextCodec.ValidFieldType(operation.ExpectedValue) ||
                 !PptxTextCodec.ValidFieldType(operation.Value) ||
                 PptxTextCodec.IsAutomaticFieldType(operation.ExpectedValue) ||
                 PptxTextCodec.IsAutomaticFieldType(operation.Value)))
                throw new CodecException("invalid_presentation_edit_operation", $"PPTX edit operation {operation.OperationId} field type must be a changed valid non-automatic token.");
            if (leafKind is ("textFieldId" or "tableTextFieldId") &&
                (!PptxTextCodec.ValidFieldId(operation.ExpectedValue) ||
                 !PptxTextCodec.ValidFieldId(operation.Value)))
                throw new CodecException("invalid_presentation_edit_operation", $"PPTX edit operation {operation.OperationId} field ID must be a brace-wrapped UUID.");
            ...
            if (leafKind is "shadowBlurRadiusEmu" or "imageShadowBlurRadiusEmu" or "shadowDistanceEmu" or "imageShadowDistanceEmu" or "shadowDirectionDegrees" or "imageShadowDirectionDegrees" or "shadowAlignment" or "imageShadowAlignment" or "textShadowBlurRadiusEmu" or "textShadowDistanceEmu" or "textShadowDirectionDegrees" or "textShadowAlignment" or "textDefaultShadowBlurRadiusEmu" or "textDefaultShadowDistanceEmu" or "textDefaultShadowDirectionDegrees" or "textDefaultShadowAlignment" or "textDefaultReflectionAlignment")
            {
            ...
            if (leafKind is "textShadowScaleX" or "textShadowScaleY" or "textDefaultShadowScaleX" or "textDefaultShadowScaleY")
            {
                if (!ValidShadowScaleToken(operation.ExpectedValue) || !ValidShadowScaleToken(operation.Value) ||
                    operation.ExpectedValue == operation.Value)
                    throw new CodecException("invalid_presentation_edit_operation", $"PPTX edit operation {operation.OperationId} {leafKind} must use a changed canonical signed Int32 scale token.");
            }
            if (leafKind is "textShadowSkewX" or "textShadowSkewY" or "textDefaultShadowSkewX" or "textDefaultShadowSkewY")
            {
                if (!ValidShadowSkewToken(operation.ExpectedValue) || !ValidShadowSkewToken(operation.Value) ||
                    operation.ExpectedValue == operation.Value)
                    throw new CodecException("invalid_presentation_edit_operation", $"PPTX edit operation {operation.OperationId} {leafKind} must use a changed canonical skew token strictly between -5400000 and 5400000.");
            }
            if (leafKind is "shadowOpacityThousandthPercent" or "imageShadowOpacityThousandthPercent" or "textDefaultShadowOpacityThousandthPercent" or "textShadowOpacityThousandthPercent")
            {
                if (!ValidOpacityToken(operation.ExpectedValue) || !ValidOpacityToken(operation.Value) ||
                    operation.ExpectedValue == operation.Value)
            ...
            if (leafKind is "textDefaultShadowRotateWithShape" or "textDefaultReflectionRotateWithShape" or "textReflectionRotateWithShape" or "textShadowRotateWithShape")
            {
                if (!ValidBooleanToken(operation.ExpectedValue) || !ValidBooleanToken(operation.Value) ||
                    operation.ExpectedValue == operation.Value)
                    throw new CodecException("invalid_presentation_edit_operation", $"PPTX edit operation {operation.OperationId} {leafKind} must use a changed canonical boolean token 0 or 1.");
            ...
            if (leafKind is "shadowColorRgb" or "imageShadowColorRgb" or "textShadowColorRgb" or "textShadowColorScheme" or "textDefaultShadowColorRgb" or "shadowColorScheme" or "imageShadowColorScheme" or "textDefaultShadowColorScheme")
            {
                if (leafKind is "shadowColorRgb" or "imageShadowColorRgb" or "textShadowColorRgb" or "textDefaultShadowColorRgb")
                {
            ...
            if (leafKind is "textShadowBlurRadiusEmu" or "textShadowDistanceEmu" or "textShadowDirectionDegrees" or "textShadowAlignment")
            {
                if (!ValidShadowGeometryToken(leafKind, operation.ExpectedValue) ||
                    !ValidShadowGeometryToken(leafKind, operation.Value) ||
                    operation.ExpectedValue == operation.Value)
            ...
            if (leafKind is "textReflectionBlurRadiusEmu" or "textDefaultReflectionBlurRadiusEmu" or "textDefaultReflectionDistanceEmu" or "textReflectionDistanceEmu" or "textReflectionDirectionDegrees" or "textReflectionFadeAngleDegrees" or "textReflectionScaleX" or "textReflectionScaleY" or "textReflectionSkewX" or "textReflectionSkewY" or "textDefaultReflectionDirectionDegrees" or "textDefaultReflectionFadeAngleDegrees" or "textDefaultReflectionScaleX" or "textDefaultReflectionScaleY" or "textDefaultReflectionSkewX" or "textDefaultReflectionSkewY" or "textDefaultReflectionAlignment" or "textReflectionAlignment")
            {
            ...
            if (leafKind is "textReflectionStartPosition" or "textReflectionEndPosition" or "textDefaultReflectionStartPosition" or "textDefaultReflectionEndPosition")
            {
                if (!ValidTextReflectionPositionToken(operation.ExpectedValue) ||
                    !ValidTextReflectionPositionToken(operation.Value) ||
                    operation.ExpectedValue == operation.Value)
                    throw new CodecException("invalid_presentation_edit_operation", $"PPTX edit operation {operation.OperationId} {leafKind} must use a changed canonical value from 0 through 100000.");
            }
        ...
        EffectiveCodecLimits limits)
    {
        ...
        foreach (var operation in request.Operations)
        {
            ...
            if (element is P.Shape fieldShape &&
                projectedElement.ContentCase == PresentationElement.ContentOneofCase.Shape &&
                projectedElement.Source.TextEditable &&
                LeafKind(operation) is ("textFieldType" or "textFieldId") &&
                PptxCodec.SupportsBoundTextLeaf(fieldShape))
            {
                ProveLeafValue(fieldShape, operation);
                proofs.Add(new PptxEditPlanProof(operation, elementHash, operation.SlidePartPath));
                continue;
            }
            if (element is P.GraphicFrame tableField &&
                projectedElement.ContentCase == PresentationElement.ContentOneofCase.Table &&
                projectedElement.Source.Editable &&
                LeafKind(operation) is ("tableTextFieldType" or "tableTextFieldId") &&
                PptxTableCodec.TryRead(tableField, out _))
            {
                ProveLeafValue(tableField, operation);
                proofs.Add(new PptxEditPlanProof(operation, elementHash, operation.SlidePartPath));
                continue;
            }
            if (element is P.Shape defaultTextShape &&
                projectedElement.ContentCase == PresentationElement.ContentOneofCase.Shape &&
                projectedElement.Source.TextEditable &&
                LeafKind(operation) is "textDefaultGlowColorRgb" or "textDefaultGlowColorScheme" or "textDefaultGlowOpacityThousandthPercent" or "textDefaultShadowBlurRadiusEmu" or "textDefaultShadowDistanceEmu" or "textDefaultShadowDirectionDegrees" or "textDefaultShadowScaleX" or "textDefaultShadowScaleY" or "textDefaultShadowSkewX" or "textDefaultShadowSkewY" or "textDefaultShadowAlignment" or "textDefaultShadowColorRgb" or "textDefaultShadowColorScheme" or "textDefaultShadowOpacityThousandthPercent" or "textDefaultShadowRotateWithShape" or "textDefaultInnerShadowBlurRadiusEmu" or "textDefaultInnerShadowDistanceEmu" or "textDefaultInnerShadowDirectionDegrees" or "textDefaultInnerShadowColorRgb" or "textDefaultInnerShadowColorScheme" or "textDefaultInnerShadowOpacityThousandthPercent" or "textDefaultReflectionBlurRadiusEmu" or "textDefaultReflectionDistanceEmu" or "textDefaultReflectionStartOpacityThousandthPercent" or "textDefaultReflectionEndOpacityThousandthPercent" or "textDefaultReflectionDirectionDegrees" or "textDefaultReflectionFadeAngleDegrees" or "textDefaultReflectionScaleX" or "textDefaultReflectionScaleY" or "textDefaultReflectionSkewX" or "textDefaultReflectionSkewY" or "textDefaultReflectionAlignment" or "textDefaultReflectionRotateWithShape" or "textDefaultReflectionStartPosition" or "textDefaultReflectionEndPosition")
            {
    ...
    private static PptxXmlPatch[] CompileXmlPatches(byte[] partBytes, IReadOnlyList<PptxEditPlanProof> proofs)
    {
        ...
        foreach (var proof in proofs)
        {
            ...
            if (leafKind is "textShadowBlurRadiusEmu" or "textShadowDistanceEmu" or "textShadowDirectionDegrees" or "textShadowScaleX" or "textShadowScaleY" or "textShadowSkewX" or "textShadowSkewY" or "textShadowRotateWithShape" or "textShadowAlignment")
            {
                if (range.LocalName != "sp")
                    throw new CodecException("presentation_edit_target_mismatch", $"PPTX edit operation {operation.OperationId} {leafKind} target has the wrong native element type.", operation.SlidePartPath);
                patches.Add(CompileTextShadowXmlPatch(xml, range, proof, drawingPrefixes));
                continue;
            }
            if (leafKind == "textShadowOpacityThousandthPercent")
            {
                if (range.LocalName != "sp")
                    throw new CodecException("presentation_edit_target_mismatch", $"PPTX edit operation {operation.OperationId} {leafKind} target has the wrong native element type.", operation.SlidePartPath);
                patches.Add(CompileTextShadowOpacityXmlPatch(xml, range, proof, drawingPrefixes));
                continue;
            }
            if (leafKind is "textShadowColorRgb" or "textShadowColorScheme")
            {
                if (range.LocalName != "sp")
                    throw new CodecException("presentation_edit_target_mismatch", $"PPTX edit operation {operation.OperationId} {leafKind} target has the wrong native element type.", operation.SlidePartPath);
                patches.Add(CompileTextShadowColorXmlPatch(xml, range, proof, drawingPrefixes));
                continue;
            }
            ...
            if (leafKind is "textDefaultShadowBlurRadiusEmu" or "textDefaultShadowDistanceEmu" or "textDefaultShadowDirectionDegrees" or "textDefaultShadowScaleX" or "textDefaultShadowScaleY" or "textDefaultShadowSkewX" or "textDefaultShadowSkewY" or "textDefaultShadowAlignment")
            {
                if (range.LocalName != "sp")
                    throw new CodecException("presentation_edit_target_mismatch", $"PPTX edit operation {operation.OperationId} {leafKind} target has the wrong native element type.", operation.SlidePartPath);
            ...
            if (leafKind is "textDefaultReflectionBlurRadiusEmu" or "textDefaultReflectionDistanceEmu" or "textDefaultReflectionStartOpacityThousandthPercent" or "textDefaultReflectionEndOpacityThousandthPercent" or "textDefaultReflectionDirectionDegrees" or "textDefaultReflectionFadeAngleDegrees" or "textDefaultReflectionScaleX" or "textDefaultReflectionScaleY" or "textDefaultReflectionSkewX" or "textDefaultReflectionSkewY" or "textDefaultReflectionAlignment" or "textDefaultReflectionRotateWithShape" or "textDefaultReflectionStartPosition" or "textDefaultReflectionEndPosition")
            {
                if (range.LocalName != "sp")
                    throw new CodecException("presentation_edit_target_mismatch", $"PPTX edit operation {operation.OperationId} {leafKind} target has the wrong native element type.", operation.SlidePartPath);
            ...
            if (leafKind is "textReflectionStartPosition" or "textReflectionEndPosition" or "textReflectionFadeAngleDegrees" or "textReflectionScaleX" or "textReflectionScaleY" or "textReflectionSkewX" or "textReflectionSkewY" or "textReflectionAlignment" or "textReflectionRotateWithShape" or "textReflectionBlurRadiusEmu" or "textReflectionStartOpacityThousandthPercent" or "textReflectionEndOpacityThousandthPercent" or "textReflectionDistanceEmu" or "textReflectionDirectionDegrees")
            {
                if (range.LocalName != "sp")
                    throw new CodecException("presentation_edit_target_mismatch", $"PPTX edit operation {operation.OperationId} {leafKind} target has the wrong native element type.", operation.SlidePartPath);
            ...
            if (leafKind == "textFieldType")
            {
                if (range.LocalName != "sp")
                    throw new CodecException("presentation_edit_target_mismatch", $"PPTX edit operation {operation.OperationId} textFieldType target has the wrong native element type.", operation.SlidePartPath);
                patches.Add(CompileTextFieldTypeXmlPatch(xml, range, proof, drawingPrefixes));
                continue;
            }
            if (leafKind == "textFieldId")
            {
                if (range.LocalName != "sp")
                    throw new CodecException("presentation_edit_target_mismatch", $"PPTX edit operation {operation.OperationId} textFieldId target has the wrong native element type.", operation.SlidePartPath);
                patches.Add(CompileTextFieldIdXmlPatch(xml, range, proof, drawingPrefixes));
                continue;
            }
            if (leafKind == "tableTextFieldType")
            {
                if (range.LocalName != "graphicFrame")
                    throw new CodecException("presentation_edit_target_mismatch", $"PPTX edit operation {operation.OperationId} tableTextFieldType target has the wrong native element type.", operation.SlidePartPath);
                patches.Add(CompileTableCellFieldTypeXmlPatch(xml, range, proof, drawingPrefixes));
                continue;
            }
            if (leafKind == "tableTextFieldId")
            {
                if (range.LocalName != "graphicFrame")
                    throw new CodecException("presentation_edit_target_mismatch", $"PPTX edit operation {operation.OperationId} tableTextFieldId target has the wrong native element type.", operation.SlidePartPath);
                patches.Add(CompileTableCellFieldIdXmlPatch(xml, range, proof, drawingPrefixes));
                continue;
            }
    ...
    private static PptxXmlPatch CompileTextFieldTypeXmlPatch(
        string xml,
        ...
        IReadOnlySet<string> drawingPrefixes)
    {
        ...
        var fields = FieldPattern().Matches(elementXml)
            .Where(match => drawingPrefixes.Contains(match.Groups["prefix"].Value))
            .ToArray();
        if (operation.NativeLeafIndex >= (uint)fields.Length)
            throw new CodecException("presentation_edit_target_missing", $"PPTX edit operation {operation.OperationId} field index is out of range.", operation.SlidePartPath);
        var field = fields[operation.NativeLeafIndex];
        var startTag = XmlTokenPattern().Matches(field.Value).Cast<Match>()
            .FirstOrDefault(match => !match.Value.StartsWith("</", StringComparison.Ordinal) && LocalName(match.Value) == "fld")
            ?? throw new CodecException("presentation_edit_target_missing", $"PPTX edit operation {operation.OperationId} field start tag was not found.", operation.SlidePartPath);
        var attributes = XmlAttributePattern().Matches(startTag.Value).Cast<Match>()
            .Where(match => LocalAttributeName(match.Groups["name"].Value) == "type")
            .ToArray();
        if (attributes.Length != 1)
            throw new CodecException("presentation_edit_target_mismatch", $"PPTX edit operation {operation.OperationId} field type attribute is missing or ambiguous.", operation.SlidePartPath);
        ...
        if (actual != operation.ExpectedValue)
            throw new CodecException("presentation_leaf_precondition_failed", $"PPTX edit operation {operation.OperationId} raw field type does not match the expected value.", operation.SlidePartPath);
        if (!PptxTextCodec.ValidFieldType(operation.Value) || PptxTextCodec.IsAutomaticFieldType(operation.Value) ||
            PptxTextCodec.IsAutomaticFieldType(actual))
            throw new CodecException("invalid_presentation_edit_operation", $"PPTX edit operation {operation.OperationId} field type must remain a valid non-automatic token.", operation.SlidePartPath);
        var start = elementRange.Start + field.Index + startTag.Index + valueGroup.Index;
    ...
    private static PptxXmlPatch CompileTextFieldIdXmlPatch(
        string xml,
        XmlRange elementRange,
        PptxEditPlanProof proof,
        IReadOnlySet<string> drawingPrefixes)
    {
        ...
        var startTag = XmlTokenPattern().Matches(field.Value).Cast<Match>()
            ...
            ?? throw new CodecException("presentation_edit_target_missing", $"PPTX edit operation {operation.OperationId} field start tag was not found.", operation.SlidePartPath);
        var attributes = XmlAttributePattern().Matches(startTag.Value).Cast<Match>()
            .Where(match => LocalAttributeName(match.Groups["name"].Value) == "id")
            .ToArray();
        if (attributes.Length != 1)
            throw new CodecException("presentation_edit_target_mismatch", $"PPTX edit operation {operation.OperationId} field ID attribute is missing or ambiguous.", operation.SlidePartPath);
        var valueGroup = attributes[0].Groups["value"];
        var actual = System.Net.WebUtility.HtmlDecode(valueGroup.Value);
        if (actual != operation.ExpectedValue)
            throw new CodecException("presentation_leaf_precondition_failed", $"PPTX edit operation {operation.OperationId} raw field ID does not match the expected value.", operation.SlidePartPath);
        if (!PptxTextCodec.ValidFieldId(actual) || !PptxTextCodec.ValidFieldId(operation.Value))
            throw new CodecException("invalid_presentation_edit_operation", $"PPTX edit operation {operation.OperationId} field ID must be a brace-wrapped UUID.", operation.SlidePartPath);
        return new PptxXmlPatch(
            operation,
            elementRange.Start + field.Index + startTag.Index + valueGroup.Index,
            elementRange.Start + field.Index + startTag.Index + valueGroup.Index + valueGroup.Length,
            EscapeAttribute(operation.Value),
            proof.SourceElementSha256,
            proof.MutationPartPath);
    }

    private static PptxXmlPatch CompileTableCellFieldTypeXmlPatch(
        string xml,
        XmlRange elementRange,
        PptxEditPlanProof proof,
        IReadOnlySet<string> drawingPrefixes)
    {
        ...
        var fields = FieldPattern().Matches(cell.Value)
            .Where(match => drawingPrefixes.Contains(match.Groups["prefix"].Value))
            .ToArray();
        if (operation.NativeLeafIndex >= (uint)fields.Length)
            throw new CodecException("presentation_edit_target_missing", $"PPTX edit operation {operation.OperationId} table-cell field index is out of range.", operation.SlidePartPath);
        var field = fields[operation.NativeLeafIndex];
        var startTag = XmlTokenPattern().Matches(field.Value).Cast<Match>()
            .FirstOrDefault(match => !match.Value.StartsWith("</", StringComparison.Ordinal) && LocalName(match.Value) == "fld")
            ?? throw new CodecException("presentation_edit_target_missing", $"PPTX edit operation {operation.OperationId} table-cell field start tag was not found.", operation.SlidePartPath);
        var attributes = XmlAttributePattern().Matches(startTag.Value).Cast<Match>()
            .Where(match => LocalAttributeName(match.Groups["name"].Value) == "type")
            .ToArray();
        if (attributes.Length != 1)
            throw new CodecException("presentation_edit_target_mismatch", $"PPTX edit operation {operation.OperationId} table-cell field type attribute is missing or ambiguous.", operation.SlidePartPath);
        var valueGroup = attributes[0].Groups["value"];
        var actual = System.Net.WebUtility.HtmlDecode(valueGroup.Value);
        if (actual != operation.ExpectedValue)
            throw new CodecException("presentation_leaf_precondition_failed", $"PPTX edit operation {operation.OperationId} raw table-cell field type does not match the expected value.", operation.SlidePartPath);
        if (!PptxTextCodec.ValidFieldType(operation.Value) || PptxTextCodec.IsAutomaticFieldType(operation.Value) ||
            !PptxTextCodec.ValidFieldType(actual) || PptxTextCodec.IsAutomaticFieldType(actual))
            throw new CodecException("invalid_presentation_edit_operation", $"PPTX edit operation {operation.OperationId} table-cell field type must remain a valid non-automatic token.", operation.SlidePartPath);
        var start = elementRange.Start + cell.Index + field.Index + startTag.Index + valueGroup.Index;
        return new PptxXmlPatch(operation, start, start + valueGroup.Length, EscapeAttribute(operation.Value), proof.SourceElementSha256, proof.MutationPartPath);
    }

    private static PptxXmlPatch CompileTableCellFieldIdXmlPatch(
        string xml,
        XmlRange elementRange,
        PptxEditPlanProof proof,
        IReadOnlySet<string> drawingPrefixes)
    {
        ...
        var attributes = XmlAttributePattern().Matches(startTag.Value).Cast<Match>()
            ...
            .ToArray();
        if (attributes.Length != 1)
            throw new CodecException("presentation_edit_target_mismatch", $"PPTX edit operation {operation.OperationId} table-cell field ID attribute is missing or ambiguous.", operation.SlidePartPath);
        var valueGroup = attributes[0].Groups["value"];
        var actual = System.Net.WebUtility.HtmlDecode(valueGroup.Value);
        if (actual != operation.ExpectedValue)
            throw new CodecException("presentation_leaf_precondition_failed", $"PPTX edit operation {operation.OperationId} raw table-cell field ID does not match the expected value.", operation.SlidePartPath);
        if (!PptxTextCodec.ValidFieldId(actual) || !PptxTextCodec.ValidFieldId(operation.Value))
            throw new CodecException("invalid_presentation_edit_operation", $"PPTX edit operation {operation.OperationId} table-cell field ID must be a brace-wrapped UUID.", operation.SlidePartPath);
        var start = elementRange.Start + cell.Index + field.Index + startTag.Index + valueGroup.Index;
        return new PptxXmlPatch(operation, start, start + valueGroup.Length, EscapeAttribute(operation.Value), proof.SourceElementSha256, proof.MutationPartPath);
    }
    ...
    private static bool HasSafeNativeShadowGeometry(OpenXmlCompositeElement properties, string kind)
    {
        ...
        if (kind is "shadowBlurRadiusEmu" or "imageShadowBlurRadiusEmu" or "shadowDistanceEmu" or "imageShadowDistanceEmu")
            return long.TryParse(attributes[0].Value, System.Globalization.NumberStyles.None, System.Globalization.CultureInfo.InvariantCulture, out var coordinate) &&
                coordinate is >= 0 && (kind == "textShadowBlurRadiusEmu" ? coordinate <= 12_700_000 : coordinate <= 2_147_483_647) &&
    ...
    private static bool HasSafeNativeTextShadow(OpenXmlCompositeElement properties, string kind)
    {
        if (!PptxShadowCodec.TryRead(properties, out var shadow, allowTransforms: true) || shadow is null)
            return false;
        return kind switch
        {
            "textShadowScaleX" => PptxShadowCodec.IsSafeDirectTextRun(shadow, allowScaleX: true, allowScaleY: false),
            "textShadowScaleY" => PptxShadowCodec.IsSafeDirectTextRun(shadow, allowScaleX: false, allowScaleY: true),
            "textShadowSkewX" => PptxShadowCodec.IsSafeDirectTextRun(shadow, allowScaleX: false, allowScaleY: false, allowSkewX: true),
            "textShadowSkewY" => PptxShadowCodec.IsSafeDirectTextRun(shadow, allowScaleX: false, allowScaleY: false, allowSkewY: true),
            "textShadowRotateWithShape" => PptxShadowCodec.IsSafeDirectTextRun(shadow, allowScaleX: false, allowScaleY: false, allowRotateWithShape: true) && PptxShadowCodec.HasCanonicalRotateWithShapeToken(properties),
            _ => PptxShadowCodec.IsSafeDirectTextRun(shadow, allowScaleX: false, allowScaleY: false),
        };
    }
    ...
    private static bool HasSafeNativeTextReflection(OpenXmlCompositeElement properties, string kind)
    {
        var allowVariableStart = kind == "textReflectionStartPosition";
        var allowVariableEnd = kind == "textReflectionEndPosition";
        var allowFadeAngle = kind == "textReflectionFadeAngleDegrees";
        var allowScaleX = kind == "textReflectionScaleX";
        var allowScaleY = kind == "textReflectionScaleY";
        var allowSkewX = kind == "textReflectionSkewX";
        var allowSkewY = kind == "textReflectionSkewY";
        var allowAlignment = kind == "textReflectionAlignment";
        var allowRotateWithShape = kind == "textReflectionRotateWithShape";
        if (!PptxReflectionCodec.TryRead(properties, out var reflection,
                allowTransforms: allowFadeAngle || allowScaleX || allowScaleY || allowSkewX || allowSkewY || allowAlignment || allowRotateWithShape,
                allowVariablePositions: allowVariableStart || allowVariableEnd) || reflection is null)
            return false;
        if (allowVariableStart)
            return reflection.HasStartPositionThousandthPercent &&
                   (!reflection.HasEndPositionThousandthPercent || reflection.EndPositionThousandthPercent == 100_000);
        if (allowVariableEnd)
            return reflection.HasEndPositionThousandthPercent &&
        ...
        if (allowFadeAngle)
        {
            return reflection.HasFadeDirectionAngle60000 &&
                   fullSpan &&
        ...
        if (allowScaleY)
        {
            return reflection.HasScaleYThousandthPercent &&
                   fullSpan &&
                   !reflection.HasFadeDirectionAngle60000 &&
                   !reflection.HasScaleXThousandthPercent &&
                   !reflection.HasSkewXAngle60000 &&
        ...
        if (allowSkewX)
        {
            return reflection.HasSkewXAngle60000 &&
                   fullSpan &&
                   !reflection.HasFadeDirectionAngle60000 &&
                   !reflection.HasScaleXThousandthPercent &&
                   !reflection.HasScaleYThousandthPercent &&
        ...
        if (allowSkewY)
        {
            return reflection.HasSkewYAngle60000 &&
                   fullSpan &&
                   !reflection.HasFadeDirectionAngle60000 &&
                   !reflection.HasScaleXThousandthPercent &&
                   !reflection.HasScaleYThousandthPercent &&
        ...
        if (allowRotateWithShape)
        {
            return reflection.HasRotateWithShape &&
                   fullSpan &&
                   !reflection.HasFadeDirectionAngle60000 &&
                   !reflection.HasScaleXThousandthPercent &&
                   !reflection.HasScaleYThousandthPercent &&
    ...
    private static bool TryReadSafeNativeParagraphDefaultReflection(
        OpenXmlCompositeElement properties,
        out PresentationReflection reflection,
        bool allowReflectionTransforms = false,
        bool allowReflectionStartPosition = false,
        bool allowReflectionEndPosition = false)
    {
        reflection = null!;
        if (!PptxReflectionCodec.TryRead(
                properties,
                out var parsed,
                allowTransforms: true,
                allowVariablePositions: allowReflectionStartPosition || allowReflectionEndPosition) ||
            parsed is null)
            return false;
        if (allowReflectionStartPosition &&
            parsed.HasEndPositionThousandthPercent &&
            parsed.EndPositionThousandthPercent != 100_000)
            return false;
        if (allowReflectionEndPosition &&
            parsed.HasStartPositionThousandthPercent &&
            parsed.StartPositionThousandthPercent != 0)
            return false;
        reflection = parsed;
        return allowReflectionTransforms ||
               (!reflection.HasScaleXThousandthPercent && !reflection.HasScaleYThousandthPercent &&
                !reflection.HasSkewXAngle60000 && !reflection.HasSkewYAngle60000);
    }
    ...
    private static string ReadLeafValue(OpenXmlElement element, PresentationEditOperation operation)
    {
        ...
        {
            if (element is not P.Shape shape)
                throw new CodecException("presentation_edit_target_mismatch", $"PPTX edit operation {operation.OperationId} textFieldType target is not a shape.", operation.SlidePartPath);
            var fields = shape.Descendants<A.Field>().ToArray();
            if (operation.NativeLeafIndex >= (uint)fields.Length)
                throw new CodecException("presentation_edit_target_missing", $"PPTX edit operation {operation.OperationId} field index is out of range.", operation.SlidePartPath);
            var type = fields[operation.NativeLeafIndex].Type?.Value;
            if (!PptxTextCodec.ValidFieldType(type) || PptxTextCodec.IsAutomaticFieldType(type))
                throw new CodecException("presentation_edit_target_missing", $"PPTX edit operation {operation.OperationId} target is not a valid static field type.", operation.SlidePartPath);
            return type!;
        }
        if (kind == "textFieldId")
        {
            if (element is not P.Shape shape)
                throw new CodecException("presentation_edit_target_mismatch", $"PPTX edit operation {operation.OperationId} textFieldId target is not a shape.", operation.SlidePartPath);
            var fields = shape.Descendants<A.Field>().ToArray();
            if (operation.NativeLeafIndex >= (uint)fields.Length)
                throw new CodecException("presentation_edit_target_missing", $"PPTX edit operation {operation.OperationId} field index is out of range.", operation.SlidePartPath);
            var field = fields[operation.NativeLeafIndex];
            var id = field.Id?.Value;
            if (!PptxTextCodec.ValidFieldId(id) ||
                !PptxTextCodec.ValidFieldType(field.Type?.Value) ||
                PptxTextCodec.IsAutomaticFieldType(field.Type?.Value))
                throw new CodecException("presentation_edit_target_missing", $"PPTX edit operation {operation.OperationId} target is not a valid static field ID.", operation.SlidePartPath);
            return id!;
        }
        if (kind is "tableTextFieldType" or "tableTextFieldId")
        {
            if (element is not P.GraphicFrame table || !PptxTableCodec.TryRead(table, out _))
                throw new CodecException("presentation_edit_target_mismatch", $"PPTX edit operation {operation.OperationId} {kind} target is not a supported table.", operation.SlidePartPath);
            var cells = table.Graphic?.GraphicData?.GetFirstChild<A.Table>()?.Elements<A.TableRow>()
                .SelectMany(row => row.Elements<A.TableCell>()).ToArray() ?? [];
            if (operation.TextLeafIndex >= (uint)cells.Length)
            ...
            var fields = cells[operation.TextLeafIndex].GetFirstChild<A.TextBody>()?.Descendants<A.Field>().ToArray() ?? [];
            if (operation.NativeLeafIndex >= (uint)fields.Length)
                throw new CodecException("presentation_edit_target_missing", $"PPTX edit operation {operation.OperationId} table-cell field index is out of range.", operation.SlidePartPath);
            var field = fields[operation.NativeLeafIndex];
            var type = field.Type?.Value;
            if (!PptxTextCodec.ValidFieldType(type) || PptxTextCodec.IsAutomaticFieldType(type))
                throw new CodecException("presentation_edit_target_missing", $"PPTX edit operation {operation.OperationId} target is not a valid static table-cell field type.", operation.SlidePartPath);
            if (kind == "tableTextFieldType") return type!;
            var id = field.Id?.Value;
            if (!PptxTextCodec.ValidFieldId(id))
                throw new CodecException("presentation_edit_target_missing", $"PPTX edit operation {operation.OperationId} target is not a valid static table-cell field ID.", operation.SlidePartPath);
            return id!;
        }
        ...
        if (kind is "textDefaultShadowBlurRadiusEmu" or "textDefaultShadowDistanceEmu" or "textDefaultShadowDirectionDegrees" or "textDefaultShadowScaleX" or "textDefaultShadowScaleY" or "textDefaultShadowSkewX" or "textDefaultShadowSkewY" or "textDefaultShadowAlignment" or "textDefaultShadowColorRgb" or "textDefaultShadowColorScheme" or "textDefaultShadowOpacityThousandthPercent" or "textDefaultShadowRotateWithShape")
        {
            if (element is not P.Shape shape)
                throw new CodecException("presentation_edit_target_mismatch", $"PPTX edit operation {operation.OperationId} {kind} target is not a shape.", operation.SlidePartPath);
            var paragraphs = shape.Descendants<A.Paragraph>().ToArray();
            ...
            var shadowNative = shadowProperties.GetFirstChild<A.EffectList>()?.GetFirstChild<A.OuterShadow>();
            if (shadowNative is null ||
                !PptxShadowCodec.TryReadOuterShadow(shadowNative, out var shadow, allowTransforms: true) || shadow is null ||
            ...
            if (kind == "textDefaultShadowBlurRadiusEmu" && !shadow.HasBlurRadiusEmu ||
                ...
                kind == "textDefaultShadowScaleX" && !shadow.HasScaleXThousandthPercent ||
                kind == "textDefaultShadowScaleY" && !shadow.HasScaleYThousandthPercent ||
                kind == "textDefaultShadowSkewX" && !shadow.HasSkewXAngle60000 ||
                kind == "textDefaultShadowSkewY" && !shadow.HasSkewYAngle60000 ||
            ...
            return kind switch
            {
                ...
                "textDefaultShadowScaleX" => shadow.ScaleXThousandthPercent.ToString(System.Globalization.CultureInfo.InvariantCulture),
                "textDefaultShadowScaleY" => shadow.ScaleYThousandthPercent.ToString(System.Globalization.CultureInfo.InvariantCulture),
                "textDefaultShadowSkewX" => shadow.SkewXAngle60000.ToString(System.Globalization.CultureInfo.InvariantCulture),
                "textDefaultShadowSkewY" => shadow.SkewYAngle60000.ToString(System.Globalization.CultureInfo.InvariantCulture),
        ...
        if (kind is "textShadowBlurRadiusEmu" or "textShadowDistanceEmu" or "textShadowDirectionDegrees" or "textShadowScaleX" or "textShadowScaleY" or "textShadowSkewX" or "textShadowSkewY" or "textShadowRotateWithShape" or "textShadowColorRgb" or "textShadowColorScheme" or "textShadowOpacityThousandthPercent" or "textShadowAlignment")
        {
            if (element is not P.Shape shape)
                throw new CodecException("presentation_edit_target_mismatch", $"PPTX edit operation {operation.OperationId} {kind} target is not a shape.", operation.SlidePartPath);
            var leaves = shape.Descendants<A.Text>().ToArray();
            if (operation.TextLeafIndex >= (uint)leaves.Length || leaves[operation.TextLeafIndex].Parent is not A.Run run ||
                run.RunProperties is not { } shadowProperties || !HasSafeNativeTextShadow(shadowProperties, kind))
                throw new CodecException("presentation_edit_target_missing", $"PPTX edit operation {operation.OperationId} target has no bounded explicit text outer shadow.", operation.SlidePartPath);
            if (!PptxShadowCodec.TryRead(shadowProperties, out var shadow, allowTransforms: true) || shadow is null ||
                kind == "textShadowBlurRadiusEmu" && !shadow.HasBlurRadiusEmu ||
                kind == "textShadowDistanceEmu" && !shadow.HasDistanceEmu ||
                kind == "textShadowDirectionDegrees" && !shadow.HasDirectionAngle60000 ||
                kind == "textShadowScaleX" && !shadow.HasScaleXThousandthPercent ||
                kind == "textShadowScaleY" && !shadow.HasScaleYThousandthPercent ||
                kind == "textShadowSkewX" && !shadow.HasSkewXAngle60000 ||
                kind == "textShadowSkewY" && !shadow.HasSkewYAngle60000 ||
                kind == "textShadowRotateWithShape" && !shadow.HasRotateWithShape ||
                kind == "textShadowColorRgb" && string.IsNullOrEmpty(shadow.ColorRgb) ||
                kind == "textShadowColorScheme" && string.IsNullOrEmpty(shadow.ColorScheme) ||
                kind == "textShadowOpacityThousandthPercent" && !shadow.HasOpacityThousandthPercent ||
                kind == "textShadowAlignment" && !shadow.HasAlignment)
                throw new CodecException("presentation_edit_target_missing", $"PPTX edit operation {operation.OperationId} target has no bounded explicit text outer shadow.", operation.SlidePartPath);
            return kind switch
            {
                "textShadowBlurRadiusEmu" => shadow.HasBlurRadiusEmu
                    ? shadow.BlurRadiusEmu.ToString(System.Globalization.CultureInfo.InvariantCulture)
                    : MissingLeaf(operation),
                "textShadowDistanceEmu" => shadow.HasDistanceEmu
                    ? shadow.DistanceEmu.ToString(System.Globalization.CultureInfo.InvariantCulture)
                    : MissingLeaf(operation),
                "textShadowDirectionDegrees" => shadow.HasDirectionAngle60000
                    ? shadow.DirectionAngle60000.ToString(System.Globalization.CultureInfo.InvariantCulture)
                    : MissingLeaf(operation),
                "textShadowScaleX" => shadow.HasScaleXThousandthPercent
                    ? shadow.ScaleXThousandthPercent.ToString(System.Globalization.CultureInfo.InvariantCulture)
                    : MissingLeaf(operation),
                "textShadowScaleY" => shadow.HasScaleYThousandthPercent
                    ? shadow.ScaleYThousandthPercent.ToString(System.Globalization.CultureInfo.InvariantCulture)
                    : MissingLeaf(operation),
                "textShadowSkewX" => shadow.HasSkewXAngle60000
                    ? shadow.SkewXAngle60000.ToString(System.Globalization.CultureInfo.InvariantCulture)
                    : MissingLeaf(operation),
                "textShadowSkewY" => shadow.HasSkewYAngle60000
                    ? shadow.SkewYAngle60000.ToString(System.Globalization.CultureInfo.InvariantCulture)
                    : MissingLeaf(operation),
                "textShadowRotateWithShape" => shadow.HasRotateWithShape
                    ? (shadow.RotateWithShape ? "1" : "0")
                    : MissingLeaf(operation),
                "textShadowColorRgb" => shadow.ColorRgb,
                "textShadowColorScheme" => shadow.ColorScheme,
                "textShadowOpacityThousandthPercent" => shadow.OpacityThousandthPercent.ToString(System.Globalization.CultureInfo.InvariantCulture),
                "textShadowAlignment" => shadow.Alignment,
                _ => throw new CodecException("invalid_presentation_edit_target", $"PPTX edit operation {operation.OperationId} has an unsupported direct text outer-shadow leaf.", operation.SlidePartPath),
            };
        }
        ...
        if (kind is "textDefaultReflectionBlurRadiusEmu" or "textDefaultReflectionDistanceEmu" or "textDefaultReflectionStartOpacityThousandthPercent" or "textDefaultReflectionEndOpacityThousandthPercent" or "textDefaultReflectionDirectionDegrees" or "textDefaultReflectionFadeAngleDegrees" or "textDefaultReflectionScaleX" or "textDefaultReflectionScaleY" or "textDefaultReflectionSkewX" or "textDefaultReflectionSkewY" or "textDefaultReflectionAlignment" or "textDefaultReflectionRotateWithShape" or "textDefaultReflectionStartPosition" or "textDefaultReflectionEndPosition")
        {
            if (element is not P.Shape shape)
                throw new CodecException("presentation_edit_target_mismatch", $"PPTX edit operation {operation.OperationId} {kind} target is not a shape.", operation.SlidePartPath);
            var paragraphs = shape.Descendants<A.Paragraph>().ToArray();
            if (operation.NativeLeafIndex >= (uint)paragraphs.Length ||
                ...
                !TryReadSafeNativeParagraphDefaultReflection(
                    reflectionProperties,
                    out var reflection,
                    allowReflectionTransforms: true,
                    allowReflectionStartPosition: kind == "textDefaultReflectionStartPosition",
                    allowReflectionEndPosition: kind == "textDefaultReflectionEndPosition"))
            ...
            return kind switch
            {
                ...
                "textDefaultReflectionStartPosition" when reflection.HasStartPositionThousandthPercent => reflection.StartPositionThousandthPercent.ToString(System.Globalization.CultureInfo.InvariantCulture),
                "textDefaultReflectionEndPosition" when reflection.HasEndPositionThousandthPercent => reflection.EndPositionThousandthPercent.ToString(System.Globalization.CultureInfo.InvariantCulture),
                ...
                "textDefaultReflectionFadeAngleDegrees" when reflection.HasFadeDirectionAngle60000 => reflection.FadeDirectionAngle60000.ToString(System.Globalization.CultureInfo.InvariantCulture),
                "textDefaultReflectionScaleX" when reflection.HasScaleXThousandthPercent => reflection.ScaleXThousandthPercent.ToString(System.Globalization.CultureInfo.InvariantCulture),
                "textDefaultReflectionScaleY" when reflection.HasScaleYThousandthPercent => reflection.ScaleYThousandthPercent.ToString(System.Globalization.CultureInfo.InvariantCulture),
                "textDefaultReflectionSkewX" when reflection.HasSkewXAngle60000 => reflection.SkewXAngle60000.ToString(System.Globalization.CultureInfo.InvariantCulture),
                "textDefaultReflectionSkewY" when reflection.HasSkewYAngle60000 => reflection.SkewYAngle60000.ToString(System.Globalization.CultureInfo.InvariantCulture),
                "textDefaultReflectionAlignment" when reflection.HasAlignment => reflection.Alignment,
                "textDefaultReflectionRotateWithShape" when reflection.HasRotateWithShape => reflection.RotateWithShape ? "1" : "0",
        ...
        if (kind is "textReflectionStartPosition" or "textReflectionEndPosition" or "textReflectionFadeAngleDegrees" or "textReflectionScaleX" or "textReflectionScaleY" or "textReflectionSkewX" or "textReflectionSkewY" or "textReflectionAlignment" or "textReflectionRotateWithShape" or "textReflectionBlurRadiusEmu" or "textReflectionStartOpacityThousandthPercent" or "textReflectionEndOpacityThousandthPercent" or "textReflectionDistanceEmu" or "textReflectionDirectionDegrees")
        {
            if (element is not P.Shape shape)
                throw new CodecException("presentation_edit_target_mismatch", $"PPTX edit operation {operation.OperationId} {kind} target is not a shape.", operation.SlidePartPath);
            var leaves = shape.Descendants<A.Text>().ToArray();
            if (operation.TextLeafIndex >= (uint)leaves.Length || leaves[operation.TextLeafIndex].Parent is not A.Run run ||
                run.RunProperties is not { } reflectionProperties || !HasSafeNativeTextReflection(reflectionProperties, kind))
            ...
            if (!PptxReflectionCodec.TryRead(reflectionProperties, out var reflection,
                    allowTransforms: kind is "textReflectionFadeAngleDegrees" or "textReflectionScaleX" or "textReflectionScaleY" or "textReflectionSkewX" or "textReflectionSkewY" or "textReflectionAlignment" or "textReflectionRotateWithShape",
                    allowVariablePositions: kind is "textReflectionStartPosition" or "textReflectionEndPosition") || reflection is null)
                throw new CodecException("presentation_edit_target_missing", $"PPTX edit operation {operation.OperationId} target has no bounded explicit text reflection.", operation.SlidePartPath);
            return kind switch
            {
                "textReflectionStartPosition" => reflection.HasStartPositionThousandthPercent
                    ? reflection.StartPositionThousandthPercent.ToString(System.Globalization.CultureInfo.InvariantCulture)
                    : MissingLeaf(operation),
                "textReflectionEndPosition" => reflection.HasEndPositionThousandthPercent
                    ? reflection.EndPositionThousandthPercent.ToString(System.Globalization.CultureInfo.InvariantCulture)
                    : MissingLeaf(operation),
                "textReflectionFadeAngleDegrees" => reflection.HasFadeDirectionAngle60000
                    ? reflection.FadeDirectionAngle60000.ToString(System.Globalization.CultureInfo.InvariantCulture)
                    : MissingLeaf(operation),
                "textReflectionScaleX" => reflection.HasScaleXThousandthPercent
                    ? reflection.ScaleXThousandthPercent.ToString(System.Globalization.CultureInfo.InvariantCulture)
                    : MissingLeaf(operation),
                "textReflectionScaleY" => reflection.HasScaleYThousandthPercent
                    ? reflection.ScaleYThousandthPercent.ToString(System.Globalization.CultureInfo.InvariantCulture)
                    : MissingLeaf(operation),
                "textReflectionSkewX" => reflection.HasSkewXAngle60000
                    ? reflection.SkewXAngle60000.ToString(System.Globalization.CultureInfo.InvariantCulture)
                    : MissingLeaf(operation),
                "textReflectionSkewY" => reflection.HasSkewYAngle60000
                    ? reflection.SkewYAngle60000.ToString(System.Globalization.CultureInfo.InvariantCulture)
                    : MissingLeaf(operation),
                "textReflectionAlignment" => reflection.HasAlignment
                    ? reflection.Alignment
                    : MissingLeaf(operation),
                "textReflectionRotateWithShape" => reflection.HasRotateWithShape
                    ? (reflection.RotateWithShape ? "1" : "0")
                    : MissingLeaf(operation),
    ...
    private static PptxXmlPatch CompileTextShadowXmlPatch(
        string xml,
        XmlRange elementRange,
        PptxEditPlanProof proof,
        IReadOnlySet<string> drawingPrefixes)
    {
        ...
        var startTag = XmlTokenPattern().Matches(fragment).Cast<Match>()
            ...
            ?? throw new CodecException("presentation_edit_target_missing", $"PPTX edit operation {operation.OperationId} outer-shadow tag was not found.", operation.SlidePartPath);
        var attribute = kind switch
        {
            "textShadowBlurRadiusEmu" => "blurRad",
            "textShadowDistanceEmu" => "dist",
            "textShadowDirectionDegrees" => "dir",
            "textShadowScaleX" => "sx",
            "textShadowScaleY" => "sy",
            "textShadowSkewX" => "kx",
            "textShadowSkewY" => "ky",
            "textShadowRotateWithShape" => "rotWithShape",
            "textShadowAlignment" => "algn",
            _ => throw new CodecException("invalid_presentation_edit_target", $"PPTX edit operation {operation.OperationId} has an unsupported direct text outer-shadow leaf.", operation.SlidePartPath),
        };
        var attributes = XmlAttributePattern().Matches(startTag.Value).Cast<Match>()
            .Where(match => LocalAttributeName(match.Groups["name"].Value) == attribute)
            .ToArray();
        if (attributes.Length != 1)
            throw new CodecException("presentation_edit_target_mismatch", $"PPTX edit operation {operation.OperationId} outer-shadow {attribute} attribute is missing or ambiguous.", operation.SlidePartPath);
        var valueGroup = attributes[0].Groups["value"];
        if (System.Net.WebUtility.HtmlDecode(valueGroup.Value) != operation.ExpectedValue)
            throw new CodecException("presentation_leaf_precondition_failed", $"PPTX edit operation {operation.OperationId} raw text outer-shadow {attribute} does not match the expected value.", operation.SlidePartPath);
    ...
    private static PptxXmlPatch CompileTextShadowColorXmlPatch(
        string xml,
        XmlRange elementRange,
        PptxEditPlanProof proof,
        IReadOnlySet<string> drawingPrefixes)
    {
        ...
        var outerShadow = DirectChildRange(elementXml, effectList, "effectLst", "outerShdw", operation);
        var kind = LeafKind(operation);
        var colorName = kind == "textShadowColorScheme" ? "schemeClr" : "srgbClr";
        ...
        var startTag = XmlTokenPattern().Matches(fragment).Cast<Match>()
            ...
            ?? throw new CodecException("presentation_edit_target_missing", $"PPTX edit operation {operation.OperationId} text outer-shadow {colorName} tag was not found.", operation.SlidePartPath);
        var attributes = XmlAttributePattern().Matches(startTag.Value).Cast<Match>()
            .Where(match => LocalAttributeName(match.Groups["name"].Value) == "val")
            .ToArray();
        if (attributes.Length != 1 || XmlAttributePattern().Matches(startTag.Value).Cast<Match>().Any(match => LocalAttributeName(match.Groups["name"].Value) != "val"))
            throw new CodecException("presentation_edit_target_mismatch", $"PPTX edit operation {operation.OperationId} text outer-shadow {colorName} value attribute is missing or ambiguous.", operation.SlidePartPath);
        var valueGroup = attributes[0].Groups["value"];
        var actual = System.Net.WebUtility.HtmlDecode(valueGroup.Value);
        var isRgb = kind == "textShadowColorRgb";
        ...
        if (!matches)
            throw new CodecException("presentation_leaf_precondition_failed", $"PPTX edit operation {operation.OperationId} raw text outer-shadow {colorName} does not match the expected value.", operation.SlidePartPath);
        var start = elementRange.Start + color.Start + startTag.Index + valueGroup.Index;
        return new PptxXmlPatch(operation, start, start + valueGroup.Length, replacement, proof.SourceElementSha256, proof.MutationPartPath);
    }

    private static PptxXmlPatch CompileTextShadowOpacityXmlPatch(
        string xml,
        XmlRange elementRange,
        PptxEditPlanProof proof,
        IReadOnlySet<string> drawingPrefixes)
    {
        ...
        var run = TextRunPattern().Matches(elementXml).Cast<Match>()
            ...
            .SingleOrDefault(match => match.Index <= leaf.Index && leaf.Index < match.Index + match.Length);
        if (run is null)
            throw new CodecException("presentation_edit_target_mismatch", $"PPTX edit operation {operation.OperationId} textShadowOpacityThousandthPercent leaf is not owned by an editable text run.", operation.SlidePartPath);
        ...
        var colors = DirectChildRanges(elementXml, outerShadow)
            ...
            .ToArray();
        if (colors.Length != 1)
            throw new CodecException("presentation_edit_target_mismatch", $"PPTX edit operation {operation.OperationId} text outer-shadow color is missing or ambiguous.", operation.SlidePartPath);
        var color = colors[0];
        ...
        var startTag = XmlTokenPattern().Matches(fragment).Cast<Match>()
            ...
            ?? throw new CodecException("presentation_edit_target_missing", $"PPTX edit operation {operation.OperationId} text outer-shadow alpha tag was not found.", operation.SlidePartPath);
        var attributes = XmlAttributePattern().Matches(startTag.Value).Cast<Match>()
            .Where(match => LocalAttributeName(match.Groups["name"].Value) == "val")
            .ToArray();
        if (attributes.Length != 1 || XmlAttributePattern().Matches(startTag.Value).Cast<Match>().Any(match => LocalAttributeName(match.Groups["name"].Value) != "val"))
            throw new CodecException("presentation_edit_target_mismatch", $"PPTX edit operation {operation.OperationId} text outer-shadow alpha value attribute is missing or ambiguous.", operation.SlidePartPath);
        var valueGroup = attributes[0].Groups["value"];
        var actual = System.Net.WebUtility.HtmlDecode(valueGroup.Value);
        if (!ValidOpacityToken(actual) || actual != operation.ExpectedValue)
            throw new CodecException("presentation_leaf_precondition_failed", $"PPTX edit operation {operation.OperationId} raw text outer-shadow alpha does not match the expected value.", operation.SlidePartPath);
        ...
        IReadOnlySet<string> drawingPrefixes)
    {
        ...
        var attribute = kind switch
        {
            "textReflectionStartPosition" => "stPos",
            "textReflectionEndPosition" => "endPos",
            "textReflectionFadeAngleDegrees" => "fadeDir",
            "textReflectionScaleX" => "sx",
            "textReflectionScaleY" => "sy",
            "textReflectionSkewX" => "kx",
            "textReflectionSkewY" => "ky",
            "textReflectionAlignment" => "algn",
            "textReflectionRotateWithShape" => "rotWithShape",
        ...
        PptxEditPlanProof proof)
    {
        ...
        var attribute = LeafKind(operation) switch
        {
            ...
            "textDefaultShadowScaleX" => "sx",
            "textDefaultShadowScaleY" => "sy",
            "textDefaultShadowSkewX" => "kx",
            "textDefaultShadowSkewY" => "ky",
        ...
        PptxEditPlanProof proof)
    {
        ...
        var attribute = LeafKind(operation) switch
        {
            ...
            "textDefaultReflectionStartPosition" => "stPos",
            "textDefaultReflectionEndPosition" => "endPos",
            ...
            "textDefaultReflectionFadeAngleDegrees" => "fadeDir",
            "textDefaultReflectionScaleX" => "sx",
            "textDefaultReflectionScaleY" => "sy",
            "textDefaultReflectionSkewX" => "kx",
            "textDefaultReflectionSkewY" => "ky",
            "textDefaultReflectionAlignment" => "algn",
            "textDefaultReflectionRotateWithShape" => "rotWithShape",
    ...
    {
        if (kind is "textReflectionScaleX" or "textReflectionScaleY" or "textDefaultReflectionScaleX" or "textDefaultReflectionScaleY")
            return ValidShadowScaleToken(value);
        if (kind is "textReflectionSkewX" or "textReflectionSkewY" or "textDefaultReflectionSkewX" or "textDefaultReflectionSkewY")
            return ValidShadowSkewToken(value);
        if (kind is "textReflectionAlignment" or "textDefaultReflectionAlignment")
            return ValidShadowGeometryToken(kind, value);
        ...
        return kind is ("textReflectionDirectionDegrees" or "textReflectionFadeAngleDegrees" or "textDefaultReflectionDirectionDegrees" or "textDefaultReflectionFadeAngleDegrees") &&
            long.TryParse(value, System.Globalization.NumberStyles.None, System.Globalization.CultureInfo.InvariantCulture, out var direction) &&
            direction is >= 0 and < 21_600_000 &&
            direction.ToString(System.Globalization.CultureInfo.InvariantCulture) == value;
    }
    private static bool ValidTextReflectionPositionToken(string value) =>
        long.TryParse(value, System.Globalization.NumberStyles.None, System.Globalization.CultureInfo.InvariantCulture, out var position) &&
        position is >= 0 and <= 100_000 &&
        position.ToString(System.Globalization.CultureInfo.InvariantCulture) == value;
    private static bool ValidTextSoftEdgeRadiusToken(string value) =>
        long.TryParse(value, System.Globalization.NumberStyles.None, System.Globalization.CultureInfo.InvariantCulture, out var radius) &&
        radius is >= 0 and <= 12_700_000 &&
        radius.ToString(System.Globalization.CultureInfo.InvariantCulture) == value;
    private static bool ValidShadowScaleToken(string value) =>
        int.TryParse(value, System.Globalization.NumberStyles.Integer, System.Globalization.CultureInfo.InvariantCulture, out var scale) &&
        scale.ToString(System.Globalization.CultureInfo.InvariantCulture) == value;
    private static bool ValidShadowSkewToken(string value) =>
        int.TryParse(value, System.Globalization.NumberStyles.Integer, System.Globalization.CultureInfo.InvariantCulture, out var skew) &&
        skew is > -5_400_000 and < 5_400_000 &&
        skew.ToString(System.Globalization.CultureInfo.InvariantCulture) == value;
    private static bool ValidShadowGeometryToken(string kind, string value)
    {
        if (kind is "shadowBlurRadiusEmu" or "imageShadowBlurRadiusEmu" or "textShadowBlurRadiusEmu" or "textShadowDistanceEmu" or "shadowDistanceEmu" or "imageShadowDistanceEmu" or "textDefaultShadowBlurRadiusEmu" or "textDefaultShadowDistanceEmu")
        {
            return long.TryParse(value, System.Globalization.NumberStyles.None, System.Globalization.CultureInfo.InvariantCulture, out var coordinate) &&
                coordinate >= 0 &&
                (kind is "textShadowBlurRadiusEmu" ? coordinate <= 12_700_000 :
                 kind is "textShadowDistanceEmu" or "textDefaultShadowDistanceEmu" ? coordinate <= 1_270_000_000 :
                 coordinate <= 2_147_483_647) &&
                coordinate.ToString(System.Globalization.CultureInfo.InvariantCulture) == value;
        }
        if (kind is "shadowDirectionDegrees" or "imageShadowDirectionDegrees" or "textDefaultShadowDirectionDegrees" or "textShadowDirectionDegrees")
        {
        ...
        if (kind is "textShadowSkewX" or "textShadowSkewY") return ValidShadowSkewToken(value);
        return (kind is "shadowAlignment" or "imageShadowAlignment" or "textShadowAlignment" or "textDefaultShadowAlignment" or "textDefaultReflectionAlignment" or "textReflectionAlignment") && (value is "tl" or "t" or "tr" or "l" or "ctr" or "r" or "bl" or "b" or "br");
    }
    ...
    private static bool LeafValuesEqual(string left, string right, string leafKind) =>
        leafKind is "fillRgb" or "lineRgb" or "textShadowColorRgb" or "shape3dContourRgb" or "shape3dExtrusionRgb"
            ? PptxColor.Normalize(left) == PptxColor.Normalize(right)
            : leafKind is "lineScheme" or "fillScheme" or "textShadowColorScheme" or "shape3dContourColorScheme" or "shape3dExtrusionColorScheme"
