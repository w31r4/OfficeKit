...
assert.ok(HELP_CATALOG.some((item) => item.name === "presentation.reuseSourceComponent"));
...
assert.equal(HELP_CATALOG.find((item) => item.name === "image.editSvgText")?.schema?.parameters?.nodeId?.required, true);
assert.match(HELP_CATALOG.find((item) => item.name === "image.svgEditCapability")?.summary || "", /source-revision-bound.*fill.*stroke.*opacity.*transform-scalar/i);
assert.match(HELP_CATALOG.find((item) => item.name === "image.getSvgEditLeaves")?.schema?.returns?.["SVG edit leaves"]?.description || "", /capabilities.*not raw XML.*XPath.*arbitrary attribute/i);
assert.equal(HELP_CATALOG.find((item) => item.name === "image.editSvgLeaf")?.schema?.parameters?.leafId?.required, true);
assert.match(HELP_CATALOG.find((item) => item.name === "image.editSvgLeaf")?.schema?.parameters?.update?.description || "", /#RGB.*opacity.*0\.\.1.*translate.*scale.*rotate/i);
assert.match(HELP_CATALOG.find((item) => item.name === "image.editSvgLeaf")?.summary || "", /expectedHash.*exact token splice.*cross-image.*no-op/i);
...
assert.match(HELP_CATALOG.find((item) => item.name === "workbook.xlsxFormulaSyntax")?.summary || "", /Excel.*_xlfn.*ANCHORARRAY.*original storage spelling/i);
assert.equal(HELP_CATALOG.find((item) => item.name === "fx.MODE.MULT")?.examples?.[0], "=MODE.MULT(A1:A10)");
...
assert.match(HELP_CATALOG.find((item) => item.name === "fx.LINEST")?.summary || "", /single-variable least-squares dynamic array.*1x2.*5x2.*stats.*constant known-x is removed.*mismatched shapes.*#N\/A.*multivariable.*unsupported/i);
assert.equal(HELP_CATALOG.find((item) => item.name === "fx.LINEST")?.schema?.returns?.value?.type, "unknown[][]");
assert.match(HELP_CATALOG.find((item) => item.name === "fx.TREND")?.summary || "", /single-variable linear prediction dynamic array.*row or column shape.*new-x.*const.*zero intercept.*constant known-x.*multivariable.*fail closed/i);
assert.equal(HELP_CATALOG.find((item) => item.name === "fx.TREND")?.schema?.returns?.value?.type, "unknown[][]");
assert.match(HELP_CATALOG.find((item) => item.name === "fx.LOGEST")?.summary || "", /single-variable exponential regression dynamic array.*y=b\*m\^x.*1x2.*5x2.*natural-log regression.*positive.*b=1.*multivariable.*fail closed/i);
assert.equal(HELP_CATALOG.find((item) => item.name === "fx.LOGEST")?.schema?.returns?.value?.type, "unknown[][]");
assert.match(HELP_CATALOG.find((item) => item.name === "fx.GROWTH")?.summary || "", /single-variable exponential prediction dynamic array.*y=b\*m\^x.*row or column shape.*positive.*b=1.*overflow.*multivariable.*fail closed/i);
assert.equal(HELP_CATALOG.find((item) => item.name === "fx.GROWTH")?.schema?.returns?.value?.type, "unknown[][]");
assert.match(HELP_CATALOG.find((item) => item.name === "fx.TRUE")?.summary || "", /logical value TRUE.*no arguments.*#VALUE!/i);
assert.match(HELP_CATALOG.find((item) => item.name === "fx.FALSE")?.summary || "", /logical value FALSE.*no arguments.*#VALUE!/i);
...
assert.match(HELP_CATALOG.find((item) => item.name === "presentation.slides.add")?.schema?.parameters?.background?.description || "", /direct slide background.*native image.*irregular imported graphs remain source-bound.*fail closed/i);
...
for (const name of ["ROW", "COLUMN", "ADDRESS", "SHEET", "SHEETS", "ISFORMULA", "FORMULATEXT"]) assert.ok(HELP_CATALOG.some((item) => item.name === `fx.${name}`));
...
assert.match(HELP_CATALOG.find((item) => item.name === "fx.ADDRESS")?.summary || "", /1-based row and column.*mode 1 through 4.*A1 or R1C1.*XFD1048576.*#VALUE!/i);
assert.match(HELP_CATALOG.find((item) => item.name === "fx.SHEET")?.summary || "", /1-based OfficeKit worksheet.*defined name.*table.*sheet-name string.*#N\/A.*3D spans.*Chart, macro, and dialog/i);
assert.match(HELP_CATALOG.find((item) => item.name === "fx.SHEETS")?.summary || "", /total number.*hidden worksheets.*validated single-sheet.*3D spans.*chart, macro, and dialog/i);
...
assert.equal(HELP_CATALOG.find((item) => item.name === "fx.SHEET")?.schema?.returns?.value?.type, "number");
assert.equal(HELP_CATALOG.find((item) => item.name === "fx.SHEETS")?.schema?.returns?.value?.type, "number");
...
assert.equal(pdfCatalog.length, 27);
...
assert.match(freeTextHelp?.schema?.returns?.operation?.description || "", /appearanceTextVerified=true.*fixed-helvetica-v1.*contents\/author\/subject.*complete snapshot.*appearanceTextVerified=true/i);
const areaHelp = HELP_CATALOG.find((item) => item.name === "PdfFile.editPdf.add_area_annotation");
assert.match(areaHelp?.summary || "", /source- and page-snapshot-bound.*unfilled.*Square or Circle.*rewrite-only.*painted bounds outside.*fail closed.*solid-no-fill-v1.*contents\/author\/subject\/RGB color/i);
assert.equal(areaHelp?.schema?.parameters?.sourceSha256?.required, true);
assert.equal(areaHelp?.schema?.parameters?.expectedPage?.required, true);
assert.equal(areaHelp?.schema?.parameters?.shape?.required, true);
assert.equal(areaHelp?.schema?.parameters?.bbox?.required, true);
assert.match(areaHelp?.schema?.parameters?.borderWidth?.description || "", /0\.5 through 12.*defaults to 2/i);
assert.match(areaHelp?.schema?.returns?.operation?.description || "", /solid\/no-fill.*appearanceBboxVerified=true.*solid-no-fill-v1.*contents\/author\/subject\/RGB color.*re-verifies.*complete-snapshot deletion/i);
...
assert.match(HELP_CATALOG.find((item) => item.name === "PdfFile.editPdf")?.schema?.parameters?.operations?.description || "", /add_area_annotation.*shape=rectangle\|ellipse.*0\.5–12 point borderWidth.*unfilled native Square\/Circle.*appearanceBbox.*Fill\/dash\/cloud\/opacity.*incremental save/is);
assert.match(HELP_CATALOG.find((item) => item.name === "PdfFile.editPdf")?.schema?.parameters?.operations?.description || "", /Fixed-helvetica-v1 FreeText update.*complete inspect-returned snapshot.*contents\/author\/subject.*appearance bounds.*default appearance.*alignment.*clipped or provider-unencodable/is);
...
assert.equal(HELP_CATALOG.find((item) => item.name === "fx.SHEET")?.category, "information");
assert.equal(HELP_CATALOG.find((item) => item.name === "fx.SHEETS")?.category, "information");
