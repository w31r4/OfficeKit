...
Use this style when the brief matches **seminar lecture; academic presentation**. It is designed for
researchers, students, technical reviewers. Its visual premise is Warm paper and restrained botanical geometry support long-form teaching. The style is a
...
**question → method → observation → derivation or comparison → interpretation → limitations → conclusion**. This style is not a good fit for **night operations**;
...
- Use **#F6F1E7** as the main field, **#26352E** for primary type, **#3E7658** for the decisive signal, **#D8795F** for secondary evidence, and **#D8D1C4** for rules or context.
...
- The recurring structure is **paper, variable page silhouettes, documentary imagery**. Vary the silhouette between
  cover, evidence, data, process, comparison, and decision pages rather than
  repeating a component grid.
- Use paper texture or warm surfaces as atmosphere, then build the lesson with precise lines, definitions, and figures. Surface warmth must not reduce contrast.
...
- **Page skeleton.** Use warm paper with an unequal two-column reading path and small botanical or geometric anchors. The title states the lesson or claim, then a short rule separates it from the explanation and figure.
- **Type rhythm.** Use a friendly bold title and a readable body layer. Keep accent colors for chapter marks, key numbers, or list steps rather than coloring paragraphs.
- **Evidence carriers.** Use small bar charts, process diagrams, quotation/definition fields, lists, and one contextual illustration or photograph. A pale panel must have a teaching function.
- **Images and layers.** Use a flat illustration or real classroom/context photo when it helps the learner. Never place text on a busy photo; keep screenshots bounded and source-labelled.
- **Pacing.** Alternate explanation, worked example, and a short conclusion. Chapter pages can breathe; body pages still need enough substance to teach the stated point.
- **Selection rule.** Choose this style for seminars, education, and long-form explanatory decks with a human, paper-like tone.

## Signature moves and variation limits

- Let **paper** recur as a quiet identity cue on selected pages; it must
