...
modulation transforms. A direct imported accent2 `a:blueMod` remains opaque,
...
- Project a canonical shared ThemePart whose accent2 direct RGB leaf contains
  ...
  `design.theme.accentTransforms.accent2.blueMod`, using a 0..1 fraction.
- Issue `setThemeAccent2BlueMod` for that field and compile one authorized
...
- `presentation-theme-accent2-bluemod`: Strict source-bound projection and
  source-preserving editing of one direct accent2 `a:blueMod` leaf.
