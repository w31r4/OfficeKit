...
- 35 cases: 21 PDF, 7 Documents, and 7 Spreadsheets. PDF is 60% of the suite and remains an absolute majority, so a meaningful Office vertical slice never creates unrelated PDF filler prompts.
- All 35 cases are `ready`. Presentation acceptance moved to `evals/presentation-program-json/`; PromptBench no longer exposes or grades the retired Presentation/MJS authoring surface.
...
### Presentation acceptance in 2.0

The six pre-2.0 Presentation PromptBench cases and their JavaScript-authoring graders are retired. Their historical measurements remain in version-control history and release notes; they are not runnable product contracts in 2.0. Current Presentation acceptance uses PPJ-native authored, recovery, and source-bound import scenarios under `evals/presentation-program-json/`.
