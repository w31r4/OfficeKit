...
Use this style when the brief matches **operations review; work presentation**. It is designed for
operators, managers, cross-functional teams. Its visual premise is Cyan carries attainment while amber is reserved for exceptions. The style is a
clean-room OfficeKit grammar: the included images teach a range of decisions,
not pages or coordinates to copy.

The deck should help the audience help a team see operating reality, the constraint behind it, and the next action that will change the result. The natural story arc is
**current state → signal → cause → trade-off → intervention → owner/date → follow-through**. This style is not a good fit for **heritage brand**;
...
- Use **#070A10** as the main field, **#F2F6F8** for primary type, **#20BBD8** for the decisive signal, **#87DCEB** for secondary evidence, and **#343B45** for rules or context.
...
- Work / Operations guidance: Every status has a definition, time window, and source. Make exceptions visible without turning them into alarm colors. Bind actions to owners and dates. Let one primary signal dominate each page.
...
- The recurring structure is **glow, variable page silhouettes, documentary imagery**. Vary the silhouette between
  cover, evidence, data, process, comparison, and decision pages rather than
  repeating a component grid.
- Use a controlled luminous accent only at attainment or exception points. Preserve a calm structural field and make the primary signal readable without the glow.
...
For this category, favor **status snapshot, operating trend, incident or cause map, service journey, roadmap/gates, decision and owner close**. The style's content shapes are
status, cause, next action; select one primary carrier per page and explain why it earns the
space.

### Information carriers

- baseline-versus-target lines, milestone tracks, exception bars, service funnels, process maps, responsibility matrices, and annotated timelines; distinguish actual, target, and forecast visually and in text
- use authentic workplace or product context when it explains the work; crop it as evidence or a chapter field, not as a generic hero. Keep operational labels and connectors above tested scrims.
...
The expected density is **dense**: concentrate it around the title-to-body transition, chart
edges, column gutters, and the final implication.

## Style-specific playbook

- **Page skeleton.** Use a near-black operating field with a stable four-corner skeleton: status or section at the top, primary signal in the middle, and owner/date at the bottom. Cyan and ice-blue carry attainment; amber marks an exception.
- **Type rhythm.** Use a compact sans with distinct weights for status, conclusion, and operational detail. Keep labels short and put the time window and definition next to every status.
- **Evidence carriers.** Prefer baseline-versus-target lines, exception bars, service funnels, gates, responsibility tracks, and annotated timelines. A status light is useful only when its state definition is written nearby.
- **Images and layers.** Photography belongs on a cover or a real incident/context page. On evidence pages, keep the field calm and editable; never use glowing imagery to hide an absent metric.
- **Pacing.** Alternate a headline operating judgment with a dense proof page, then end each chapter with owner, date, and gate. Keep the cyan signal readable without relying on glow.
- **Selection rule.** Choose this style for operations reviews, incident readouts, reliability work, and cross-functional action tracking.

## Signature moves and variation limits

- Let **glow** recur as a quiet identity cue on selected pages; it must
...
- Avoid status-light mosaics, unreadable dashboards, decorative progress rings, unexplained “good/bad” color semantics, and roadmaps without gates or accountability.
...
- Check baseline/target units, ownership, temporal order, connector visibility, label legibility, and whether the page tells the team what to do next.
