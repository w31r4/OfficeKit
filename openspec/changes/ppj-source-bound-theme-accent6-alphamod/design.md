...
transform and the direct accent6 tint, shade, luminance modulation, and
luminance offset leaves at a time, then patches the existing DrawingML token.
This change applies the same ownership boundary to accent6 alpha modulation
...
- Accent6 transforms other than `alphaMod`, transforms on other accent roles,
...
1. **Use the existing native transform field.**
   ...
   emits it. The importer records a role of `accent6`; the projector converts
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:alphaMod`. The source writer patches `Accent6Color` only when the authored
   transform is the one existing accent6 alphaMod and rejects all other
   transform flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent6AlphaMod` is accepted only with
   `accentTransforms.accent6.alphaMod` in the source-issued capability. This
