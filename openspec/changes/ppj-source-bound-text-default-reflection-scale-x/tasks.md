...
- [x] 1.1 Add `textDefaultReflectionScaleX` to the PPJ schema, native leaf
      registry, capability reference, and generated manual; verify JSON and
      maintainer contract checks.
- [x] 1.2 Project an existing paragraph default reflection `sx` as a signed
      1/100000 ratio leaf and verify the value and paragraph binding in the
      focused fixture.

## 2. Source-bound codec

- [x] 2.1 Extend edit-plan validation and expected-value proof for canonical
      signed Int32 scale tokens; verify stale, malformed, and unsupported
      reflection graphs fail closed.
- [x] 2.2 Splice only `a:reflection/@sx` in the owning SlidePart; verify fadeDir,
      sibling effects, text topology, and non-target package parts remain
      unchanged.
