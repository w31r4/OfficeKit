...
direct accent6 hue-offset token while preserving the rest of an imported theme.

## ADDED Requirements

### Requirement: Strict source-bound accent6 hueOff projection

The PPTX projector MUST expose
`design.theme.accentTransforms.accent6.hueOff` and
`setThemeAccent6HueOff` only when the canonical shared ThemePart has one
direct accent6 RGB leaf with exactly one direct `a:hueOff/@val` child, no
...
#### Scenario: Project a direct accent6 hueOff leaf
...
- **THEN** the theme contains `accentTransforms.accent6.hueOff` equal to
  `210`
- **AND** the native reference contains `setThemeAccent6HueOff` authorized
  for `accentTransforms.accent6.hueOff`

#### Scenario: Reject unsupported accent6 hueOff topology
...
- **THEN** the source-bound accent6 hueOff field and capability are omitted

### Requirement: Source-bound accent6 hueOff editing

The source-preserving compiler MUST accept one authorized change to the
existing `design.theme.accentTransforms.accent6.hueOff` field, convert a
...
#### Scenario: Edit and reproject one accent6 hueOff token

- **GIVEN** a projected direct accent6 hueOff owner at `210`
...
#### Scenario: Fail closed for unsafe accent6 hueOff edits

- **GIVEN** a source-bound accent6 hueOff owner
