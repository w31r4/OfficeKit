...
{
    [Fact]
    public void PpjSourceBoundThemeAccent5SatOffEditsOnlySatOffAndReprojects()
    {
        ...
        {
            Role = "accent5",
            SaturationOffsetThousandth = -25_000,
        });
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            {
                SourceUri = "deck.assets/source/theme-accent5-satOff.pptx",
            },
        });
        ...
        Assert.Equal(-0.25, projectedTransform["satOff"]!.GetValue<double>());
        var nativeRef = Assert.IsType<JsonObject>(projectedTheme["nativeRef"]);
        Assert.Contains(nativeRef["capabilities"]!.AsArray(), capability =>
            capability!["operation"]!.GetValue<string>() == "setThemeAccent5SatOff" &&
            capability["fields"]!.AsArray().Any(field => field!.GetValue<string>() == "accentTransforms.accent5.satOff"));
        ...
        editedProgram["design"]!["theme"]!["accentTransforms"]!["accent5"]!["satOff"] = 0.4;
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            Assert.Equal(40_000, Assert.Single(accent5.Elements<A.SaturationOffset>()).Val!.Value);
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/theme-accent5-satOff.pptx",
            },
        });
        Assert.True(reprojected.Ok, Diagnostics(reprojected));
        var reprojectedProgram = JsonNode.Parse(reprojected.PresentationProgram.ProgramJson.ToByteArray())!.AsObject();
        Assert.Equal(0.4, reprojectedProgram["design"]!["theme"]!["accentTransforms"]!["accent5"]!["satOff"]!.GetValue<double>());

        var unauthorizedProgram = JsonNode.Parse(projectedProgram.ToJsonString())!.AsObject();
        unauthorizedProgram["design"]!["theme"]!["accentTransforms"]!["accent5"]!["satOff"] = 0.8;
        unauthorizedProgram["design"]!["theme"]!["nativeRef"]!["capabilities"]!.AsArray()
            .Single(capability => capability!["operation"]!.GetValue<string>() == "setThemeAccent5SatOff")!["fields"] = new JsonArray("accentTransforms.accent5.tint");
        ...
        deletionProgram["design"]!["theme"]!["accentTransforms"]!["accent5"]!.AsObject().Remove("satOff");
        ...
        outOfRangeProgram["design"]!["theme"]!["accentTransforms"]!["accent5"]!["satOff"] = 1.1;
