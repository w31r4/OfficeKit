...
{
    [Fact]
    public void PpjSourceBoundThemeAccent1AlphaModEditsOnlyAlphaModAndReprojects()
    {
        ...
        {
            Role = "accent1",
            AlphaModulationThousandth = 60_000,
        });
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            {
                SourceUri = "deck.assets/source/theme-accent1-alphaMod.pptx",
            },
        });
        ...
        Assert.Equal(0.6, projectedTransform["alphaMod"]!.GetValue<double>());
        var nativeRef = Assert.IsType<JsonObject>(projectedTheme["nativeRef"]);
        Assert.Contains(nativeRef["capabilities"]!.AsArray(), capability =>
            capability!["operation"]!.GetValue<string>() == "setThemeAccent1AlphaMod" &&
            capability["fields"]!.AsArray().Any(field => field!.GetValue<string>() == "accentTransforms.accent1.alphaMod"));
        ...
        editedProgram["design"]!["theme"]!["accentTransforms"]!["accent1"]!["alphaMod"] = 0.4;
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            Assert.Equal(40_000, Assert.Single(accent1.Elements<A.AlphaModulation>()).Val!.Value);
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/theme-accent1-alphaMod.pptx",
            },
        });
        Assert.True(reprojected.Ok, Diagnostics(reprojected));
        var reprojectedProgram = JsonNode.Parse(reprojected.PresentationProgram.ProgramJson.ToByteArray())!.AsObject();
        Assert.Equal(0.4, reprojectedProgram["design"]!["theme"]!["accentTransforms"]!["accent1"]!["alphaMod"]!.GetValue<double>());

        var unauthorizedProgram = JsonNode.Parse(projectedProgram.ToJsonString())!.AsObject();
        unauthorizedProgram["design"]!["theme"]!["accentTransforms"]!["accent1"]!["alphaMod"] = 0.8;
        unauthorizedProgram["design"]!["theme"]!["nativeRef"]!["capabilities"]!.AsArray()
            .Single(capability => capability!["operation"]!.GetValue<string>() == "setThemeAccent1AlphaMod")!["fields"] = new JsonArray("accentTransforms.accent1.tint");
        ...
        deletionProgram["design"]!["theme"]!["accentTransforms"]!["accent1"]!.AsObject().Remove("alphaMod");
        ...
        outOfRangeProgram["design"]!["theme"]!["accentTransforms"]!["accent1"]!["alphaMod"] = 1.1;
