...
the same ownership boundary to accent4 red-channel modulation without
...
- Accent4 transforms other than `redMod`, transforms on accent4 through
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:redMod`. The source writer patches `Accent4Color` only when the authored
   transform is the one existing accent4 redMod and rejects all other transform
   flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent4RedMod` is accepted only with
   `accentTransforms.accent4.redMod` in the source-issued capability. This
