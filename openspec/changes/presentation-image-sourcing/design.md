...
### Keep the two provider adapters local and bounded

OfficeKit calls only the documented Openverse and Wikimedia HTTP APIs through two leaf-local adapters. The adapters normalize the small candidate surface required here and do not introduce cache, downloader, browser fallback, MCP, or CLI behavior. Lucide is loaded from `@iconify-json/lucide@1.2.126` and rendered deterministically from Iconify path data.

Alternative considered: reusing `webfetch-core@0.1.5`. Rejected during integration because its optional `sharp@^0.33` peer conflicts with OfficeKit's current renderer dependency and would make an ordinary `npm ci` fail. Its download behavior is also intentionally not reused because OfficeKit needs stricter DNS, redirect, type, byte, and pixel controls.
...
2. Add the exact Lucide dependency, notices, package inventory, and current `2.0.0` release metadata.
