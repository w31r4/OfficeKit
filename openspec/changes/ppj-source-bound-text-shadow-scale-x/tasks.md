...
- [x] 1.1 Add `textShadowScaleX` to direct-run parsing, projection, safety proof, readback, dispatch, token validation, and `sx` patching.
...
- [x] 2.1 Add an authored → imported/source-bound → second-projection fixture covering negative and positive signed scale values and XML/package preservation.
- [x] 2.2 Add missing/invalid token, other-transform, sibling/unknown-child, stale-proof, and no-op cases.

## 3. OpenSpec and gates

- [x] 3.1 Record the increment in backlog and coverage notes; validate with `openspec validate ppj-source-bound-text-shadow-scale-x --strict`.
