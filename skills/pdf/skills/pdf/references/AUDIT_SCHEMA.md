...
For attachment quarantine, use `savePolicy.strategy: "read-only"`, `operation.type: "extract-attachments"`, and bind `output` to the delivered `attachments.json` manifest. The audit envelope must directly repeat `validation.sourceUnchanged: true`, `validation.allHashesVerified: true`, `validation.allPathsContained: true`, `validation.duplicateNamesSeparated: true`, and the exact boolean `validation.attachmentsOpenedOrExecuted: false`; the manifest carries the same facts for the extracted payloads. The quarantine file hashes and contained paths remain task-specific validation evidence inside that manifest and the audit `validation` object. The validator rejects a missing or incorrect value, so a workflow cannot pass while leaving these security facts only in prose or in the manifest.
