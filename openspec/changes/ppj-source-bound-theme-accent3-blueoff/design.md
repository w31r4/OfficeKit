...
transform and the direct accent3 tint, shade, luminance, alpha, saturation, and
red modulation leaves at a time, then patches the existing DrawingML token.
This change applies the same ownership boundary to accent3 blue-channel offset
...
- Recognize only a canonical shared ThemePart whose accent3 color is a direct
  six-digit RGB leaf with one direct `a:blueOff/@val` child.
...
- Accent3 transforms other than `blueOff`, transforms on other accent roles, inherited or effect theme state, or arbitrary theme XML editing.
- Creating or deleting a missing blueOff node, changing multiple theme fields,
  or claiming PowerPoint host color rendering.

## Decisions

1. **Use the existing native transform field.**
  `PresentationThemeColorTransform.BlueOffsetThousandth` already represents
  the DrawingML signed -100000..100000 offset integer and the authored compiler
  already emits it. The importer records a role of `accent3`; the projector
  converts the signed value by dividing by 100000, and the source-bound compiler rounds by
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:blueOff`. The source writer patches `Accent3Color` only when the authored
   transform is the one existing accent3 blueOff and rejects all other transform
   flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent3BlueOff` is accepted only with
   `accentTransforms.accent3.blueOff` in the source-issued capability. This
