...
- Give a direct literal `a:ahXY/@minY` bound a stable native index and explicit
  EMU value.
- Keep guide identity, x bounds, the paired maximum bound, position, handle
  order, and custom-geometry topology source-bound.
- Prove a SlidePart-only bound edit and second projection with a valid range.

**Non-Goals:**

- Editing x bounds, maximum y bounds, handle position, guide identity,
  adjustment values, formulas, polar handles, paths, or list topology.
...
2. **Require a canonical non-negative integer.** The resolved model can hide a
   source reference, so exact integer round-tripping and the shape-height bound
   are required. Reference-backed bounds remain opaque.

3. **Keep the paired range graph intact.** The writer proves the direct
   `ahXY` attribute structure and changes only `minY`; it does not add/remove
   the range or edit x bounds, `maxY`, current adjustment, or other bounds. The
...
- An invalid min/max/current-adjustment relationship could affect host
...
- Changing a bound may alter how a host interprets a drag handle. The
  operation is scalar source-bound and does not promise host UI behavior or
