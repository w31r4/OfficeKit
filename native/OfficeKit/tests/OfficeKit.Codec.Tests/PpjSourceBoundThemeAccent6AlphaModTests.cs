...
{
    [Fact]
    public void PpjSourceBoundThemeAccent6AlphaModEditsOnlyAlphaModAndReprojects()
    {
        ...
        {
            Role = "accent6",
            AlphaModulationThousandth = 25_000,
        });
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            {
                SourceUri = "deck.assets/source/theme-accent6-alphamod.pptx",
            },
        });
        ...
        var projectedTransform = projectedTheme["accentTransforms"]!["accent6"]!;
        Assert.Equal(0.25, projectedTransform["alphaMod"]!.GetValue<double>());
        var nativeRef = Assert.IsType<JsonObject>(projectedTheme["nativeRef"]);
        Assert.Contains(nativeRef["capabilities"]!.AsArray(), capability =>
            capability!["operation"]!.GetValue<string>() == "setThemeAccent6AlphaMod" &&
            capability["fields"]!.AsArray().Any(field => field!.GetValue<string>() == "accentTransforms.accent6.alphaMod"));
        ...
        editedProgram["design"]!["theme"]!["accentTransforms"]!["accent6"]!["alphaMod"] = 0.5;
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            Assert.Equal("667788", accent6.Val!.Value);
            Assert.Equal(50_000, Assert.Single(accent6.Elements<A.AlphaModulation>()).Val!.Value);
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/theme-accent6-alphamod.pptx",
            },
        });
        Assert.True(reprojected.Ok, Diagnostics(reprojected));
        var reprojectedProgram = JsonNode.Parse(reprojected.PresentationProgram.ProgramJson.ToByteArray())!.AsObject();
        Assert.Equal(0.5, reprojectedProgram["design"]!["theme"]!["accentTransforms"]!["accent6"]!["alphaMod"]!.GetValue<double>());

        var unauthorizedProgram = JsonNode.Parse(projectedProgram.ToJsonString())!.AsObject();
        unauthorizedProgram["design"]!["theme"]!["accentTransforms"]!["accent6"]!["alphaMod"] = 0.6;
        unauthorizedProgram["design"]!["theme"]!["nativeRef"]!["capabilities"]!.AsArray()
            .Single(capability => capability!["operation"]!.GetValue<string>() == "setThemeAccent6AlphaMod")!["fields"] = new JsonArray("accentTransforms.accent6.shade");
        ...
        deletionProgram["design"]!["theme"]!["accentTransforms"]!["accent6"]!.AsObject().Remove("alphaMod");
        ...
        siblingProgram["design"]!["theme"]!["accentTransforms"]!["accent6"]!["shade"] = 0.1;
        ...
        outOfRangeProgram["design"]!["theme"]!["accentTransforms"]!["accent6"]!["alphaMod"] = 1.01;
