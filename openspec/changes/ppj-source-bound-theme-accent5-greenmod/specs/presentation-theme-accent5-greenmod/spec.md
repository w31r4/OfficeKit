...
direct accent5 green-channel modulation token while preserving the rest of an
imported theme.

## ADDED Requirements

### Requirement: Strict source-bound accent5 greenMod projection

The PPTX projector MUST expose
`design.theme.accentTransforms.accent5.greenMod` and
`setThemeAccent5GreenMod` only when the canonical shared ThemePart has one
direct accent5 RGB leaf with exactly one direct `a:greenMod/@val` child, no
...
#### Scenario: Project a direct accent5 greenMod leaf
...
- **THEN** the theme contains `accentTransforms.accent5.greenMod` equal to
  `0.25`
- **AND** the native reference contains `setThemeAccent5GreenMod` authorized
  for `accentTransforms.accent5.greenMod`

#### Scenario: Reject unsupported accent5 greenMod topology
...
- **THEN** the source-bound accent5 greenMod field and capability are omitted

### Requirement: Source-bound accent5 greenMod editing

The source-preserving compiler MUST accept one authorized change to the
existing `design.theme.accentTransforms.accent5.greenMod` field, convert a
...
#### Scenario: Edit and reproject one accent5 greenMod token

- **GIVEN** a projected direct accent5 greenMod owner at `0.25`
...
#### Scenario: Fail closed for unsafe accent5 greenMod edits

- **GIVEN** a source-bound accent5 greenMod owner
