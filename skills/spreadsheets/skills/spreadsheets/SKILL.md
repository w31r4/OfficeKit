...
- `examples/officekit-pivot-table-workflow.mjs`: Run or adapt this canonical example for bounded native PivotTable authoring, source and cached-value audit, inspect/render/verify, OfficeKit roundtrip, and the one source-bound `refreshOnLoad=true` → `false` hardening operation on a uniquely owned imported cache. Other imported Pivot topology remains read-only.
