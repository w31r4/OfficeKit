...
accent1 plus accent3 tint, shade, luminance, alpha, and saturation transforms.
A direct imported accent3 `a:redMod` remains opaque, leaving one concrete F-15
...
- Project a canonical shared ThemePart whose accent3 direct RGB leaf contains
  ...
  `design.theme.accentTransforms.accent3.redMod`, using a 0..1 fraction.
- Issue `setThemeAccent3RedMod` for that field and compile one authorized
...
- `presentation-theme-accent3-redmod`: Strict source-bound projection and
  source-preserving editing of one direct accent3 `a:redMod` leaf.
