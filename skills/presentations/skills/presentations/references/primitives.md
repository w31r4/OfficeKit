...
| Text and typography | Titles, body copy, runs, notes and native text leaves | `shape.text.set`, `presentation.textRange`, `nativeObject.setDiagramNodeText`, `nativeObject.setDiagramNodeRunText` | Preserve run topology, facts and source styling unless the requested operation explicitly changes them. Direct source-bound run leaves include font size, typeface, color, decoration, `fontKerningPoints`, and bounded `fontBaselinePercent` when the imported `a:rPr/@kern` or `a:rPr/@baseline` token is canonical. |
