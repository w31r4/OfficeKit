...
- Give a fully literal, in-frame text-rectangle right edge a stable native
...
- Editing left/top/bottom edges, guide references, guide formulas, handles,
...
1. **Use one native index for the right edge.** The leaf represents the one
...
3. **Preserve representation.** A direct source keeps `a:rect/@r` numeric. An
   authored/private source keeps `r="officeKitTextRight"` and changes only the
...
- A right-edge change can alter text layout inside a custom shape. Restrict the
