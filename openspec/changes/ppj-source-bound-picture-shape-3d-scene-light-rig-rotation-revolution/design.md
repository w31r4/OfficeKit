...
- Expose one canonical signed `a:rot/@rev` angle in 60000ths of a degree.
- Replace only `lightRig/a:rot/@rev` in the owning SlidePart.
- Preserve latitude, longitude, camera state, light-rig state, picture
...
- Reuse `TryReadSceneLightRigRotationRevolution` and its requested-value
...
- Token-splice only the direct `lightRig/a:rot/@rev` attribute.

# Risks / Trade-offs

- [Risk] A revolution token without a complete rotation owner could be
  mistaken for a safe scene. → Require the shared full-rotation reader.
