...
- Owner: `p:sp/p:spPr/a:scene3d/a:lightRig/a:rot/@rev`.
- Native leaf: `shape3dSceneLightRigRotationRevolution60000`.
- The owner must be the single direct `a:scene3d` child of `a:spPr`. The
  ...
  modeled.
- Projection stores only the rotation revolution in the native shape model.
  The edit plan rechecks the complete rotation owner and token-splices only
  `rot/@rev`, preserving latitude, longitude, camera/light-rig identity, root
