...
function renderElement(e, assets, diagnostics) {
  ...
  if (e.type === "chart") {
    ...
    return `<g${common}><rect x="${f.x}" y="${f.y}" width="${f.width}" height="${f.height}" fill="#F8FAFC" stroke="#98A2B3"/><line x1="${f.x + 12}" y1="${base}" x2="${f.x + f.width - 8}" y2="${base}" stroke="#667085"/>${bars}${lines}${scatter}${pie}</g>`;
  }
  if (e.type === "shape" || e.geometry) {
    const opacity = num(e.opacity ?? e.style?.opacity, 1);
    return `<rect${common} x="${f.x}" y="${f.y}" width="${f.width}" height="${f.height}" rx="${num(e.geometry?.radius, 0)}" fill="${color(e.style?.fill, "#D9E2F3")}" fill-opacity="${opacity}" stroke="${color(e.style?.stroke?.color, "#667085")}" stroke-width="${num(e.style?.stroke?.width, 1)}"/>`;
  }
