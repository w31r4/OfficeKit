...
direct accent5 saturation offset token while preserving the rest of an
imported theme.

## ADDED Requirements

### Requirement: Strict source-bound accent5 satOff projection

The PPTX projector MUST expose
`design.theme.accentTransforms.accent5.satOff` and
`setThemeAccent5SatOff` only when the canonical shared ThemePart has one
direct accent5 RGB leaf with exactly one direct `a:satOff/@val` child, no
...
#### Scenario: Project a direct accent5 satOff leaf

- **GIVEN** a canonical imported ThemePart with accent5 `334455` and
  `a:satOff/@val="-25000"`
- **WHEN** the presentation is projected to PPJ
- **THEN** the theme contains `accentTransforms.accent5.satOff` equal to
  `-0.25`
- **AND** the native reference contains `setThemeAccent5SatOff` authorized
  for `accentTransforms.accent5.satOff`

#### Scenario: Reject unsupported accent5 satOff topology
...
- **THEN** the source-bound accent5 satOff field and capability are omitted

### Requirement: Source-bound accent5 satOff editing

The source-preserving compiler MUST accept one authorized change to the
existing `design.theme.accentTransforms.accent5.satOff` field, convert a
...
#### Scenario: Edit and reproject one accent5 satOff token

- **GIVEN** a projected direct accent5 satOff owner at `-0.25`
...
#### Scenario: Fail closed for unsafe accent5 satOff edits

- **GIVEN** a source-bound accent5 satOff owner
