...
- [x] 1.1 Add the source-bound accent1 greenOff proposal, delta spec, and design, then verify `openspec validate ppj-source-bound-theme-accent1-green-off --strict` passes
- [x] 1.2 Add `setThemeAccent1GreenOff` and `accentTransforms.accent1.greenOff` to the PPJ schema, semantic capability map, registry, and generated/reference documentation; verify schema and capability coverage checks pass

## 2. Native implementation

- [x] 2.1 Read a strict direct accent1 `a:greenOff` leaf during PPTX projection and expose the authored thousandth value; verify focused projection assertions pass
- [x] 2.2 Compile one authorized greenOff field by patching only the existing direct `a:greenOff/@val`, rejecting unsupported topology and combined edits; verify the native codec builds
- [x] 2.3 Add the source-bound greenOff regression covering projection, no-op bytes, one-token edit, re-projection, deletion, siblings, combined edits, range, and capability tampering; verify the focused test passes
...
- `PpjSourceBoundThemeAccent1GreenOff`: 1 passed; `PpjSourceBoundTheme`: 31 passed.
