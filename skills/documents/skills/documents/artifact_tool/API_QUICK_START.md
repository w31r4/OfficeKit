...
For one ordinary imported header or footer, prefer the matching packaged
transaction:
...
Each binds an explicit section/reference/text target, refuses existing output or
audit paths, proves one HeaderPart or FooterPart ZIP scope and normalized XML
residual, reimports, verifies, model-renders, and emits a byte-bound audit. A
header transaction never mutates a footer, a footer transaction never mutates a
header, and neither broadens the profile to PAGE/simple fields.
...
if (footnote) {
  footnote.paragraphs = [
    "Approval remains conditional on final render QA.",
    "The native-render record is retained with the delivery audit.",
  ];
}
...
unknown attributes remain source-bound. Duplicate, child-bearing,
extension-bearing, or invalid lexical pagination/border markup fails closed on
semantic replacement. Use native Word or LibreOffice rendering to confirm the
...
- Plain-text footnotes/endnotes with 1 through 16 physical paragraphs anchored at the end of one paragraph or list item; recognized imported note bodies expose `paragraphs` plus LF-joined `text` and allow fixed-count text-only edits
