...
| F-15 | 完整 PowerPoint 差距 | 部分：theme color/font/style + bounded tint/shade/alpha/channel/discrete/hue transforms | grammar color token 的 tint/shade、authored `design.theme.accentColors` 六角色色板、`design.theme.accentTransforms` 六角色的直接 `a:tint`/`a:shade`/`a:lumMod`/`a:lumOff`/`a:alphaMod`/`a:alphaOff`/`a:satMod`/`a:satOff`/`a:redMod`/`a:redOff`/`a:greenMod`/`a:greenOff`/`a:blueMod`/`a:blueOff`/`a:hueMod`/`a:hueOff`/`a:gray`/`a:comp`/`a:inv`/`a:gamma`/`a:invGamma`、`design.theme.colorRoles` 六个 dark/light/hyperlink 角色（含有限 RGBA alpha）和 `design.theme.fontScheme.major/minor/majorEastAsia/minorEastAsia/majorComplexScript/minorComplexScript` 已有 bounded lowering；复杂脚本字体 direct `a:cs` owner 已有 authored/imported/source-bound 单叶闭环；新增 source-bound `design.theme.accentColors.accent1`、`design.theme.accentColors.accent2`、`design.theme.accentColors.accent3`、`design.theme.accentColors.accent4`、`design.theme.accentColors.accent5`、`design.theme.accentColors.accent6`、`design.theme.colorRoles.dark1`、`design.theme.colorRoles.light1`、`design.theme.colorRoles.dark2`、`design.theme.colorRoles.light2`、`design.theme.colorRoles.hyperlink` 与 `design.theme.colorRoles.followedHyperlink` 单字段 owner；新增 source-bound `design.theme.accentTransforms.accent1.tint` 单字段 owner：仅当共享 ThemePart 的 accent1 是直接 RGB 加唯一 `a:tint/@val` 时投影并只改该 tint；新增同边界的 `design.theme.accentTransforms.accent1.shade` 单字段 owner，只改唯一 `a:shade/@val`；新增同边界的 `design.theme.accentTransforms.accent1.lumMod` 单字段 owner，只改唯一 `a:lumMod/@val`；新增同边界的 `design.theme.accentTransforms.accent1.lumOff` 单字段 owner，只改唯一 `a:lumOff/@val`；新增同边界的 `design.theme.accentTransforms.accent1.alphaMod` 单字段 owner，只改唯一 `a:alphaMod/@val`；新增同边界的 `design.theme.accentTransforms.accent1.alphaOff` 单字段 owner，只改唯一 `a:alphaOff/@val`；新增同边界的 `design.theme.accentTransforms.accent1.greenMod` 单字段 owner，只改唯一 `a:greenMod/@val`；新增同边界的 `design.theme.accentTransforms.accent1.greenOff` 单字段 owner，只改唯一 `a:greenOff/@val`；新增同边界的 `design.theme.accentTransforms.accent1.blueMod` 单字段 owner，只改唯一 `a:blueMod/@val`；新增同边界的 `design.theme.accentTransforms.accent1.blueOff` 单字段 owner，只改唯一 `a:blueOff/@val`；新增同边界的 `design.theme.accentTransforms.accent1.hueMod` 单字段 owner，只改唯一 `a:hueMod/@val`；新增同一严格拓扑下的 source-bound `design.theme.accentTransforms.accent1.hueOff` 单字段 owner，只改唯一 `a:hueOff/@val`；新增同一严格拓扑下的 source-bound `design.theme.accentTransforms.accent1.gray` 单字段 owner：仅当共享 ThemePart 的 accent1 是直接 RGB 加唯一 `a:gray` 子节点时投影 `true`，并允许以 `false` 删除这个已有子节点，`true` 保持 no-op；缺失 gray、组合编辑、删除 transform 对象和能力篡改 fail closed；新增同一严格拓扑下的 source-bound `design.theme.accentTransforms.accent2.tint` 单字段 owner，只改唯一 `a:accent2Color/a:srgbClr/a:tint/@val`；新增同一严格拓扑下的 source-bound `design.theme.accentTransforms.accent2.shade` 单字段 owner，只改唯一 `a:accent2Color/a:srgbClr/a:shade/@val`；新增同一严格拓扑下的 source-bound `design.theme.accentTransforms.accent2.lumMod` 单字段 owner，只改唯一 `a:accent2Color/a:srgbClr/a:lumMod/@val`；其它 transform 组合、继承和 effect graph 仍为 source-owned；完整 transform/effect/font scheme 未完成 | 未验收 |
...
`size` 为 1–768 点，精度 0.01；`sizePercent` 为 0.25–4 的相对倍数，
精度 0.00001，`1` 表示 100%。倍数 1、跟随正文和缺省分别保留。
普通文本框和形状支持新增、赋值、切换、删除和恢复；切换要求新旧字段
各自的精确权限。删除只清除直接字号声明，标记、字体、颜色及原始数值
写法、run、相邻段落和非目标 XML/ZIP 保持原状。异常、重复或含未知
属性的原生字号保持源内容并拒绝覆盖，非法数字不再导致导入失败。
最小实验 **4/4 通过**，使用去嵌入 PPJ 的实际源文件验证三类标记，
包括导入的嵌入图片；同时复现并修复了高精度请求与原生舍入值之间的
写回校验不一致。相关回归先通过 36 项；另两项旧编号拒绝断言更新为
组合修改成功后的回投影检查，单独复跑 **2/2 通过**。预览诊断、生成资料、
Skill 同步和严格 OpenSpec 检查通过。直接点数字号预览保留字形布局限制，
相对/跟随正文字号明确 unavailable。未重建 NativeAOT；完整继承与宿主排版仍有缺口，
F-03 继续开放。
...
允许的 Unicode 码点，必须含非空白内容；schema 上限从 256 对齐为 255。
普通文本框和形状支持字体新增、赋值、切换、删除和恢复。删除清除直接
字体声明，显式 `true` 则写入“跟随正文”，两者分别回投影；切换时要求
被删除和新增字段各自的精确权限。其它列表样式、标记、run、相邻段落、
未知 XML 与非目标 ZIP 内容保持原状。未知属性、非法值和重复字体标记
继续保留，拒绝覆盖。最小实验 **4/4 通过**，覆盖字符、编号、外链图片
以及导入的嵌入图片标记；补充实验复现并修复了字体编辑重写其它颜色和
字号节点的问题，现保留 RGB 大小写及数值的正号、前导零写法。含最小实验
在内的段落、列表、表格和母版回归 **30/30 通过，0 跳过**；Help、生成资料、
Skill 同步、预览诊断和 OpenSpec 检查通过。未重建 NativeAOT。
直接字符字体预览保留布局限制，“跟随正文”的字体
布局明确 unavailable；完整字体继承和宿主字形排版仍有缺口，F-03 继续开放。
...
属性，保留编号类型、scheme、项目符号样式、其它段落属性、run、相邻段落、
未知 XML 和非目标 ZIP 内容。编辑要求 startAt 的精确字段权限；同时将
能力字段数组上限从 32 调整为 64，以容纳现有 33 项段落字段，仍校验闭合
词表和唯一性。非法原生起始值不再导致投影异常，未知格式和重复标记继续
保留原文、拒绝覆盖。最小实验 **3/3 通过**；包含该实验的列表、母版、
表格图片列表及段落回归 **19/19 通过，0 跳过**。Help、schema、生成资料、
预览诊断、Skill 同步和 OpenSpec 检查通过。未重建 NativeAOT；编号自动
求值、继承列表布局和预览仍有缺口，完整 F-03 继续开放。
...
**直接 rich-text run 反射 fadeAngle 增量（2026-09-11）：** 直接
`text.paragraphs[].runs[].style.reflection.fadeAngle` 已接通
`textReflectionFadeAngleDegrees` native leaf。对直接
`a:rPr/a:effectLst/a:reflection`，只有 full-span 位置（`stPos` 缺省或 `0`、
`endPos` 缺省或 `100000`）且存在单一规范 `fadeDir`、没有
`scale/skew/alignment/rotateWithShape` 时才投影；编辑只替换所属 SlidePart 的
`reflection/@fadeDir`，保留其它效果、run 拓扑和非目标 ZIP，二次投影恢复角度值。
`PpjSourceBoundTextReflectionFadeAngleEditsCanonicalTokenAndReprojects` 覆盖
`2730000` → `5415000`、45.5° → 90.25°、Open XML 校验和只改 slide part；
...
**直接 rich-text run 反射 scaleY 增量（2026-09-11）：** 直接
`text.paragraphs[].runs[].style.reflection.scaleY` 已接通
`textReflectionScaleY` native leaf。只有 full-span 位置和单一规范 `sy`、且没有
fade、scaleX、skew、alignment 或 `rotateWithShape` 时才投影；编辑只替换所属
SlidePart 的 `reflection/@sy`，保留其它效果、run 拓扑和非目标 ZIP，二次投影恢复
有符号比例。`PpjSourceBoundTextReflectionScaleYEditsCanonicalTokenAndReprojects` 覆盖
`125000` → `-75000`、1.25 → -0.75、Open XML 校验和只改 slide part；
其它 transform、可变端点、异常 token 和复杂 effect graph 继续 source-owned。本轮不宣称
PowerPoint 宿主反射显示。

**直接 rich-text run 反射 skewX 增量（2026-09-11）：** 直接
`text.paragraphs[].runs[].style.reflection.skewX` 已接通
`textReflectionSkewX` native leaf。只有 full-span 位置和单一规范 `kx`、且没有
fade、scale、`skewY`、alignment 或 `rotateWithShape` 时才投影；编辑只替换所属
SlidePart 的 `reflection/@kx`，保留其它效果、run 拓扑和非目标 ZIP，二次投影恢复
有符号角度。`PpjSourceBoundTextReflectionSkewXEditsCanonicalTokenAndReprojects` 覆盖
`-750000` → `1200000`、-12.5° → 20°、Open XML 校验和只改 slide part；其它
transform、可变端点、异常 token 和复杂 effect graph 继续 source-owned。本轮不宣称
PowerPoint 宿主反射显示。

**直接 rich-text run 反射 skewY 增量（2026-09-11）：** 直接
`text.paragraphs[].runs[].style.reflection.skewY` 已接通
`textReflectionSkewY` native leaf。只有 full-span 位置和单一规范 `ky`、且没有
fade、scale、`skewX`、alignment 或 `rotateWithShape` 时才投影；编辑只替换所属
SlidePart 的 `reflection/@ky`，保留其它效果、run 拓扑和非目标 ZIP，二次投影恢复
有符号角度。`PpjSourceBoundTextReflectionSkewYEditsCanonicalTokenAndReprojects` 覆盖
`-750000` → `1200000`、-12.5° → 20°、Open XML 校验和只改 slide part；其它
transform、可变端点、异常 token 和复杂 effect graph 继续 source-owned。本轮不宣称
PowerPoint 宿主反射显示。

**直接 rich-text run 反射 alignment 增量（2026-09-11）：** 直接
`text.paragraphs[].runs[].style.reflection.alignment` 已接通
`textReflectionAlignment` native leaf。只有 full-span 位置和单一规范 `algn`、且没有
fade、scale、skew 或 `rotateWithShape` 时才投影；编辑只替换所属 SlidePart 的
`reflection/@algn`，保留其它效果、run 拓扑和非目标 ZIP，二次投影恢复九值对齐 token。
`PpjSourceBoundTextReflectionAlignmentEditsCanonicalTokenAndReprojects` 覆盖
`b` → `tr`、Open XML 校验和只改 slide part；其它 transform、可变端点、异常 token
和复杂 effect graph 继续 source-owned。本轮不宣称 PowerPoint 宿主反射显示。

**直接 rich-text run 反射 rotateWithShape 增量（2026-09-12）：** 直接
`text.paragraphs[].runs[].style.reflection.rotateWithShape` 已接通
`textReflectionRotateWithShape` native leaf。只有 full-span 位置和单一规范
`rotWithShape`（`0`/`1`）且没有 fade、scale、skew 或 alignment 时才投影；编辑只替换
所属 SlidePart 的 `reflection/@rotWithShape`，保留其它效果、run 拓扑和非目标 ZIP，
二次投影恢复显式布尔值（包括 `false`）。`PpjSourceBoundTextReflectionRotateWithShapeEditsCanonicalTokenAndReprojects`
覆盖 `1` → `0`、Open XML 校验和只改 slide part；其它 transform、可变端点、异常 token
和复杂 effect graph 继续 source-owned。本轮不宣称 PowerPoint 宿主反射显示。

**直接 rich-text run 外阴影 blur 增量（2026-09-13）：** 直接
`text.paragraphs[].runs[].style.shadow.blur` 已接通
`textShadowBlurRadiusEmu` native leaf。只接受一个直接
`a:rPr/a:effectLst/a:outerShdw`、已有规范非负 `blurRad`、一个 RGB/theme
颜色子节点，并排除 transform 属性、兄弟 effect、未知后代和异常拓扑；
编辑只替换所属 SlidePart 的 `outerShdw/@blurRad`，保留距离、方向、颜色、
透明度、run 拓扑和非目标 ZIP，二次投影恢复新 blur。最小回归
`PpjSourceBoundTextShadowBlurEditsDirectRunOwnerAndReprojects` 覆盖
38100 → 50800 EMU（3pt → 4pt）、Open XML、SlidePart-only changed-part、
非目标 ZIP 保留、二次投影和兄弟 effect fail-closed；本轮不宣称
PowerPoint 宿主阴影显示。直接 outer shadow 的 distance/angle/color/opacity
及其它 transform 叶子仍按后续 owner 逐项补齐。

**直接 rich-text run 外阴影 distance 增量（2026-09-13）：** 直接
`text.paragraphs[].runs[].style.shadow.distance` 已接通
`textShadowDistanceEmu` native leaf。只有直接 `a:rPr/a:effectLst/a:outerShdw`
已有规范非负 `dist`（0–1,270,000,000 EMU）、一个 RGB/theme 颜色子节点且无
transform、兄弟 effect 或未知后代时才投影；编辑只替换所属 SlidePart 的
`outerShdw/@dist`，保留 blur、方向、颜色、透明度、run 拓扑和非目标 ZIP，二次
投影恢复新 distance。聚焦回归覆盖合法改值、Open XML、SlidePart-only、非目标
...
**直接 rich-text run 外阴影 theme color 增量（2026-09-13）：** 直接
`text.paragraphs[].runs[].style.shadow.color` 的主题色形式已接通
`textShadowColorScheme` native leaf。只有同一严格 direct
`a:rPr/a:effectLst/a:outerShdw` 拥有一个裸 `schemeClr/@val`、有界
blur/distance/direction、无颜色或阴影变换、兄弟 effect 和未知后代时才投影；
编辑只替换所属 SlidePart 的 `outerShdw/schemeClr/@val`，保留几何、alpha、
run 拓扑和非目标 ZIP，二次投影恢复新的主题 token。RGB、tint/shade、非法或
重复颜色、缺少几何和复杂 effect graph 继续 source-owned 或 fail-closed；最小回归
`PpjSourceBoundTextShadowColorSchemeEditsDirectRunOwnerAndReprojects` 覆盖
`accent1` → `accent2`、no-op/stale/非法编辑、duplicate/transform/RGB/
missing-geometry/unknown-descendant fail-closed、Open XML 和 SlidePart-only。
本轮不宣称 PowerPoint 宿主阴影显示。

**直接 rich-text run 外阴影 opacity 增量（2026-09-13）：** 直接
`text.paragraphs[].runs[].style.shadow.opacity` 已接通
`textShadowOpacityThousandthPercent` native leaf。只有同一严格 direct
`a:rPr/a:effectLst/a:outerShdw` 拥有一个已有的规范 `a:alpha/@val`（0–100000），
一个 RGB 或 theme 颜色子节点，并且已有规范 blur/distance/direction、无
transform、兄弟 effect 或未知后代时才投影；编辑只替换所属 SlidePart 的
`outerShdw/(srgbClr|schemeClr)/alpha/@val`，保留颜色、几何、run 拓扑和非目标
ZIP，二次投影恢复新的 opacity。最小回归
`PpjSourceBoundTextShadowOpacityEditsDirectRunOwnerAndReprojects` 覆盖
`72000` → `66000`、RGB 与 theme alpha、Open XML、SlidePart-only、非目标 ZIP
保留、stale/缺失/非法/变换/兄弟 effect 拒绝和二次投影；本轮不宣称
PowerPoint 宿主阴影显示。

**直接 rich-text run 外阴影 alignment 增量（2026-09-13）：** 直接
`text.paragraphs[].runs[].style.shadow.alignment` 已接通
`textShadowAlignment` native leaf。只有一个直接
`a:rPr/a:effectLst/a:outerShdw`、已有规范 `algn`（`tl`、`t`、`tr`、`l`、
`ctr`、`r`、`bl`、`b`、`br`）、至少一个有界 blur/distance/direction、一个
RGB/theme 颜色子节点，且没有 transform、兄弟 effect 或未知后代时才投影；编辑
只替换所属 SlidePart 的 `outerShdw/@algn`，保留颜色、几何、alpha、run 拓扑和
非目标 ZIP，二次投影恢复新 token。`PpjSourceBoundTextShadowAlignmentEditsDirectRunOwnerAndReprojects`
覆盖 `ctr` → `br`、RGB/theme projection、Open XML、SlidePart-only、非目标 ZIP
保留、stale/非法/缺失/变换/兄弟/未知图 fail-closed 和二次投影；本轮不宣称
PowerPoint 宿主阴影显示。

**直接 rich-text run 外阴影 scaleX 增量（2026-09-13）：** 直接
`text.paragraphs[].runs[].style.shadow.scaleX` 已接通
`textShadowScaleX` native leaf。只有一个直接
`a:rPr/a:effectLst/a:outerShdw`、已有规范有符号 `sx`、至少一个有界
blur/distance/direction、一个 RGB/theme 颜色子节点，且没有 `sy`/`kx`/`ky`/
`rotWithShape`、兄弟 effect 或未知后代时才投影；编辑只替换所属 SlidePart 的
`outerShdw/@sx`，按 0.00001 精度保留负值和正值，保留颜色、几何、alpha、run
拓扑和非目标 ZIP，二次投影恢复新的比例。最小回归
`PpjSourceBoundTextShadowScaleXEditsDirectRunOwnerAndReprojects` 覆盖
`-50000` → `125000`（`-0.5` → `1.25`）、stale/非法/缺失/变换/兄弟/未知图
fail-closed 和 Open XML；本轮不宣称 PowerPoint 宿主阴影显示。

**直接 rich-text run 外阴影 scaleY 增量（2026-09-13）：** 直接
`text.paragraphs[].runs[].style.shadow.scaleY` 已接通
`textShadowScaleY` native leaf。只有一个直接
`a:rPr/a:effectLst/a:outerShdw`、已有规范有符号 `sy`、至少一个有界
blur/distance/direction、一个 RGB/theme 颜色子节点，且没有 `sx`/`kx`/`ky`/
`rotWithShape`、兄弟 effect 或未知后代时才投影；编辑只替换所属 SlidePart 的
`outerShdw/@sy`，按 0.00001 精度保留负值和正值，保留颜色、几何、alpha、run
拓扑和非目标 ZIP，二次投影恢复新的比例。最小回归
`PpjSourceBoundTextShadowScaleYEditsDirectRunOwnerAndReprojects` 覆盖
`-50000` → `125000`（`-0.5` → `1.25`）、stale/非法/缺失/变换/兄弟/未知图
fail-closed 和 Open XML；本轮不宣称 PowerPoint 宿主阴影显示。

**直接 rich-text run 外阴影 skewX 增量（2026-09-13）：** 直接
`text.paragraphs[].runs[].style.shadow.skewX` 已接通
`textShadowSkewX` native leaf。只有一个直接
`a:rPr/a:effectLst/a:outerShdw`、已有规范 `kx`（严格位于 -5400000 与
5400000 之间，1/60000 度）、至少一个有界 blur/distance/direction、一个
RGB/theme 颜色子节点，且没有 `sx`/`sy`/`ky`/`rotWithShape`、兄弟 effect 或未知
后代时才投影；编辑只替换所属 SlidePart 的 `outerShdw/@kx`，保留颜色、几何、
alpha、run 拓扑和非目标 ZIP，二次投影恢复新的偏斜角。最小回归
`PpjSourceBoundTextShadowSkewXEditsDirectRunOwnerAndReprojects` 覆盖
`-75000` → `75000`（`-1.25°` → `1.25°`）、stale/非法/缺失/变换/兄弟/未知图
fail-closed 和 Open XML；本轮不宣称 PowerPoint 宿主阴影显示。

**直接 rich-text run 外阴影 skewY 增量（2026-09-13）：** 直接
`text.paragraphs[].runs[].style.shadow.skewY` 已接通
`textShadowSkewY` native leaf。只有一个直接
`a:rPr/a:effectLst/a:outerShdw`、已有规范 `ky`（严格位于 -5400000 与
5400000 之间，1/60000 度）、至少一个有界 blur/distance/direction、一个
RGB/theme 颜色子节点，且没有 `sx`/`sy`/`kx`/`rotWithShape`、兄弟 effect 或未知
后代时才投影；编辑只替换所属 SlidePart 的 `outerShdw/@ky`，保留颜色、几何、
alpha、run 拓扑和非目标 ZIP，二次投影恢复新的偏斜角。最小回归
`PpjSourceBoundTextShadowSkewYEditsDirectRunOwnerAndReprojects` 覆盖
`-75000` → `75000`（`-1.25°` → `1.25°`）、stale/非法/缺失/变换/兄弟/未知图
fail-closed 和 Open XML；本轮不宣称 PowerPoint 宿主阴影显示。

**直接 rich-text run 外阴影 rotateWithShape 增量（2026-09-13）：** 直接
`text.paragraphs[].runs[].style.shadow.rotateWithShape` 已接通
`textShadowRotateWithShape` native leaf。只有一个直接
`a:rPr/a:effectLst/a:outerShdw`、已有规范 `rotWithShape` `0/1`、至少一个
有界 blur/distance/direction、一个 RGB/theme 颜色子节点，且没有
`sx`/`sy`/`kx`/`ky`、兄弟 effect 或未知后代时才投影；显式 `false` 也保留为
布尔叶子。编辑只替换所属 SlidePart 的 `outerShdw/@rotWithShape`，保留颜色、
几何、alpha、run 拓扑和非目标 ZIP，二次投影恢复新的布尔值。最小回归
`PpjSourceBoundTextShadowRotateWithShapeEditsDirectRunOwnerAndReprojects` 与显式
false 用例覆盖 `1` → `0`、stale/非法/缺失/变换/兄弟/未知图 fail-closed、Open XML
和 SlidePart-only；本轮不宣称 PowerPoint 宿主阴影显示。
...
**直接文本字段 ID 增量（2026-09-13）：** 普通文本/形状中已有有效花括号 UUID、静态且非 automatic 的 direct `a:fld` 现在额外投影 `text.paragraphs[].runs[].field.id` 的 `textFieldId` native leaf，并复用 `setTextField` capability。source-bound 只替换所属 SlidePart 的 `a:fld/@id` token，保留 field type、缓存文本、自动字段状态、inline topology、其它 XML 与非目标 ZIP；`PpjTextBodyPropertyLifecycleTests.StaticTextFieldIdLifecyclePatchesOnlyFieldIdToken` 覆盖 authored → 去嵌入 → no-op/单 ID token splice → Open XML → 二次投影，以及 stale、malformed、automatic、ID+缓存文本组合拒绝。表格字段 ID、automatic 字段求值/刷新、字段关系语义和宿主 PowerPoint 接受仍 source-owned。

**表格字段 ID 增量（2026-09-13）：** 固定拓扑矩形 table cell 中已有有效花括号 UUID、静态且非 automatic 的 direct `a:fld` 现在额外投影 `table.rows[].cells[].text.paragraphs[].runs[].field.id` 的 `tableTextFieldId` native leaf，并复用 `setTextField` capability。source-bound 只替换所属 SlidePart 的该 `a:fld/@id` token，保留 field type、缓存文本、自动字段状态、inline topology、表格网格、合并关系与非目标 ZIP；`PpjTableTextFieldIdLifecycleTests` 覆盖 authored → 去嵌入 → 单 ID token splice → Open XML → 二次投影，以及 stale、malformed、automatic、ID+缓存文本组合拒绝。表格字段关系、自动字段求值/刷新和宿主 PowerPoint 接受仍 source-owned。
...
**文本 reflection 增量：** `textStyle.reflection` 已复用同一 rich-text run/default-run owner 路径，覆盖命名 text style、layout/master 默认 text style 的 authored 解析；它接受 `blur`、`startOpacity`/`endOpacity`、`distance` 和 `angle`，NativeAOT 写入直接 `a:effectLst/a:reflection`，使用确定性的 `stPos=0`/`endPos=100000`，并保持 glow → inner shadow → outer shadow → reflection 顺序。`PpjAuthoredTextReflectionEffectWritesRunAndDefaultRunOwners` 已验证 run 与 `a:defRPr` 两个 owner 的几何、起止 alpha、全跨度位置、嵌入 PPJ 恢复和非法 opacity 拒绝。对去嵌入 PPJ 后的 imported/source-bound 直接 rich-text run，只有一个 full-span 直接 `a:reflection` 或其前接一个有界 `a:outerShdw` 的 effect list 会投影为 `textStyle.reflection`，并颁发存在的 blur/startOpacity/endOpacity/distance/direction 五类 native leaf；`PpjSourceBoundTextReflectionEditsDirectRunOwnerAndReprojects` 已证明五个值只改所属 SlidePart、保留 full-span owner proof 并二次投影恢复。段落 `defaultText` 现在也颁发 `textDefaultReflectionBlurRadiusEmu`、`textDefaultReflectionDistanceEmu`、`textDefaultReflectionStartOpacityThousandthPercent`、`textDefaultReflectionEndOpacityThousandthPercent`、`textDefaultReflectionDirectionDegrees`、`textDefaultReflectionFadeAngleDegrees`、`textDefaultReflectionScaleX`、`textDefaultReflectionScaleY`、`textDefaultReflectionSkewX`、`textDefaultReflectionSkewY`、`textDefaultReflectionAlignment` 和 `textDefaultReflectionRotateWithShape`（仅 direct full-span blur/distance/start/end opacity/direction/fadeDir/sx/sy/kx/ky/algn/rotWithShape，允许的 reflection transform/flag 仅为 sx、sy、fadeDir、kx、ky 与 rotWithShape，并要求各自 token 已存在），并分别颁发 glow radius、shadow blur、inner-shadow blur 与 soft-edge radius leaf；非 full-span、transform/unknown/extension effect graph 继续 source-owned 或 fail closed。

**文本 defaultText reflection scaleX 增量（2026-09-11）：** 在既有段落默认 reflection strict owner 上新增 `textDefaultReflectionScaleX`，对应 `paragraph.style.defaultText.reflection.scaleX`，按原生 `a:reflection/@sx` 的 signed Int32 和 1/100000 ratio 精度表达。去嵌入后的 native leaf 只在 direct full-span `a:reflection` 已有合法 `sx` 时出现，且允许与既有 `fadeDir` 共存；编辑只替换该属性，保留 fadeDir、其它 reflection 属性、effect 顺序、段落/run 拓扑和非目标 ZIP 成员。focused fixture 已覆盖 `-0.5 → 1.25`、仅所属 SlidePart 改变、Open XML 校验、二次投影及 `algn`/unknown/复杂 effect fail closed；记录见 `openspec/changes/ppj-source-bound-text-default-reflection-scale-x/`。宿主 PowerPoint reflection 显示和 skew/WordArt 仍不在本轮范围。

**文本 defaultText reflection scaleY 增量（2026-09-11）：** 在既有段落默认 reflection strict owner 上新增 `textDefaultReflectionScaleY`，对应 `paragraph.style.defaultText.reflection.scaleY`，按原生 `a:reflection/@sy` 的 signed Int32 和 1/100000 ratio 精度表达。去嵌入后的 native leaf 只在 direct full-span `a:reflection` 已有合法 `sy` 时出现，且允许与既有 `sx`、`fadeDir` 共存；编辑只替换该属性，保留 `sx`、fadeDir、其它 reflection 属性、effect 顺序、段落/run 拓扑和非目标 ZIP 成员。focused fixture 已覆盖 `1.75 → -1.5`、仅所属 SlidePart 改变、Open XML 校验、二次投影及 `algn`/unknown/复杂 effect fail closed；记录见 `openspec/changes/ppj-source-bound-text-default-reflection-scale-y/`。宿主 PowerPoint reflection 显示和 skew/WordArt 仍不在本轮范围。

**文本 defaultText reflection skewX 增量（2026-09-11）：** 在既有段落默认 reflection strict owner 上新增 `textDefaultReflectionSkewX`，对应 `paragraph.style.defaultText.reflection.skewX`，按原生 `a:reflection/@kx` 的 canonical signed Int32 和 1/60000 度精度表达，舍入后严格位于 -90～90 度。去嵌入后的 native leaf 只在 direct full-span `a:reflection` 已有合法 `kx` 时出现，且允许与既有 `sx`、`sy`、`fadeDir` 共存；编辑只替换该属性，保留其它 reflection 属性、effect 顺序、段落/run 拓扑和非目标 ZIP 成员。focused fixture 已覆盖 `-12.5 → 20`、仅所属 SlidePart 改变、Open XML 校验、二次投影及 `algn`/unknown/复杂 effect fail closed；记录见 `openspec/changes/ppj-source-bound-text-default-reflection-skew-x/`。宿主 PowerPoint reflection 显示和 WordArt 仍不在本轮范围。

**文本 defaultText reflection skewY 增量（2026-09-11）：** 在既有段落默认 reflection strict owner 上新增 `textDefaultReflectionSkewY`，对应 `paragraph.style.defaultText.reflection.skewY`，按原生 `a:reflection/@ky` 的 canonical signed Int32 和 1/60000 度精度表达，舍入后严格位于 -90～90 度。去嵌入后的 native leaf 只在 direct full-span `a:reflection` 已有合法 `ky` 时出现，且允许与既有 `sx`、`sy`、`fadeDir`、`kx` 共存；编辑只替换该属性，保留其它 reflection 属性、effect 顺序、段落/run 拓扑和非目标 ZIP 成员。focused fixture 已覆盖 `8.25 → -20`、仅所属 SlidePart 改变、Open XML 校验、二次投影及 `algn`/unknown/复杂 effect fail closed；记录见 `openspec/changes/ppj-source-bound-text-default-reflection-skew-y/`。宿主 PowerPoint reflection 显示和 WordArt 仍不在本轮范围。

**文本 defaultText reflection alignment 增量（2026-09-11）：** 在既有段落默认 reflection strict owner 上新增 `textDefaultReflectionAlignment`，对应 `paragraph.style.defaultText.reflection.alignment`，按原生 `a:reflection/@algn` 的有限枚举 token 表达：`tl`、`t`、`tr`、`l`、`ctr`、`r`、`bl`、`b`、`br`。去嵌入后的 native leaf 只在 direct full-span `a:reflection` 已有合法 `algn` 时出现；编辑只替换该属性，保留 reflection 的其它属性、effect 顺序、段落/run 拓扑和非目标 ZIP 成员。focused fixture 已覆盖 `b → tr`、仅所属 SlidePart 改变、Open XML 校验、二次投影及 `rotWithShape`/unknown/复杂 effect fail closed；记录见 `openspec/changes/ppj-source-bound-text-default-reflection-alignment/`。宿主 PowerPoint reflection 显示和 WordArt 仍不在本轮范围。

**文本 defaultText reflection rotateWithShape 增量（2026-09-11）：** 在既有段落默认 reflection strict owner 上新增 `textDefaultReflectionRotateWithShape`，对应 `paragraph.style.defaultText.reflection.rotateWithShape`，按原生 `a:reflection/@rotWithShape` 的显式 `0`/`1` token 表达。去嵌入后的 native leaf 只在 direct full-span `a:reflection` 已有合法 rotate-with-shape token 时出现；编辑只替换该属性，保留 reflection 的其它属性、effect 顺序、段落/run 拓扑和非目标 ZIP 成员。focused fixture 已覆盖 `1 → 0`、仅所属 SlidePart 改变、Open XML 校验、二次投影及 non-full-span/unknown/复杂 effect fail closed；记录见 `openspec/changes/ppj-source-bound-text-default-reflection-rotate-with-shape/`。宿主 PowerPoint reflection 显示和 WordArt 仍不在本轮范围。

**文本 defaultText reflection startPosition 增量（2026-09-11）：** 在既有段落默认 reflection strict owner 上新增 `textDefaultReflectionStartPosition`，对应 `paragraph.style.defaultText.reflection.startPosition`，按原生 `a:reflection/@stPos` 的 0～100000 token 映射为 PPJ 0～1。去嵌入后的 native leaf 只在 direct reflection 的 `stPos` 已存在且合法、`endPos` 缺省或为 `100000` 时出现；source-bound 编辑只替换 `stPos`，保留 `endPos`、其它 reflection 属性、effect 顺序、段落/run 拓扑和非目标 ZIP 成员。`PpjSourceBoundTextDefaultReflectionStartPositionEditsLeafAndReprojects` 已覆盖 `0.2 → 0.35`、仅所属 SlidePart 改变、Open XML 校验、二次投影；partial-span `endPos=80000` 保持 opaque，记录见 `openspec/changes/ppj-source-bound-text-default-reflection-start-position/`。宿主 PowerPoint reflection 显示和其它未覆盖 effect leaf 仍不在本轮范围。

**文本 defaultText reflection endPosition 增量（2026-09-11）：** 在既有段落默认 reflection strict owner 上新增 `textDefaultReflectionEndPosition`，对应 `paragraph.style.defaultText.reflection.endPosition`，按原生 `a:reflection/@endPos` 的 0～100000 token 映射为 PPJ 0～1。去嵌入后的 native leaf 只在 direct reflection 的 `endPos` 已存在且合法、`startPos` 缺省或为 `0` 时出现；source-bound 编辑只替换 `endPos`，保留 `startPos`、其它 reflection 属性、effect 顺序、段落/run 拓扑和非目标 ZIP 成员。`PpjSourceBoundTextDefaultReflectionEndPositionEditsLeafAndReprojects` 已覆盖 `0.8 → 0.65`、仅所属 SlidePart 改变、Open XML 校验、二次投影；双变量 `stPos=20000/endPos=80000` 保持 opaque，记录见 `openspec/changes/ppj-source-bound-text-default-reflection-end-position/`。宿主 PowerPoint reflection 显示和两端同时变化仍不在本轮范围。

**文本 soft edge 增量：** `textStyle.softEdge` 已复用同一 rich-text run/default-run owner 路径，覆盖命名 text style、layout/master 默认 text style 的 authored 解析；它接受 `0..1000` point radius，NativeAOT 写入直接 `a:effectLst/a:softEdge`，并保持 glow → inner shadow → outer shadow → reflection → soft edge 顺序。`PpjAuthoredTextSoftEdgeEffectWritesRunAndDefaultRunOwners` 已验证 run 与 `a:defRPr` 两个 owner 的半径、四项 effect 顺序、嵌入 PPJ 恢复和非法半径拒绝。对去嵌入 PPJ 后的 imported/source-bound 直接 rich-text run，只有一个直接 `a:softEdge`，或其前接一个已验证的 `a:outerShdw` / full-span `a:reflection`，会投影为 `textStyle.softEdge`，并颁发 `textSoftEdgeRadiusEmu`；段落 `defaultText` 的同一 strict owner 现在颁发 `textDefaultSoftEdgeRadiusEmu`，按段落 index 定位 `a:defRPr` 并只 splice `softEdge/@rad`；扩展后的 focused fixture 还证明 `textDefaultGlowRadiusEmu`、`textDefaultGlowColorRgb`、`textDefaultGlowColorScheme`、`textDefaultGlowOpacityThousandthPercent`、`textDefaultShadowBlurRadiusEmu`、`textDefaultShadowDistanceEmu`、`textDefaultShadowDirectionDegrees`、`textDefaultShadowAlignment`、`textDefaultShadowColorRgb`、`textDefaultShadowColorScheme`、`textDefaultShadowOpacityThousandthPercent`、`textDefaultShadowRotateWithShape`、`textDefaultInnerShadowBlurRadiusEmu`、`textDefaultInnerShadowDistanceEmu`、`textDefaultInnerShadowDirectionDegrees`、`textDefaultInnerShadowColorRgb`/`textDefaultInnerShadowColorScheme`、`textDefaultInnerShadowOpacityThousandthPercent` 与 `textDefaultReflectionBlurRadiusEmu`、`textDefaultReflectionDistanceEmu`、`textDefaultReflectionStartOpacityThousandthPercent`、`textDefaultReflectionEndOpacityThousandthPercent`、`textDefaultReflectionDirectionDegrees` 与 `textDefaultReflectionRotateWithShape` 的独立回写，二十五类 owner 只改所属 SlidePart 并二次投影恢复。多 effect chain、reflection→softEdge 之外的其它复杂拓扑、非 full-span/transform/unknown/extension graph 继续 source-owned 或 fail closed。
...
**文本 defaultText shadow scaleX 增量（2026-09-10）：** 在既有段落默认外阴影
strict owner 上新增 `textDefaultShadowScaleX`，对应
`paragraph.style.defaultText.shadow.scaleX` 的有符号比例，按原生
`outerShdw/@sx` 的 signed Int32 和 1/100000 精度表达。去嵌入后的 native leaf
只在直接 `a:p/a:pPr/a:defRPr/a:effectLst/a:outerShdw` 已有合法 `sx` 时出现；编辑
只替换该属性，保留颜色、透明度、其它阴影变换、段落/run 拓扑和非目标 ZIP
成员。聚焦实验已覆盖 `-0.5 → 1.25`、仅所属 SlidePart 改变、Open XML 校验、二次投影及 malformed/overflow/复杂 effect fail closed；记录见
`openspec/changes/ppj-source-bound-text-default-shadow-scale-x/`。宿主 PowerPoint
阴影显示和 scaleY/skew/WordArt 仍不在本轮范围。

**文本 defaultText shadow scaleY 增量（2026-09-10）：** 在同一段落默认外阴影
strict owner 上新增 `textDefaultShadowScaleY`，对应
`paragraph.style.defaultText.shadow.scaleY` 的有符号比例，按原生
`outerShdw/@sy` 的 signed Int32 和 1/100000 精度表达。去嵌入后的 native leaf
只在直接 `a:p/a:pPr/a:defRPr/a:effectLst/a:outerShdw` 已有合法 `sy` 时出现；编辑
只替换该属性，保留 `sx`、颜色、透明度、其它阴影变换、段落/run 拓扑和非目标 ZIP
成员。聚焦实验已覆盖 `1.75 → -1.5`、仅所属 SlidePart 改变、Open XML 校验、
二次投影及 malformed/overflow/复杂 effect fail closed；记录见
`openspec/changes/ppj-source-bound-text-default-shadow-scale-y/`。宿主 PowerPoint
阴影显示和 WordArt 留在后续范围；skewX 由下面的增量接续，skewY 由下一段接续。

**文本 defaultText shadow skewX 增量（2026-09-10）：** 在同一段落默认外阴影
strict owner 上新增 `textDefaultShadowSkewX`，对应
`paragraph.style.defaultText.shadow.skewX` 的水平倾斜角，按原生
`outerShdw/@kx` 的 signed Int32 和 1/60000 度精度表达，舍入后严格位于
-90～90 度。去嵌入后的 native leaf 只在直接
`a:p/a:pPr/a:defRPr/a:effectLst/a:outerShdw` 已有合法 `kx` 时出现；编辑只替换
该属性，保留 `sx`/`sy`、颜色、透明度、其它阴影变换、段落/run 拓扑和非目标 ZIP
成员。聚焦实验已覆盖 `-12.5 → 20`、仅所属 SlidePart 改变、Open XML 校验、二次
投影及 malformed/overflow/bound/复杂 effect fail closed；记录见
`openspec/changes/ppj-source-bound-text-default-shadow-skew-x/`。宿主 PowerPoint
阴影显示和 WordArt 仍不在本轮范围。

**文本 defaultText shadow skewY 增量（2026-09-10）：** 在同一段落默认外阴影
strict owner 上新增 `textDefaultShadowSkewY`，对应
`paragraph.style.defaultText.shadow.skewY` 的垂直倾斜角，按原生
`outerShdw/@ky` 的 signed Int32 和 1/60000 度精度表达，舍入后严格位于
-90～90 度。去嵌入后的 native leaf 只在直接
`a:p/a:pPr/a:defRPr/a:effectLst/a:outerShdw` 已有合法 `ky` 时出现；编辑只替换
该属性，保留 `sx`/`sy`/`kx`、颜色、透明度、其它阴影变换、段落/run 拓扑和非目标 ZIP
成员。聚焦实验已覆盖 `8.25 → -20`、仅所属 SlidePart 改变、Open XML 校验、二次
投影及 malformed/overflow/bound/复杂 effect fail closed；记录见
`openspec/changes/ppj-source-bound-text-default-shadow-skew-y/`。宿主 PowerPoint
阴影显示、WordArt 和其它未建模 effect leaf 仍不在本轮范围。

**直接 rich-text reflection startPosition 增量（2026-09-11）：**
新增 `text.paragraphs[].runs[].style.reflection.startPosition` 对应的
`textReflectionStartPosition` native leaf。对直接 `a:rPr/a:effectLst/a:reflection`，
只有显式规范整数 `stPos` 且 `endPos` 缺省或为 `100000` 时才投影；编辑只替换
所属 SlidePart 的 `reflection/@stPos`，保留其它效果、run 拓扑和非目标 ZIP，
二次投影恢复 0..1 值。`PpjSourceBoundTextReflectionStartPositionEditsCanonicalTokenAndReprojects`
覆盖 `20000` → `35000`、0.2 → 0.35、Open XML 校验和只改 slide part；
...
**文本 direct run outer-shadow blur 增量验收：** 直接 rich-text run 的
`run.style.shadow.blur` 只在一个已有、边界内的直接
`a:outerShdw/@blurRad`、一个 RGB/theme 颜色子节点、无 transform/兄弟 effect
时颁发 `textShadowBlurRadiusEmu`；单字段编辑必须只改变所属 SlidePart 的
`blurRad`，保留距离、方向、颜色、透明度、其它 effect 和 run 拓扑，并在
二次投影恢复新 blur。缺失/非法 blur、transform 属性、兄弟 effect、未知后代、
重复节点和异常图保持 source-owned 或 fail closed；其它 outer-shadow 字段继续
按独立 leaf 逐项补齐。

**文本 direct run outer-shadow distance 增量验收：** 直接 rich-text run 的
`run.style.shadow.distance` 只在一个已有、边界内的直接
`a:outerShdw/@dist`、一个 RGB/theme 颜色子节点、无 transform/兄弟 effect
时颁发 `textShadowDistanceEmu`；单字段编辑必须只改变所属 SlidePart 的
`dist`，保留 blur、方向、颜色、透明度、其它 effect 和 run 拓扑，并在二次
投影恢复新 distance。缺失/非法 distance、transform 属性、兄弟 effect、未知
后代、重复节点和异常图保持 source-owned 或 fail closed；本轮不宣称
PowerPoint 宿主阴影显示。

**文本 reflection 增量验收：** authored run 与 paragraph default-run 均须在二次导入中恢复 `blur`/`startOpacity`/`endOpacity`/`distance`/`angle`；direct rich-text run 的 source-bound profile 还须恢复存在的五个 native leaf，并在单字段编辑后只改变所属 SlidePart，同时保留 `stPos=0`/`endPos=100000`；paragraph `defaultText` 的 strict owner 还须恢复 `textDefaultReflectionBlurRadiusEmu`、`textDefaultReflectionDistanceEmu`、`textDefaultReflectionStartOpacityThousandthPercent`、`textDefaultReflectionEndOpacityThousandthPercent`、`textDefaultReflectionDirectionDegrees`、`textDefaultReflectionFadeAngleDegrees`、`textDefaultReflectionScaleX`、`textDefaultReflectionScaleY`、`textDefaultReflectionSkewX`、`textDefaultReflectionSkewY`、`textDefaultReflectionAlignment`、`textDefaultReflectionRotateWithShape`、`textDefaultReflectionStartPosition` 与 `textDefaultReflectionEndPosition`，只编辑直接 `reflection/@blurRad`/`reflection/@dist`/`reflection/@stPos`/`reflection/@endPos`/`reflection/@stA`/`reflection/@endA`/`reflection/@dir`/`reflection/@fadeDir`/`reflection/@sx`/`reflection/@sy`/`reflection/@kx`/`reflection/@ky`/`reflection/@algn`/`reflection/@rotWithShape` 并保留 paragraph index；一个 direct `a:reflection`（`stPos` 缺省或为 `0` 时允许 `endPos`，`endPos` 缺省或为 `100000` 时允许 `stPos`）或 `a:outerShdw`→`a:reflection` 进入该 profile，允许的 reflection transform/flag 仅为 `stPos`、`endPos`、`sx`、`sy`、`fadeDir`、`kx`、`ky` 与 `rotWithShape`，alignment 仅接受已有合法 `algn` token，rotate-with-shape 仅接受已有 `0`/`1` token；超出几何/opacity、两端同时为变量、其它 transform/unknown/extension 或带其它未建模 effect graph 时保持 source-owned 或 fail closed，不把 reflection 转成普通文本样式。
...
**字段增量：** 固定拓扑 table cell 现在可以投影 direct text/field 混合 body；`run.field` 保留 typed `type`、`text` 和 ID。对拥有 GUID field ID、静态且非 automatic 的字段，`table.rows[].cells[].text.paragraphs[].runs[].field.type` 作为 `tableTextFieldType` native leaf 暴露，source-bound 只改直接 `a:fld/@type`，保留字段 ID、缓存文本、自动字段状态、段落/run 拓扑、表格网格与合并关系；缓存 display text 仍有独立编辑路径。`StaticTableTextFieldTypeLifecyclePatchesOnlyFieldTypeToken` 覆盖 authored → 去嵌入 PPJ → type-token 写回 → 二次投影、SlidePart-only footprint、Open XML 校验，以及 automatic、stale、invalid、unsupported 和身份/缓存/拓扑组合变更的 fail-closed 拒绝；自动字段求值/刷新仍由宿主负责。
同一窄 profile 还为静态字段投影 `tableTextFieldId`，source-bound 只改直接 `a:fld/@id`，保留 type、缓存文本、自动字段状态、inline/table topology 和表格网格；`PpjTableTextFieldIdLifecycleTests` 覆盖单 ID token 写回、SlidePart-only footprint、Open XML 校验、二次投影，以及 stale、invalid、automatic 和 ID+缓存文本组合拒绝。
...
再补五个无数值的离散颜色变换字段 `gray`、`comp`、`inv`、`gamma` 和 `invGamma`；它们只接受显式 `true`，分别写入对应的 `a:gray`、`a:comp`、`a:inv`、`a:gamma` 和 `a:invGamma` 空子节点。省略的操作不补写；authored 路径的 `false` 和空对象在编译入口失败，source-bound 的 `gray:false` 只用于删除已存在的直接 gray 子节点；该增量仍只属于 authored 六角色 accent owner。
...
本轮再补一个 source-bound 主题变换字段 `design.theme.accentTransforms.accent1.shade`；当共享 ThemePart 的 accent1 是直接六位 RGB 加唯一 `a:shade/@val` 时，PPJ 投影该 0..1 分数并只允许写回 shade token，其他变换和主题图继续保持 source-owned。

本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent1.lumMod`；当共享 ThemePart 的 accent1 是直接六位 RGB 加唯一 `a:lumMod/@val` 时，PPJ 投影该 0..1 分数并只允许写回 lumMod token，其他变换和主题图继续保持 source-owned。

本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent1.lumOff`；当共享 ThemePart 的 accent1 是直接六位 RGB 加唯一 `a:lumOff/@val` 时，PPJ 投影该 -1..1 有符号分数并只允许写回 lumOff token，其他变换和主题图继续保持 source-owned。

本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent1.alphaMod`；当共享 ThemePart 的 accent1 是直接六位 RGB 加唯一 `a:alphaMod/@val` 时，PPJ 投影该 0..1 分数并只允许写回 alphaMod token，其他变换和主题图继续保持 source-owned。

本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent1.alphaOff`；当共享 ThemePart 的 accent1 是直接六位 RGB 加唯一 `a:alphaOff/@val` 时，PPJ 投影该 -1..1 有符号分数并只允许写回 alphaOff token，其他变换和主题图继续保持 source-owned。

本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent1.satMod`；当共享 ThemePart 的 accent1 是直接六位 RGB 加唯一 `a:satMod/@val` 时，PPJ 投影该 0..1 分数并只允许写回 satMod token，其他变换和主题图继续保持 source-owned。

本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent1.satOff`；当共享 ThemePart 的 accent1 是直接六位 RGB 加唯一 `a:satOff/@val` 时，PPJ 投影该 -1..1 有符号分数并只允许写回 satOff token，其他变换和主题图继续保持 source-owned。

本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent1.redMod`；当共享 ThemePart 的 accent1 是直接六位 RGB 加唯一 `a:redMod/@val` 时，PPJ 投影该 0..1 分数并只允许写回 redMod token，其他变换和主题图继续保持 source-owned。

本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent1.redOff`；当共享 ThemePart 的 accent1 是直接六位 RGB 加唯一 `a:redOff/@val` 时，PPJ 投影该 -1..1 有符号分数并只允许写回 redOff token，其他变换和主题图继续保持 source-owned。

本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent1.greenMod`；当共享 ThemePart 的 accent1 是直接六位 RGB 加唯一 `a:greenMod/@val` 时，PPJ 投影该 0..1 分数并只允许写回 greenMod token，其他变换和主题图继续保持 source-owned。

本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent1.greenOff`；当共享 ThemePart 的 accent1 是直接六位 RGB 加唯一 `a:greenOff/@val` 时，PPJ 投影该 -1..1 有符号分数并只允许写回 greenOff token，其他变换和主题图继续保持 source-owned。

本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent1.blueMod`；当共享 ThemePart 的 accent1 是直接六位 RGB 加唯一 `a:blueMod/@val` 时，PPJ 投影该 0..1 分数并只允许写回 blueMod token，其他变换和主题图继续保持 source-owned。

本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent1.blueOff`；当共享 ThemePart 的 accent1 是直接六位 RGB 加唯一 `a:blueOff/@val` 时，PPJ 投影该 -1..1 有符号分数并只允许写回 blueOff token，其他变换和主题图继续保持 source-owned。

本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent1.hueMod`；当共享 ThemePart 的 accent1 是直接六位 RGB 加唯一 `a:hueMod/@val` 时，PPJ 投影该 0..1 分数并只允许写回 hueMod token，其他变换和主题图继续保持 source-owned。

本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent1.hueOff`；当共享 ThemePart 的 accent1 是直接六位 RGB 加唯一 `a:hueOff/@val` 时，PPJ 投影该 -360..360 度数并只允许写回 hueOff token，其他变换和主题图继续保持 source-owned。

本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent1.gray` source-bound 字段；当共享 ThemePart 的 accent1 是直接六位 RGB 加唯一 `a:gray` 子节点时，PPJ 投影 `true`，并只允许用 `false` 删除已有 gray 子节点，`true` 为 no-op；缺失 gray、兄弟/组合编辑、删除 transform 对象和 capability 篡改继续 fail closed。

本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent2.tint`；当共享 ThemePart 的 accent2 是直接六位 RGB 加唯一 `a:tint/@val` 时，PPJ 投影该 0..1 分数并只允许写回 tint token，其他变换和主题图继续保持 source-owned。

本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent2.shade`；当共享 ThemePart 的 accent2 是直接六位 RGB 加唯一 `a:shade/@val` 时，PPJ 投影该 0..1 分数并只允许写回 shade token，其他变换和主题图继续保持 source-owned。

本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent2.lumMod`；当共享 ThemePart 的 accent2 是直接六位 RGB 加唯一 `a:lumMod/@val` 时，PPJ 投影该 0..1 分数并只允许写回 lumMod token，其他变换和主题图继续保持 source-owned。

本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent2.lumOff`；当共享 ThemePart 的 accent2 是直接六位 RGB 加唯一 `a:lumOff/@val` 时，PPJ 投影该 -1..1 有符号分数并只允许写回 lumOff token，其他变换和主题图继续保持 source-owned。

本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent2.alphaMod`；当共享 ThemePart 的 accent2 是直接六位 RGB 加唯一 `a:alphaMod/@val` 时，PPJ 投影该 0..1 分数并只允许写回 alphaMod token，其他变换和主题图继续保持 source-owned。

本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent2.alphaOff`；当共享 ThemePart 的 accent2 是直接六位 RGB 加唯一 `a:alphaOff/@val` 时，PPJ 投影该 -1..1 有符号分数并只允许写回 alphaOff token，其他变换和主题图继续保持 source-owned。
...
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent2.redMod`；当共享 ThemePart 的 accent2 是直接六位 RGB 加唯一 `a:redMod/@val` 时，PPJ 投影该 0..1 分数并只允许写回 redMod token，其他变换和主题图继续保持 source-owned。

本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent2.redOff`；当共享 ThemePart 的 accent2 是直接六位 RGB 加唯一 `a:redOff/@val` 时，PPJ 投影该 -1..1 有符号分数并只允许写回 redOff token，其他变换和主题图继续保持 source-owned。

本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent2.greenMod`；当共享 ThemePart 的 accent2 是直接六位 RGB 加唯一 `a:greenMod/@val` 时，PPJ 投影该 0..1 分数并只允许写回 greenMod token，其他变换和主题图继续保持 source-owned。

本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent2.greenOff`；当共享 ThemePart 的 accent2 是直接六位 RGB 加唯一 `a:greenOff/@val` 时，PPJ 投影该 -1..1 有符号分数并只允许写回 greenOff token，其他变换和主题图继续保持 source-owned。

本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent2.blueMod`；当共享 ThemePart 的 accent2 是直接六位 RGB 加唯一 `a:blueMod/@val` 时，PPJ 投影该 0..1 分数并只允许写回 blueMod token，其他变换和主题图继续保持 source-owned。

本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent2.blueOff`；当共享 ThemePart 的 accent2 是直接六位 RGB 加唯一 `a:blueOff/@val` 时，PPJ 投影该 -1..1 有符号分数并只允许写回 blueOff token，其他变换和主题图继续保持 source-owned。

本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent2.hueMod`；当共享 ThemePart 的 accent2 是直接六位 RGB 加唯一 `a:hueMod/@val` 时，PPJ 投影该 0..1 分数并只允许写回 hueMod token，其他变换和主题图继续保持 source-owned。

本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent2.hueOff`；当共享 ThemePart 的 accent2 是直接六位 RGB 加唯一 `a:hueOff/@val` 时，PPJ 投影该 -360..360 度数并只允许写回 hueOff token，其他变换和主题图继续保持 source-owned。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent3.tint`；当共享 ThemePart 的 accent3 是直接六位 RGB 加唯一 `a:tint/@val` 时，PPJ 投影该 0..1 分数并只允许写回 tint token，其他变换和主题图继续保持 source-owned。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent3.shade`；当共享 ThemePart 的 accent3 是直接六位 RGB 加唯一 `a:shade/@val` 时，PPJ 投影该 0..1 分数并只允许写回 shade token，其他变换和主题图继续保持 source-owned。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent3.lumMod`；当共享 ThemePart 的 accent3 是直接六位 RGB 加唯一 `a:lumMod/@val` 时，PPJ 投影该 0..1 分数并只允许写回 lumMod token，其他变换和主题图继续保持 source-owned。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent3.lumOff`；当共享 ThemePart 的 accent3 是直接六位 RGB 加唯一 `a:lumOff/@val` 时，PPJ 投影该 -1..1 分数并只允许写回 lumOff token，其他变换和主题图继续保持 source-owned。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent3.alphaMod`；当共享 ThemePart 的 accent3 是直接六位 RGB 加唯一 `a:alphaMod/@val` 时，PPJ 投影该 0..1 分数并只允许写回 alphaMod token，其他变换和主题图继续保持 source-owned。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent3.alphaOff`；当共享 ThemePart 的 accent3 是直接六位 RGB 加唯一 `a:alphaOff/@val` 时，PPJ 投影该 -1..1 有符号分数并只允许写回 alphaOff token，其他变换和主题图继续保持 source-owned。
...
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent3.greenMod` source-bound 字段：直接 RGB accent3 叶子下唯一 `a:greenMod/@val`，按 0..1 fraction 投影和回写，仅改已有 token；删除、兄弟字段、组合编辑、越界和 capability 篡改继续 fail closed。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent3.greenOff` source-bound 字段：直接 RGB accent3 叶子下唯一 `a:greenOff/@val`，按 -1..1 signed fraction 投影和回写，仅改已有 token；删除、兄弟字段、组合编辑、越界和 capability 篡改继续 fail closed。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent3.blueMod` source-bound 字段：直接 RGB accent3 叶子下唯一 `a:blueMod/@val`，按 0..1 fraction 投影和回写，仅改已有 token；删除、兄弟字段、组合编辑、越界和 capability 篡改继续 fail closed。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent3.blueOff` source-bound 字段：直接 RGB accent3 叶子下唯一 `a:blueOff/@val`，按 -1..1 signed fraction 投影和回写，仅改已有 token；删除、兄弟字段、组合编辑、越界和 capability 篡改继续 fail closed。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent3.hueMod` source-bound 字段：直接 RGB accent3 叶子下唯一 `a:hueMod/@val`，按 0..1 fraction 投影和回写，仅改已有 token；删除、兄弟字段、组合编辑、越界和 capability 篡改继续 fail closed。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent3.hueOff` source-bound 字段：直接 RGB accent3 叶子下唯一 `a:hueOff/@val`，按 -360..360 度数投影和回写，仅改已有 token；删除、兄弟字段、组合编辑、越界和 capability 篡改继续 fail closed。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent4.tint` source-bound 字段：直接 RGB accent4 叶子下唯一 `a:tint/@val`，按 0..1 fraction 投影和回写，仅改已有 token；删除、兄弟字段、组合编辑、越界和 capability 篡改继续 fail closed。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent4.shade` source-bound 字段：直接 RGB accent4 叶子下唯一 `a:shade/@val`，按 0..1 fraction 投影和回写，仅改已有 token；删除、兄弟字段、组合编辑、越界和 capability 篡改继续 fail closed。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent4.lumMod` source-bound 字段：直接 RGB accent4 叶子下唯一 `a:lumMod/@val`，按 0..1 fraction 投影和回写，仅改已有 token；删除、兄弟字段、组合编辑、越界和 capability 篡改继续 fail closed。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent4.lumOff` source-bound 字段：直接 RGB accent4 叶子下唯一 `a:lumOff/@val`，按 -1..1 fraction 投影和回写，仅改已有 token；删除、兄弟字段、组合编辑、越界和 capability 篡改继续 fail closed。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent4.alphaMod` source-bound 字段：直接 RGB accent4 叶子下唯一 `a:alphaMod/@val`，按 0..1 fraction 投影和回写，仅改已有 token；删除、兄弟字段、组合编辑、越界和 capability 篡改继续 fail closed。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent4.alphaOff` source-bound 字段：直接 RGB accent4 叶子下唯一 `a:alphaOff/@val`，按 -1..1 fraction 投影和回写，仅改已有 token；删除、兄弟字段、组合编辑、越界和 capability 篡改继续 fail closed。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent4.satMod` source-bound 字段：直接 RGB accent4 叶子下唯一 `a:satMod/@val`，按 0..1 fraction 投影和回写，仅改已有 token；删除、兄弟字段、组合编辑、越界和 capability 篡改继续 fail closed。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent4.satOff` source-bound 字段：直接 RGB accent4 叶子下唯一 `a:satOff/@val`，按 -1..1 signed fraction 投影和回写，仅改已有 token；删除、兄弟字段、组合编辑、越界和 capability 篡改继续 fail closed。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent4.redMod` source-bound 字段：直接 RGB accent4 叶子下唯一 `a:redMod/@val`，按 0..1 fraction 投影和回写，仅改已有 token；删除、兄弟字段、组合编辑、越界和 capability 篡改继续 fail closed。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent4.greenMod` source-bound 字段：直接 RGB accent4 叶子下唯一 `a:greenMod/@val`，按 0..1 fraction 投影和回写，仅改已有 token；删除、兄弟字段、组合编辑、越界和 capability 篡改继续 fail closed。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent4.greenOff` source-bound 字段：直接 RGB accent4 叶子下唯一 `a:greenOff/@val`，按 -1..1 signed fraction 投影和回写，仅改已有 token；删除、兄弟字段、组合编辑、越界和 capability 篡改继续 fail closed。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent4.blueMod` source-bound 字段：直接 RGB accent4 叶子下唯一 `a:blueMod/@val`，按 0..1 fraction 投影和回写，仅改已有 token；删除、兄弟字段、组合编辑、越界和 capability 篡改继续 fail closed。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent4.blueOff` source-bound 字段：直接 RGB accent4 叶子下唯一 `a:blueOff/@val`，按 -1..1 signed fraction 投影和回写，仅改已有 token；删除、兄弟字段、组合编辑、越界和 capability 篡改继续 fail closed。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent4.hueMod` source-bound 字段：直接 RGB accent4 叶子下唯一 `a:hueMod/@val`，按 0..1 fraction 投影和回写，仅改已有 token；删除、兄弟字段、组合编辑、越界和 capability 篡改继续 fail closed。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent4.hueOff` source-bound 字段：直接 RGB accent4 叶子下唯一 `a:hueOff/@val`，按 -360..360 度数投影和回写，仅改已有 token；删除、兄弟字段、组合编辑、越界和 capability 篡改继续 fail closed。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent4.redOff` source-bound 字段：直接 RGB accent4 叶子下唯一 `a:redOff/@val`，按 -1..1 signed fraction 投影和回写，仅改已有 token；删除、兄弟字段、组合编辑、越界和 capability 篡改继续 fail closed。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent5.tint` source-bound 字段：直接 RGB accent5 叶子下唯一 `a:tint/@val`，按 0..1 fraction 投影和回写，仅改已有 token；删除、兄弟字段、组合编辑、越界和 capability 篡改继续 fail closed。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent5.shade` source-bound 字段：直接 RGB accent5 叶子下唯一 `a:shade/@val`，按 0..1 fraction 投影和回写，仅改已有 token；删除、兄弟字段、组合编辑、越界和 capability 篡改继续 fail closed。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent5.lumMod` source-bound 字段：直接 RGB accent5 叶子下唯一 `a:lumMod/@val`，按 0..1 fraction 投影和回写，仅改已有 token；删除、兄弟字段、组合编辑、越界和 capability 篡改继续 fail closed。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent5.lumOff` source-bound 字段：直接 RGB accent5 叶子下唯一 `a:lumOff/@val`，按 -1..1 signed fraction 投影和回写，仅改已有 token；删除、兄弟字段、组合编辑、越界和 capability 篡改继续 fail closed。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent5.alphaMod` source-bound 字段：直接 RGB accent5 叶子下唯一 `a:alphaMod/@val`，按 0..1 fraction 投影和回写，仅改已有 token；删除、兄弟字段、组合编辑、越界和 capability 篡改继续 fail closed。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent5.alphaOff` source-bound 字段：直接 RGB accent5 叶子下唯一 `a:alphaOff/@val`，按 -1..1 signed fraction 投影和回写，仅改已有 token；删除、兄弟字段、组合编辑、越界和 capability 篡改继续 fail closed。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent5.satMod` source-bound 字段：直接 RGB accent5 叶子下唯一 `a:satMod/@val`，按 0..1 fraction 投影和回写，仅改已有 token；删除、兄弟字段、组合编辑、越界和 capability 篡改继续 fail closed。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent5.satOff` source-bound 字段：直接 RGB accent5 叶子下唯一 `a:satOff/@val`，按 -1..1 signed fraction 投影和回写，仅改已有 token；删除、兄弟字段、组合编辑、越界和 capability 篡改继续 fail closed。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent5.redMod` source-bound 字段：直接 RGB accent5 叶子下唯一 `a:redMod/@val`，按 0..1 fraction 投影和回写，仅改已有 token；删除、兄弟字段、组合编辑、越界和 capability 篡改继续 fail closed。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent5.redOff` source-bound 字段：直接 RGB accent5 叶子下唯一 `a:redOff/@val`，按 -1..1 signed fraction 投影和回写，仅改已有 token；删除、兄弟字段、组合编辑、越界和 capability 篡改继续 fail closed。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent5.greenMod` source-bound 字段：直接 RGB accent5 叶子下唯一 `a:greenMod/@val`，按 0..1 fraction 投影和回写，仅改已有 token；删除、兄弟字段、组合编辑、越界和 capability 篡改继续 fail closed。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent5.greenOff` source-bound 字段：直接 RGB accent5 叶子下唯一 `a:greenOff/@val`，按 -1..1 signed fraction 投影和回写，仅改已有 token；删除、兄弟字段、组合编辑、越界和 capability 篡改继续 fail closed。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent5.blueMod` source-bound 字段：直接 RGB accent5 叶子下唯一 `a:blueMod/@val`，按 0..1 fraction 投影和回写，仅改已有 token；删除、兄弟字段、组合编辑、越界和 capability 篡改继续 fail closed。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent5.blueOff` source-bound 字段：直接 RGB accent5 叶子下唯一 `a:blueOff/@val`，按 -1..1 signed fraction 投影和回写，仅改已有 token；删除、兄弟字段、组合编辑、越界和 capability 篡改继续 fail closed。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent5.hueMod` source-bound 字段：直接 RGB accent5 叶子下唯一 `a:hueMod/@val`，按 0..1 fraction 投影和回写，仅改已有 token；删除、兄弟字段、组合编辑、越界和 capability 篡改继续 fail closed。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent5.hueOff` source-bound 字段：直接 RGB accent5 叶子下唯一 `a:hueOff/@val`，按 -360..360 degree offset 投影和回写，仅改已有 token；删除、兄弟字段、组合编辑、越界和 capability 篡改继续 fail closed。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent6.tint` source-bound 字段：直接 RGB accent6 叶子下唯一 `a:tint/@val`，按 0..1 fraction 投影和回写，仅改已有 token；删除、兄弟字段、组合编辑、越界和 capability 篡改继续 fail closed。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent6.shade` source-bound 字段：直接 RGB accent6 叶子下唯一 `a:shade/@val`，按 0..1 fraction 投影和回写，仅改已有 token；删除、兄弟字段、组合编辑、越界和 capability 篡改继续 fail closed。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent6.lumMod` source-bound 字段：直接 RGB accent6 叶子下唯一 `a:lumMod/@val`，按 0..1 fraction 投影和回写，仅改已有 token；删除、兄弟字段、组合编辑、越界和 capability 篡改继续 fail closed。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent6.lumOff` source-bound 字段：直接 RGB accent6 叶子下唯一 `a:lumOff/@val`，按 -1..1 fraction 投影和回写，仅改已有 token；删除、兄弟字段、组合编辑、越界和 capability 篡改继续 fail closed。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent6.alphaMod` source-bound 字段：直接 RGB accent6 叶子下唯一 `a:alphaMod/@val`，按 0..1 fraction 投影和回写，仅改已有 token；删除、兄弟字段、组合编辑、越界和 capability 篡改继续 fail closed。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent6.alphaOff` source-bound 字段：直接 RGB accent6 叶子下唯一 `a:alphaOff/@val`，按 -1..1 fraction 投影和回写，仅改已有 token；删除、兄弟字段、组合编辑、越界和 capability 篡改继续 fail closed。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent6.satMod` source-bound 字段：直接 RGB accent6 叶子下唯一 `a:satMod/@val`，按 0..1 fraction 投影和回写，仅改已有 token；删除、兄弟字段、组合编辑、越界和 capability 篡改继续 fail closed。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent6.satOff` source-bound 字段：直接 RGB accent6 叶子下唯一 `a:satOff/@val`，按 -1..1 fraction 投影和回写，仅改已有 token；删除、兄弟字段、组合编辑、越界和 capability 篡改继续 fail closed。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent6.redMod` source-bound 字段：直接 RGB accent6 叶子下唯一 `a:redMod/@val`，按 0..1 fraction 投影和回写，仅改已有 token；删除、兄弟字段、组合编辑、越界和 capability 篡改继续 fail closed。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent6.redOff` source-bound 字段：直接 RGB accent6 叶子下唯一 `a:redOff/@val`，按 -1..1 signed fraction 投影和回写，仅改已有 token；删除、兄弟字段、组合编辑、越界和 capability 篡改继续 fail closed。
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent6.greenMod` source-bound 字段：直接 RGB accent6 叶子下唯一 `a:greenMod/@val`，按 0..1 fraction 投影和回写，仅改已有 token；删除、兄弟字段、组合编辑、越界和 capability 篡改继续 fail closed。
...
本轮继续增加同一严格拓扑下的 `design.theme.accentTransforms.accent6.hueOff` source-bound 字段：直接 RGB accent6 叶子下唯一 `a:hueOff/@val`，按 -360..360 degree offset 投影和回写，仅改已有 token；删除、兄弟字段、组合编辑、越界和 capability 篡改继续 fail closed。
...
**本轮后续增量（2026-09-13）：** `design.theme.name` 现在作为一个 source-bound 元数据字段闭合：当所有 master 共享一个已有、非空名称的 ThemePart 时，投影器颁发 `setThemeName` capability；编辑只改对应 ThemePart 的 `a:theme/@name`，并由二次投影恢复。随后同一 owner 的已有 `a:majorFont/a:latin/@typeface` 与 `a:minorFont/a:latin/@typeface` 分别作为 `design.theme.fontScheme.major`、`.minor` 闭合，`setThemeFontScheme` capability 按字段授权；现有 `a:majorFont/a:ea/@typeface` 与 `a:minorFont/a:ea/@typeface` 也分别作为 `design.theme.fontScheme.majorEastAsia`、`.minorEastAsia` 闭合，`a:majorFont/a:cs/@typeface` 与 `a:minorFont/a:cs/@typeface` 作为 `.majorComplexScript`、`.minorComplexScript` 闭合，均颁发独立字段 capability。每次编译只改一个字体槽位。又对同一 owner 的六个 direct RGB accent 节点开放 `design.theme.accentColors.accent1`：投影保留六角色观察值，只颁发 `setThemeAccent1Color`，编辑只改 `a:accent1Color/a:srgbClr/@val`。随后为同一六角色观察对象增加 `design.theme.accentColors.accent2`：颁发 `setThemeAccent2Color`，编辑只改 `a:accent2Color/a:srgbClr/@val`；本轮再增加 `design.theme.accentColors.accent3`：颁发 `setThemeAccent3Color`，编辑只改 `a:accent3Color/a:srgbClr/@val`；随后增加 `design.theme.accentColors.accent4`：颁发 `setThemeAccent4Color`，编辑只改 `a:accent4Color/a:srgbClr/@val`；本轮再增加 `setThemeAccent5Color`：它按字段授权 `design.theme.accentColors.accent5`，导出只改 `a:accent5Color/a:srgbClr/@val`；本轮再增加 `setThemeAccent6Color`：它按字段授权 `design.theme.accentColors.accent6`，导出只改 `a:accent6Color/a:srgbClr/@val`；本轮再增加 `setThemeColorRoleDark1`：它按字段授权 `design.theme.colorRoles.dark1`，导出只改 `a:dk1/a:srgbClr/@val`。本轮再增加 `setThemeColorRoleLight1`：它按字段授权 `design.theme.colorRoles.light1`，导出只改 `a:lt1/a:srgbClr/@val`。本轮再增加 `setThemeColorRoleDark2`：它按字段授权 `design.theme.colorRoles.dark2`，导出只改 `a:dk2/a:srgbClr/@val`。本轮再增加 `setThemeColorRoleLight2`：它按字段授权 `design.theme.colorRoles.light2`，导出只改 `a:lt2/a:srgbClr/@val`；本轮再增加 `setThemeColorRoleHyperlink`：它按字段授权 `design.theme.colorRoles.hyperlink`，导出只改 `a:hlink/a:srgbClr/@val`；本轮再增加 `setThemeColorRoleFollowedHyperlink`：它按字段授权 `design.theme.colorRoles.followedHyperlink`，导出只改 `a:folHlink/a:srgbClr/@val`。缺失或多 ThemePart、缺少所需字体/颜色槽位、变换色、字段删除及其余 theme graph 仍保持 source-owned；focused native 回归覆盖 no-op 字节、changed-part、Open XML、非目标 ZIP 和 capability authority。本轮再补 `design.theme.accentColors.accent1` 的 source-bound RGBA owner：当 accent1 direct RGB 下已有一个可由 `#RRGGBBAA` 精确表示的 `a:alpha` 时，投影保留八位颜色，编辑只改既有 `a:srgbClr/@val` 与 `a:alpha/@val`；其他 accent alpha、非规范 alpha、拓扑变化和主题图继续 source-owned。本轮再补 `design.theme.accentColors.accent2` 的 source-bound RGBA owner：当 accent2 direct RGB 下已有一个可由 `#RRGGBBAA` 精确表示的 `a:alpha` 时，投影保留八位颜色，编辑只改既有 `a:srgbClr/@val` 与 `a:alpha/@val`；其他 accent alpha、非规范 alpha、拓扑变化和主题图继续 source-owned。本轮再补 `design.theme.accentColors.accent3` 的 source-bound RGBA owner：当 accent3 direct RGB 下已有一个可由 `#RRGGBBAA` 精确表示的 `a:alpha` 时，投影保留八位颜色，编辑只改既有 `a:srgbClr/@val` 与 `a:alpha/@val`；其他 accent alpha、非规范 alpha、拓扑变化和主题图继续 source-owned。本轮再补 `design.theme.accentColors.accent4` 的 source-bound RGBA owner：当 accent4 direct RGB 下已有一个可由 `#RRGGBBAA` 精确表示的 `a:alpha` 时，投影保留八位颜色，编辑只改既有 `a:srgbClr/@val` 与 `a:alpha/@val`；其他 accent alpha、非规范 alpha、拓扑变化和主题图继续 source-owned。本轮再补 `design.theme.accentColors.accent5` 的 source-bound RGBA owner：当 accent5 direct RGB 下已有一个可由 `#RRGGBBAA` 精确表示的 `a:alpha` 时，投影保留八位颜色，编辑只改既有 `a:srgbClr/@val` 与 `a:alpha/@val`；其他 accent alpha、非规范 alpha、拓扑变化和主题图继续 source-owned。本轮再补 `design.theme.accentColors.accent6` 的 source-bound RGBA owner：当 accent6 direct RGB 下已有一个可由 `#RRGGBBAA` 精确表示的 `a:alpha` 时，投影保留八位颜色，编辑只改既有 `a:srgbClr/@val` 与 `a:alpha/@val`；其他 accent alpha、非规范 alpha、拓扑变化和主题图继续 source-owned。本轮再补 `design.theme.colorRoles.dark1` 的 source-bound RGBA owner：当 `a:dk1` direct RGB 下已有一个可由 `#RRGGBBAA` 精确表示的 `a:alpha` 时，投影保留八位颜色，编辑只改既有 `a:srgbClr/@val` 与 `a:alpha/@val`，并保持 alpha presence 不变；其他 color role alpha、非规范 alpha、拓扑变化和主题图继续 source-owned。本轮再补 `design.theme.colorRoles.light1` 的 source-bound RGBA owner：当 `a:lt1` direct RGB 下已有一个可由 `#RRGGBBAA` 精确表示的 `a:alpha` 时，投影保留八位颜色，编辑只改既有 `a:srgbClr/@val` 与 `a:alpha/@val`，并保持 alpha presence 不变；其他 color role alpha、非规范 alpha、拓扑变化和主题图继续 source-owned。 本轮再补 `design.theme.colorRoles.dark2` 的 source-bound RGBA owner：当 `a:dk2` direct RGB 下已有一个可由 `#RRGGBBAA` 精确表示的 `a:alpha` 时，投影保留八位颜色，编辑只改既有 `a:srgbClr/@val` 与 `a:alpha/@val`，并保持 alpha presence 不变；其他 color role alpha、非规范 alpha、拓扑变化和主题图继续 source-owned。 本轮再补 `design.theme.colorRoles.light2` 的 source-bound RGBA owner：当 `a:lt2` direct RGB 下已有一个可由 `#RRGGBBAA` 精确表示的 `a:alpha` 时，投影保留八位颜色，编辑只改既有 `a:srgbClr/@val` 与 `a:alpha/@val`，并保持 alpha presence 不变；其他 color role alpha、非规范 alpha、拓扑变化和主题图继续 source-owned。 本轮再补 `design.theme.colorRoles.hyperlink` 的 source-bound RGBA owner：当 `a:hlink` direct RGB 下已有一个可由 `#RRGGBBAA` 精确表示的 `a:alpha` 时，投影保留八位颜色，编辑只改既有 `a:srgbClr/@val` 与 `a:alpha/@val`，并保持 alpha presence 不变；其他 color role alpha、非规范 alpha、拓扑变化和主题图继续 source-owned。 本轮再补 `design.theme.colorRoles.followedHyperlink` 的 source-bound RGBA owner：当 `a:folHlink` direct RGB 下已有一个可由 `#RRGGBBAA` 精确表示的 `a:alpha` 时，投影保留八位颜色，编辑只改既有 `a:srgbClr/@val` 与 `a:alpha/@val`，并保持 alpha presence 不变；其他 color role alpha、非规范 alpha、拓扑变化和主题图继续 source-owned。

**本轮后续增量（2026-09-15）：** F-15 再补齐一个独立元数据字段：已有共享 ThemePart 的 `a:fontScheme/@name` 现在投影为 `design.theme.fontScheme.name`，按字段颁发 `setThemeFontSchemeName`；source-bound 编译只改该属性，删除、空值、与其它字体槽位合并或伪造 capability 均 fail closed。字体回退、effect scheme 和其余 theme graph 仍保持 source-owned。
**仍缺：** 完整 `theme1.xml` color/font/effect scheme、更多颜色变换、继承后的 effect style、更完整的逐脚本字体回退、字体嵌入/替换、复杂语言字体映射的其余组合、WordArt 和宿主字体回退差异；当前 accent1/accent2/accent3/accent4/accent5/accent6 与 colorRoles.dark1（含已有 direct alpha）、colorRoles.light1（含已有 direct alpha）、colorRoles.dark2（含已有 direct alpha）、colorRoles.light2（含已有 direct alpha）、colorRoles.hyperlink（含已有 direct alpha）、colorRoles.followedHyperlink（含已有 direct alpha） source-bound owner、其余 color roles、alpha、tint/shade 与复杂脚本 direct owner 都只覆盖 PPJ 的有限 authored/native lowering，不会改写任意 imported theme XML。
...
| F-03 | fields/WordArt/完整文本 | 部分完成（rich text 子集、静态 typed field display source-bound 编辑、bounded automatic `slidenum` cache refresh 与 `author`/`datetime`/`datetimeFigureOut`/`datetime1`/`datetime2`/`datetime3`/`datetime4`/`datetime5`/`datetime6`/`datetime7`/`datetime8`/`datetime9`/`datetime10`/`datetime11`/`datetime12`/`datetime13`/`uaqdatetime1`/`uaqdatetime2`/`uaqdatetime3`/`uaqdatetime4`/`uaqdatetime5`/`uaqdatetime6`/`uaqdatetime7` host-refreshable cache、复杂脚本 direct `a:cs` owner、formal `text.language` owner、source-free authored text glow/inner shadow/reflection/soft edge、direct rich-text run（含 startPosition/endPosition/fadeAngle/rotateWithShape）及 paragraph `defaultText` glow-radius/color/theme-color/opacity/shadow-blur/distance/direction/alignment/color/theme-color/opacity/rotate-with-shape/scaleX/scaleY/skewX/skewY/inner-shadow-blur-distance-direction-color-theme-color-alpha/reflection-blur-distance-start-opacity-end-opacity-direction/fade-angle/scale-x/scale-y/skew-x/skew-y/alignment/soft-edge radius source-bound leaves 已交付） | 未覆盖的自动字段求值、其它 automatic field、WordArt、defaultText 其它 effect leaf、复杂 effect 与完整语言/脚本字体回退组合 | P0 |
...
| F-15 | theme/effect/font scheme | 部分完成（bounded grammar tint/shade + authored `design.theme.fontScheme.major/minor/majorEastAsia/minorEastAsia/majorComplexScript/minorComplexScript` + `design.theme.accentColors`/`accentTransforms`（含 `tint`/`shade`/`lumMod`/`lumOff`/`alphaMod`/`alphaOff`/`satMod`/`satOff`/`redMod`/`redOff`/`greenMod`/`greenOff`/`blueMod`/`blueOff`/`hueMod`/`hueOff`/`gray`/`comp`/`inv`/`gamma`/`invGamma`）/`colorRoles`，含有限 authored alpha 与 accent1/accent2/accent3/accent4/accent5/accent6 与 colorRoles.dark1/light1/dark2/light2/hyperlink/followedHyperlink source-bound direct alpha，以及复杂脚本 direct `a:cs` 字体 owner；source-bound `design.theme.name`、`design.theme.fontScheme.name`、`design.theme.accentColors.accent1`/`design.theme.accentColors.accent2`/`design.theme.accentColors.accent3`/`design.theme.accentColors.accent4`/`design.theme.accentColors.accent5`/`design.theme.accentColors.accent6`/`design.theme.colorRoles.dark1`/`design.theme.colorRoles.light1`/`design.theme.colorRoles.dark2`/`design.theme.colorRoles.light2`/`design.theme.colorRoles.hyperlink`/`design.theme.colorRoles.followedHyperlink`/`design.theme.accentTransforms.accent1.lumMod`/`design.theme.accentTransforms.accent1.lumOff`/`design.theme.accentTransforms.accent1.alphaMod`/`design.theme.accentTransforms.accent1.alphaOff`/`design.theme.accentTransforms.accent1.satMod`/`design.theme.accentTransforms.accent1.satOff`/`design.theme.accentTransforms.accent1.redMod`/`design.theme.accentTransforms.accent1.redOff`/`design.theme.accentTransforms.accent1.greenMod`/`design.theme.accentTransforms.accent1.greenOff`/`design.theme.accentTransforms.accent1.blueMod`/`design.theme.accentTransforms.accent1.blueOff` 与唯一共享 ThemePart 上的 `design.theme.fontScheme.major/minor/majorEastAsia/minorEastAsia/majorComplexScript/minorComplexScript` 可按字段改写） | transform/inheritance/fallback/完整语言脚本字体回退/其余 theme graph | P1 |
