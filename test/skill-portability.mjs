...
assert.match(repl, /reopen that reviewed revision.*`inspect` again/is);
assert.match(repl, /Native leaf IDs and expected hashes are bound to\s+one revision/is);
assert.match(repl, /rebuilt the node index from reviewed\s+bytes/is);
assert.match(repl, /operations.*audit evidence.*not a replacement for reinspection/is);
