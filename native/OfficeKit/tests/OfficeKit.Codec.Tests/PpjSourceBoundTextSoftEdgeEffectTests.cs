...
public sealed partial class PptxCodecTests
{
    ...
    public void PpjSourceBoundTextSoftEdgeEditsDirectRunOwnerAndReprojects()
    {
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            Assert.Equal(-750_000, defaultShadow.HorizontalSkew!.Value);
            Assert.Equal(495_000, defaultShadow.VerticalSkew!.Value);
        ...
        var defaultShadowScaleXLeaves = projectedShadowElement["nativeRef"]!["leaves"]!.AsArray()
            .Select(leaf => leaf!.AsObject())
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "textDefaultShadowScaleX")
            .ToArray();
        ...
        var defaultShadowSkewXLeaves = projectedShadowElement["nativeRef"]!["leaves"]!.AsArray()
            .Select(leaf => leaf!.AsObject())
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "textDefaultShadowSkewX")
            .ToArray();
        Assert.Single(defaultShadowSkewXLeaves);
        Assert.Equal(-12.5, defaultShadowSkewXLeaves[0]["value"]!.GetValue<double>(), precision: 6);
        var defaultShadowSkewYLeaves = projectedShadowElement["nativeRef"]!["leaves"]!.AsArray()
            .Select(leaf => leaf!.AsObject())
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "textDefaultShadowSkewY")
            .ToArray();
        Assert.Single(defaultShadowSkewYLeaves);
        Assert.Equal(8.25, defaultShadowSkewYLeaves[0]["value"]!.GetValue<double>(), precision: 6);
        ...
        var defaultReflectionSkewXLeaves = projectedElement["nativeRef"]!["leaves"]!.AsArray()
            .Select(leaf => leaf!.AsObject())
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "textDefaultReflectionSkewX")
            .ToArray();
        Assert.Single(defaultReflectionSkewXLeaves);
        Assert.Equal(-12.5, defaultReflectionSkewXLeaves[0]["value"]!.GetValue<double>(), precision: 6);
        var defaultReflectionSkewYLeaves = projectedElement["nativeRef"]!["leaves"]!.AsArray()
            .Select(leaf => leaf!.AsObject())
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "textDefaultReflectionSkewY")
            .ToArray();
        Assert.Single(defaultReflectionSkewYLeaves);
        Assert.Equal(8.25, defaultReflectionSkewYLeaves[0]["value"]!.GetValue<double>(), precision: 6);
        var defaultReflectionAlignmentLeaves = projectedElement["nativeRef"]!["leaves"]!.AsArray()
            .Select(leaf => leaf!.AsObject())
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "textDefaultReflectionAlignment")
            .ToArray();
        ...
        defaultShadowSkewXLeaves[0]["value"] = 20d;
        defaultShadowSkewYLeaves[0]["value"] = -20d;
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            Assert.Equal(1_200_000, defaultShadow.HorizontalSkew!.Value);
            Assert.Equal(-1_200_000, defaultShadow.VerticalSkew!.Value);
        ...
        Assert.Equal(20, reprojectedProgram["pages"]![0]!["elements"]![2]!["text"]!["paragraphs"]![0]!["style"]!["defaultText"]!["shadow"]!["skewX"]!.GetValue<double>(), precision: 6);
        Assert.Equal(-20, reprojectedProgram["pages"]![0]!["elements"]![2]!["text"]!["paragraphs"]![0]!["style"]!["defaultText"]!["shadow"]!["skewY"]!.GetValue<double>(), precision: 6);
