# Design: source-bound paragraph default-text reflection distance
...
`dist`, optionally preceded by one valid direct `a:outerShdw`. The reflection
keeps its blur, start/end opacity, direction and full-span positions
...
The leaf kind is `textDefaultReflectionDistanceEmu` and its PPJ location is
`paragraph.style.defaultText.reflection.distance`. The edit plan re-proves the
selected `a:p`, direct `a:pPr`, `a:defRPr`, strict full-span effect list, and
direct `a:reflection` before token-splicing only `dist` in the owning
SlidePart. No protobuf or wire-version change is needed. The focused fixture
removes the embedded PPJ, changes the paragraph default distance alongside
the existing default-text and inline-run effect leaves, verifies SlidePart-only
