...
{
    [Fact]
    public void PpjSourceBoundPictureShape3dSceneLightRigRotationRevolutionLeafEditsAndReprojects()
    {
        ...
        var source = AddPictureShape3dSceneLightRigRotationRevolution(RemoveEmbeddedPpj(authored.File.ToByteArray()), "6000000");
        var projected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/source/picture-shape-3d-scene-light-rig-rotation-revolution.pptx",
            },
        });
        ...
        var target = Assert.Single(projectedImage["nativeRef"]!["leaves"]!.AsArray()
            ...
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "shape3dSceneLightRigRotationRevolution60000"));
        Assert.Equal(6_000_000, target["value"]!.GetValue<long>());
        target["value"] = 7_200_000;
        ...
        Assert.Contains("lon=\"0\"", editedXml, StringComparison.Ordinal);
        Assert.Contains("rev=\"7200000\"", editedXml, StringComparison.Ordinal);
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            Assert.Equal("0", rotation.GetAttribute("lon", string.Empty).Value);
            Assert.Equal("7200000", rotation.GetAttribute("rev", string.Empty).Value);
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/picture-shape-3d-scene-light-rig-rotation-revolution.pptx",
            },
        });
        ...
        var reprojectedTarget = Assert.Single(reprojectedImage["nativeRef"]!["leaves"]!.AsArray()
            .Select(leaf => leaf!.AsObject())
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "shape3dSceneLightRigRotationRevolution60000"));
        Assert.Equal(7_200_000, reprojectedTarget["value"]!.GetValue<long>());
    }

    private static byte[] AddPictureShape3dSceneLightRigRotationRevolution(byte[] source, string revolution)
    {
        ...
        using (var package = PresentationDocument.Open(stream, true))
        {
            ...
            rotation.SetAttribute(new OpenXmlAttribute("lon", string.Empty, "0"));
            rotation.SetAttribute(new OpenXmlAttribute("rev", string.Empty, revolution));
            lightRig.AppendChild(rotation);
