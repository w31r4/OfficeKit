...
change applies the same ownership boundary to accent5 saturation offset without
...
- Accent5 transforms other than `satOff`, transforms on accent5 through
...
1. **Use the existing native transform field.**
   ...
   compiler already emits it. The importer records a role of `accent5`; the
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:satOff`. The source writer patches `Accent5Color` only when the authored
   transform is the one existing accent5 satOff and rejects all other transform
   flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent5SatOff` is accepted only with
   `accentTransforms.accent5.satOff` in the source-issued capability. This
