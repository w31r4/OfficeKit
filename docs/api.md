...
- `paragraphFormat` (object) — Optional modeled paragraph formatting for the wrapped paragraph, including keepNext, boolean keepLinesTogether, boolean widowControl, 0-through-9 outlineLevel, pageBreakBefore, and presence-aware boolean suppressLineNumbers with the same direct/style inheritance and fail-closed source rules as document.addParagraph.
