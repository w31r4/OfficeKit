...
This change applies the same ownership boundary to accent3 tint without
...
- Accent3 transforms other than `tint`, transforms on accent4 through accent6,
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:tint`. The source writer patches `Accent3Color` only when the authored
   transform is the one existing accent3 tint and rejects all other transform
   flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent3Tint` is accepted only with
   `accentTransforms.accent3.tint` in the source-issued capability. This keeps
