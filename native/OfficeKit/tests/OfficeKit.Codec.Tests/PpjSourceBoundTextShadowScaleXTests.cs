...
{
    [Fact]
    public void PpjSourceBoundTextShadowScaleXEditsDirectRunOwnerAndReprojects()
    {
        var program = ShadowProgram(scaleX: -0.5);
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            Assert.Equal(-50_000, shadow.HorizontalRatio!.Value);
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/source/text-shadow-scale-x.pptx",
            },
        });
        ...
        Assert.Equal(-0.5, shadowStyle["scaleX"]!.GetValue<double>(), precision: 6);
        var scaleLeaf = Assert.Single(projectedElement["nativeRef"]!["leaves"]!.AsArray(),
            leaf => leaf!["kind"]!.GetValue<string>() == "textShadowScaleX");
        ...
        using (var themeStream = new MemoryStream())
        {
            ...
            var themeProjected = Invoke(new CodecRequest
            {
                ...
                PresentationProgram = new PresentationProgramRequest
                {
                    SourceUri = "deck.assets/source/text-shadow-scale-x-theme.pptx",
                },
            });
            ...
            Assert.Equal(-0.5, themeElement["text"]!["paragraphs"]![0]!["runs"]![0]!["style"]!["shadow"]!["scaleX"]!.GetValue<double>(), precision: 6);
        ...
        using (var staleStream = new MemoryStream())
        {
            ...
            using (var stalePackage = PresentationDocument.Open(staleStream, true))
            {
                ...
                staleShadow.HorizontalRatio = 125_000;
            }
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            Assert.Equal(19_050, shadow.Distance!.Value);
            Assert.Equal(5_400_000, shadow.Direction!.Value);
            Assert.Equal(125_000, shadow.HorizontalRatio!.Value);
            Assert.Equal("16324F", shadow.GetFirstChild<A.RgbColorModelHex>()!.Val!.Value);
            Assert.Equal(72_000, shadow.Descendants<A.Alpha>().Single().Val!.Value);
        }
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/text-shadow-scale-x.pptx",
            },
        });
        ...
        Assert.Equal(1.25, reprojectedShadow["scaleX"]!.GetValue<double>(), precision: 6);
        Assert.Equal(1.25, reprojectedElement["nativeRef"]!["leaves"]!.AsArray()
            .Select(leaf => leaf!.AsObject())
            .Single(leaf => leaf["kind"]!.GetValue<string>() == "textShadowScaleX")["value"]!.GetValue<double>(), precision: 6);
        ...
        invalidProgram["pages"]![0]!["elements"]![0]!["nativeRef"]!["leaves"]!.AsArray()
            .Single(leaf => leaf!["kind"]!.GetValue<string>() == "textShadowScaleX")!["value"] = 2_147_483_648;
    ...
    [Fact]
    public void PpjSourceBoundTextShadowScaleXLeavesStayOpaqueForUnsupportedGraphs()
    {
        var authored = CompileScaleXShadow(ShadowProgram(scaleX: -0.5));
        Assert.True(authored.Ok, Diagnostics(authored));
        var source = RemoveEmbeddedPpj(authored.File.ToByteArray());

        foreach (var (label, mutate) in new[]
        {
            ("transform", (Action<A.OuterShadow>)(shadow => shadow.VerticalRatio = 120_000)),
            ("missing-scale", (Action<A.OuterShadow>)(shadow => shadow.HorizontalRatio = null)),
            ("invalid-scale", (Action<A.OuterShadow>)(shadow => shadow.SetAttribute(new OpenXmlAttribute("sx", "", "invalid")))),
        ...
        })
        {
            ...
            var projected = Invoke(new CodecRequest
            {
                ...
                PresentationProgram = new PresentationProgramRequest
                {
                    SourceUri = $"deck.assets/source/text-shadow-scale-x-{label}.pptx",
                },
            });
        ...
        var siblingAuthored = CompileScaleXShadow(ShadowProgram(scaleX: -0.5, siblingGlow: true));
        ...
        var siblingProjected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/source/text-shadow-scale-x-sibling.pptx",
            },
        });
    ...
    private static JsonObject ShadowProgram(double scaleX, bool siblingGlow = false)
    {
    ...
    private static CodecResponse CompileScaleXShadow(JsonObject program) => Invoke(new CodecRequest
    {
        ProtocolVersion = CodecProtocol.ProtocolVersion,
        Operation = CodecOperation.CompilePpjToPptx,
        Family = ArtifactFamily.Presentation,
