...
{
    [Fact]
    public void PpjSourceBoundTextShadowDirectionEditsDirectRunOwnerAndReprojects()
    {
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/source/text-shadow-direction.pptx",
            },
        });
        ...
        var leaves = projectedElement["nativeRef"]!["leaves"]!.AsArray()
            ...
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "textShadowDirectionDegrees")
            .ToArray();
        var directionLeaf = Assert.Single(leaves);
        Assert.Equal(90, directionLeaf["value"]!.GetValue<double>(), precision: 6);
        ...
        using (var staleStream = new MemoryStream())
        {
            ...
            using (var stalePackage = PresentationDocument.Open(staleStream, true))
            {
                ...
                staleShadow.Direction = 6_000_000;
            }
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            Assert.Equal(38_100, shadow.BlurRadius!.Value);
            Assert.Equal(19_050, shadow.Distance!.Value);
            Assert.Equal(7_200_000, shadow.Direction!.Value);
            Assert.Equal("16324F", shadow.GetFirstChild<A.RgbColorModelHex>()!.Val!.Value);
            Assert.Equal(72_000, shadow.Descendants<A.Alpha>().Single().Val!.Value);
        }
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/text-shadow-direction.pptx",
            },
        });
        ...
        Assert.Equal(120, reprojectedShadow["angle"]!.GetValue<double>(), precision: 6);
        Assert.Equal(0.72, reprojectedShadow["opacity"]!.GetValue<double>(), precision: 6);
        Assert.Equal(120, reprojectedElement["nativeRef"]!["leaves"]!.AsArray()
            .Select(leaf => leaf!.AsObject())
            .Single(leaf => leaf["kind"]!.GetValue<string>() == "textShadowDirectionDegrees")["value"]!.GetValue<double>(), precision: 6);
        ...
        var invalidLeaf = invalidProgram["pages"]![0]!["elements"]![0]!["nativeRef"]!["leaves"]!.AsArray()
            .Single(leaf => leaf!["kind"]!.GetValue<string>() == "textShadowDirectionDegrees")!.AsObject();
        invalidLeaf["value"] = 360;
    ...
    [Fact]
    public void PpjSourceBoundTextShadowDirectionLeavesStayOpaqueForSiblingEffects()
    {
        var root = new DirectoryInfo(AppContext.BaseDirectory);
        while (root is not null && !File.Exists(Path.Combine(root.FullName, "package.json")))
            root = root.Parent;
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/source/text-shadow-direction-complex.pptx",
            },
        });
        Assert.True(projected.Ok, Diagnostics(projected));
        var projectedProgram = JsonNode.Parse(projected.PresentationProgram.ProgramJson.ToByteArray())!.AsObject();
        ...
        Assert.DoesNotContain(
            ...
            leaf => leaf!["kind"]!.GetValue<string>() == "textShadowDirectionDegrees");

        foreach (var (label, mutate) in new[]
        {
            ("transform", (Action<A.OuterShadow>)(shadow => shadow.HorizontalRatio = 120_000)),
            ("missing-direction", (Action<A.OuterShadow>)(shadow => shadow.Direction = null)),
        ...
        })
        {
            ...
            var unsupportedProjected = Invoke(new CodecRequest
            {
                ...
                PresentationProgram = new PresentationProgramRequest
                {
                    SourceUri = $"deck.assets/source/text-shadow-direction-{label}.pptx",
                },
            });
