...
- Expose one canonical signed `a:backdrop/a:norm/@dx` coordinate in EMU.
- Replace only `backdrop/a:norm/@dx` in the owning SlidePart.
...
- Reuse `TryReadSceneBackdropNormalDx` and its requested-value validator so
...
- Token-splice only the direct `backdrop/a:norm/@dx` attribute.

# Risks / Trade-offs

- [Risk] A normal X token without a complete backdrop owner could be mistaken
