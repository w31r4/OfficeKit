...
{
    [Fact]
    public void PpjSourceBoundCustomGeometryPathMoveToYEditsAndReprojects()
    {
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            {
                SourceUri = "deck.assets/source/custom-geometry-path-move-to-y.pptx",
            },
        });
        ...
        var moveLeaves = projectedShape["nativeRef"]!["leaves"]!.AsArray()
            .Select(leaf => leaf!.AsObject())
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "customGeometryPathMoveToY")
            .ToArray();
        var target = Assert.Single(moveLeaves);
        ...
        Assert.Contains("x=\"100\" y=\"250\"", editedXml, StringComparison.Ordinal);
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/custom-geometry-path-move-to-y.pptx",
            },
        });
        ...
        var reprojectedLeaves = reprojectedShape["nativeRef"]!["leaves"]!.AsArray()
            ...
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "customGeometryPathMoveToY")
            .ToArray();
        Assert.Equal(250, Assert.Single(reprojectedLeaves)["value"]!.GetValue<long>());
    }
}
