...
- Expose one canonical signed `a:backdrop/a:anchor/@z` coordinate in EMU.
- Replace only `backdrop/a:anchor/@z` in the owning SlidePart.
...
- Reuse `TryReadSceneBackdropAnchorZ` and its requested-value validator so
...
- Token-splice only the direct `backdrop/a:anchor/@z` attribute.

# Risks / Trade-offs

- [Risk] An anchor Z token without a complete backdrop owner could be mistaken
