...
This change applies the same ownership boundary to accent3 blue-channel modulation
...
- Accent3 transforms other than `blueMod`, transforms on other accent roles, inherited or effect theme state, or arbitrary theme XML editing.
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:blueMod`. The source writer patches `Accent3Color` only when the authored
   transform is the one existing accent3 blueMod and rejects all other transform
   flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent3BlueMod` is accepted only with
   `accentTransforms.accent3.blueMod` in the source-issued capability. This
