...
transform and the direct accent5 tint, shade, luminance, alpha, saturation, and
red modulation leaves at a time, then patches the existing DrawingML token.
This change applies the same ownership boundary to accent5 blue-channel modulation
...
- Accent5 transforms other than `blueMod`, transforms on other accent roles, inherited or effect theme state, or arbitrary theme XML editing.
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:blueMod`. The source writer patches `Accent5Color` only when the authored
   transform is the one existing accent5 blueMod and rejects all other transform
   flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent5BlueMod` is accepted only with
   `accentTransforms.accent5.blueMod` in the source-issued capability. This
