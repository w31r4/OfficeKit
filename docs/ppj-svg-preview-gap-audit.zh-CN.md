...
五种多边形轮廓阶段记录（2026-09-10）：triangle、rtTriangle、trapezoid、parallelogram、chevron 共用预设调整和显示路径，覆盖普通形状、形状图片填充与图片遮罩。TzaE9j 在同一 035472e9 默认包下通过 66 项作者/源生命周期回归，其中 36 项独立调整值修改/清零/删除保持非目标内容；正式入口共 331 项，新增结果均为 requires-review。外部缺省连接对照有 2/5 失败，新增 line-join-inherited 诊断；显式 round 的独立对照 5/5 通过。这一阶段曾复现预设文字落到轮廓外，当前文字区域修复证据见上段，未混称整张形状保真。原十二项删除失败、其余预设/主题/效果及完整目标保持开放，任务 10/15；详见[当前 G-02 多边形](ppj-preview-current-gaps.zh-CN.md#五种多边形共享轮廓与尚未解决的文字区域)。下方每次运行的数量均为历史快照，不重复累加。

文字方向进展（2026-09-10）：horizontal/vertical/vertical270 已按阅读坐标交换宽高、映射物理边距和锚定，并与独立文字角度及防镜像组合。xTtqoh 验证 75 项真实方向/源生命周期案例通过，正式入口共 265 项；新增结果均为 requires-review，原十二项删除失败仍使整套退出 1。九个自生成矩形案例另与本机 LibreOffice 对照，文字墨迹边界最大差为 1px，仅作该字号、边距与方向的局部证据，不代表完整字体、第三方文件、PowerPoint 或人类验收。多列、其他竖排模式、upright、AutoFit 等仍缺，任务仍 10/15。详见[当前 G-02 文字方向](ppj-preview-current-gaps.zh-CN.md#文字方向阅读坐标与物理边距)。
...
默认包交付与 dataset 折线检查更新（2026-09-10）：冻结源码 `035472e9` 已通过仓库构建流程生成本机默认 linux-x64 双 profile 包，旧包完整备份；无测试 preload 的 presentation 4/4、快照 scene 47/47、双构建 9 文件一致、包内容与打包预检通过。正式入口已可使用，包身份和跨格式依赖失败见[当前差距第 3.6 节](ppj-preview-current-gaps.zh-CN.md#36-默认运行时交付与复验2026-09-10)。下方“默认旧包未更新”均为交付前历史记录。

最终 L4nZbV 集成的十二项正式入口案例通过限定断言。dataset 折线的 1→2 变体有真实点位移动、缺失/零区别和独立显式数据对照；仅在原生折线及其祖先完成绘制时解除旧通道忽略误报。缺捕获、隐藏、混合归属或未支持图表保留错误，encoded heatmap 仍为 failed。整套仍因四项变换/七项间距删除失败退出 1；10/15 任务和完整目标不变，详见[当前差距 G-11/G-12](ppj-preview-current-gaps.zh-CN.md#g-11g-12已有检查和发布契约不是绘制完成)。

正式入口接入进展（2026-09-10）：`renderPpjToSvg` 已请求并消费原生 scene，移除旧组件/图表猜测，强制合并输入检查，沿用身份校验与非覆盖发布。DUtafR 十项组件/样式/dataset/源候选正式函数案例通过内容区全像素及证据/保留断言；实际 CLI 作者、源 no-op 和覆盖拒绝也在匹配包下通过。默认安装旧包仍报 preview.scene.missing，未替换安装包，必须配套更新再复验；dataset 旧事实误报及十一项原生删除失败仍保留。下方“正式入口未切换”为对应旧轮次记录，当前边界见[当前差距第 2 节](ppj-preview-current-gaps.zh-CN.md#2-正式入口已接场景为什么仍需配套原生包)。

线性渐变进展（2026-09-10）：形状和表格单元格共享直接色标/透明度/物理方向绘制。PfrKIJ 七项 0°/45°/90° 作者、源 no-op 和源方向修改通过实际像素、重新投影与非目标保留，45° PNG 已查看。整套仍为 failed：四项变换删除及七项段落间距删除失败。当前差距文档开头已同步该报告，不再仅列旧的四项失败；径向/背景等消费者仍缺，见[当前差距 G-04](ppj-preview-current-gaps.zh-CN.md#g-04样式主题和效果)。

字符列表进展（2026-09-10）：内部 painter 开始消费直接字符/字体/RGB/point 字号和悬挂缩进，续行不重复符号；不确定样式与定位继续失败。BPCuPP 作者/源 no-op/源字符与颜色修改及独立对照通过实际符号像素、正文位置、重新投影和仅目标 slide 变化，已查看 PNG。Presentation 4/4 和生成检查通过；自动编号、图片列表、继承、十一项删除失败及正式入口仍未完成，见[当前差距 G-02](ppj-preview-current-gaps.zh-CN.md#g-02文字和形状)。

折线事实映射进展（2026-09-10）：内部 native profile 将实际可见点捕获与完整原始/原生数据匹配，严格保留 null/zero 区分。nPwMZz 四个真实作者/源候选状态各解除两条系列类型旧误报，缺捕获恢复、缺 registry 拒绝，现有缺口/marker/零像素断言仍通过。孤立点错误逐字段解除另有合成边界检查，未混称真实触发；整套仍有十一项删除失败，正式入口与 4.1 未完成，详见[当前差距 G-11/G-12](ppj-preview-current-gaps.zh-CN.md#g-11g-12已有检查和发布契约不是绘制完成)。

媒体静态封面进展（2026-09-10）：内部原生 media 以已验证 posterAssetId 绘制图片并增加可见静态提示；audio/video 合成检查、缺失资产拒绝和透明度保留通过。rnrCq1 两个真实作者封面变体通过颜色/透明/警示像素，源 no-op 保持字节和 opaque 边界；不声称播放或导入封面已支持。Presentation 4/4 和生成检查通过，整套仍有十一项删除失败，详见[当前差距 G-07](ppj-preview-current-gaps.zh-CN.md#g-07source-boundopaque-和静态检查边界)。

大小写显示进展（2026-09-10）：内部消费 fontCaps 的 all/none，保持源文字不变；small 和未知 token 明确未支持。je2i3l 五个真实作者/源 no-op/源编辑案例通过独立文字像素对照、重新投影和非目标部件保留。未重建原生包或切换正式入口，整套仍有十一项删除失败，详见[当前差距 G-02](ppj-preview-current-gaps.zh-CN.md#g-02文字和形状)。

源间距删除反例（2026-09-10）：新增行距/段前/段后七种删除组合，最终 rsK9nS 全部在 PPJ 编译阶段被 unsupportedMutation 拒绝，未生成候选；原源和逐请求证据已保存，失败保持致命，不能用已有清零成功替代。底层 codec 已支持删除，缺口在 PPJ 富文本变更映射，原生修复待规划范围确认。整套现在有四项 frame 删除失败及七项间距拒绝，完整目标仍开放，详见[当前差距 G-02](ppj-preview-current-gaps.zh-CN.md#g-02文字和形状)。
