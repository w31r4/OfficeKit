...
{
    [Fact]
    public void PpjSourceBoundTextShadowColorSchemeEditsDirectRunOwnerAndReprojects()
    {
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            var authoredColor = Assert.IsType<A.SchemeColor>(shadow.FirstChild);
            Assert.Equal(A.SchemeColorValues.Accent1, authoredColor.Val!.Value);
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/source/text-shadow-color-scheme.pptx",
            },
        });
        ...
        Assert.Equal("accent1", shadowStyle["color"]!["token"]!.GetValue<string>());
        ...
        var leaves = projectedElement["nativeRef"]!["leaves"]!.AsArray()
            ...
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "textShadowColorScheme")
            .ToArray();
        var colorLeaf = Assert.Single(leaves);
        Assert.Equal("accent1", colorLeaf["value"]!.GetValue<string>());
        ...
        using (var staleStream = new MemoryStream())
        {
            ...
            using (var stalePackage = PresentationDocument.Open(staleStream, true))
            {
                ...
                staleShadow.GetFirstChild<A.SchemeColor>()!.Val = A.SchemeColorValues.Accent2;
            }
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            Assert.Equal(19_050, shadow.Distance!.Value);
            Assert.Equal(5_400_000, shadow.Direction!.Value);
            var editedColor = Assert.IsType<A.SchemeColor>(shadow.FirstChild);
            Assert.Equal(A.SchemeColorValues.Accent2, editedColor.Val!.Value);
            Assert.Equal(72_000, shadow.Descendants<A.Alpha>().Single().Val!.Value);
        }
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/text-shadow-color-scheme.pptx",
            },
        });
        ...
        Assert.Equal("accent2", reprojectedShadow["color"]!["token"]!.GetValue<string>());
        ...
        Assert.Equal("accent2", reprojectedElement["nativeRef"]!["leaves"]!.AsArray()
            .Select(leaf => leaf!.AsObject())
            .Single(leaf => leaf["kind"]!.GetValue<string>() == "textShadowColorScheme")["value"]!.GetValue<string>());
        ...
        var invalidLeaf = invalidProgram["pages"]![0]!["elements"]![0]!["nativeRef"]!["leaves"]!.AsArray()
            .Single(leaf => leaf!["kind"]!.GetValue<string>() == "textShadowColorScheme")!.AsObject();
        invalidLeaf["value"] = "not-a-theme-color";
    ...
    [Fact]
    public void PpjSourceBoundTextShadowColorSchemeLeavesStayOpaqueForUnsupportedGraphs()
    {
        var root = new DirectoryInfo(AppContext.BaseDirectory);
        while (root is not null && !File.Exists(Path.Combine(root.FullName, "package.json")))
            root = root.Parent;
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/source/text-shadow-color-scheme-complex.pptx",
            },
        });
        Assert.True(projected.Ok, Diagnostics(projected));
        var projectedProgram = JsonNode.Parse(projected.PresentationProgram.ProgramJson.ToByteArray())!.AsObject();
        ...
        Assert.DoesNotContain(
            ...
            leaf => leaf!["kind"]!.GetValue<string>() == "textShadowColorScheme");
        ...
        foreach (var (label, mutate) in new[]
        {
            ...
            ("missing-color", (Action<A.OuterShadow>)(shadow => shadow.GetFirstChild<A.SchemeColor>()!.Remove())),
            ("duplicate-color", (Action<A.OuterShadow>)(shadow => shadow.AppendChild(new A.SchemeColor { Val = A.SchemeColorValues.Accent2 }))),
            ("malformed-color", (Action<A.OuterShadow>)(shadow => shadow.GetFirstChild<A.SchemeColor>()!.SetAttribute(new OpenXmlAttribute("val", "", "future")))),
            ("color-transform", (Action<A.OuterShadow>)(shadow => shadow.GetFirstChild<A.SchemeColor>()!.AppendChild(new A.Tint { Val = 50_000 }))),
            ...
            ("rgb-color", (Action<A.OuterShadow>)(shadow => { shadow.GetFirstChild<A.SchemeColor>()!.Remove(); shadow.AppendChild(new A.RgbColorModelHex { Val = "16324F" }); })),
        })
        {
            ...
            var unsupportedProjected = Invoke(new CodecRequest
            {
                ...
                PresentationProgram = new PresentationProgramRequest
                {
                    SourceUri = $"deck.assets/source/text-shadow-color-scheme-{label}.pptx",
                },
            });
            if (label == "malformed-color")
            {
                Assert.False(unsupportedProjected.Ok, Diagnostics(unsupportedProjected));
                continue;
            }
            Assert.True(unsupportedProjected.Ok, label + ": " + Diagnostics(unsupportedProjected));
