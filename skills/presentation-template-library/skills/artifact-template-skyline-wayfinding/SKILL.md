...
Use this style when the brief matches **service operations; work presentation**. It is designed for
operators, managers, cross-functional teams. Its visual premise is Sky-blue markers turn service operations into one visible journey. The style is a
...
**current state → signal → cause → trade-off → intervention → owner/date → follow-through**. This style is not a good fit for **fashion story**;
...
- Use **#F8FAFC** as the main field, **#162A4A** for primary type, **#4C9DD8** for the decisive signal, **#EE9A3D** for secondary evidence, and **#D9E2EA** for rules or context.
...
- The recurring structure is **wayfinding, variable page silhouettes, native data graphics**. Vary the silhouette between
  cover, evidence, data, process, comparison, and decision pages rather than
  repeating a component grid.
- Use routes, landmarks, and checkpoints to make a service or place legible. Connectors should have clear endpoints and never pass through labels.
...
- **Page skeleton.** Use white operations pages with a pale-blue section rail and a route-like arc for project progress. Put the conclusion at the top, the data or route in the middle, and the current period or action at the edge.
- **Type rhythm.** Use a consistent sans and small label system. Blue belongs to titles, rules, and the current route; keep long-form copy black and avoid coloring whole paragraphs.
- **Evidence carriers.** Use waterfalls, progress bars, paired columns, multi-period tables, and the project arc. Leader lines and nodes must name a stage, location, or milestone rather than decorate the margin.
- **Images and layers.** Use a photographic section board only for an opener, transition, Q&A, or appendix. Ordinary data pages remain light, editable, and free of full-slide photography.
- **Pacing.** Journey → current state → gap → milestone → next action. Keep the route visible but let each page answer one operational question.
- **Selection rule.** Choose this style for service operations, project progress, milestone reviews, and wayfinding-like reports.

## Signature moves and variation limits

- Let **wayfinding** recur as a quiet identity cue on selected pages; it must
