...
| Text and typography | Titles, body copy, runs, notes and native text leaves | `shape.text.set`, `presentation.textRange`, `nativeObject.setDiagramNodeText`, `nativeObject.setDiagramNodeRunText` | Preserve run topology, facts and source styling unless the requested operation explicitly changes them. Direct source-bound run leaves include font size, typeface, color, decoration, `fontKerningPoints`, bounded `fontBaselinePercent`, and bounded `fontSpacingPoints` when the imported `a:rPr/@kern`, `a:rPr/@baseline`, or `a:rPr/@spc` token is canonical. |
