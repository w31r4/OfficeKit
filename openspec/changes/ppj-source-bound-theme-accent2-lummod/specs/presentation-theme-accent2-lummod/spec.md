...
direct accent2 luminance modulation token while preserving the rest of an
imported theme.

## ADDED Requirements

### Requirement: Strict source-bound accent2 lumMod projection

The PPTX projector MUST expose
`design.theme.accentTransforms.accent2.lumMod` and
`setThemeAccent2LumMod` only when the canonical shared ThemePart has one
direct accent2 RGB leaf with exactly one direct `a:lumMod/@val` child, no extra
attributes or descendants, and an integer value in `[0, 100000]`. The
projected value MUST be the native value divided by 100000.

#### Scenario: Project a direct accent2 lumMod leaf

- **GIVEN** a canonical imported ThemePart with accent2 `223344` and
  `a:lumMod/@val="25000"`
- **WHEN** the presentation is projected to PPJ
- **THEN** the theme contains `accentTransforms.accent2.lumMod` equal to `0.25`
- **AND** the native reference contains `setThemeAccent2LumMod` authorized for
  `accentTransforms.accent2.lumMod`

#### Scenario: Reject unsupported accent2 lumMod topology
...
- **THEN** the source-bound accent2 lumMod field and capability are omitted

### Requirement: Source-bound accent2 lumMod editing

The source-preserving compiler MUST accept one authorized change to the
existing `design.theme.accentTransforms.accent2.lumMod` field, convert a finite
...
#### Scenario: Edit and reproject one accent2 lumMod token

- **GIVEN** a projected direct accent2 lumMod owner at `0.25`
...
#### Scenario: Fail closed for unsafe accent2 lumMod edits

- **GIVEN** a source-bound accent2 lumMod owner
