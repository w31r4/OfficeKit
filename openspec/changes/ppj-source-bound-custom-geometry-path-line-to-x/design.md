...
- Owner: `p:sp/p:spPr/a:custGeom/a:pathLst/a:path/a:lnTo/a:pt/@x`.
- PPJ surface: `shape.nativeRef.leaves[]`.
- Native leaf: `customGeometryPathLineToX`.
...
- The geometry must pass the existing bounded custom-geometry recognition
  profile. The selected `lnTo` and `pt` must be attribute-only, and `x` must
...
topology, the selected direct `lnTo/pt`, the canonical numeric x token, and
...
removes embedded PPJ, projects both line-to x leaves, changes one leaf, checks
that only the owning SlidePart changed and the package remains valid, then
projects again to recover both x coordinates.
