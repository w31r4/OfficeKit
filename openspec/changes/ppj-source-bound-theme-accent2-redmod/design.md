...
the same ownership boundary to accent2 red-channel modulation without
...
- Accent2 transforms other than `redMod`, transforms on accent3 through
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:redMod`. The source writer patches `Accent2Color` only when the authored
   transform is the one existing accent2 redMod and rejects all other transform
   flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent2RedMod` is accepted only with
   `accentTransforms.accent2.redMod` in the source-issued capability. This
