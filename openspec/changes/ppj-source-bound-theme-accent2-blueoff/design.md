...
This change applies the same ownership boundary to accent2 blue-channel offset
...
- Accent2 transforms other than `blueOff`, transforms on accent3 through
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:blueOff`. The source writer patches `Accent2Color` only when the authored
   transform is the one existing accent2 blueOff and rejects all other transform
   flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent2BlueOff` is accepted only with
   `accentTransforms.accent2.blueOff` in the source-issued capability. This
