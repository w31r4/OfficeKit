...
{
    [Fact]
    public void PpjSourceBoundThemeAccent6RgbaEditsOnlyAccent6AndReprojects()
    {
        var authoredRequest = ExportRequest();
        var authoredTheme = new PresentationThemeArtifact();
        authoredTheme.AccentRgb.Add(new[] { "112233", "223344", "334455", "445566", "556677", "66778880" });
        ...
        Assert.Equal("#66778880", projectedColors["accent6"]!.GetValue<string>());
        ...
        editedProgram["design"]!["theme"]!["accentColors"]!["accent6"] = "#ABCDEF40";
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            Assert.Equal(25098, scheme.Accent6Color.RgbColorModelHex.GetFirstChild<A.Alpha>()!.Val!.Value);
        }
        ...
        Assert.Equal("#ABCDEF40", reprojectedProgram["design"]!["theme"]!["accentColors"]!["accent6"]!.GetValue<string>());

        var alphaPresenceProgram = JsonNode.Parse(projectedProgram.ToJsonString())!.AsObject();
        alphaPresenceProgram["design"]!["theme"]!["accentColors"]!["accent6"] = "#ABCDEF";
        ...
        tamperedProgram["design"]!["theme"]!["nativeRef"]!["capabilities"]!.AsArray()
            .Single(capability => capability!["operation"]!.GetValue<string>() == "setThemeAccent6Color")!["fields"] = new JsonArray("accentColors.accent1");
