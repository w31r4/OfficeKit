...
modulation transforms. A direct imported accent2 `a:greenMod` remains opaque,
...
- Project a canonical shared ThemePart whose accent2 direct RGB leaf contains
  ...
  `design.theme.accentTransforms.accent2.greenMod`, using a 0..1 fraction.
- Issue `setThemeAccent2GreenMod` for that field and compile one authorized
  change by rewriting only the existing `a:greenMod/@val` token.
- Convert the PPJ fraction to DrawingML percentage thousandths with away-from-zero
...
- `presentation-theme-accent2-greenmod`: Strict source-bound projection and
  source-preserving editing of one direct accent2 `a:greenMod` leaf.
