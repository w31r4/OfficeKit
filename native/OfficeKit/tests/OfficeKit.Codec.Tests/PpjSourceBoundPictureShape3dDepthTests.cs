...
{
    [Fact]
    public void PpjSourceBoundPictureShape3dDepthLeafEditsAndReprojects()
    {
        ...
        var source = AddPictureShape3dDepth(RemoveEmbeddedPpj(authored.File.ToByteArray()), -1_200);
        var projected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/source/picture-shape-3d-depth.pptx",
            },
        });
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            Assert.Equal(-2_400, picture.ShapeProperties!.GetFirstChild<A.Shape3DType>()!.Z!.Value);
        }
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/picture-shape-3d-depth.pptx",
            },
        });
        ...
        Assert.Equal(-2_400, reprojectedImage["nativeRef"]!["leaves"]!.AsArray()
            .Select(leaf => leaf!.AsObject())
            .Single(leaf => leaf["kind"]!.GetValue<string>() == "shape3dDepthEmu")["value"]!.GetValue<long>());
    }

    private static byte[] AddPictureShape3dDepth(byte[] source, long depth)
    {
        ...
        using (var package = PresentationDocument.Open(stream, true))
        {
            var matches = package.PresentationPart!.SlideParts
                ...
                .ToArray();
            if (matches.Length != 1)
                throw new InvalidOperationException($"Expected one evidence identity picture, found {matches.Length}");
