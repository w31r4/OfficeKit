...
- Extend `setThemeColorRoleLight1` for `design.theme.colorRoles.light1` while keeping the existing dark1 capability intact.
- Patch only an existing direct `a:lt1/a:srgbClr/@val` and its existing `a:alpha/@val` when present in the canonical shared ThemePart.
...
2. **Issue an independent field capability.** Extend `setThemeColorRoleLight1` with only `colorRoles.light1`; retain the existing dark1 capability so a projection remains truthful about both independently owned fields.
...
4. **Patch the existing XML token.** The writer rereads the canonical ThemePart, verifies the direct RGB light1 topology, and changes only `a:lt1/a:srgbClr/@val` plus the existing `a:alpha/@val` when present. It never synthesizes a missing node or serializes unsupported descendants.
