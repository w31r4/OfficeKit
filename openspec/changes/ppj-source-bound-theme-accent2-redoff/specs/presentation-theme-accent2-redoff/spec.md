...
direct accent2 red-channel offset token while preserving the rest of an
imported theme.

## ADDED Requirements

### Requirement: Strict source-bound accent2 redOff projection

The PPTX projector MUST expose
`design.theme.accentTransforms.accent2.redOff` and
`setThemeAccent2RedOff` only when the canonical shared ThemePart has one
direct accent2 RGB leaf with exactly one direct `a:redOff/@val` child, no
...
#### Scenario: Project a direct accent2 redOff leaf

- **GIVEN** a canonical imported ThemePart with accent2 `223344` and
  `a:redOff/@val="-25000"`
- **WHEN** the presentation is projected to PPJ
- **THEN** the theme contains `accentTransforms.accent2.redOff` equal to
  `-0.25`
- **AND** the native reference contains `setThemeAccent2RedOff` authorized
  for `accentTransforms.accent2.redOff`

#### Scenario: Reject unsupported accent2 redOff topology
...
- **THEN** the source-bound accent2 redOff field and capability are omitted

### Requirement: Source-bound accent2 redOff editing

The source-preserving compiler MUST accept one authorized change to the
existing `design.theme.accentTransforms.accent2.redOff` field, convert a
...
#### Scenario: Edit and reproject one accent2 redOff token

- **GIVEN** a projected direct accent2 redOff owner at `-0.25`
...
#### Scenario: Fail closed for unsafe accent2 redOff edits

- **GIVEN** a source-bound accent2 redOff owner
