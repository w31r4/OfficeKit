...
public sealed class XlsxCodecTests
{
    ...
    public void ImportPopulatesOpaquePartContentTypesAndOutgoingRelationships()
    {
        ...
        Assert.Equal("application/vnd.office-kit.native+xml", nativePart.ContentType);
        ...
        nativePart.ContentType = "application/vnd.office-kit.native+xml";
    ...
    public void SourcePreservingThemeEditRetainsSystemColorAndResidualContent()
    {
        ...
        Assert.True(imported.Artifact.Workbook.Theme.Source.Editable);
        imported.Artifact.Workbook.Theme.Name = "OfficeKit Edited";
        ...
        Assert.Contains("name=\"OfficeKit Edited\"", xml);
        ...
        using (var document = SpreadsheetDocument.Open(stream, false))
            Assert.Empty(new OpenXmlValidator(FileFormatVersions.Office2021).Validate(document));
        var reimported = Import(exported.File.ToByteArray());
        Assert.Equal("OfficeKit Edited", reimported.Artifact.Workbook.Theme.Name);
    ...
    private static byte[] MutateTheme(byte[] bytes, bool unsupportedColor)
    {
        ...
        using (var archive = new ZipArchive(stream, ZipArchiveMode.Update, leaveOpen: true))
        {
            ...
            XNamespace probe = "urn:officekit:test";
            ...
            theme.Root.Add(new XElement(drawing + "extLst", new XElement(drawing + "ext", new XAttribute("uri", "urn:officekit:test"), new XElement(probe + "probe", new XAttribute("value", "preserve-me")))));
            entry.Delete();
    ...
    private static byte[] AddChartResidual(byte[] bytes)
    {
        ...
        using (var document = SpreadsheetDocument.Open(stream, true))
        {
            ...
            XNamespace fixture = "urn:officekit:worksheet-chart";
    ...
    private static byte[] AddPictureLocksResidual(byte[] bytes)
    {
        ...
        using (var document = SpreadsheetDocument.Open(stream, true))
        {
            ...
            extension.Append(document.WorkbookPart.WorksheetParts.Single().DrawingsPart!.CreateUnknownElement("<fixture:probe xmlns:fixture=\"urn:officekit:picture-effect\">preserve-me</fixture:probe>"));
        ...
        bool omitConnectionRefreshOnLoad = false)
    {
        ...
        using (var document = SpreadsheetDocument.Open(stream, true))
        {
            ...
            WritePart(connectionsPart, $$"""
                ...
                <x:connections xmlns:x="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:fixture="urn:office-kit:query-fixture">
            ...
            if (addUnsupportedRelationship)
                tablePart.AddExternalRelationship("urn:office-kit:unsupported-query-companion", new Uri("https://fixture.invalid/query"), "rIdUnsupportedQueryCompanion");
    ...
    private static byte[] AddExternalWorkbookRelationship(byte[] bytes)
    {
        ...
        using (var archive = new ZipArchive(stream, ZipArchiveMode.Update, leaveOpen: true))
        {
            ...
            using var writer = new StreamWriter(replacement.Open());
            writer.Write(xml.Replace("</Relationships>", "<Relationship Id=\"rIdExternal\" Type=\"urn:office-kit:test\" Target=\"https://example.invalid/data\" TargetMode=\"External\"/></Relationships>", StringComparison.Ordinal));
        }
        return stream.ToArray();
    }
    ...
    private static byte[] AddOpaqueOpcGraph(byte[] bytes)
    {
        ...
        using (var archive = new ZipArchive(stream, ZipArchiveMode.Update, leaveOpen: true))
        {
            ...
            using (var writer = new StreamWriter(archive.CreateEntry("[Content_Types].xml").Open()))
                writer.Write(contentTypesXml.Replace("</Types>", "<Override PartName=\"/xl/custom/native.xml\" ContentType=\"application/vnd.office-kit.native+xml\"/></Types>", StringComparison.Ordinal));
            using (var writer = new StreamWriter(archive.CreateEntry("xl/custom/native.xml").Open()))
                writer.Write("<native xmlns=\"urn:office-kit:native\">preserve me</native>");
            using (var writer = new StreamWriter(archive.CreateEntry("xl/custom/_rels/native.xml.rels").Open()))
                writer.Write("<?xml version=\"1.0\" encoding=\"UTF-8\"?><Relationships xmlns=\"http://schemas.openxmlformats.org/package/2006/relationships\"><Relationship Id=\"rIdPayload\" Type=\"urn:office-kit:native-payload\" Target=\"https://example.invalid/native-payload\" TargetMode=\"External\"/></Relationships>");
        }
        return stream.ToArray();
    }
