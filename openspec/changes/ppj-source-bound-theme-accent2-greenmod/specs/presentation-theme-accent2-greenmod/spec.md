...
direct accent2 green-channel modulation token while preserving the rest of an
imported theme.

## ADDED Requirements

### Requirement: Strict source-bound accent2 greenMod projection

The PPTX projector MUST expose
`design.theme.accentTransforms.accent2.greenMod` and
`setThemeAccent2GreenMod` only when the canonical shared ThemePart has one
direct accent2 RGB leaf with exactly one direct `a:greenMod/@val` child, no
...
#### Scenario: Project a direct accent2 greenMod leaf
...
- **THEN** the theme contains `accentTransforms.accent2.greenMod` equal to
  `0.25`
- **AND** the native reference contains `setThemeAccent2GreenMod` authorized
  for `accentTransforms.accent2.greenMod`

#### Scenario: Reject unsupported accent2 greenMod topology
...
- **THEN** the source-bound accent2 greenMod field and capability are omitted

### Requirement: Source-bound accent2 greenMod editing

The source-preserving compiler MUST accept one authorized change to the
existing `design.theme.accentTransforms.accent2.greenMod` field, convert a
...
#### Scenario: Edit and reproject one accent2 greenMod token

- **GIVEN** a projected direct accent2 greenMod owner at `0.25`
...
#### Scenario: Fail closed for unsafe accent2 greenMod edits

- **GIVEN** a source-bound accent2 greenMod owner
