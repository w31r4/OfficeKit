...
- 37 `ready` cases: 21 PDF cases, five bounded XLSX workflows (direct threaded-comment reply success, nested-reply refusal, formula-assumption update, source-bound connection refresh-on-open, and source-bound Pivot refresh-on-open), six DOCX workflows (classic-comment, nested-modern-comment refusal, complex-table-topology refusal, source-bound header/footer text, and source-bound section page numbering), and five PPTX workflows (title plus fixed-topology rich-speaker-notes run edit, source-bound slide-name edit, source-bound complete section-boundary edit, closed-leaf slide clone, and connected-SmartArt compound refusal).
- Fourteen locked, self-authored PDF corpus inputs are now ready. Eight are auditable safe-refusal boundaries: AES-256 owner/user permission split, native annotation reply chain, untagged complex report, mixed bilingual scan orientation/deskew, Dynamic XFA packet, print-production risk structure, a real DocMDP P=1 certification signature with a public test root, and a `/3D` plus `/RichMedia` opaque-object graph. The RichMedia fixture has a parser-visible model/default-view/activation/asset/configuration/JavaScript graph, but deliberately does not claim a runnable viewer; its correct current result is a source-bound audit-only refusal unless a provider can prove runtime preservation. One is a real DocMDP P=2/FieldMDP form whose one permitted controlled finalisation is independently graded. Two form a qpdf structural-repair pair: one two-page native-text PDF whose only damage is `startxref = 0` plus a document attachment, and one malformed no-trailer/no-page-tree control that must be rejected. The remaining four-page multi-channel redact fixture places one fictional canary in visible and white selectable text, raster content and an invisible OCR layer, annotation/widget/attachment, metadata/XMP, an unreferenced decoded stream, and one prior revision. The thirteenth fixture is a three-page ruled `Regional Revenue` table whose merged headers, fixed columns, text canaries, and footnote define one bounded read-only profile rather than a generic table-recognition claim. The fourteenth is a two-page PAdES-LTA test document with a visible final-page approval box and four disclosed disposable test-PKI files. Its evaluator invokes pyHanko directly, checks the test root/CRL, CMS, signature timestamp, DocumentTimeStamp, and DSS, and never treats that bounded test profile as a production TSA route or a PAdES conformance certificate. Every asset has a committed SHA-256/byte manifest and parser oracle; generic “no output” does not pass either kind of case.
- 4 `asset-required` cases still await their pinned corpus files. They remain excluded from readiness and repeat-matrix claims.
...
npm run eval:agents -- prepare pdf-cross-page-table-extraction --subject candidate --trial 1
npm run eval:agents -- run pdf-cross-page-table-extraction --subject candidate --trial 1
npm run eval:agents -- prepare pdf-pades-ltv-signature --subject candidate --trial 1
npm run eval:agents -- run pdf-pades-ltv-signature --subject candidate --trial 1
...
npm run eval:agents -- prepare docx-complex-table-topology-boundary --subject candidate --trial 1
npm run eval:agents -- run docx-complex-table-topology-boundary --subject candidate --trial 1
...
npm run eval:agents -- prepare pptx-smartart-notes-comments-boundary --subject candidate --trial 1
npm run eval:agents -- run pptx-smartart-notes-comments-boundary --subject candidate --trial 1
```

Generated PDF fixtures and the locked-corpus verifier require Python with ReportLab, pypdf, and Pillow. Artifact-producing PDF graders additionally require pdfplumber where they inspect text geometry; applicable visual oracles require `pdftoppm`. Set `OFFICE_KIT_AGENT_EVAL_PYTHON` to that evaluator interpreter and, only when it is not on `PATH`, set `OFFICE_KIT_AGENT_EVAL_PDFTOPPM`. A prepared PDF prompt binds its actual provider interpreter from `OFFICE_KIT_AGENT_EVAL_PROVIDER_PYTHON`, then legacy `OFFICE_KIT_PDF_PROVIDER_PYTHON`, then the evaluator interpreter. For a complete evaluator-supplied Poppler QA toolchain, set all of `OFFICE_KIT_AGENT_EVAL_PDFINFO`, `OFFICE_KIT_AGENT_EVAL_PDFTOPPM`, and `OFFICE_KIT_AGENT_EVAL_PDFTOTEXT`; the trial exports those exact paths and, when they share a directory, prepends only that directory for the Agent. This keeps a small parser/render evaluator separate from a policy-authorized managed specialist runtime such as pyHanko; `run.json` records the selected paths. In Codex, use the Python executable returned by `load_workspace_dependencies` for the evaluator and a managed provider path only when the case needs it.

## Locked boundary corpus

The fourteen ready locked PDF inputs and the four disclosed PAdES test-PKI
companion files live below `evals/assets/`, outside the npm payload.
`integrity.json` records every declared file's SHA-256 and byte count;
...
except for the expressly public, disposable PAdES test P12 files. Those two
files are evaluation material only, contain no production identity or secret,
and must never be installed as a trust credential or reused outside the case.
...
never executes any of them. The PAdES case uses a separate evaluator-only
pyHanko validator instead of the candidate adapter: it requires exactly one
`/ETSI.CAdES.detached` approval signature, one `/ETSI.RFC3161`
DocumentTimeStamp, an explicit test root and CRL with no network fetching, and
nonempty DSS certificates/CRLs/VRI records. The repair pair is independently checked for a raw
...
There are five ready PPTX graders. The source-bound slide-name edit uses the canonical `launch-review.pptx` two-slide fixture; the rich-notes edit uses a separate `rich-notes-review.pptx` two-slide fixture; the section-boundary edit uses a four-slide complete native-section partition fixture; the clone uses `release-review.pptx` with a deliberately small notes/comments/chart leaf profile plus one relationship-free custom-show run action; and `strategy-review.pptx` has one connected SmartArt graph plus separate review canaries.

The connected-SmartArt refusal has one explicit audit contract. The prompt
routes this exact atomic case to the installed
`officekit-connected-smartart-refusal-workflow.mjs`, which records the source
hash, no output, `officeKit.actualProvider`/`actualVersion` and
`silentFallback: false`; `target.smartArtName`, `nodeModelId`, `slide`,
`notesSlide`, and `modernCommentThread`; and public import, inspect, capability
decision, and `Presentation.verify({ visualQa: true })` preflight records. The
grader rejects renamed target keys or an audit that merely claims a successful
verification without identifying that public verification call.

The ready XLSX, DOCX, and modeled PPTX baselines are generated through the public OfficeKit facade and need no Python provider. The connected-SmartArt fixture then uses the evaluator-only explicit package patch surface to establish its deliberately opaque source graph; it is never an Agent fallback route. Their independent native visual graders require `soffice`, `pdfinfo`, and `pdftoppm`; a missing executable reports `grader-unavailable`, never a product pass or failure. The rich title/notes case requires the named title and one exact ordinary notes run edit, fixed paragraph/run counts and order, a preserved character bullet plus auto-numbered sibling paragraph, fixed slide topology/direct background/supporting text, exactly the target slide XML and notes XML as changed package parts, a byte-stable appendix, byte-bound audit provenance, second import, and LibreOffice/Poppler page evidence. The source-bound slide-name case changes only the first SlidePart's `p:cSld/@name`; it independently requires fixed topology, all non-target package parts byte-identical, source-part/name/attribute audit binding, successful second import, and pixel-identical native renders of both pages. It deliberately permits Open XML SDK canonicalization inside the changed target SlidePart rather than claiming a lexical one-attribute XML delta.
...
The following thirteen ready PDF cases have complete case-specific grading:
...
| `pdf-pades-ltv-signature` | Independent pypdf requires one visible final-page `ApprovalSignature`, `/ETSI.CAdES.detached`, one `/ETSI.RFC3161` DocumentTimeStamp, and a bounded DSS surface; evaluator-only pyHanko then validates CMS, the disclosed explicit root/CRL, approval timestamp, document timestamp, and every required LTV evidence count | Poppler/Pillow keeps page 1 pixel-identical and confines page-2 pixels to the declared approval box | The source is a strict output prefix; 2–4 bounded incremental revisions, exact root/CRL provenance, one approval and one timestamp record, and DSS certificate/CRL/VRI evidence are required. The profile is test-only and cannot claim PAdES conformance. | Public bounded test workflow or equivalent typed pyHanko sign+verify sequence; no fallback, no ad-hoc writer or secret dump, strict postflight explicit-root/CRL validation, and Poppler review |
...
| `pdf-cross-page-table-extraction` | The hidden oracle reads immutable source text with pypdf and requires the exact three-page title, merged two-row header, four labels, eight typed rows, footnote, and narrative/rotated-text exclusions in JSON and flattened CSV | Poppler/Pillow renders all source pages; the public audit must bind one nonblank bbox-overlay review for each selected segment | JSON, CSV, source, and audit hashes must agree; every cell must carry a finite in-page bbox, span, row/column identity, and ruled-grid confidence proof; source bytes stay immutable | `pdfplumber`/version, explicit `read-only`, no fallback, published ruled-table workflow, completed preflight, and Poppler overlay review; generic candidate or manual table routes fail |
| `pdf-richmedia-opaque-preservation` | Independent pypdf traversal requires exactly one page-two `/3D` and one `/RichMedia` annotation, one binary `/3DD` stream, default view and activation dictionaries, RichMedia asset/configuration/settings graph, JavaScript action, and two pinned opaque-stream hashes | Not applicable: a safe refusal writes no modified PDF | Immutable source hash, canonical audit-only output, explicit no-mutation/no-partial-artifact proof, and no assertion that the non-executed runtime remains usable | Read-only structure inspection before refusal, declared no-mutation save policy, no fallback and no mutation command |

The frozen-evaluator six-record `pdf-pades-ltv-signature` matrix passed all three candidate-Skill trials at `100/100`; copied-reference passed two of three. All records used the same candidate tarball and oracle fingerprint. The failed copied-reference trial made a valid signature and passed the cryptographic, source-prefix, DSS, timestamp, and Poppler checks, but then rewrote the canonical audit `operation` string into an object. The audit-provenance hard gate correctly forced zero rather than accepting a semantically similar but uncontracted output. The copied reference text does not independently implement this new PAdES workflow: successful runs discovered the public workflow in the same installed candidate tarball. This measures Agent/package reachability and a strict output-contract gap, not standalone reference PAdES support or a general PAdES conformance claim.

At clean commit `d72a47ef3a678024ab5ac6476a790e10e2703029`, the ruled-table case completed its first six-record repeat matrix. All three candidate-Skill trials passed `100/100`. The copied-reference Skill passed two trials at `100/100`; its third run produced the correct JSON, CSV, audit, source proof, and Poppler review, but wrote six undeclared review files under `outputs/render-evidence`, so the exact-deliverables hard gate correctly forced its score to zero. All six trials used candidate tarball SHA-256 `56486e189dee959560f7e6207c2eb83707bc7cf51acf67a9907521dcf3d003f6` and oracle fingerprint `97f27b0ca2fa5c3ee906bdfd6606e7f72318386a02ba360f9bdbf387476e120e`; the copied candidate Skill hash was `a3add101286e553eec0cb3ad65abd4de182598d0ff0ff4fb8d11f28bb6cd631a`, and the copied reference Skill hash was `0a09e468825a8be83345fd6c34e848c9c383bea66fc67e09dc36ecb5dfb2f0b1`. The reference text did not itself contain this new workflow; in each successful reference trial the Agent discovered the public workflow in the same installed candidate tarball. This records package discoverability and one output-contract compatibility gap, not byte-equivalent reference-Skill coverage or a general table-extraction claim.
...
The bounded contract deliberately rejects reply-of-reply and branched graphs. `xlsx-threaded-nested-reply-boundary` is now a locked 5,172-byte source (`46a7450657be342f5c5a360a8432da2e44da9bf914a0fffc55ce96f64f5c3c39`) with one root, one direct reply, one reply-of-reply, three person identities, one threaded-comments part, one persons part, and their exact content-type and relationship graph. Its raw-OPC oracle verifies all of those canaries without using the public model. `SpreadsheetFile.importXlsx()` intentionally exposes zero editable threads for this graph; a no-op export preserves every package-part payload even though ZIP container bytes may be rewritten. An Agent must use the public Spreadsheets import/inspect preflight and fail closed without exporting or ZIP-patching a modified workbook. The repository tests include identity/topology/content-type/relationship adversarial cases.

Frozen evaluator matrix `v24` completed six records: the candidate Skill passed `3/3` at `100/100`, and the copied reference Skill also passed `3/3` at `100/100`. Every record used candidate tarball SHA-256 `88ef8b55e43bb5f1d6039ed2ce24616de861bd9ba5d3e9ed30d206be6f3f8304`, the locked source SHA-256 above, and oracle SHA-256 `c65994c03e9147bae6a8496662fd830129b743495c7ad678f317eb12542c6d9c`. The candidate Skill hash was `9b9374c439bd6acd4f89b7f97d1ebfec070e6cb2bf9a5bf03c2442ee89345a73`; the copied reference Skill hash was `ddc38cce46b41ff29271d235ee3168bde44e6b33b0556c49fbe9cdc00be454c5`. Reference preparation changed only bare package-name occurrences in its copied `SKILL.md` and API quick-start document, then installed the same candidate tarball. All six trials preserved the source, produced only `outputs/audit.json`, and recorded public import/inspect, zero public target threads, an explicit identity-preserving nested-reply boundary, no mutation, and no fallback. This measures workflow and audit-contract compatibility on the same package, not independent implementations.
...
`docx-modern-comment-reply-boundary` is ready with a locked 3,904-byte input (`43e70c765eb7c616e521bda2e9f17727ee8ccfa186de20e3c98e599158abac00`). It contains one native Word modern-comment root, its direct reply, and a reply-of-reply, together with `commentsExtended`, `commentsIds`, `commentsExtensible`, `people`, durable IDs, fixed dates, resolved state, and one root-only body anchor. The independent raw-OPC oracle checks that whole graph and its relationships/content types without using the OfficeKit document model. The requested extra nested reply remains outside the bounded contract: an Agent must inspect through the public Documents workflow and fail closed without exporting or patching a DOCX.

At a frozen local evaluator snapshot, the six-record repeat matrix used the same source hash above, candidate tarball SHA-256 `88ef8b55e43bb5f1d6039ed2ce24616de861bd9ba5d3e9ed30d206be6f3f8304`, and oracle SHA-256 `2065ccec7c632fed99dd7b54ad016150a76baf1e59a21cbe99b234865a07296e`. The candidate Skill (`875b1fafd1d0629c7ccf4c42dba94ac4d650fbac8361a544fe9e79e2af75c6df`) passed all three trials at `100/100`. The copied reference Skill (`88ef86377ff44161ee0f19cedc5028a5520dbae30e406821ccfa2845ca8e1635`) safely refused and preserved the source in all three trials, but passed none of the complete task contracts: two scored `40/100` because their audits omitted the required typed failed-closed preflight evidence, and one scored `0/100` after also failing the package-write/trace hard gates. This is a workflow/audit-contract compatibility result, not an independent implementation comparison.

`docx-complex-table-topology-boundary` is ready with a locked 2,789-byte input (`1ba3aba2dc6d4de094cba14da25675c124bdc142c38105da432ea7b5fdf4906d`). Its second table, `ClinicalDoseMatrix`, combines one vertical merge, one directly nested table, custom table style `ClinicalProtocolTable`, one inserted revision, and a block-level `ROUTE_CONTROL` SDT. The raw-OPC oracle requires that exact topology and canaries; public import preserves both tables as source-bound and intentionally exposes no editable control for the nested table-cell SDT. Inserting the requested `Frequency` column is therefore an audit-only safe refusal: the Agent must use public import/inspect preflight, declare the unsupported capability, verify the source, write no DOCX, and never reach for ZIP/XML/package reconstruction.

Frozen local evaluator matrix `v27` at clean commit `f3f48e3f84cd975479f6a061d8f79c5162cbc1ab` completed all six records: the candidate Skill and copied-reference Skill each passed `3/3` at `100/100`, with every hard gate. Every record used candidate tarball SHA-256 `88ef8b55e43bb5f1d6039ed2ce24616de861bd9ba5d3e9ed30d206be6f3f8304`, the locked source SHA-256 above, and oracle SHA-256 `6b7f21d56e868b310d6859b172b3582e6d6e934ad60133b331785cbe6b7b1852`; the candidate Skill hash was `875b1fafd1d0629c7ccf4c42dba94ac4d650fbac8361a544fe9e79e2af75c6df` and the copied-reference Skill hash was `88ef86377ff44161ee0f19cedc5028a5520dbae30e406821ccfa2845ca8e1635`. Both subjects used the same candidate package, so this records workflow/audit-contract compatibility rather than independent implementation parity.
...
`pptx-smartart-notes-comments-boundary` is a compound safe-refusal slice, not a claim that every SmartArt, note, or comment is uneditable. The locked four-slide source gives only slide 1 a named SmartArt frame whose `dgm:relIds` bind four standard diagram parts; its data part owns one external hyperlink relationship. Slide 4 has ordinary speaker notes and a modern root/direct reply thread as independent canaries. The request deliberately combines the third node, the notes, and the reply as one atomic transaction. Its published refusal workflow binds the graph/node and review canaries through public import, semantic/package inspection, and verification, then writes only the canonical audit. The raw OPC oracle binds all four diagram relationships/content types, the three model-ID/text nodes, the connected external child relationship, notes/comments/authors graphs, and related hashes. A passing refusal must use public import/inspect/verify preflight, bind the target IDs and source hash in an audit-only result, explain that the connected SmartArt graph is source-bound, and leave no modified package or partial review edit. Repository smoke rejects relationship/node drift, audit omissions, a claimed success, direct ZIP access, and export/patch traces.

At clean commit `59cbb5794312955cb44069ccbec358adfe2d94c8`, a fresh six-record local matrix completed at `100/100`: candidate `3/3` and copied-reference `3/3`, with every hard gate passing. Every record used candidate tarball SHA-256 `4d1ec7c589c8a46544e7fc1386009eca29172f110b7791752386340056a1a018`, source SHA-256 `121e26ff58fac91f9bd3819078815c949cab3110b68fbfb3307a1adaab639fb2`, and oracle SHA-256 `65891397c1cb50fdf372cdb98acfe2555e3a380af23814e3a76f8ee1c5f6c3d9`. The candidate Skill hash was `0b6b357eb652f3a3479d461138a7e3ec7e0f1f1d73bb328951cbe837942a3593`; the copied-reference Skill hash was `fa9f6badd10fc4957b489ce10c9ccc2d4edc6357cf2396df38c447de6e7d0506`. Reference preparation changed bare package-name text in 34 isolated copied files and still installed the same candidate tarball, so this is workflow/audit-contract compatibility evidence, not an independent implementation comparison.

One clean autonomous candidate Agent trial at `0e8824cb3dac8332ff631b0cb75850ebe2a56f6b` scored `100/100`: all hard gates plus 6 machine, 2 visual, 2 security, and 7 trace checks passed. The Agent selected the published workflow, supplied the required closed-leaf opt-in, and did not write scratch OOXML. The run bound tarball SHA-256 `b9759b4ef9712958c335712166887a2dd96d5b4eaa6b1b5fba1127b10b1fcac6`, copied-Skill SHA-256 `971096a78fec3c01c178cae76d6a458712a1b40df344a724cadde3b3acc6a9ff`, prompt SHA-256 `2bd75b885f0d4fa0b36e3395b82bcde17fe559088c54f9b9c78a54d3bdc908ea`, input SHA-256 `fc3b4ce9bbfbf101cea046e30c9c8a7550e9165771874061536ed131f15ef63e`, and oracle SHA-256 `0e8a68981b16824d464e86bacf3744bab45593ef451812b458e36923e9f4225d`. The source worktree was clean. This is one trial, not the default three-repeat matrix or a reference-Skill comparison.
