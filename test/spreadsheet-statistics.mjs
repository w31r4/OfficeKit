...
orderSheet.getRange("B1:B23").formulas = [
  ...
  ["=RANK.AVG(2,A1:A7,1)"],
  ["=RANK.AVG(5,A1:A7,0)"],
  ["=PERCENTILE.EXC(A1:A7,0.5)"],
  ["=PERCENTILE.EXC(A1:A7,0.125)"],
  ["=PERCENTILE.EXC(A1:A7,0.124)"],
  ["=PERCENTILE.EXC(A1:A7,0.875)"],
  ["=PERCENTILE.EXC(A1:A7,0.876)"],
  ["=QUARTILE.EXC(A1:A7,1)"],
  ["=QUARTILE.EXC(A1:A7,2)"],
  ["=QUARTILE.EXC(A1:A7,3)"],
  ["=QUARTILE.EXC(A1:A7,0)"],
  ["=QUARTILE.EXC(A1:A7,4)"],
  ["=QUARTILE.EXC(A1:A7,1.9)"],
  ["=TRIMMEAN(A1:A7,0.2)"],
  ["=TRIMMEAN(A1:A7,0.3)"],
  ["=TRIMMEAN(A1:A7,0.99)"],
  ["=TRIMMEAN(A1:A7,1)"],
...
orderSheet.getRange("D1").formulas = [["=MODE.MULT(A1:A7)"]];
orderSheet.getRange("F1").formulas = [["=MODE.MULT(1,2,3)"]];
...
const orderZip = await JSZip.loadAsync(new Uint8Array(await orderXlsx.arrayBuffer()));
const orderWorksheetXml = await orderZip.file("xl/worksheets/sheet3.xml").async("text");
assert.match(orderWorksheetXml, /_xlfn\.RANK\.AVG\(2,A1:A7,0\)/);
assert.match(orderWorksheetXml, /_xlfn\.PERCENTILE\.EXC\(A1:A7,0\.5\)/);
assert.match(orderWorksheetXml, /_xlfn\.QUARTILE\.EXC\(A1:A7,1\)/);
assert.match(orderWorksheetXml, /_xlfn\.MODE\.MULT\(A1:A7\)/);
...
assert.equal(importedOrderSheet.store.get("D1").formula, "=MODE.MULT(A1:A7)");
