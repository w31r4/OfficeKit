...
{
    [Fact]
    public void PpjSourceBoundThemeAccent1RgbaEditsOnlyAccent1AndReprojects()
    {
        var authoredRequest = ExportRequest();
        var authoredTheme = new PresentationThemeArtifact();
        authoredTheme.AccentRgb.Add(new[] { "11223380", "223344", "334455", "445566", "556677", "667788" });
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            {
                SourceUri = "deck.assets/source/theme-accent1.pptx",
            },
        });
        ...
        Assert.Equal("#11223380", projectedColors["accent1"]!.GetValue<string>());
        Assert.Equal("#223344", projectedColors["accent2"]!.GetValue<string>());
        ...
        Assert.Contains(nativeRef["capabilities"]!.AsArray(), capability =>
            capability!["operation"]!.GetValue<string>() == "setThemeAccent1Color" &&
            capability["fields"]!.AsArray().Any(field => field!.GetValue<string>() == "accentColors.accent1"));
        ...
        editedProgram["design"]!["theme"]!["accentColors"]!["accent1"] = "#ABCDEF40";
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            Assert.Equal("ABCDEF", scheme.Accent1Color!.RgbColorModelHex!.Val!.Value);
            Assert.Equal(25098, scheme.Accent1Color.RgbColorModelHex.GetFirstChild<A.Alpha>()!.Val!.Value);
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/theme-accent1.pptx",
            },
        });
        Assert.True(reprojected.Ok, Diagnostics(reprojected));
        var reprojectedProgram = JsonNode.Parse(reprojected.PresentationProgram.ProgramJson.ToByteArray())!.AsObject();
        Assert.Equal("#ABCDEF40", reprojectedProgram["design"]!["theme"]!["accentColors"]!["accent1"]!.GetValue<string>());

        var alphaPresenceProgram = JsonNode.Parse(projectedProgram.ToJsonString())!.AsObject();
        alphaPresenceProgram["design"]!["theme"]!["accentColors"]!["accent1"] = "#ABCDEF";
        ...
        unauthorizedProgram["design"]!["theme"]!["accentColors"]!["accent2"] = "#AABBCC";
        ...
        tamperedProgram["design"]!["theme"]!["nativeRef"]!["capabilities"]!.AsArray()
            .Single(capability => capability!["operation"]!.GetValue<string>() == "setThemeAccent1Color")!["fields"] = new JsonArray("accentColors.accent2");
