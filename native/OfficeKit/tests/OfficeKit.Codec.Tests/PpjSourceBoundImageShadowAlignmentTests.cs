...
{
    [Fact]
    public void PpjSourceBoundImageShadowAlignmentEditsLeafAndReprojects()
    {
        ...
        image["shadow"]!.AsObject()["alignment"] = "br";
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/source/image-shadow-alignment.pptx",
            },
        });
        ...
        Assert.Equal("br", projectedImage["shadow"]!["alignment"]!.GetValue<string>());
        Assert.True(projectedImage["shadow"]!["rotateWithShape"]!.GetValue<bool>());
        var alignmentLeaves = projectedImage["nativeRef"]!["leaves"]!.AsArray()
            .Select(leaf => leaf!.AsObject())
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "imageShadowAlignment")
            .ToArray();
        Assert.Single(alignmentLeaves);
        Assert.Equal("br", alignmentLeaves[0]["value"]!.GetValue<string>());

        alignmentLeaves[0]["value"] = "tl";
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            var picture = package.PresentationPart!.SlideParts
                ...
                .Single(item => item.NonVisualPictureProperties?.NonVisualDrawingProperties?.Name?.Value == "evidence identity");
            var shadow = Assert.IsType<A.OuterShadow>(Assert.Single(picture.ShapeProperties!.GetFirstChild<A.EffectList>()!.ChildElements));
            Assert.Equal(A.RectangleAlignmentValues.TopLeft, shadow.Alignment!.Value);
            Assert.True(shadow.RotateWithShape!.Value);
            Assert.Equal(76_200U, shadow.BlurRadius!.Value);
            Assert.Equal(38_100U, shadow.Distance!.Value);
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/image-shadow-alignment.pptx",
            },
        });
        ...
        var reprojectedImage = reprojectedProgram["pages"]![0]!["elements"]!.AsArray()
            .Select(item => item!.AsObject())
            .Single(item => item["name"]!.GetValue<string>() == "evidence identity");
        Assert.Equal("tl", reprojectedImage["shadow"]!["alignment"]!.GetValue<string>());
        Assert.True(reprojectedImage["shadow"]!["rotateWithShape"]!.GetValue<bool>());
        Assert.Equal("tl", reprojectedImage["nativeRef"]!["leaves"]!.AsArray().Single(leaf =>
            leaf!["kind"]!.GetValue<string>() == "imageShadowAlignment")!["value"]!.GetValue<string>());
    }
}
