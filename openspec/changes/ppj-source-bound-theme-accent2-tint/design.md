# Design: source-bound accent2 tint

## Context

The existing source-preserving theme path already recognizes one direct
accent1 transform at a time and patches its existing DrawingML token. This
change applies the same ownership boundary to accent2 without broadening the
...
- Accent2 transforms other than `tint`, transforms on accent3 through accent6,
...
1. **Use the existing native transform field.**
   ...
   DrawingML 0..100000 integer and the authored compiler already emits it. The
   importer records a role of `accent2`; the projector converts by dividing by
   100000, and the source-bound compiler rounds by
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:tint`. The source writer patches `Accent2Color` only when the authored
   transform is the one existing accent2 tint and rejects all other transform
   flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent2Tint` is accepted only with
   `accentTransforms.accent2.tint` in the source-issued capability. This keeps
