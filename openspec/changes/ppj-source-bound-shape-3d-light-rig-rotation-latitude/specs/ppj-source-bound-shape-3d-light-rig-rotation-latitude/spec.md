# ppj-source-bound-shape-3d-light-rig-rotation-latitude Specification

## ADDED Requirements

### Requirement: Strict direct light-rig rotation latitude projection

The presentation projection MUST expose
`shape3dSceneLightRigRotationLatitude60000` only when a shape has one direct
...
#### Scenario: Project a light-rig rotation latitude
...
- **THEN** exactly one `shape3dSceneLightRigRotationLatitude60000` native leaf
...
- **WHEN** the PPTX is projected to PPJ
- **THEN** no `shape3dSceneLightRigRotationLatitude60000` leaf is emitted

### Requirement: Source-bound light-rig rotation latitude token splice

The PPJ edit plan MUST permit
`shape3dSceneLightRigRotationLatitude60000` only when the source-bound shape
...
#### Scenario: Edit and reproject light-rig rotation latitude

- **GIVEN** a projected strict scene owner with `lat="0"`
...
- **WHEN** an edit targets `shape3dSceneLightRigRotationLatitude60000`
