...
{
    [Fact]
    public void PpjSourceBoundThemeAccent1GreenModEditsOnlyGreenModAndReprojects()
    {
        ...
        {
            Role = "accent1",
            GreenModulationThousandth = 35_000,
        });
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            {
                SourceUri = "deck.assets/source/theme-accent1-greenMod.pptx",
            },
        });
        ...
        var projectedTransform = projectedTheme["accentTransforms"]!["accent1"]!;
        Assert.Equal(0.35, projectedTransform["greenMod"]!.GetValue<double>());
        var nativeRef = Assert.IsType<JsonObject>(projectedTheme["nativeRef"]);
        Assert.Contains(nativeRef["capabilities"]!.AsArray(), capability =>
            capability!["operation"]!.GetValue<string>() == "setThemeAccent1GreenMod" &&
            capability["fields"]!.AsArray().Any(field => field!.GetValue<string>() == "accentTransforms.accent1.greenMod"));
        ...
        editedProgram["design"]!["theme"]!["accentTransforms"]!["accent1"]!["greenMod"] = 0.4;
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            Assert.Equal("112233", accent1.Val!.Value);
            Assert.Equal(40_000, Assert.Single(accent1.Elements<A.GreenModulation>()).Val!.Value);
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/theme-accent1-greenMod.pptx",
            },
        });
        Assert.True(reprojected.Ok, Diagnostics(reprojected));
        var reprojectedProgram = JsonNode.Parse(reprojected.PresentationProgram.ProgramJson.ToByteArray())!.AsObject();
        Assert.Equal(0.4, reprojectedProgram["design"]!["theme"]!["accentTransforms"]!["accent1"]!["greenMod"]!.GetValue<double>());

        var unauthorizedProgram = JsonNode.Parse(projectedProgram.ToJsonString())!.AsObject();
        unauthorizedProgram["design"]!["theme"]!["accentTransforms"]!["accent1"]!["greenMod"] = 0.8;
        unauthorizedProgram["design"]!["theme"]!["nativeRef"]!["capabilities"]!.AsArray()
            .Single(capability => capability!["operation"]!.GetValue<string>() == "setThemeAccent1GreenMod")!["fields"] = new JsonArray("accentTransforms.accent1.tint");
        ...
        deletionProgram["design"]!["theme"]!["accentTransforms"]!["accent1"]!.AsObject().Remove("greenMod");
        ...
        siblingProgram["design"]!["theme"]!["accentTransforms"]!["accent1"]!["tint"] = 0.1;
        ...
        outOfRangeProgram["design"]!["theme"]!["accentTransforms"]!["accent1"]!["greenMod"] = 1.1;
