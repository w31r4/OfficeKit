# Design: source-bound accent5 lumMod

## Context

The existing source-preserving theme path recognizes one direct accent1
transform and the direct accent5 tint/shade leaves at a time, then patches the
existing DrawingML token. This change applies the same ownership boundary to
accent5 luminance modulation without broadening the theme graph or changing
...
- Accent5 transforms other than `lumMod`, transforms on accent5 through
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:lumMod`. The source writer patches `Accent5Color` only when the authored
   transform is the one existing accent5 lumMod and rejects all other
   transform flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent5LumMod` is accepted only with
   `accentTransforms.accent5.lumMod` in the source-issued capability. This
