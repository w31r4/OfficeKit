...
change applies the same ownership boundary to accent3 saturation offset without
...
- Accent3 transforms other than `satOff`, transforms on accent4 through
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:satOff`. The source writer patches `Accent3Color` only when the authored
   transform is the one existing accent3 satOff and rejects all other transform
   flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent3SatOff` is accepted only with
   `accentTransforms.accent3.satOff` in the source-issued capability. This
