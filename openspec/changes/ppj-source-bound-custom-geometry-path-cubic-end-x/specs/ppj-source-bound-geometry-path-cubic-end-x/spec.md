# Custom geometry cubic end-point x coordinate

## ADDED Requirements

### Requirement: expose one bounded cubic end-point x coordinate

The system MUST expose a `customGeometryPathCubicEndX` native leaf only for a
direct ordered `a:path/a:cubicBezTo` with exactly three direct `a:pt` children,
where the third point's `x` is a canonical signed integer within the bounded
...
- **WHEN** an imported custom geometry contains a recognized path with a
  literal cubic end-point x coordinate
- **THEN** the projected shape exposes that coordinate with path order in
  ...
  control points, the paired end-point y, and the rest of the path graph remain
  source-owned

#### Scenario: edit only the selected end-point x token

- **WHEN** a source-bound PPJ edit changes an issued cubic end-point x leaf to
  another canonical bounded coordinate
- **THEN** only that third point's `x` value changes in the owning SlidePart and
