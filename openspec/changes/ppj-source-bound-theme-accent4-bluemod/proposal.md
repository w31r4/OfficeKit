...
modulation transforms. A direct imported accent4 `a:blueMod` remains opaque,
...
- Project a canonical shared ThemePart whose accent4 direct RGB leaf contains
  ...
  `design.theme.accentTransforms.accent4.blueMod`, using a 0..1 fraction.
- Issue `setThemeAccent4BlueMod` for that field and compile one authorized
...
- `presentation-theme-accent4-bluemod`: Strict source-bound projection and
  source-preserving editing of one direct accent4 `a:blueMod` leaf.
