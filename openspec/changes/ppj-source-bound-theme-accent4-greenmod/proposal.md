...
modulation transforms. A direct imported accent4 `a:greenMod` remains opaque,
...
- Project a canonical shared ThemePart whose accent4 direct RGB leaf contains
  ...
  `design.theme.accentTransforms.accent4.greenMod`, using a 0..1 fraction.
- Issue `setThemeAccent4GreenMod` for that field and compile one authorized
...
- `presentation-theme-accent4-greenmod`: Strict source-bound projection and
  source-preserving editing of one direct accent4 `a:greenMod` leaf.
