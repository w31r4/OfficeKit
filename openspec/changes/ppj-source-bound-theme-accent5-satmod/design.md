...
transform and the direct accent5 tint, shade, luminance, and alpha leaves at a
time, then patches the existing DrawingML token. This change applies the same
ownership boundary to accent5 saturation modulation without broadening the
...
- Recognize only a canonical shared ThemePart whose accent5 color is a direct
  six-digit RGB leaf with one direct `a:satMod/@val` child.
...
- Accent5 transforms other than `satMod`, transforms on accent5 through
  accent6, inherited or effect theme state, or arbitrary theme XML editing.
- Creating or deleting a missing satMod node, changing multiple theme fields,
  or claiming PowerPoint host color rendering.

## Decisions

1. **Use the existing native transform field.**
   `PresentationThemeColorTransform.SaturationModulationThousandth` already
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:satMod`. The source writer patches `Accent5Color` only when the authored
   transform is the one existing accent5 satMod and rejects all other
   transform flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent5SatMod` is accepted only with
   `accentTransforms.accent5.satMod` in the source-issued capability. This
