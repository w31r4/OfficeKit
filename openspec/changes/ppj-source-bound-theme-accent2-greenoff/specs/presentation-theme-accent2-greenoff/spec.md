...
direct accent2 green-channel offset token while preserving the rest of an
imported theme.

## ADDED Requirements

### Requirement: Strict source-bound accent2 greenOff projection

The PPTX projector MUST expose
`design.theme.accentTransforms.accent2.greenOff` and
`setThemeAccent2GreenOff` only when the canonical shared ThemePart has one
direct accent2 RGB leaf with exactly one direct `a:greenOff/@val` child, no
...
#### Scenario: Project a direct accent2 greenOff leaf
...
- **THEN** the theme contains `accentTransforms.accent2.greenOff` equal to
  `-0.25`
- **AND** the native reference contains `setThemeAccent2GreenOff` authorized
  for `accentTransforms.accent2.greenOff`

#### Scenario: Reject unsupported accent2 greenOff topology
...
- **THEN** the source-bound accent2 greenOff field and capability are omitted

### Requirement: Source-bound accent2 greenOff editing

The source-preserving compiler MUST accept one authorized change to the
existing `design.theme.accentTransforms.accent2.greenOff` field, convert a
...
#### Scenario: Edit and reproject one accent2 greenOff token

- **GIVEN** a projected direct accent2 greenOff owner at `-0.25`
...
#### Scenario: Fail closed for unsafe accent2 greenOff edits

- **GIVEN** a source-bound accent2 greenOff owner
