...
{
    [Fact]
    public void PpjSourceBoundThemeAccent2AlphaOffEditsOnlyAlphaOffAndReprojects()
    {
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            {
                SourceUri = "deck.assets/source/theme-accent2-alphaoff.pptx",
            },
        });
        ...
        Assert.Equal(-0.25, projectedTransform["alphaOff"]!.GetValue<double>());
        var nativeRef = Assert.IsType<JsonObject>(projectedTheme["nativeRef"]);
        Assert.Contains(nativeRef["capabilities"]!.AsArray(), capability =>
            capability!["operation"]!.GetValue<string>() == "setThemeAccent2AlphaOff" &&
            capability["fields"]!.AsArray().Any(field => field!.GetValue<string>() == "accentTransforms.accent2.alphaOff"));
        ...
        editedProgram["design"]!["theme"]!["accentTransforms"]!["accent2"]!["alphaOff"] = 0.5;
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            Assert.Equal(50_000, Assert.Single(accent2.Elements<A.AlphaOffset>()).Val!.Value);
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/theme-accent2-alphaoff.pptx",
            },
        });
        Assert.True(reprojected.Ok, Diagnostics(reprojected));
        var reprojectedProgram = JsonNode.Parse(reprojected.PresentationProgram.ProgramJson.ToByteArray())!.AsObject();
        Assert.Equal(0.5, reprojectedProgram["design"]!["theme"]!["accentTransforms"]!["accent2"]!["alphaOff"]!.GetValue<double>());

        var unauthorizedProgram = JsonNode.Parse(projectedProgram.ToJsonString())!.AsObject();
        unauthorizedProgram["design"]!["theme"]!["accentTransforms"]!["accent2"]!["alphaOff"] = 0.6;
        unauthorizedProgram["design"]!["theme"]!["nativeRef"]!["capabilities"]!.AsArray()
            .Single(capability => capability!["operation"]!.GetValue<string>() == "setThemeAccent2AlphaOff")!["fields"] = new JsonArray("accentTransforms.accent2.shade");
        ...
        deletionProgram["design"]!["theme"]!["accentTransforms"]!["accent2"]!.AsObject().Remove("alphaOff");
        ...
        outOfRangeProgram["design"]!["theme"]!["accentTransforms"]!["accent2"]!["alphaOff"] = 1.01;
