...
function invalid(message, code = "invalid_spreadsheet_pivot") {
  return new OfficeKitCodecError(message, [], { code });
}
...
export function wireWorksheetPivots(workbook, sheet, state, cells) {
  ...
  for (const slot of state?.slots || []) {
    ...
    if (snapshot(slot.pivot.sourceValues()) !== slot.sourceValuesSnapshot ||
      ...
      throw invalid(`Imported PivotTable ${slot.pivot.name}, its source data, and its cached output are read-only in the first OfficeKit native profile.`, "unsupported_spreadsheet_pivot_edit");
    }
    output.push(pivotSnapshot(slot.pivot) === slot.publicSnapshot
      ? slot.wire
      : wireSourceBoundPivotRefreshOnLoadHardening(slot));
  }
  if (state && remaining.size) throw invalid(`Source-bound worksheet ${sheet.name} cannot add PivotTables in the first OfficeKit native profile.`, "unsupported_spreadsheet_pivot_edit");
