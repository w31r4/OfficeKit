...
direct accent3 alpha modulation token while preserving the rest of an imported
theme.

## ADDED Requirements

### Requirement: Strict source-bound accent3 alphaMod projection

The PPTX projector MUST expose
`design.theme.accentTransforms.accent3.alphaMod` and
`setThemeAccent3AlphaMod` only when the canonical shared ThemePart has one
direct accent3 RGB leaf with exactly one direct `a:alphaMod/@val` child, no
...
#### Scenario: Project a direct accent3 alphaMod leaf
...
- **THEN** the theme contains `accentTransforms.accent3.alphaMod` equal to `0.25`
- **AND** the native reference contains `setThemeAccent3AlphaMod` authorized for
  `accentTransforms.accent3.alphaMod`

#### Scenario: Reject unsupported accent3 alphaMod topology
...
- **THEN** the source-bound accent3 alphaMod field and capability are omitted

### Requirement: Source-bound accent3 alphaMod editing

The source-preserving compiler MUST accept one authorized change to the
existing `design.theme.accentTransforms.accent3.alphaMod` field, convert a
...
#### Scenario: Edit and reproject one accent3 alphaMod token

- **GIVEN** a projected direct accent3 alphaMod owner at `0.25`
...
#### Scenario: Fail closed for unsafe accent3 alphaMod edits

- **GIVEN** a source-bound accent3 alphaMod owner
