...
Use this style when the brief matches **due diligence; consulting presentation**. It is designed for
executives, strategy teams, decision makers. Its visual premise is Red editorial marks turn a dense dossier into a visible decision path. The style is a
...
**question → hypothesis → framework → evidence → implication → recommendation → action**. This style is not a good fit for **festival campaign**;
...
- Use **#FFFDF8** as the main field, **#20201E** for primary type, **#B63A42** for the decisive signal, **#85736E** for secondary evidence, and **#E8E1D8** for rules or context.
...
- The recurring structure is **redline, variable page silhouettes, native data graphics**. Vary the silhouette between
  cover, evidence, data, process, comparison, and decision pages rather than
  repeating a component grid.
- Use fine annotation marks and red editorial signals to show what changed or needs a decision. Let the underlying paper field and type carry most of the hierarchy.
...
- **Page skeleton.** Treat the deck as an editorial dossier: figure number and complete judgment first, chart or table second, notes and method immediately below. Use a generous outer margin and let a red annotation mark identify the decision-relevant change.
- **Type rhythm.** Pair a readable serif body layer with a neutral sans for titles, chart labels, and navigation. Figure captions should be stronger than notes but weaker than the main assertion.
- **Evidence carriers.** Use driver trees, waterfalls, ranked comparisons, scenario tables, annotated before/after bars, and a phased roadmap. Every framework must answer the question stated in its caption.
- **Images and layers.** Keep photography to a real operating context or article opener. Never let a stock image compete with the evidence field; cite and crop any supplied image deliberately.
- **Pacing.** Alternate article-like explanation with figure-led proof. Let recommendation pages grow denser than the opener, but close with implementation conditions rather than a slogan.
- **Selection rule.** Choose this style when a complex diligence or strategy question must resolve into a visible recommendation and its conditions.

## Signature moves and variation limits

- Let **redline** recur as a quiet identity cue on selected pages; it must
