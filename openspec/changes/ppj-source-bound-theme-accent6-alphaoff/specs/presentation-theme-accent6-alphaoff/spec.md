...
direct accent6 alpha offset token while preserving the rest of an imported
theme.

## ADDED Requirements

### Requirement: Strict source-bound accent6 alphaOff projection

The PPTX projector MUST expose
`design.theme.accentTransforms.accent6.alphaOff` and
`setThemeAccent6AlphaOff` only when the canonical shared ThemePart has one
direct accent6 RGB leaf with exactly one direct `a:alphaOff/@val` child, no
...
#### Scenario: Project a direct accent6 alphaOff leaf
...
- **THEN** the theme contains `accentTransforms.accent6.alphaOff` equal to
  `-0.25`
- **AND** the native reference contains `setThemeAccent6AlphaOff` authorized
  for `accentTransforms.accent6.alphaOff`

#### Scenario: Reject unsupported accent6 alphaOff topology
...
- **THEN** the source-bound accent6 alphaOff field and capability are omitted

### Requirement: Source-bound accent6 alphaOff editing

The source-preserving compiler MUST accept one authorized change to the
existing `design.theme.accentTransforms.accent6.alphaOff` field, convert a
...
#### Scenario: Edit and reproject one accent6 alphaOff token

- **GIVEN** a projected direct accent6 alphaOff owner at `-0.25`
...
#### Scenario: Fail closed for unsafe accent6 alphaOff edits

- **GIVEN** a source-bound accent6 alphaOff owner
