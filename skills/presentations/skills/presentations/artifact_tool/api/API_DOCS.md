...
- Put non-visible PowerPoint title/description on meaningful ordinary shapes, connectors, groups, tables, and charts with `accessibility`; for imports, require the object's `accessibilityCapability.editable` before `setAccessibilityMetadata(...)`. A chart's visible title is separate.
