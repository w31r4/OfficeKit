# ppj-source-bound-shape-3d-backdrop-up-dx Specification

## ADDED Requirements

### Requirement: Strict direct backdrop up-vector X projection

The presentation projection MUST expose `shape3dSceneBackdropUpDxEmu` only
...
The leaf value MUST be the up vector's `dx` coordinate. Existing sibling 3-D
owners MAY remain and MUST be preserved.

#### Scenario: Project a backdrop up-vector X coordinate
...
- **THEN** exactly one `shape3dSceneBackdropUpDxEmu` native leaf is emitted
  with value `0`
...
- **WHEN** the PPTX is projected to PPJ
- **THEN** no `shape3dSceneBackdropUpDxEmu` leaf is emitted

### Requirement: Source-bound backdrop up-vector X token splice

The PPJ edit plan MUST permit `shape3dSceneBackdropUpDxEmu` only when the
...
replace only `a:backdrop/a:up/@dx` in the owning SlidePart, preserve all other
scene and package content, and allow the new coordinate to be recovered on a
second projection.

#### Scenario: Edit and reproject backdrop up-vector X

- **GIVEN** a projected strict backdrop owner with `up dx="0"`
...
- **WHEN** an edit targets `shape3dSceneBackdropUpDxEmu`
