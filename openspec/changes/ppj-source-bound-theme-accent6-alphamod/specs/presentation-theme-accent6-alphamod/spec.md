...
direct accent6 alpha modulation token while preserving the rest of an imported
theme.

## ADDED Requirements

### Requirement: Strict source-bound accent6 alphaMod projection

The PPTX projector MUST expose
`design.theme.accentTransforms.accent6.alphaMod` and
`setThemeAccent6AlphaMod` only when the canonical shared ThemePart has one
direct accent6 RGB leaf with exactly one direct `a:alphaMod/@val` child, no
...
#### Scenario: Project a direct accent6 alphaMod leaf
...
- **THEN** the theme contains `accentTransforms.accent6.alphaMod` equal to `0.25`
- **AND** the native reference contains `setThemeAccent6AlphaMod` authorized for
  `accentTransforms.accent6.alphaMod`

#### Scenario: Reject unsupported accent6 alphaMod topology
...
- **THEN** the source-bound accent6 alphaMod field and capability are omitted

### Requirement: Source-bound accent6 alphaMod editing

The source-preserving compiler MUST accept one authorized change to the
existing `design.theme.accentTransforms.accent6.alphaMod` field, convert a
...
#### Scenario: Edit and reproject one accent6 alphaMod token

- **GIVEN** a projected direct accent6 alphaMod owner at `0.25`
...
#### Scenario: Fail closed for unsafe accent6 alphaMod edits

- **GIVEN** a source-bound accent6 alphaMod owner
