...
assert.match(buildPilotPrompt({ manifest, ...buildPilotMatrix(manifest, { taskId: "business-review-01", arm: "C", trial: 1 })[0] }), /minimumBodyFontSize[\s\S]*22[\s\S]*minimumCaptionFontSize[\s\S]*20[\s\S]*four distinct composition silhouettes[\s\S]*cardWallPattern[\s\S]*replace at least one repeated card surface[\s\S]*parallel comparison[\s\S]*no unrecorded design warnings[\s\S]*intentionalWarnings[\s\S]*dominant reading anchor[\s\S]*contrast[\s\S]*pale card surface[\s\S]*palette\.strict false[\s\S]*closed brand palette[\s\S]*every allowed emitted color[\s\S]*presentation\.validateLayout[\s\S]*quantitative claim[\s\S]*Compose nodes[\s\S]*slide\.compose[\s\S]*do not pass a compose node[\s\S]*head[\s\S]*from[\s\S]*tail[\s\S]*bringToFront/i);
