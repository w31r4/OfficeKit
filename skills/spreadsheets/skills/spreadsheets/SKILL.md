...
- `examples/officekit-robust-statistics-workflow.mjs`: Run or adapt this `Data` / `Analysis` / `Checks` example for tied average rank, exclusive percentiles/quartiles, multi-mode spill results, and a symmetric trimmed mean with independent formula reconciliations, SVG review, and two canonical OfficeKit roundtrips. The workflow uses standard `_xlfn.` package spellings for post-2010 Excel functions while OfficeKit calculation accepts both prefixed and bare names.
