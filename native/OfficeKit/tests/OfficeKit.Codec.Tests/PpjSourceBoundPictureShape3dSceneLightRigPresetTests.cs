...
{
    [Fact]
    public void PpjSourceBoundPictureShape3dSceneLightRigPresetLeafEditsAndReprojects()
    {
        ...
        var source = AddPictureShape3dSceneLightRigPreset(RemoveEmbeddedPpj(authored.File.ToByteArray()), "threePt");
        var projected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/source/picture-shape-3d-scene-light-rig-preset.pptx",
            },
        });
        ...
        Assert.Contains("rig=\"soft\"", editedXml, StringComparison.Ordinal);
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            Assert.Equal("soft", lightRig.GetAttribute("rig", string.Empty).Value);
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/picture-shape-3d-scene-light-rig-preset.pptx",
            },
        });
    ...
    private static byte[] AddPictureShape3dSceneLightRigPreset(byte[] source, string preset)
    {
