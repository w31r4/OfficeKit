# Design: source-bound accent6 lumMod

## Context

The existing source-preserving theme path recognizes one direct accent1
transform and the direct accent6 tint/shade leaves at a time, then patches the
existing DrawingML token. This change applies the same ownership boundary to
accent6 luminance modulation without broadening the theme graph or changing
...
- Accent6 transforms other than `lumMod`, transforms on other accent roles,
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:lumMod`. The source writer patches `Accent6Color` only when the authored
   transform is the one existing accent6 lumMod and rejects all other
   transform flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent6LumMod` is accepted only with
   `accentTransforms.accent6.lumMod` in the source-issued capability. This
