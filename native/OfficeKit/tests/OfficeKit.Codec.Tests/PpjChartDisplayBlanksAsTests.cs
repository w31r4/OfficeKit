...
{
    [Fact]
    public void PpjChartDisplayBlanksAsAuthorAndEditSourceChart()
    {
        ...
        {
            ["displayBlanksAs"] = new JsonObject { ["kind"] = "string", ["value"] = "span" },
        };
        ...
        chart["displayBlanksAs"] = new JsonObject { ["token"] = "displayBlanksAs" };
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest { SourceUri = "display-blanks-as/source.pptx" },
        });
        ...
        var projectedChart = projectedProgram["pages"]!.AsArray()
            ...
            .Select(item => item!.AsObject())
            .Single(item => item["type"]!.GetValue<string>() == "chart");
        Assert.Equal("span", projectedChart["displayBlanksAs"]!.GetValue<string>());

        projectedProgram["design"]!["grammar"]!["tokens"] = new JsonObject
        {
            ["displayBlanksAsEdited"] = new JsonObject { ["kind"] = "string", ["value"] = "zero" },
        };
        projectedChart["displayBlanksAs"] = new JsonObject { ["token"] = "displayBlanksAsEdited" };
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            Family = ArtifactFamily.Presentation,
            File = edited.File,
            PresentationProgram = new PresentationProgramRequest { SourceUri = "display-blanks-as/edited.pptx" },
        });
        ...
        var reprojectedChart = reprojectedProgram["pages"]!.AsArray()
            ...
            .Select(item => item!.AsObject())
            .Single(item => item["type"]!.GetValue<string>() == "chart");
        Assert.Equal("zero", reprojectedChart["displayBlanksAs"]!.GetValue<string>());
    }
}
