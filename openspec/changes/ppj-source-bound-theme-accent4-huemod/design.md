...
transform and direct accent4 tint, shade, luminance, alpha, saturation, red,
and blue transform leaves at a time, then patches the existing DrawingML token.
This change applies the same ownership boundary to accent4 hue modulation
...
- Accent4 transforms other than `hueMod`, transforms on other accent roles, inherited or effect theme state, or arbitrary theme XML editing.
...
1. **Use the existing native transform field.**
   ...
   emits it. The importer records a role of `accent4`; the projector converts
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:hueMod`. The source writer patches `Accent4Color` only when the authored
   transform is the one existing accent4 hueMod and rejects all other transform
   flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent4HueMod` is accepted only with
   `accentTransforms.accent4.hueMod` in the source-issued capability. This
