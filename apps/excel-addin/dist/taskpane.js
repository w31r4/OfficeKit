...
const workbookName = byId("workbook-name");
...
async function refresh() {
    ...
    try {
        const client = await workbookDescriptor();
        const response = await requestJson(`/v1/browser/sessions/${encodeURIComponent(activeSession.id)}/refresh`, { client });
        activeSession = response.session;
        showSession(activeSession);
    }
    catch (error) {
...
async function workbookDescriptor() {
...
async function executeOperation(request) {
    try {
        ...
        const result = await Excel.run(async (context) => executeWithContext(context, request.operation, request.args));
        if (jsonByteLength(result) > maxResultBytes) {
            const error = new Error(`Excel operation result exceeds the ${maxResultBytes}-byte bridge limit.`);
...
function requireOperationCapability(operation, args) {
    ...
    if (!Office.context.requirements.isSetSupported(requirement.set, requirement.version)) {
        const error = new Error(`${operation} requires ${requirement.set} ${requirement.version} or later in this Excel client.`);
    ...
    if (operation === "update_sheet_view" && args.zoom !== undefined && !Office.context.requirements.isSetSupported("ExcelApiDesktop", "1.1")) {
        const error = new Error("update_sheet_view.zoom requires ExcelApiDesktop 1.1 or later in this Excel client.");
        error.code = "unsupported-capability";
        throw error;
    }
}
...
async function readRanges(context, args) {
    const sheet = context.workbook.worksheets.getItem(args.sheet);
    const ranges = args.ranges.map((address) => sheet.getRange(address));
    ...
    if (cells > maxReadCells)
        throw boundedOperationError("read_ranges", maxReadCells, "cells");
    for (const range of ranges)
        range.load(include.join(","));
    await context.sync();
    return {
        sheet: args.sheet,
...
async function searchWorkbook(context, args) {
    const options = args.options ?? {};
    const maximum = Math.min(Number.isSafeInteger(options.maxResults) ? options.maxResults : 100, 500);
    const worksheets = context.workbook.worksheets;
    worksheets.load("items/name");
    await context.sync();
    const ranges = worksheets.items.map((sheet) => sheet.getUsedRangeOrNullObject());
    for (const range of ranges)
        range.load("isNullObject,rowCount,columnCount,text,address");
    await context.sync();
    let cells = 0;
    for (const range of ranges) {
    ...
    if (cells > maxSearchCells)
        throw boundedOperationError("search_workbook", maxSearchCells, "cells");
    for (const range of ranges)
        if (!range.isNullObject)
            range.load("text");
    await context.sync();
...
async function listItems(context, args) {
    if (args.kind === "worksheets")
        return readSheetsMetadata(context);
    if (args.kind === "tables") {
        const tables = context.workbook.tables;
        tables.load("items/name,items/id,items/showHeaders,items/showTotals");
        await context.sync();
        return { kind: args.kind, items: tables.items.map((item) => ({ name: item.name, id: item.id, showHeaders: item.showHeaders, showTotals: item.showTotals })) };
    }
    if (args.kind === "pivotTables") {
        const pivots = context.workbook.pivotTables;
        pivots.load("items/name,items/id");
        await context.sync();
        return { kind: args.kind, items: pivots.items.map((item) => ({ name: item.name, id: item.id })) };
    }
    if (args.kind === "names") {
        const names = context.workbook.names;
        names.load("items/name,items/formula,items/value");
        await context.sync();
        return { kind: args.kind, items: names.items.map((item) => ({ name: item.name, formula: item.formula, value: item.value })) };
    }
    const sheets = context.workbook.worksheets;
    sheets.load("items/name");
    await context.sync();
    const collections = sheets.items.map((sheet) => sheet.charts);
    for (const collection of collections)
        collection.load("items/name,items/id");
    await context.sync();
    return {
        kind: args.kind,
        items: collections.flatMap((collection, index) => collection.items.map((item) => ({ sheet: sheets.items[index].name, name: item.name, id: item.id }))),
    };
}
async function writeRange(context, args) {
...
async function clearRange(context, args) {
...
async function readRangeImage(context, args) {
    const range = sheetRange(context, args);
    range.load("rowCount,columnCount");
    await context.sync();
...
async function readSheetsMetadata(context) {
    const sheets = context.workbook.worksheets;
    sheets.load("items/name,items/id,items/position,items/visibility");
    await context.sync();
    return { sheets: sheets.items.map((sheet) => ({ name: sheet.name, id: sheet.id, position: sheet.position, visibility: sheet.visibility })) };
}
async function resizeRange(context, args) {
    const range = sheetRange(context, args);
    if (args.columnWidth !== undefined)
        range.format.columnWidth = args.columnWidth;
    if (args.rowHeight !== undefined)
        range.format.rowHeight = args.rowHeight;
...
async function updateSheetView(context, args) {
    const sheet = context.workbook.worksheets.getItem(args.sheet);
    if (args.freezeRows !== undefined || args.freezeColumns !== undefined) {
        sheet.freezePanes.unfreeze();
        if (args.freezeRows)
            sheet.freezePanes.freezeRows(args.freezeRows);
        if (args.freezeColumns)
            sheet.freezePanes.freezeColumns(args.freezeColumns);
    }
    if (args.showGridlines !== undefined)
        sheet.showGridlines = args.showGridlines;
    if (args.zoom !== undefined)
        context.workbook.application.activeWindow.zoom = args.zoom;
...
async function chart(context, args) {
    const sheet = context.workbook.worksheets.getItem(args.sheet);
    if (args.action === "create") {
        ...
        if (args.position?.start && args.position?.end)
            created.setPosition(args.position.start, args.position.end);
    }
    else {
        ...
        if (args.action === "delete")
            ...
            if (args.title !== undefined)
                existing.title.text = args.title;
            if (args.sourceRange !== undefined)
                existing.setData(sheet.getRange(args.sourceRange), Excel.ChartSeriesBy.auto);
            if (args.position?.start && args.position?.end)
                existing.setPosition(args.position.start, args.position.end);
        }
    }
    await context.sync();
    return { action: args.action, sheet: args.sheet, name: args.name };
}
async function table(context, args) {
    const sheet = context.workbook.worksheets.getItem(args.sheet);
    if (args.action === "create") {
        ...
        if (args.action === "add_rows")
            existing.rows.add(null, args.rows);
        if (args.action === "delete_rows") {
            for (let index = 0; index < args.count; index += 1)
                existing.rows.getItemAt(args.index).delete();
        }
    }
    await context.sync();
    return { action: args.action, sheet: args.sheet, name: args.name };
}
async function pivotTable(context, args) {
    const pivots = context.workbook.pivotTables;
    if (args.action === "create")
        pivots.add(args.name, args.source, `${args.sheet}!${args.destination}`);
    else {
...
function sheetRange(context, args) {
...
async function submitResult(sessionId, requestId, result) {
    let payload = { requestId, ...result };
...
function boundedOperationError(operation, limit, unit) {
    const error = new Error(`${operation} exceeds the ${limit}-${unit} live operation limit.`);
    error.code = "response-too-large";
    return error;
}
...
function detectedWebView() {
    const userAgent = typeof navigator === "undefined" ? "" : navigator.userAgent;
    if (/WebView2|Edg\//iu.test(userAgent))
        return "webview2-or-edge";
    if (/AppleWebKit/iu.test(userAgent))
        return "webkit";
    return "unknown";
}
function cellAddress(usedRangeAddress, rowOffset, columnOffset) {
...
async function parseResponse(response) {
    ...
    if (!response.ok)
        throw new Error(payload?.error?.message ?? `OfficeKit bridge returned ${response.status}.`);
    return payload;
}
function setSetup(message, canConnect) {
...
function byId(id) {
    ...
    if (element == null)
        throw new Error(`Missing OfficeKit element: ${id}`);
    return element;
}
