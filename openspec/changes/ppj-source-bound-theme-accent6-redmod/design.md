...
the same ownership boundary to accent6 red-channel modulation without
...
- Accent6 transforms other than `redMod`, transforms on accent6 through
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:redMod`. The source writer patches `Accent6Color` only when the authored
   transform is the one existing accent6 redMod and rejects all other transform
   flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent6RedMod` is accepted only with
   `accentTransforms.accent6.redMod` in the source-issued capability. This
