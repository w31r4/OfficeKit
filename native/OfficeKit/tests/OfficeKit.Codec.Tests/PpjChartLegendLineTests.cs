...
{
    [Fact]
    public void PpjChartLegendLineAuthorAndEditSourceChart()
    {
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest { SourceUri = "legend-line/source.pptx" },
        });
        ...
        var projectedChart = projectedProgram["pages"]!.AsArray()
            ...
            .Select(item => item!.AsObject())
            .Single(item => item["type"]!.GetValue<string>() == "chart");
        var projectedLine = projectedChart["style"]!["legendLine"]!;
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            Family = ArtifactFamily.Presentation,
            File = edited.File,
            PresentationProgram = new PresentationProgramRequest { SourceUri = "legend-line/edited.pptx" },
        });
        ...
        var reprojectedChart = reprojectedProgram["pages"]!.AsArray()
            ...
            .Select(item => item!.AsObject())
            .Single(item => item["type"]!.GetValue<string>() == "chart");
        var reprojectedLine = reprojectedChart["style"]!["legendLine"]!;
