...
{
    [Fact]
    public void PpjSourceBoundCustomGeometryPathCubicControl2XEditsAndReprojects()
    {
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            {
                SourceUri = "deck.assets/source/custom-geometry-path-cubic-control2-x.pptx",
            },
        });
        ...
        var cubicLeaves = projectedShape["nativeRef"]!["leaves"]!.AsArray()
            ...
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "customGeometryPathCubicControl2X")
            .ToArray();
        var target = Assert.Single(cubicLeaves);
        Assert.Equal(500, target["value"]!.GetValue<long>());
        target["value"] = 550;
        ...
        Assert.Contains("x=\"550\" y=\"600\"", editedXml, StringComparison.Ordinal);
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/custom-geometry-path-cubic-control2-x.pptx",
            },
        });
        ...
        var reprojectedLeaves = reprojectedShape["nativeRef"]!["leaves"]!.AsArray()
            ...
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "customGeometryPathCubicControl2X")
            .ToArray();
        Assert.Equal(550, Assert.Single(reprojectedLeaves)["value"]!.GetValue<long>());
    }
}
