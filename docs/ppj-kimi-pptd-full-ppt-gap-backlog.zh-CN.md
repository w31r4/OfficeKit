...
本轮后续增量（2026-09-06）：`textField.automatic` 已进入 PPJ/schema/wire，并在当前有界 profile 中支持 `type: "slidenum"`、默认 `type: "datetime"` 与 `type: "datetime1"`。`slidenum` 的 authoring/source-bound 缓存按所属 slide 序号刷新；`datetime` 与 `datetime1` 写入并回读 host-refreshable 标记及缓存文本，其中 `datetime1` 请求宿主使用 MM/DD/YYYY 格式；OfficeKit 不伪造本机时间。PPJ 二次投影恢复 `automatic: true`；`datetime2` 等日期格式、author 等未覆盖的自动字段和完整字段指令图继续 fail closed。
