# Shape 3-D bottom-bevel height

## ADDED Requirements

### Requirement: Project one direct bottom-bevel height

The system MUST expose one `shape3dBevelBottomHeightEmu` native leaf only for
an ordinary shape with exactly one direct `a:sp3d` whose only child is a direct
`a:bevelB` and whose `h` value is a canonical non-negative 32-bit coordinate.

#### Scenario: Project a recognized bottom bevel height

- **WHEN** an imported shape contains a direct `a:sp3d/a:bevelB/@h="800"`
  with only bounded root and bevel attributes
- **THEN** PPJ exposes `shape3dBevelBottomHeightEmu` with value `800`
- **AND** the projection preserves the other bevel and 3-D attributes as
  source state.

### Requirement: Edit only the bottom-bevel height token

The system MUST allow a source-bound edit to replace only the proven
`a:bevelB/@h` attribute in the owning SlidePart.

#### Scenario: Reproject an edited height

- **WHEN** the `shape3dBevelBottomHeightEmu` leaf changes from `800` to `1600`
...
- **THEN** the projection emits no `shape3dBevelBottomHeightEmu` leaf
