...
Use this style when the brief matches **annual operations report; work presentation**. It is designed for
operators, managers, cross-functional teams. Its visual premise is Warm ivory and jade structure guide a mature annual brief. The style is a
...
**current state → signal → cause → trade-off → intervention → owner/date → follow-through**. This style is not a good fit for **youth campaign**;
...
- Use **#F2F0E7** as the main field, **#222222** for primary type, **#B7A530** for the decisive signal, **#6B7E65** for secondary evidence, and **#C8C5B9** for rules or context.
...
- The recurring structure is **jade, variable page silhouettes, native data graphics**. Vary the silhouette between
  cover, evidence, data, process, comparison, and decision pages rather than
  repeating a component grid.
- Use a measured green signal for progress and stewardship. Combine it with warm neutral surfaces and fine rules; avoid turning every positive value green.
...
- **Page skeleton.** Use a warm ivory annual-report field with a small navigation micro-header. Keep ESG or data sections on white, mark the current period with amber, and reserve stronger fields for section transitions.
- **Type rhythm.** Pair sans-serif titles, labels, and figures with a serif reading layer for longer narrative. Never replace a dense but readable paragraph with a tiny sans wall.
- **Evidence carriers.** Use composition donuts only for genuine shares, stacked bars for period mix, financial tables with a highlighted current column, and a small set of trend charts.
- **Images and layers.** Use real people, place, or operating photographs for narrative pages. Keep image backgrounds separate from tables and source notes; no image should hide a period label.
- **Pacing.** Let navigation and current-period emphasis repeat quietly while page silhouettes change between narrative, table, chart, and close. Amber is one current signal, not a general brand fill.
- **Selection rule.** Choose this style for annual reports, ESG reviews, and mature operating updates that need continuity across many pages.

## Signature moves and variation limits

- Let **jade** recur as a quiet identity cue on selected pages; it must
