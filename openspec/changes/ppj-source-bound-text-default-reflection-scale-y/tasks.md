...
- [x] 1.1 Add `textDefaultReflectionScaleY` to the PPJ schema, native leaf
      registry, capability reference, and generated manual; verify JSON and
      maintainer contract checks.
- [x] 1.2 Project an existing paragraph default reflection `sy` as a signed
...
- [x] 2.2 Splice only `a:reflection/@sy` in the owning SlidePart; verify `sx`,
      fadeDir, sibling effects, text topology, and non-target package parts
      remain unchanged.
