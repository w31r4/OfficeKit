...
accent1 plus accent2 tint, shade, and luminance modulation. A direct imported
accent2 `a:lumOff` remains opaque, leaving one concrete F-15 transform field
...
- Project a canonical shared ThemePart whose accent2 direct RGB leaf contains
  ...
  `design.theme.accentTransforms.accent2.lumOff`, using a -1..1 fraction.
- Issue `setThemeAccent2LumOff` for that field and compile one authorized
...
- `presentation-theme-accent2-lumoff`: Strict source-bound projection and
  source-preserving editing of one direct accent2 `a:lumOff` leaf.
