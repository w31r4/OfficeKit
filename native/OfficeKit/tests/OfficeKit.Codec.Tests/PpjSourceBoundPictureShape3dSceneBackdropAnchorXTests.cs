...
{
    [Fact]
    public void PpjSourceBoundPictureShape3dSceneBackdropAnchorXLeafEditsAndReprojects()
    {
        ...
        var source = AddPictureShape3dSceneBackdropAnchorX(RemoveEmbeddedPpj(authored.File.ToByteArray()), "0");
        var projected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/source/picture-shape-3d-scene-backdrop-anchor-x.pptx",
            },
        });
        ...
        var target = Assert.Single(projectedImage["nativeRef"]!["leaves"]!.AsArray()
            ...
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "shape3dSceneBackdropAnchorXEmu"));
        ...
        Assert.Contains("<a:anchor x=\"500\" y=\"100\" z=\"200\"", editedXml, StringComparison.Ordinal);
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            Assert.Equal("500", anchor.GetAttribute("x", string.Empty).Value);
            Assert.Equal("100", anchor.GetAttribute("y", string.Empty).Value);
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/picture-shape-3d-scene-backdrop-anchor-x.pptx",
            },
        });
    ...
    private static byte[] AddPictureShape3dSceneBackdropAnchorX(byte[] source, string anchorX)
    {
        ...
        using (var package = PresentationDocument.Open(stream, true))
        {
            ...
            anchor.SetAttribute(new OpenXmlAttribute("x", string.Empty, anchorX));
