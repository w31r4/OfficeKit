...
本轮新增显式 `design.theme.fontScheme.major/minor` authored 字段；它们优先于 `design.fonts` 的位置约定，写入真实 `a:majorFont`/`a:minorFont`。其中 `major` 提供 Latin 和 complex-script 槽位，新增的可选 `majorEastAsia` 只覆盖 major 的 East Asian 槽位，缺省时回落到 `major`；缺少整个显式 scheme 时仍保留旧回退。source-bound 不把主题 XML 猜成可写能力。
...
**仍缺：** 完整 `theme1.xml` color/font/effect scheme、更多颜色变换、继承后的 effect style、minor 与更完整的逐脚本字体回退、字体嵌入/替换、复杂语言字体映射的其余组合、WordArt 和宿主字体回退差异；当前 color roles、alpha、tint/shade、major East Asian slot 与复杂脚本 direct owner 都只覆盖 PPJ 的有限 authored/native lowering，不会改写任意 imported theme XML。
