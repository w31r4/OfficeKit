...
This capability gives PPJ a safe, field-qualified source-bound owner for one
 direct accent5 hue-modulation token while preserving the rest of an imported theme.

## ADDED Requirements

### Requirement: Strict source-bound accent5 hueMod projection

The PPTX projector MUST expose
`design.theme.accentTransforms.accent5.hueMod` and
`setThemeAccent5HueMod` only when the canonical shared ThemePart has one
direct accent5 RGB leaf with exactly one direct `a:hueMod/@val` child, no
...
#### Scenario: Project a direct accent5 hueMod leaf

- **GIVEN** a canonical imported ThemePart with accent5 `334455` and
  `a:hueMod/@val="35000"`
- **WHEN** the presentation is projected to PPJ
- **THEN** the theme contains `accentTransforms.accent5.hueMod` equal to
  `0.35`
- **AND** the native reference contains `setThemeAccent5HueMod` authorized
  for `accentTransforms.accent5.hueMod`

#### Scenario: Reject unsupported accent5 hueMod topology
...
- **THEN** the source-bound accent5 hueMod field and capability are omitted

### Requirement: Source-bound accent5 hueMod editing

The source-preserving compiler MUST accept one authorized change to the
existing `design.theme.accentTransforms.accent5.hueMod` field, convert a
...
#### Scenario: Edit and reproject one accent5 hueMod token

- **GIVEN** a projected direct accent5 hueMod owner at `0.35`
...
#### Scenario: Fail closed for unsafe accent5 hueMod edits

- **GIVEN** a source-bound accent5 hueMod owner
