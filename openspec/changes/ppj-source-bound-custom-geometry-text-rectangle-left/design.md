...
- Give a fully literal, in-frame text-rectangle left edge a stable native leaf
...
- Prove both the authored private-guide profile and a SlidePart-only edit with
  a second projection.

**Non-Goals:**

- Editing top/right/bottom edges, guide references, guide formulas, handles,
...
1. **Use one native index for the left edge.** The leaf represents the one
...
3. **Preserve representation.** A direct source keeps `a:rect/@l` numeric. An
   authored/private source keeps `l="officeKitTextLeft"` and changes only the
...
- A left-edge change can alter text layout inside a custom shape. Restrict the
