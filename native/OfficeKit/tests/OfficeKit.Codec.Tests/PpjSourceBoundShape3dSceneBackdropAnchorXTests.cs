...
{
    [Fact]
    public void PpjSourceBoundShape3dSceneBackdropAnchorXLeafEditsAndReprojects()
    {
        var authored = Invoke(ExportRequest());
        Assert.True(authored.Ok, Diagnostics(authored));
        var source = AddShape3dSceneBackdropAnchorX(RemoveEmbeddedPpj(authored.File.ToByteArray()), "100");
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            {
                SourceUri = "deck.assets/source/shape-3d-scene-backdrop-anchor-x.pptx",
            },
        });
        ...
        Assert.Contains("<a:anchor x=\"200\" y=\"200\" z=\"300\"", editedXml, StringComparison.Ordinal);
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/shape-3d-scene-backdrop-anchor-x.pptx",
            },
        });
        ...
        var reprojectedTarget = Assert.Single(reprojectedShape["nativeRef"]!["leaves"]!.AsArray()
            .Select(leaf => leaf!.AsObject())
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "shape3dSceneBackdropAnchorXEmu"));
        Assert.Equal(200, reprojectedTarget["value"]!.GetValue<long>());
    }

    private static byte[] AddShape3dSceneBackdropAnchorX(byte[] source, string anchorX)
    {
