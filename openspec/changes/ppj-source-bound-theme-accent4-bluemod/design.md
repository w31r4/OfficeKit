...
This change applies the same ownership boundary to accent4 blue-channel modulation
...
- Accent4 transforms other than `blueMod`, transforms on other accent roles, inherited or effect theme state, or arbitrary theme XML editing.
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:blueMod`. The source writer patches `Accent4Color` only when the authored
   transform is the one existing accent4 blueMod and rejects all other transform
   flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent4BlueMod` is accepted only with
   `accentTransforms.accent4.blueMod` in the source-issued capability. This
