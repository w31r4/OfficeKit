...
Use this style when the brief matches **annual portfolio report; finance presentation**. It is designed for
investors, finance leaders, analysts. Its visual premise is Warm paper and rotating chapter signals turn reporting into a yearbook. The style is a
...
**stance → drivers → evidence → scenarios → risks → decision or allocation**. This style is not a good fit for **minimal luxury**;
...
- Use **#F3EFE3** as the main field, **#17191A** for primary type, **#2B64B0** for the decisive signal, **#CB642D** for secondary evidence, and **#D8D1C4** for rules or context.
...
- The recurring structure is **tabs, variable page silhouettes, documentary imagery**. Vary the silhouette between
  cover, evidence, data, process, comparison, and decision pages rather than
  repeating a component grid.
- Use small chapter tabs or indexed marks to help a reader navigate a long report. Keep them sparse and subordinate to the current conclusion.
...
- **Page skeleton.** Use warm paper as the ordinary ground, heavy black titles, and a chapter accent that rotates only at genuine section boundaries. Special dark or colored pages carry the annual stance; body pages carry evidence.
- **Type rhythm.** Use a strong sans for titles and giant figures with a regular reading face for supporting copy. Highlight short phrases, not whole paragraphs, and keep units beside numbers.
- **Evidence carriers.** Use bar/line, stacked composition, tables, timelines, and occasional small multiples. Match the chart type to the question and keep a dense table or comparison when the evidence requires it.
- **Images and layers.** Use photography or illustration only for a chapter, sample, or human context. Do not let an image replace a financial basis or turn every page into a magazine spread.
- **Pacing.** Annual reports need a long arc: highlights → diagnosis → chapters → outlook. Rotate the chapter signal but keep typography, source treatment, and data discipline stable.
- **Selection rule.** Choose this style for yearbooks, annual performance reviews, and mixed narrative/data reports.

## Signature moves and variation limits

- Let **tabs** recur as a quiet identity cue on selected pages; it must
