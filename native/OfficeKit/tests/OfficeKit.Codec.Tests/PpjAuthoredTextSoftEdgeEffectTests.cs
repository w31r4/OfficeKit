...
{
    [Fact]
    public void PpjAuthoredTextSoftEdgeEffectWritesRunAndDefaultRunOwners()
    {
        ...
        runStyle["softEdge"] = new JsonObject { ["radius"] = 8 };
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            Assert.Collection(
                ...
                reflection => Assert.IsType<A.Reflection>(reflection),
                softEdge =>
                {
                    var native = Assert.IsType<A.SoftEdge>(softEdge);
        ...
        var recoveredRunSoftEdge = recoveredProgram["pages"]!.AsArray().SelectMany(page => page!["elements"]!.AsArray())
            .Select(item => item!.AsObject())
            .Single(item => item["id"]!.GetValue<string>() == "claim-title")["text"]!["paragraphs"]![0]!["runs"]![0]!["style"]!["softEdge"]!;
        Assert.Equal(8, recoveredRunSoftEdge["radius"]!.GetValue<double>());
        var recoveredDefaultSoftEdge = recoveredProgram["pages"]!.AsArray().SelectMany(page => page!["elements"]!.AsArray())
            .Select(item => item!.AsObject())
            .Single(item => item["id"]!.GetValue<string>() == "claim-title")["text"]!["paragraphs"]![0]!["style"]!["defaultText"]!["softEdge"]!;
        Assert.Equal(4, recoveredDefaultSoftEdge["radius"]!.GetValue<double>());

        var invalidProgram = program.DeepClone().AsObject();
        invalidProgram["pages"]!.AsArray().SelectMany(page => page!["elements"]!.AsArray())
            .Select(item => item!.AsObject())
            .Single(item => item["id"]!.GetValue<string>() == "claim-title")["text"]!["paragraphs"]![0]!["runs"]![0]!["style"]!["softEdge"]!["radius"] = 1001;
