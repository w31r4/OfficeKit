...
{
    [Fact]
    public void PpjSourceBoundTextBodyColumnGapEditsLeafAndReprojects()
    {
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            Assert.Equal(2 * 12_700, bodyProperties.ColumnSpacing!.Value);
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/source/text-body-column-gap.pptx",
            },
        });
        Assert.True(projected.Ok, Diagnostics(projected));
        var projectedProgram = JsonNode.Parse(projected.PresentationProgram.ProgramJson.ToByteArray())!.AsObject();
        var projectedShape = projectedProgram["pages"]![0]!["elements"]!.AsArray().Single()!.AsObject();
        Assert.Equal(2, projectedShape["textStyle"]!["columnGap"]!.GetValue<double>(), 3);
        var leaves = projectedShape["nativeRef"]!["leaves"]!.AsArray()
            .Select(leaf => leaf!.AsObject())
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "textBodyColumnGapEmu")
            .ToArray();
        Assert.Single(leaves);
        Assert.Equal(2 * 12_700, leaves[0]["value"]!.GetValue<long>());

        leaves[0]["value"] = 4 * 12_700;
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/text-body-column-gap.pptx",
            },
        });
        ...
        Assert.Equal(4, reprojectedShape["textStyle"]!["columnGap"]!.GetValue<double>(), 3);
        Assert.Equal(4 * 12_700, reprojectedShape["nativeRef"]!["leaves"]!.AsArray().Single(leaf =>
            leaf!["kind"]!.GetValue<string>() == "textBodyColumnGapEmu")["value"]!.GetValue<long>());
    }
}
