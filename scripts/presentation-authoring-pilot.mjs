...
function buildPilotPrompt({ manifest, task, arm, trial, armOrder }) {
  const route = manifest.arms[arm].routeInstruction;
  return `You are a fresh OfficeKit Agent context in a blinded authoring pilot.\n\nTask: ${task.goal}\nScenario: ${task.scenario}\nTrial: ${trial}\nArm route: ${route}\nArm order token: ${armOrder.join(",")}\n\nUse only the installed public OfficeKit Skills and the office-kit package. Work from the sentence above, create a useful multi-page PPTX, and keep the task durable: use officekit repl, commit a reviewed draft, start a fresh REPL process to continue one local edit, review again, and publish outputs/result.pptx. The deck must be created with typed OfficeKit primitives and reimported before delivery. Use visualReview: unavailable when no visual tool exists; never claim visual completion. Do not use @oai/artifact-tool, Python, HTML/PPTD, raw OOXML, ZIP/XML patching, another writer, or a silent fallback. Do not ask a question unless a missing answer changes the audience or conclusion. Finish only after the published output exists.`;
}
