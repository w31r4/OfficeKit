...
A direct imported accent5 `a:greenOff` remains opaque, leaving one concrete F-15
...
- Project a canonical shared ThemePart whose accent5 direct RGB leaf contains
  ...
  `design.theme.accentTransforms.accent5.greenOff`, using a -1..1 fraction.
- Issue `setThemeAccent5GreenOff` for that field and compile one authorized
...
- `presentation-theme-accent5-greenoff`: Strict source-bound projection and
  source-preserving editing of one direct accent5 `a:greenOff` leaf.
