...
export const DEFAULT_BRIEF = Object.freeze({
  ...
  summary: "The core authoring, OfficeKit codec, package, and render gates are stable. Remaining work is concentrated in environment-specific compatibility and release operations.",
  evidence: [
...
export async function createDocument(outputPath, spec = DEFAULT_BRIEF) {
  ...
  assert.ok(recommendation, "OfficeKit must re-import the recommendation paragraph");
  ...
  assert.equal(
    ...
    true,
    "OfficeKit must advertise an explicit recommendation text capability",
  );
  assert.ok(
    imported.resolve(`${recommendation.id}/text`),
    "OfficeKit must resolve the advertised recommendation text range",
  );

  const evidence = imported.blocks.find((block) => block.kind === "table");
  assert.ok(evidence, "OfficeKit must re-import the evidence table");
  ...
  assert.ok(trackedInsertion, "OfficeKit must re-import the tracked insertion");
...
if (entry === import.meta.url) {
  const outputPath = path.resolve(process.argv[2] || "officekit-decision-brief.docx");
