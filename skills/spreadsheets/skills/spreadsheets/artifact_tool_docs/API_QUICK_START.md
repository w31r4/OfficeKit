...
- For bounded order and robust statistics, `RANK.AVG` averages tied positions; `PERCENTILE.INC`/`QUARTILE.INC` use inclusive interpolation; `PERCENTILE.EXC`/`QUARTILE.EXC` reject endpoints that cannot be interpolated; `MODE.MULT` spills every tied mode vertically and returns `#N/A` when no value repeats; and `TRIMMEAN` removes an even count symmetrically from both tails. Reference text, logicals, and blanks are ignored while formula errors and invalid selectors fail closed. Use the bare function names shown in Excel; OfficeKit Codec writes their required XLSX future-function prefixes and removes them again on import. Keep sources bounded and query `workbook.help("fx.RANK.AVG")` for the exact contract.
...
- For source-aware formula audits, `ROW()`/`COLUMN()` report the current cell's 1-based position, or accept one explicit single-cell reference. `ISFORMULA` distinguishes formula-backed cells, and `FORMULATEXT` returns their stored formula text or `#N/A` for a non-formula cell. Ranges, spills, computed matrices, and invalid input return `#VALUE!`; use `workbook.help("fx.FORMULATEXT")` before attempting any source-bound edit.
...
- `PMT`, `IPMT`, `PPMT`, `CUMIPMT`, `CUMPRINC`, `PV`, `FV`, `NPER`, `RATE`, `SLN`, `SYD`, `DB`, `DDB`, `NPV`, `MIRR`, `XNPV`, `IRR`, and `XIRR` calculate in the JavaScript model as well as being written to XLSX. Put rates, loan terms, cost/salvage/life assumptions, cash-flow vectors, and dates in visible input cells; do not hide them as magic numbers in a long formula.
...
- `CUMIPMT` / `CUMPRINC` accept `rate, nper, pv, start_period, end_period, type` and sum the corresponding `IPMT` / `PPMT` values across an inclusive discrete period range. All six arguments are required: rate and present value must be positive, `start_period` / `end_period` must be ordered integers from 1 through `nper`, the ending period is capped at 9,999, and `type` is 0 or 1. Use them to reconcile a visible amortization schedule rather than hiding an aggregate loan cost.
...
- `PV` accepts `rate, nper, pmt, [fv], [type]`; `FV` accepts `rate, nper, pmt, [pv], [type]`; and `NPER` accepts `rate, pmt, pv, [fv], [type]`. `PV`/`FV` require a positive finite term and preserve standard cash-flow signs. `NPER` uses the same equation in closed form and may validly return zero or a negative period count for the supplied signs; a zero payment at zero rate or an invalid real solution returns `#NUM!`.
- `SLN` accepts `cost, salvage, life` for straight-line expense. `SYD` accepts `cost, salvage, life, period` for one sum-of-years-digits period. `DB` accepts `cost, salvage, life, period, [month]` and preserves its native three-decimal fixed-declining rate with optional first/final partial-year proration. `DDB` accepts `cost, salvage, life, period, [factor]`, defaults factor to `2`, and caps expense at the remaining amount above salvage. `SYD`/`DB`/`DDB` require nonnegative cost/salvage, salvage no greater than cost, integer useful life/periods through 9,999, and valid month/factor values where applicable; invalid inputs fail as `#VALUE!` or `#NUM!` rather than being coerced.
...
- `MIRR(values, finance_rate, reinvest_rate)` requires 2 through 10,000 finite cash flows containing both signs and two rates greater than `-1`. It discounts negative flows at the finance rate, compounds positive flows to the final period at the reinvestment rate, and returns one modified periodic return; keep both assumptions visible instead of implying a single host-selected rate.
- Return formulas require both a positive and a negative finite cash flow. `IRR`/`XIRR` use an optional finite guess (default `0.1`) and return `#NUM!` if they cannot converge to a valid rate greater than `-1`; `MIRR` returns `#NUM!` for an invalid sign/rate profile. Do not present any such error as an investment result.
...
- Recognized imported native PivotTables are inspectable but their config, source values, cached output, and package topology are read-only in this profile. The sole exception is `pivot.disableRefreshOnLoad()`: only when `pivot.sourceCapabilities.refreshOnLoadHardenable` is true may it turn an explicitly present, uniquely owned cache's `refreshOnLoad` from true to false. It re-proves cache residual/source bindings and does not run or otherwise block manual, macro, external-data, or host-triggered refreshes. Exact item filters and absolute whole-day date filters (`dateEqual` through `dateNotBetween`, with ISO date bounds) on native axes are supported. Relative-clock, sub-day, grouping, calculated/other-condition filters, more than 8 row fields, multiple column fields, compact/subtotal-bearing multi-row graphs, more than 32 values, and source-bound additions fail closed rather than becoming a lossy worksheet reconstruction.
- Treat native rendering and native resaving as different compatibility claims. The shipped workbook renders the date-filtered result through LibreOffice/Poppler, but LibreOffice resave drops that advanced date filter and recalculates the Pivot unfiltered. OfficeKit's own export/import preserves the filter; do not route a workbook through LibreOffice resave when that semantic must remain intact.
