...
- Owner: `p:sp/p:spPr/a:custGeom/a:pathLst/a:path/a:cubicBezTo/a:pt[0]/@x`.
- PPJ surface: `shape.nativeRef.leaves[]`.
- Native leaf: `customGeometryPathCubicControl1X`.
...
- The geometry must pass the existing bounded custom-geometry recognition
  ...
  attribute-only, and the first control-point x must be a canonical signed
...
one end point, the canonical numeric first-control-point x token, and the
requested changed bounded coordinate. The writer then replaces only the
selected `x` value token on the first point.
...
- No line-to, move-to, quadratic, cubic second-control-point or end-point,
  first-control-point y, formula/reference, path property, or multi-path
  topology edits.
...
cubic command, removes embedded PPJ, projects the cubic first-control-point x
