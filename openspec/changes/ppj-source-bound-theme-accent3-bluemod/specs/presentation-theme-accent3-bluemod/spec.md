...
direct accent3 blue-channel modulation token while preserving the rest of an
imported theme.

## ADDED Requirements

### Requirement: Strict source-bound accent3 blueMod projection

The PPTX projector MUST expose
`design.theme.accentTransforms.accent3.blueMod` and
`setThemeAccent3BlueMod` only when the canonical shared ThemePart has one
direct accent3 RGB leaf with exactly one direct `a:blueMod/@val` child, no
...
#### Scenario: Project a direct accent3 blueMod leaf

- **GIVEN** a canonical imported ThemePart with accent3 `334455` and
  `a:blueMod/@val="25000"`
- **WHEN** the presentation is projected to PPJ
- **THEN** the theme contains `accentTransforms.accent3.blueMod` equal to
  `0.25`
- **AND** the native reference contains `setThemeAccent3BlueMod` authorized
  for `accentTransforms.accent3.blueMod`

#### Scenario: Reject unsupported accent3 blueMod topology
...
- **THEN** the source-bound accent3 blueMod field and capability are omitted

### Requirement: Source-bound accent3 blueMod editing

The source-preserving compiler MUST accept one authorized change to the
existing `design.theme.accentTransforms.accent3.blueMod` field, convert a
...
#### Scenario: Edit and reproject one accent3 blueMod token

- **GIVEN** a projected direct accent3 blueMod owner at `0.25`
...
#### Scenario: Fail closed for unsafe accent3 blueMod edits

- **GIVEN** a source-bound accent3 blueMod owner
