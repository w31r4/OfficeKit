...
{
    [Fact]
    public void PpjSourceBoundPictureShape3dSceneLightRigDirectionLeafEditsAndReprojects()
    {
        ...
        var source = AddPictureShape3dSceneLightRigDirection(RemoveEmbeddedPpj(authored.File.ToByteArray()), "t");
        var projected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/source/picture-shape-3d-scene-light-rig-direction.pptx",
            },
        });
        ...
        var target = Assert.Single(projectedImage["nativeRef"]!["leaves"]!.AsArray()
            ...
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "shape3dSceneLightRigDirection"));
        Assert.Equal("t", target["value"]!.GetValue<string>());
        target["value"] = "br";
        ...
        Assert.Contains("<a:lightRig", editedXml, StringComparison.Ordinal);
        Assert.Contains("rig=\"threePt\"", editedXml, StringComparison.Ordinal);
        Assert.Contains("dir=\"br\"", editedXml, StringComparison.Ordinal);
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            Assert.Equal("br", lightRig.GetAttribute("dir", string.Empty).Value);
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/picture-shape-3d-scene-light-rig-direction.pptx",
            },
        });
        ...
        var reprojectedTarget = Assert.Single(reprojectedImage["nativeRef"]!["leaves"]!.AsArray()
            .Select(leaf => leaf!.AsObject())
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "shape3dSceneLightRigDirection"));
        Assert.Equal("br", reprojectedTarget["value"]!.GetValue<string>());
    }

    private static byte[] AddPictureShape3dSceneLightRigDirection(byte[] source, string direction)
    {
        ...
        using (var package = PresentationDocument.Open(stream, true))
        {
            ...
            lightRig.SetAttribute(new OpenXmlAttribute("dir", string.Empty, direction));
