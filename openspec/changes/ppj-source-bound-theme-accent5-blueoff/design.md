...
This change applies the same ownership boundary to accent5 blue-channel offset
...
- Accent5 transforms other than `blueOff`, transforms on other accent roles, inherited or effect theme state, or arbitrary theme XML editing.
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:blueOff`. The source writer patches `Accent5Color` only when the authored
   transform is the one existing accent5 blueOff and rejects all other transform
   flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent5BlueOff` is accepted only with
   `accentTransforms.accent5.blueOff` in the source-issued capability. This
