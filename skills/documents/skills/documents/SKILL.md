...
- Public `document.addSection({ lineNumbering: { countBy: 5, start: 0, distance: 360, restart: "newPage" } })` authors canonical `w:lnNumType` before each text column. `countBy` defaults to 1; native `start` is zero-based; `distance` uses twentieths of a point; restart may be `newPage`, `newSection`, or `continuous`. Use `paragraphFormat: { suppressLineNumbers: true }` on a paragraph or named paragraph style to exclude its lines from display/calculation; explicit `false` overrides inherited style suppression and omission inherits. `keepNext`, boolean `keepLinesTogether`, boolean `widowControl`, and `pageBreakBefore` are separate native paragraph pagination directives; they do not calculate pages. Canonical imported direct/style leaves are editable; irregular suppression/section/pagination leaves remain source-owned and fail closed → `tasks/sections_layout.md`, `artifact_tool/API_QUICK_START.md`
