...
- Give a direct literal `a:ahXY/@maxX` bound a stable native index and explicit
  EMU value.
- Keep guide identity, the paired minimum bound, y bounds, position, handle
...
- Editing minimum bounds, y bounds, handle position, guide identity, adjustment
...
3. **Keep the paired range graph intact.** The writer proves the direct
   `ahXY` attribute structure and changes only `maxX`; it does not add/remove
   the range or edit `minX`, current adjustment, or other bounds. The focused
