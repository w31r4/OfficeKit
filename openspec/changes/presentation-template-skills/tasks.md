...
- [x] 1.3 Update the existing template search contract tests with one v3 candidate and one v2 rejection.
...
- [x] 2.4 Add one creator create/update contract flow to the existing template tests.
...
- [x] 3.3 Remove Grid defaults, fixed-layout fallback, source materialization, and old template-edit instructions from the presentation path.
...
- [x] 5.1 Delete all bundled presentation reference PPTX files and legacy previews.
- [x] 5.2 Delete embedded Grid modules, fixed layouts, registries, screenshots, support scripts, and fallback guidance.
- [x] 5.3 Update template provenance, package inventory, third-party notices, and old round-trip claims without changing DOCX/XLSX templates.

## 6. Release evidence

- [x] 6.1 Add package scans for the eight v3 style Skills and absence of PPTX/code/DSL template assets.
- [ ] 6.2 Run one fresh-context unrelated four-page creation through a selected style and record only concrete failures. The deterministic four-page dogfood passed; the separate fresh Agent attempt was blocked before execution by Enter HTTP 403 and is not counted as passed.
- [x] 6.3 Update README, coverage, and package version to 1.1.0; finalize release hashes after platform builds.
