...
Tables must use explicit Word geometry. Compute exact DXA widths, then pass matching `widthDxa`, `indentDxa`, `columnWidthsDxa`, and `cellMarginsDxa` values to `DocumentModel.addTable(...)`. Use `verticalAlignment: "center"` or `"bottom"` only when the same alignment should apply to every physical cell; omit it for Word's ordinary top alignment. Imported tables permit this semantic change only after OfficeKit recognizes the complete fixed-layout formatting profile. Set table indent to the start cell margin token (`120` DXA by default) so the visible outer border aligns with surrounding paragraph text. `scripts/table_geometry.py` is an audit or explicit package-patch tool for an existing DOCX, not the ordinary authoring path. Do not rely on autofit, percentage widths, centered default tables, fixed row heights, or tables as layout/divider hacks.
