...
representative renders, exact authored-program recovery, and synthetic-data
provenance. No user or third-party source deck enters the package.
...
DOCX, XLSX, and PPTX use the canonical OfficeKit C# NativeAOT codec. PDF keeps
...
The self-contained releases embed the same package with Node 24.18.0 and
exactly one codec for the target platform. Archive sizes and SHA-256 values
must be regenerated by the release lane after the NativeAOT migration; the
retired WASM archive hashes are not valid release evidence.
...
self-directed palettes advisory unless a closed brand palette is supplied;
...
The approved C rollout also keeps a 30-task, five-scenario brief-only catalog
in `evals/presentation-authoring-compiler/expansion.v1.json` for future
experiment design. The full live matrix is intentionally disabled; its
standalone runner requires an explicit selection of at most three task IDs.
No expansion run is part of completion, the daily gate, or the release slow
gate. Selected continuation evidence is `23/23` (`100%`), and no planned task
...
`debb7a331056e39e0f44c4e6e75d04bf9642abc0` passed the 45-step fast gate. The
92-step slow table completed as one run through the first 29 foundation steps,
followed by final-tree reruns of the repaired Presentation segment and every
remaining segment; the already-passed foundation prefix was not repeated.
OfficeKit Codec 441/441 and OfficeBridge
...
12-hour release-candidate cooldown.
...
## Current 1.0.0 milestone (2026-08-27): NativeAOT Office codec host

The JavaScript object model, `.mjs` runner, REPL, six Office facades, and
protobuf protocol 2 remain unchanged. The C# codec now runs in one lazy,
persistent NativeAOT child process over a private transport-v1 framed-stdio
channel. The child validates the handshake and frame budget, exits on stdin
EOF, is unreferenced while idle, and is reset after transport failure without
request replay or a WASM fallback. Large exchanges release their transient C#
Open XML/protobuf graph before JavaScript expands the response, avoiding the
worst cross-runtime overlap without discarding source-package or opaque-graph
state.

Distribution is split into exact-version optional packages for
`darwin-arm64`, `linux-x64`, and `win32-x64`. Each package contains only its
target executable plus a SHA-256 manifest, licenses, and CycloneDX SBOM. The
root runtime resolves an absolute package path, verifies identity, size, mode,
and SHA-256, and never searches `PATH`. Standalone archives copy only their
target package. The old .NET WASM host and generated `runtime/office-kit`
payload are removed.

Narrow local evidence on Apple Silicon: the canonical DOCX/XLSX/PPTX codec
smoke passed. A fresh Node process imported the locked 7,330,920-byte, 24-slide
template in 1.31 seconds wall-clock (1.03 seconds inside the import). The
observed Node-plus-codec process-tree peak was 250.6 MiB: the codec had fallen
to about 66 MiB while the complete JavaScript model reached about 184 MiB.
Thus the 1.5-second target passes, but the 200-MB target does not. Per the
migration boundary, reducing that remaining peak is a separate no-copy/file-
handle/streaming-wire task rather than a lossy change to Office preservation.
...
format-specific adapter rather than the aggregate compatibility module. The
published `office-kit/codec` surface is unchanged and re-exports the exact same
PPTX function bindings. Source-graph, binding-identity, package-content, PPTX
round-trip, and aggregate OfficeKit tests guard this boundary.

## Unreleased: format-local Spreadsheet codec loading
...
The compression-ratio option accepts any finite positive number, including
fractional ceilings, and invalid/non-positive values leave that optional guard
disabled rather than changing the byte budgets.
...
## 0.3.0 DOCX source-bound section columns transaction

The Documents Skill now ships
`officekit-section-columns-edit-workflow.mjs` for one exact imported canonical
Word section. The task supplies its inspected block index plus complete
normalized equal-width (`count`, `spacing`, `separator`) or explicit-width
(`definitions`, `separator`) source and replacement profiles. A transaction
retains that native profile shape: it neither adds/removes `w:cols` nor converts
an equal-width graph into an explicit-width graph, or the reverse.

Before publication the workflow binds the semantic section identity and one raw
canonical `w:cols` element. It keeps the input immutable, creates distinct
output and audit files without overwrite, permits only `word/document.xml` to
change, masks that element in a namespace-tolerant residual check, reimports
the full section projection, verifies the model render, and records
source/output hashes plus the OfficeKit provider version. Duplicate,
child/extension-bearing, non-Word, noncanonical-numeric, stale, absent, no-op,
profile-converting, or package-drift inputs fail before an output is published.
It does not calculate visible column flow; a native Word or LibreOffice plus
Poppler render remains required for final display review.

### Columns transaction integration evidence

On 2026-07-26, the integrated candidate passed `npm test`, generated API
documentation with no diff, `npm run proto:check`, deterministic
`npm run verify:office-kit-build`, clean-install `npm run test:pack`,
OfficeBridge `5/5`, OfficeKit Codec `375/375`, and the offline release-metadata
check. The candidate tarball has 527 files, 9,449,909 compressed bytes, and
25,476,745 unpacked bytes (`SHA-1
f41e6dd7a7a4fdd8368085ee9b7a0bf57ca4f237`), leaving 18,255 bytes below the
...
## 0.3.0 DOCX source-bound table direct-formatting transaction

The Documents Skill now ships
`officekit-table-formatting-edit-workflow.mjs` for one exact imported flat,
rectangular, unmerged fixed-layout Word table. The task supplies its inspected
block index and complete source/replacement profile: table indentation, four
cell margins, six uniform border values, and one uniform first-row fill. It
retains the table width, grid and physical-cell widths, text, style, and row /
cell / merge topology rather than pretending to be a generic table-style
editor.

Before publication the workflow binds the semantic table identity and raw
canonical `w:tblInd`, `w:tblBorders`, `w:tblCellMar`, and first-row `w:shd`
leaves. It keeps the input immutable, creates distinct output and audit paths
without overwrite, permits only `word/document.xml`, masks exactly those leaves
for a namespace-tolerant residual comparison, reimports the complete table
projection, verifies the model render, and records source/output hashes plus
the OfficeKit provider version. Incomplete, custom/mixed/conditional,
nested/merged/content-control, stale/no-op, noncanonical, or package-drift
sources fail before an output is published. A native Word or LibreOffice plus
Poppler render remains required for final visible wrapping and page-flow review.

### Table direct-formatting transaction integration evidence

On 2026-07-26, the integrated candidate passed `npm test`, generated API
documentation with no diff, `npm run proto:check`, deterministic
`npm run verify:office-kit-build`, clean-install `npm run test:pack`,
OfficeBridge `5/5`, OfficeKit Codec `375/375`, and the offline release-metadata
check. The candidate tarball has 530 files, 9,461,177 compressed bytes, and
25,555,825 unpacked bytes (`SHA-1
1800171af43d3f9805cddad497966e9062340169`), leaving 9,175 bytes below the
...
- Current PromptBench inventory supersedes those historical counts: 41 cases (21 PDF, 20 Office), 38 ready cases, and 3 still asset-required. Twelve self-authored locked PDF corpus inputs are now ready and verified by independent parser oracles: AES-256 owner/user permissions, `/IRT` annotation reply chains, untagged complex PDF/UA overclaim, Dynamic XFA, print-production DeviceN/overprint/OCG/OutputIntent, RichMedia/3D opaque runtime preservation, a four-page multichannel redaction source with raster/OCR and old-revision residues, a DocMDP P=1 certification signature, a DocMDP P=2/FieldMDP form, the qpdf repair pair, the three-page Regional Revenue table with merged/repeated headers, geometry/spans, JSON/CSV parity, and Poppler overlay review, plus the eight-page mixed-scan fixture with six image-only rotated/skewed pages and two born-digital canaries. The PAdES/TSA/LTV case reuses the signed P=1 source and public root to prove the baseline, then requires an explicit fail-closed refusal when the shipped provider reports no timestamp or LTV embedding capability; the mixed-scan case similarly requires an explicit refusal because the shipped OCR adapter does not expose rotate/deskew preprocessing. Neither claims a signature or OCR artifact. The boundary cases accept only source-hash-bound, inspected, no-mutation safe refusals; the signed form, redaction, repair pair, and table extraction independently grade their permitted success paths. The evaluator normalizes explicit no-mutation/no-artifact evidence across the published audit envelope and equivalent structured forms, then independently proves the trial output directory contains only the audit for refusal cases; generic no-artifact gates do not count. The ready `pptx-closed-leaf-slide-clone` fixture now drives the shipped duplicate workflow with explicit `--allow-closed-leaves` over one closed literal-data ChartPart plus canonical NotesSlide and legacy SlideComments leaves. Its independent OPC oracle permits only the adjacent clone and exact cloned parts, requires a distinct byte-identical ChartPart with the same slide-local relationship ID/type and no child graph, checks retained-source byte hashes and shared NotesMaster/CommentAuthors catalogs, requires second import, and compares source/clone/appendix LibreOffice/Poppler pixels. One clean autonomous candidate Agent run at `0e8824cb3dac8332ff631b0cb75850ebe2a56f6b` passed every hard gate and scored `100/100` across 6 machine, 2 visual, 2 security, and 7 trace checks; its tarball/Skill/prompt/input/oracle fingerprints are recorded in [Agent black-box evaluations](agent-evals.md). This is one trial, not the default repeat or reference-Skill matrix. The newly locked RichMedia fixture currently has the same structural/refusal smoke as the other corpus boundaries; its clean candidate/reference repeat matrix remains open. The multichannel redaction fixture adds an independent OCR/residue/old-revision success grader; its clean candidate/reference repeat matrix remains open.
