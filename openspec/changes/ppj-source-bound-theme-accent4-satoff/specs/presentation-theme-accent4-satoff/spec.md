...
direct accent4 saturation offset token while preserving the rest of an
imported theme.

## ADDED Requirements

### Requirement: Strict source-bound accent4 satOff projection

The PPTX projector MUST expose
`design.theme.accentTransforms.accent4.satOff` and
`setThemeAccent4SatOff` only when the canonical shared ThemePart has one
direct accent4 RGB leaf with exactly one direct `a:satOff/@val` child, no
...
#### Scenario: Project a direct accent4 satOff leaf

- **GIVEN** a canonical imported ThemePart with accent4 `334455` and
  `a:satOff/@val="-25000"`
- **WHEN** the presentation is projected to PPJ
- **THEN** the theme contains `accentTransforms.accent4.satOff` equal to
  `-0.25`
- **AND** the native reference contains `setThemeAccent4SatOff` authorized
  for `accentTransforms.accent4.satOff`

#### Scenario: Reject unsupported accent4 satOff topology
...
- **THEN** the source-bound accent4 satOff field and capability are omitted

### Requirement: Source-bound accent4 satOff editing

The source-preserving compiler MUST accept one authorized change to the
existing `design.theme.accentTransforms.accent4.satOff` field, convert a
...
#### Scenario: Edit and reproject one accent4 satOff token

- **GIVEN** a projected direct accent4 satOff owner at `-0.25`
...
#### Scenario: Fail closed for unsafe accent4 satOff edits

- **GIVEN** a source-bound accent4 satOff owner
