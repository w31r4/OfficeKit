# Design: source-bound paragraph default-text reflection start opacity
...
`stA`, optionally preceded by one valid direct `a:outerShdw`. The reflection
keeps its blur, end opacity, distance, direction and full-span positions
...
The leaf kind is `textDefaultReflectionStartOpacityThousandthPercent` and its
PPJ location is `paragraph.style.defaultText.reflection.startOpacity`. The edit
plan re-proves the selected `a:p`, direct `a:pPr`, `a:defRPr`, strict full-span
effect list, and direct `a:reflection` before token-splicing only `stA` in the
owning SlidePart. No protobuf or wire-version change is needed. The focused
fixture removes the embedded PPJ, changes the paragraph default start opacity
alongside the existing default-text and inline-run effect leaves, verifies
