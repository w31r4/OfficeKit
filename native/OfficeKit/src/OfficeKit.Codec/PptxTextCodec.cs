...
internal static class PptxTextCodec
{
    ...
    internal static PresentationTextBody Read(P.TextBody? source, PptxPartContext? slideContext = null)
    {
        ...
        foreach (var sourceParagraph in paragraphs)
        {
            ...
            PptxParagraphPropertiesCodec.Read(paragraph, properties, slideContext, readLevel: true);
    ...
    internal static void Validate(PresentationShape shape)
    {
        ...
        foreach (var paragraph in shape.TextBody.Paragraphs)
        {
            ...
            foreach (var run in paragraph.Runs)
            {
                ...
                if (run.GradientFill is not null && (run.HasColorRgb || run.HasColorScheme))
                    throw new CodecException("invalid_presentation_text", "Presentation run cannot specify both a solid color and a gradient.");
    ...
    internal static void ScrubModeledContent(P.TextBody? body, PptxPartContext? slideContext = null)
    {
        ...
        foreach (var paragraph in body.Elements<A.Paragraph>())
        {
            ...
            {
                PptxParagraphPropertiesCodec.Scrub(paragraphProperties, slideContext, includeLevel: true);
            }
            foreach (var inline in ParagraphInlines(paragraph))
            {
    ...
    private static void NormalizeBodyPropertiesEditIntent(PresentationTextBody body)
    {
        ...
        if (properties.HasNoForceAntiAlias)
        {
            properties.ClearForceAntiAlias();
            properties.ClearNoForceAntiAlias();
        }
        if (properties.HasNoSpaceFirstLastParagraph)
        {
        ...
        if (properties.HasNoFromWordArt)
        {
            properties.ClearFromWordArt();
            properties.ClearNoFromWordArt();
        }
        if (properties.HasNoFlatTextZ)
        {
        ...
        if (properties.HorizontalOverflowCase == PresentationTextBodyProperties.HorizontalOverflowOneofCase.NoHorizontalOverflowMode) properties.ClearHorizontalOverflow();
        if (properties.ColumnCountCase == PresentationTextBodyProperties.ColumnCountOneofCase.NoColumns) properties.ClearColumnCount();
        if (properties.ColumnSpacingCase == PresentationTextBodyProperties.ColumnSpacingOneofCase.NoColumnSpacing) properties.ClearColumnSpacing();
        if (properties.ColumnDirectionCase == PresentationTextBodyProperties.ColumnDirectionOneofCase.NoColumnDirection) properties.ClearColumnDirection();
        if (properties.UprightTextCase == PresentationTextBodyProperties.UprightTextOneofCase.NoUpright) properties.ClearUprightText();
    ...
    private static PresentationTextRun ReadInline(OpenXmlElement source, PptxPartContext? slideContext)
    {
        ...
        OpenXmlElement[] fills = properties is null ? [] : FillChoices(properties).ToArray();
        if (fills.Length == 1 && fills[0] is A.SolidFill solid && PptxColor.TryDirectSolidRgbWithOpacity(solid, out var rgb, out var rgbOpacity))
        {
            run.ColorRgb = rgb;
            if (rgbOpacity is { } opacity) run.ColorOpacityThousandthPercent = opacity;
        }
        else if (fills.Length == 1 && fills[0] is A.SolidFill schemeFill && PptxColor.TryDirectSolidSchemeWithOpacity(schemeFill, out var scheme, out var schemeOpacity))
        {
            run.ColorScheme = scheme;
            if (schemeOpacity is { } opacity) run.ColorOpacityThousandthPercent = opacity;
        }
        else if (fills.Length == 1 && fills[0] is A.GradientFill gradientFill && PptxGradientFillCodec.TryRead(gradientFill, out var gradient))
        {
            run.GradientFill = gradient;
        }
        if (PptxShadowCodec.TryRead(properties, out var shadow, allowTransforms: true) &&
            shadow is not null && PptxShadowCodec.IsSafeDirectTextRunWithSingleScale(shadow))
            run.Shadow = shadow;
    ...
    private static bool IsSafeDirectRunReflection(PresentationReflection reflection)
    {
        ...
        if (fullSpan)
        {
            ...
            var hasUnsupportedSkewXCombination = reflection.HasSkewXAngle60000 &&
            ...
            var hasUnsupportedAlignmentCombination = reflection.HasAlignment &&
                                                       (reflection.HasFadeDirectionAngle60000 ||
                                                        reflection.HasScaleXThousandthPercent ||
                                                        reflection.HasScaleYThousandthPercent ||
                                                        reflection.HasSkewXAngle60000 ||
    ...
    internal static A.Paragraph BuildParagraph(PresentationTextParagraph source, PptxPartContext? slideContext)
    {
        ...
        if (PptxParagraphPropertiesCodec.HasAuthoredProperties(source, includeLevel: true))
        {
            ...
            PptxParagraphPropertiesCodec.Append(properties, source, slideContext, includeLevel: true);
        ...
        switch (source.ContentCase)
        {
            ...
            case PresentationTextRun.ContentOneofCase.Field:
                ...
                if (source.Field.Automatic && !IsAutomaticFieldType(source.Field.Type))
                    throw new CodecException("invalid_presentation_text", "The bounded automatic presentation field profile only supports type slidenum, author, datetime, datetimeFigureOut, datetime1, datetime2, datetime3, datetime4, datetime5, datetime6, datetime7, datetime8, datetime9, datetime10, datetime11, datetime12, datetime13, uaqdatetime1, uaqdatetime2, uaqdatetime3, uaqdatetime4, uaqdatetime5, uaqdatetime6, or uaqdatetime7.");
                return;
    ...
    internal static bool IsAutomaticFieldType(string? value) =>
        ...
        string.Equals(value, "author", StringComparison.OrdinalIgnoreCase) ||
        string.Equals(value, "datetime", StringComparison.OrdinalIgnoreCase) ||
        string.Equals(value, "datetimeFigureOut", StringComparison.OrdinalIgnoreCase) ||
        string.Equals(value, "datetime1", StringComparison.OrdinalIgnoreCase) ||
        string.Equals(value, "datetime2", StringComparison.OrdinalIgnoreCase) ||
        string.Equals(value, "datetime3", StringComparison.OrdinalIgnoreCase) ||
        string.Equals(value, "datetime4", StringComparison.OrdinalIgnoreCase) ||
        string.Equals(value, "datetime5", StringComparison.OrdinalIgnoreCase) ||
        string.Equals(value, "datetime6", StringComparison.OrdinalIgnoreCase) ||
        string.Equals(value, "datetime7", StringComparison.OrdinalIgnoreCase) ||
        string.Equals(value, "datetime8", StringComparison.OrdinalIgnoreCase) ||
        string.Equals(value, "datetime9", StringComparison.OrdinalIgnoreCase) ||
        string.Equals(value, "datetime10", StringComparison.OrdinalIgnoreCase) ||
        string.Equals(value, "datetime11", StringComparison.OrdinalIgnoreCase) ||
        string.Equals(value, "datetime12", StringComparison.OrdinalIgnoreCase) ||
        string.Equals(value, "datetime13", StringComparison.OrdinalIgnoreCase) ||
        string.Equals(value, "uaqdatetime1", StringComparison.OrdinalIgnoreCase) ||
        string.Equals(value, "uaqdatetime2", StringComparison.OrdinalIgnoreCase) ||
        string.Equals(value, "uaqdatetime3", StringComparison.OrdinalIgnoreCase) ||
        string.Equals(value, "uaqdatetime4", StringComparison.OrdinalIgnoreCase) ||
        string.Equals(value, "uaqdatetime5", StringComparison.OrdinalIgnoreCase) ||
        string.Equals(value, "uaqdatetime6", StringComparison.OrdinalIgnoreCase) ||
        string.Equals(value, "uaqdatetime7", StringComparison.OrdinalIgnoreCase);
    ...
    private static string FieldText(PresentationTextField field, PptxPartContext? slideContext) =>
        field.Automatic && IsSlideNumberFieldType(field.Type) && slideContext?.SlideNumber is { } slideNumber
    ...
    internal static void ValidateTabStops(PresentationTextParagraph source)
    {
    ...
    private static void ApplyRunProperties(A.RunProperties properties, PresentationTextRun requested)
    {
        ...
        var colorOpacity = requested.HasColorOpacityThousandthPercent
            ...
            : (uint?)null;
        if (requested.HasColorRgb || requested.HasColorScheme || requested.GradientFill is not null)
        {
            if (fills.Length > 1 || fills.Any(fill => !ModeledFill(fill)))
                throw new CodecException("unsupported_presentation_edit", "Source-preserving PPTX export cannot replace unmodeled run fill properties.");
            foreach (var fill in fills) fill.Remove();
            OpenXmlElement requestedFill = requested.GradientFill is not null
                ? PptxGradientFillCodec.Build(requested.GradientFill, "Presentation run gradient")
                : requested.HasColorRgb
                    ? PptxColor.BuildSolidRgb(requested.ColorRgb, colorOpacity)
                    : PptxColor.BuildSolidScheme(requested.ColorScheme, colorOpacity);
            properties.AddChild(requestedFill, true);
        }
        else if (fills.Length == 1 && ModeledFill(fills[0]))
        {
            fills[0].Remove();
        }
        if (requested.Shadow is not null)
        {
            if (!PptxShadowCodec.TryRead(properties, out _))
                throw new CodecException("unsupported_presentation_edit", "Source-preserving PPTX export cannot replace unmodeled run effect properties.");
            PptxShadowCodec.Apply(properties, requested.Shadow);
        }
        else if (PptxShadowCodec.TryRead(properties, out var existingShadow) && existingShadow is not null)
        {
    ...
    private static IEnumerable<OpenXmlElement> FillChoices(OpenXmlCompositeElement source) =>
