...
direct accent3 luminance modulation token while preserving the rest of an
imported theme.

## ADDED Requirements

### Requirement: Strict source-bound accent3 lumMod projection

The PPTX projector MUST expose
`design.theme.accentTransforms.accent3.lumMod` and
`setThemeAccent3LumMod` only when the canonical shared ThemePart has one
direct accent3 RGB leaf with exactly one direct `a:lumMod/@val` child, no extra
attributes or descendants, and an integer value in `[0, 100000]`. The
projected value MUST be the native value divided by 100000.

#### Scenario: Project a direct accent3 lumMod leaf

- **GIVEN** a canonical imported ThemePart with accent3 `334455` and
  `a:lumMod/@val="25000"`
- **WHEN** the presentation is projected to PPJ
- **THEN** the theme contains `accentTransforms.accent3.lumMod` equal to `0.25`
- **AND** the native reference contains `setThemeAccent3LumMod` authorized for
  `accentTransforms.accent3.lumMod`

#### Scenario: Reject unsupported accent3 lumMod topology
...
- **THEN** the source-bound accent3 lumMod field and capability are omitted

### Requirement: Source-bound accent3 lumMod editing

The source-preserving compiler MUST accept one authorized change to the
existing `design.theme.accentTransforms.accent3.lumMod` field, convert a finite
...
#### Scenario: Edit and reproject one accent3 lumMod token

- **GIVEN** a projected direct accent3 lumMod owner at `0.25`
...
#### Scenario: Fail closed for unsafe accent3 lumMod edits

- **GIVEN** a source-bound accent3 lumMod owner
