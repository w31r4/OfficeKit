...
The native leaf `shape3dSceneBackdropNormalDzEmu` already covers strict
imported shapes, but the same complete backdrop owner on a picture is still
opaque. Its normal Z coordinate is an independently bounded literal that can
...
- Project and source-bound-edit only a strict
  `p:pic/p:spPr/a:scene3d/a:backdrop/a:norm/@dz` token.
...
- `ppj-source-bound-picture-shape-3d-scene-backdrop-normal-dz`:
  Bounded projection and source-bound editing of a picture backdrop normal Z.
