...
transform and the direct accent5 tint, shade, luminance modulation, luminance
offset, and alpha modulation leaves at a time, then patches the existing
DrawingML token. This change applies the same ownership boundary to accent5
...
- Accent5 transforms other than `alphaOff`, transforms on accent5 through
...
1. **Use the existing native transform field.**
   ...
   already emits it. The importer records a role of `accent5`; the projector
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:alphaOff`. The source writer patches `Accent5Color` only when the
   authored transform is the one existing accent5 alphaOff and rejects all
   other transform flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent5AlphaOff` is accepted only with
   `accentTransforms.accent5.alphaOff` in the source-issued capability. This
