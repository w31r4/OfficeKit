...
Use this style when the brief matches **investment committee; finance presentation**. It is designed for
investors, finance leaders, analysts. Its visual premise is Warm paper and amber anchors expose an approval boundary. The style is a
clean-room OfficeKit grammar: the included images teach a range of decisions,
not pages or coordinates to copy.

The deck should help the audience turn financial evidence into a defensible stance, allocation choice, or approval decision. The natural story arc is
**stance → drivers → evidence → scenarios → risks → decision or allocation**. This style is not a good fit for **youth campaign**;
...
- Use **#F8F1E8** as the main field, **#18304A** for primary type, **#E57A27** for the decisive signal, **#507FA3** for secondary evidence, and **#E8CBB7** for rules or context.
...
- Finance guidance: Every number carries a unit, period, as-of date, and basis. Separate history, estimate, and forecast. Pair material risks with probability/impact, mitigation, and owner. Use ranges when a single-point forecast would imply false precision.
...
- The recurring structure is **diagonal, variable page silhouettes, native data graphics**. Vary the silhouette between
  cover, evidence, data, process, comparison, and decision pages rather than
  repeating a component grid.
- Use a single diagonal seam or directional cut to signal the decision boundary. Keep evidence, labels, and baselines on stable horizontal or vertical axes; do not turn every separator into an arrow.
...
For this category, favor **thesis cover, market or performance trend, statement/table diagnosis, valuation or sensitivity, risk–mitigation pair, decision close**. The style's content shapes are
market signal, risk, decision; select one primary carrier per page and explain why it earns the
space.

### Information carriers

- directly labelled bars and lines, bridge/waterfall views, scenario bands, sensitivity tables, composition bars, and small multiples only when the units and basis stay comparable
- photography is rare and should establish institution, place, or operating context; never use lifestyle imagery to fill a financial evidence page
...
The expected density is **dense**: every page should answer one finance question and carry enough basis to audit the answer. Alternate page
...
- **Page skeleton.** Start with a warm-paper proposition and a single diagonal decision seam. Let the body alternate between a left assertion/assumption column and a right evidence or approval rail; reserve the seam for openings, transitions, or an explicit boundary.
- **Type rhythm.** Use a humanist or restrained serif for the thesis and a compact sans for figures, units, and source notes. Keep the title assertive, then make the basis line visibly quieter.
- **Evidence carriers.** Prefer driver trees, bridge bars, scenario bands, sensitivity rows, and short approval checklists. A three-way split is valid only when the brief names three actual cases.
- **Images and layers.** Use documentary place or operating imagery only when it supplies context. Keep images square-cropped or bounded, and put assumptions and decision labels on an editable plane above any image.
- **Pacing.** Move from one calm proposition to dense evidence, then create a short decision pause. One amber focus is enough; do not turn the paper field into a repeated orange dashboard.
- **Selection rule.** Choose this style when an investment committee needs to approve, reject, or condition a financially grounded action.

## Signature moves and variation limits

- Let **diagonal** recur as a quiet identity cue on selected pages; it must
...
- Avoid decorative dashboards, 3D or perspective charts, unexplained colors, single-point return promises, and pages that end without a stance or next action.
...
- Check title read-through, source/basis footers, period consistency, number alignment, forecast markers, direct labels, and whether the decision follows from the evidence.
