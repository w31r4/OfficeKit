...
{
    [Theory]
    [InlineData("combo", 35, -40)]
    [InlineData("column", 35, -40)]
    public void PpjChartOverlapAuthorAndEditSourceChart(string chartType, int authoredOverlap, int editedOverlap)
    {
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest { SourceUri = $"chart-overlap-{chartType}/source.pptx" },
        });
        ...
        var projectedChart = projectedProgram["pages"]!.AsArray()
            ...
            .Select(item => item!.AsObject())
            .Single(item => item["type"]!.GetValue<string>() == "chart");
        Assert.Equal(authoredOverlap, projectedChart["style"]!["overlap"]!.GetValue<int>());

        projectedChart["style"]!["overlap"] = editedOverlap;
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            Family = ArtifactFamily.Presentation,
            File = edited.File,
            PresentationProgram = new PresentationProgramRequest { SourceUri = $"chart-overlap-{chartType}/edited.pptx" },
        });
        ...
        var reprojectedChart = reprojectedProgram["pages"]!.AsArray()
            ...
            .Select(item => item!.AsObject())
            .Single(item => item["type"]!.GetValue<string>() == "chart");
        Assert.Equal(editedOverlap, reprojectedChart["style"]!["overlap"]!.GetValue<int>());
    }
}
