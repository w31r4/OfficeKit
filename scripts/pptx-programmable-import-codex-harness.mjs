...
In phase 2 and phase 3, reopen the current reviewed artifact from \`ctx.task\`: import \`node:path\`, find the entry whose \`id\` is \`continued-deck\` in \`ctx.task.artifacts\`, require its \`headRevision\`, resolve \`headRevision.path\` beneath \`ctx.taskRoot\` with \`path.resolve(ctx.taskRoot, headRevision.path)\`, and load that absolute path with \`FileBlob.load\`. \`ctx.input\` stages an external immutable input; never call \`ctx.input("continued-deck")\` or pass an artifact ID to it. In phase 3 publish only with \`ctx.publish(ctx.task.commit, { artifactId: "continued-deck", name: "${path.basename(task.output)}" })\`; an artifact ID or path is not a publishable commit.
