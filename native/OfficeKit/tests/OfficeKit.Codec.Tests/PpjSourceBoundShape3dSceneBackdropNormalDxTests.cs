...
{
    [Fact]
    public void PpjSourceBoundShape3dSceneBackdropNormalDxLeafEditsAndReprojects()
    {
        var authored = Invoke(ExportRequest());
        Assert.True(authored.Ok, Diagnostics(authored));
        var source = AddShape3dSceneBackdropNormalDx(RemoveEmbeddedPpj(authored.File.ToByteArray()), "0");
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            {
                SourceUri = "deck.assets/source/shape-3d-scene-backdrop-normal-dx.pptx",
            },
        });
        ...
        var target = Assert.Single(projectedShape["nativeRef"]!["leaves"]!.AsArray()
            ...
            .Where(leaf => leaf["kind"]!.GetValue<string>() == "shape3dSceneBackdropNormalDxEmu"));
        ...
        Assert.Contains("<a:norm dx=\"500\" dy=\"0\" dz=\"1\"", editedXml, StringComparison.Ordinal);
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/shape-3d-scene-backdrop-normal-dx.pptx",
            },
        });
    ...
    private static byte[] AddShape3dSceneBackdropNormalDx(byte[] source, string normalDx)
    {
        ...
        using (var package = PresentationDocument.Open(stream, true))
        {
            ...
            normal.SetAttribute(new OpenXmlAttribute("dx", string.Empty, normalDx));
            normal.SetAttribute(new OpenXmlAttribute("dy", string.Empty, "0"));
            normal.SetAttribute(new OpenXmlAttribute("dz", string.Empty, "1"));
