...
the x-range bounds plus `minY`, but the paired `maxY` bound remains opaque. A
bounded `maxY` leaf lets an agent tune the existing handle constraint while
keeping the controlled guide identity, current geometry, and the rest of the
range graph source-faithful.

## What Changes

- Add `customGeometryAdjustmentHandleMaxYEmu` as a bounded PPJ native leaf for
  an ordered `a:ahXY` handle with a direct literal `@maxY`.
...
- Token-splice only the selected `a:ahXY/@maxY` value in the owning SlidePart;
  preserve `gdRefX/gdRefY`, y minimum, x bounds, position, other handles, and
...
- `ppj-source-bound-geometry-handle-max-y`: source-bound editing of one direct
  custom-geometry XY adjustment-handle maximum-y bound.
