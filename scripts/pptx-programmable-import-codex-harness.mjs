...
Review each bounded edit against the bytes immediately before that edit, not against the original deck with a different page count. In session 1, retain the exported clone-before-edit blob and pass it as \`baseline\` to the imported top-level \`reviewArtifact(outputBlob, options)\`. In session 2, retain both the restored c0001 path and its loaded bytes; use the blob for import/edit, but pass the absolute \`.pptx\` path as \`baseline\` to \`reviewArtifact\` so its format remains explicit. \`FileBlob.load(path)\` alone has the generic \`application/octet-stream\` type and is not a typed review baseline. Pass a not-yet-created \`outputPath\` under \`ctx.taskRoot/candidates\`, set \`layout: false\` and \`visualReview: "unavailable"\`, require a non-failed verdict, and pass the exact output blob plus returned review to \`ctx.commit(outputBlob, { ..., review })\`. Do not save the candidate path before \`reviewArtifact\` writes it.

In phase 2 and phase 3, reopen the current reviewed artifact from \`ctx.task\`: import \`node:path\`, find the entry whose \`id\` is \`continued-deck\` in \`ctx.task.artifacts\`, require its \`headRevision\`, resolve \`headRevision.path\` beneath \`ctx.taskRoot\` with \`path.resolve(ctx.taskRoot, headRevision.path)\`, and load that absolute path with \`FileBlob.load\`. In phase 2 use the resolved path itself as \`reviewArtifact(..., { baseline: restoredPath })\`. \`ctx.input\` stages an external immutable input; never call \`ctx.input("continued-deck")\` or pass an artifact ID to it. In phase 3 publish only with \`ctx.publish(ctx.task.commit, { artifactId: "continued-deck", name: "${path.basename(task.output)}" })\`; an artifact ID or path is not a publishable commit.
...
2. Start a new process with \`officekit repl <task-id> --file phase-2.mjs\`. Resolve the restored reviewed full-file revision through \`ctx.task.artifacts\` as specified above, not through \`ctx.input\` or old JavaScript heap state. Reimport it, perform only phase-2's typed edit, call \`reviewArtifact\`, then \`ctx.commit\`.
3. Start a third process with \`officekit repl <task-id> --file phase-3.mjs\`. Resolve and reimport the restored current revision the same way, verify the source slide count plus one and both exact values, then publish \`ctx.task.commit\` as \`${path.basename(task.output)}\`.
...
End every phase file with an explicit top-level \`return\`. A terminal response containing \`commit.commitId: "c0001"\` proves phase 1 already committed even if \`result\` is absent; continue directly with phase 2 and never repair that presentation-only omission by rerunning phase 1. Phase 2 must commit but must not publish. Phase 3 is the only cell that may call \`ctx.publish\`.
