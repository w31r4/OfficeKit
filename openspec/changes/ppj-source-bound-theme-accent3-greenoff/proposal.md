...
modulation transforms. A direct imported accent3 `a:greenOff` remains opaque,
...
- Project a canonical shared ThemePart whose accent3 direct RGB leaf contains
  ...
  `design.theme.accentTransforms.accent3.greenOff`, using a -1..1 fraction.
- Issue `setThemeAccent3GreenOff` for that field and compile one authorized
...
- `presentation-theme-accent3-greenoff`: Strict source-bound projection and
  source-preserving editing of one direct accent3 `a:greenOff` leaf.
