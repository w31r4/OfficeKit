# ppj-source-bound-shape-3d-camera-rotation-revolution Specification

## ADDED Requirements

### Requirement: Strict direct camera rotation revolution projection

The presentation projection MUST expose
`shape3dSceneCameraRotationRevolution60000` only when a shape has one direct
...
#### Scenario: Project a camera rotation revolution

- **GIVEN** an ordinary shape with a canonical scene whose camera has
  `prst="perspectiveFront"` and `rot lat="0" lon="0" rev="0"`, and whose
  light rig has `rig="threePt"` and `dir="t"`
- **WHEN** the PPTX is projected to PPJ
- **THEN** exactly one `shape3dSceneCameraRotationRevolution60000` native leaf
...
- **WHEN** the PPTX is projected to PPJ
- **THEN** no `shape3dSceneCameraRotationRevolution60000` leaf is emitted

### Requirement: Source-bound camera rotation revolution token splice

The PPJ edit plan MUST permit
`shape3dSceneCameraRotationRevolution60000` only when the source-bound shape
still proves the strict owner and the requested value is a different
canonical non-negative rotation token no greater than 360 degrees. The PPTX
edit MUST replace only the `a:rot/@rev` token in the owning SlidePart, preserve
the scene/camera/light-rig topology and unrelated package parts, and allow the
new revolution to be recovered on a second projection.

#### Scenario: Edit and reproject camera rotation revolution

- **GIVEN** a projected strict scene owner with camera `rev="0"`
- **WHEN** PPJ changes the leaf to `1800000`
- **THEN** only the owning SlidePart changes, the package remains Open XML
  valid, the revolution token becomes `1800000`, and a second projection
  returns `1800000`
...
- **WHEN** an edit targets `shape3dSceneCameraRotationRevolution60000`
