...
export async function runDocumentFixture(fixturePath, options = {}) {
  ...
  const imported = await DocumentFile.importDocx(docx);
    for (const edit of fixture.edits || []) {
      ...
      if (edit.kind === "comboBoxContentControls") {
        const result = imported.setComboBoxContentControls(edit.values || {}, { strict: edit.strict !== false });
        if (Object.prototype.hasOwnProperty.call(edit, "expectUpdated")) assert.equal(result.updated, edit.expectUpdated);
        continue;
      }
      if (edit.kind === "dateContentControls") {
        const result = imported.setDateContentControls(edit.values || {}, { strict: edit.strict !== false });
        if (Object.prototype.hasOwnProperty.call(edit, "expectUpdated")) assert.equal(result.updated, edit.expectUpdated);
        continue;
      }
      if (edit.kind === "image") {
