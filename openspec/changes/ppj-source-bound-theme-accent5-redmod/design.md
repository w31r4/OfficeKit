...
the same ownership boundary to accent5 red-channel modulation without
...
- Accent5 transforms other than `redMod`, transforms on accent5 through
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:redMod`. The source writer patches `Accent5Color` only when the authored
   transform is the one existing accent5 redMod and rejects all other transform
   flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent5RedMod` is accepted only with
   `accentTransforms.accent5.redMod` in the source-issued capability. This
