...
public sealed partial class PptxCodecTests
{
    ...
    public void PpjChartTextReflectionPreservesIndependentEffectsLifecycle(string type, int seriesIndex)
    {
        ...
        Assert.All(XDocument.Parse(Encoding.UTF8.GetString(ZipBytes(source, path))).Descendants(a + "reflection"), native =>
            Assert.Empty(native.Attributes().Where(attr => !attr.IsNamespaceDeclaration)));
        ...
        foreach (var (input, expected) in new (string? Input, string? Expected)[]
        {
            ...
            ("""{"endOpacity":0.25,"startPosition":0.2,"endPosition":0.7}""", """{"endOpacity":0.25,"startPosition":0.2,"endPosition":0.7}"""),
            ("""{"endOpacity":0.25,"startPosition":0.8,"endPosition":0.3}""", """{"endOpacity":0.25,"startPosition":0.8,"endPosition":0.3}"""),
    ...
    public void PpjChartTextReflectionValidatesOrderBoundsTokensAndPresence()
    {
        ...
        Assert.False(PptxReflectionCodec.TryRead(direct, out _)); // Ordinary imported guard remains full-span.
        Assert.True(PptxReflectionCodec.TryReadDirectReflection(direct.GetFirstChild<DocumentFormat.OpenXml.Drawing.EffectList>()!.GetFirstChild<DocumentFormat.OpenXml.Drawing.Reflection>()!, out var read, allowVariablePositions: true));
