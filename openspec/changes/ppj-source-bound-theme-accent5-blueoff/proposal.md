...
modulation transforms. A direct imported accent5 `a:blueOff` remains opaque,
...
- Project a canonical shared ThemePart whose accent5 direct RGB leaf contains
  ...
  `design.theme.accentTransforms.accent5.blueOff`, using a -1..1 fraction.
- Issue `setThemeAccent5BlueOff` for that field and compile one authorized
...
- `presentation-theme-accent5-blueoff`: Strict source-bound projection and
  source-preserving editing of one direct accent5 `a:blueOff` leaf.
