...
This change applies the same ownership boundary to accent4 blue-channel offset
...
- Accent4 transforms other than `blueOff`, transforms on other accent roles, inherited or effect theme state, or arbitrary theme XML editing.
...
1. **Use the existing native transform field.**
  ...
  the DrawingML -100000..100000 offset integer and the authored compiler
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:blueOff`. The source writer patches `Accent4Color` only when the authored
   transform is the one existing accent4 blueOff and rejects all other transform
   flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent4BlueOff` is accepted only with
   `accentTransforms.accent4.blueOff` in the source-issued capability. This
