...
Use this style when the brief matches **equity research; finance presentation**. It is designed for
investors, finance leaders, analysts. Its visual premise is Lake-blue evidence and violet orientation create a browseable research journal. The style is a
...
**stance → drivers → evidence → scenarios → risks → decision or allocation**. This style is not a good fit for **stage launch**;
...
- Use **#FAFCFD** as the main field, **#173D5C** for primary type, **#70508F** for the decisive signal, **#4DA8D3** for secondary evidence, and **#DCE8EF** for rules or context.
...
- The recurring structure is **journal, variable page silhouettes, native data graphics**. Vary the silhouette between
  cover, evidence, data, process, comparison, and decision pages rather than
  repeating a component grid.
- Use editorial margins, section marks, and a browseable page rhythm. Let data retain its own axes and labels instead of becoming a decorative magazine spread.
...
- **Page skeleton.** Treat the deck as a browseable research journal: pale lake-blue field, editorial margins, a small orientation mark, and pages that can switch between a thesis spread and a data spread without losing the reading path.
- **Type rhythm.** Use an approachable sans for body and charts with one violet orientation accent. Make headlines feel like article claims, not section labels; keep source and basis quiet but present.
- **Evidence carriers.** Use time series, composition bars, benchmark tables, research notes, and short explanatory callouts. A chart belongs beside the question it answers.
- **Images and layers.** A water, place, or operating image can establish context on an opener. Keep imagery rare on evidence pages and preserve an editable foreground if it is used as a field.
- **Pacing.** Alternate a concise journal thesis with a denser evidence page, then return to a short implication. Violet marks navigation or focus, not every module.
- **Selection rule.** Choose this style for equity research, market journals, and recurring analytical updates with a magazine-like browse path.

## Signature moves and variation limits

- Let **journal** recur as a quiet identity cue on selected pages; it must
