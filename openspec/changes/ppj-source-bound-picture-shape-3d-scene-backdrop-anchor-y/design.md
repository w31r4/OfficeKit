...
- Expose one canonical signed `a:backdrop/a:anchor/@y` coordinate in EMU.
- Replace only `backdrop/a:anchor/@y` in the owning SlidePart.
- Preserve the other backdrop coordinates, camera/light-rig state, picture
...
- Reuse `TryReadSceneBackdropAnchorY` and its requested-value validator so
...
- Token-splice only the direct `backdrop/a:anchor/@y` attribute.

# Risks / Trade-offs

- [Risk] An anchor Y token without a complete backdrop owner could be mistaken
