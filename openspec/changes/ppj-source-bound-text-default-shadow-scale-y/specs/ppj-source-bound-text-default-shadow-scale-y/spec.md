...
vertical scale in PPJ while preserving the source effect graph and package.

## ADDED Requirements

### Requirement: project paragraph default-shadow vertical scale

The PPJ projector MUST expose `text.paragraphs[].style.defaultText.shadow.scaleY`
...
an existing bounded `sy` attribute.
...
- **WHEN** an imported text shape has a direct paragraph default `a:outerShdw`
  with a valid `sy` value
- **THEN** the element reference contains the default-text style capability and
  a leaf whose native value is the source `sy` token and whose PPJ value is the
  corresponding scale ratio

### Requirement: edit only the default-shadow vertical scale token
...
only the owning slide's direct `a:outerShdw/@sy` attribute.

#### Scenario: scale-only edit round-trips

- **WHEN** a projected paragraph default shadow changes `scaleY` while its
...
- **THEN** compilation succeeds, only the owning SlidePart changes, package XML
  remains valid, and a fresh projection reports the new scale and all preserved
  sibling state

### Requirement: reject unsafe default-shadow scale edits

The compiler MUST fail closed for missing or malformed `sy`, values outside the
...
- **WHEN** the source shadow has duplicate effect lists, unknown descendants,
  or an invalid `sy` token and a request attempts to replace its scale
