...
{
    [Fact]
    public void PpjSourceBoundThemeAccent6TintEditsOnlyTintAndReprojects()
    {
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            {
                SourceUri = "deck.assets/source/theme-accent6-tint.pptx",
            },
        });
        ...
        Assert.Equal(0.25, projectedTransform["tint"]!.GetValue<double>());
        var nativeRef = Assert.IsType<JsonObject>(projectedTheme["nativeRef"]);
        Assert.Contains(nativeRef["capabilities"]!.AsArray(), capability =>
            capability!["operation"]!.GetValue<string>() == "setThemeAccent6Tint" &&
            capability["fields"]!.AsArray().Any(field => field!.GetValue<string>() == "accentTransforms.accent6.tint"));
        ...
        editedProgram["design"]!["theme"]!["accentTransforms"]!["accent6"]!["tint"] = 0.7;
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            Assert.Equal(70_000, Assert.Single(accent6.Elements<A.Tint>()).Val!.Value);
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/theme-accent6-tint.pptx",
            },
        });
        Assert.True(reprojected.Ok, Diagnostics(reprojected));
        var reprojectedProgram = JsonNode.Parse(reprojected.PresentationProgram.ProgramJson.ToByteArray())!.AsObject();
        Assert.Equal(0.7, reprojectedProgram["design"]!["theme"]!["accentTransforms"]!["accent6"]!["tint"]!.GetValue<double>());

        var unauthorizedProgram = JsonNode.Parse(projectedProgram.ToJsonString())!.AsObject();
        unauthorizedProgram["design"]!["theme"]!["accentTransforms"]!["accent6"]!["tint"] = 0.8;
        unauthorizedProgram["design"]!["theme"]!["nativeRef"]!["capabilities"]!.AsArray()
            .Single(capability => capability!["operation"]!.GetValue<string>() == "setThemeAccent6Tint")!["fields"] = new JsonArray("accentTransforms.accent6.shade");
        ...
        deletionProgram["design"]!["theme"]!["accentTransforms"]!["accent6"]!.AsObject().Remove("tint");
        ...
        outOfRangeProgram["design"]!["theme"]!["accentTransforms"]!["accent6"]!["tint"] = 1.01;
