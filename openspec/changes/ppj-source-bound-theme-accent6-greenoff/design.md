...
the same ownership boundary to accent6 green-channel offset without
...
- Accent6 transforms other than `greenOff`, transforms on accent6 through
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:greenOff`. The source writer patches `Accent6Color` only when the authored
   transform is the one existing accent6 greenOff and rejects all other transform
   flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent6GreenOff` is accepted only with
   `accentTransforms.accent6.greenOff` in the source-issued capability. This
