...
{
    [Fact]
    public void PpjSourceBoundThemeAccent1SatModEditsOnlySatModAndReprojects()
    {
        ...
        {
            Role = "accent1",
            SaturationModulationThousandth = 35_000,
        });
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            {
                SourceUri = "deck.assets/source/theme-accent1-satMod.pptx",
            },
        });
        ...
        Assert.Equal(0.35, projectedTransform["satMod"]!.GetValue<double>());
        var nativeRef = Assert.IsType<JsonObject>(projectedTheme["nativeRef"]);
        Assert.Contains(nativeRef["capabilities"]!.AsArray(), capability =>
            capability!["operation"]!.GetValue<string>() == "setThemeAccent1SatMod" &&
            capability["fields"]!.AsArray().Any(field => field!.GetValue<string>() == "accentTransforms.accent1.satMod"));
        ...
        editedProgram["design"]!["theme"]!["accentTransforms"]!["accent1"]!["satMod"] = 0.4;
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            Assert.Equal(40_000, Assert.Single(accent1.Elements<A.SaturationModulation>()).Val!.Value);
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/theme-accent1-satMod.pptx",
            },
        });
        Assert.True(reprojected.Ok, Diagnostics(reprojected));
        var reprojectedProgram = JsonNode.Parse(reprojected.PresentationProgram.ProgramJson.ToByteArray())!.AsObject();
        Assert.Equal(0.4, reprojectedProgram["design"]!["theme"]!["accentTransforms"]!["accent1"]!["satMod"]!.GetValue<double>());

        var unauthorizedProgram = JsonNode.Parse(projectedProgram.ToJsonString())!.AsObject();
        unauthorizedProgram["design"]!["theme"]!["accentTransforms"]!["accent1"]!["satMod"] = 0.8;
        unauthorizedProgram["design"]!["theme"]!["nativeRef"]!["capabilities"]!.AsArray()
            .Single(capability => capability!["operation"]!.GetValue<string>() == "setThemeAccent1SatMod")!["fields"] = new JsonArray("accentTransforms.accent1.tint");
        ...
        deletionProgram["design"]!["theme"]!["accentTransforms"]!["accent1"]!.AsObject().Remove("satMod");
        ...
        outOfRangeProgram["design"]!["theme"]!["accentTransforms"]!["accent1"]!["satMod"] = 1.1;
