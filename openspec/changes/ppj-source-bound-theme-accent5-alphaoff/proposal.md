...
accent1 plus accent5 tint, shade, luminance modulation, luminance offset, and
alpha modulation. A direct imported accent5 `a:alphaOff` remains opaque,
...
- Project a canonical shared ThemePart whose accent5 direct RGB leaf contains
  ...
  `design.theme.accentTransforms.accent5.alphaOff`, using a -1..1 fraction.
- Issue `setThemeAccent5AlphaOff` for that field and compile one authorized
...
- `presentation-theme-accent5-alphaoff`: Strict source-bound projection and
  source-preserving editing of one direct accent5 `a:alphaOff` leaf.
