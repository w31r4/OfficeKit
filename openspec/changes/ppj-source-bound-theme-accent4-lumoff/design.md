# Design: source-bound accent4 lumOff

## Context

The existing source-preserving theme path recognizes one direct accent1
transform and the direct accent4 tint/shade leaves at a time, then patches the
existing DrawingML token. This change applies the same ownership boundary to
accent4 luminance offset without broadening the theme graph or changing
...
- Accent4 transforms other than `lumOff`, transforms on accent4 through
...
1. **Use the existing native transform field.**
   ...
   represents the DrawingML -100000..100000 integer and the authored compiler
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:lumOff`. The source writer patches `Accent4Color` only when the authored
   transform is the one existing accent4 lumOff and rejects all other
   transform flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent4LumOff` is accepted only with
   `accentTransforms.accent4.lumOff` in the source-issued capability. This
