...
{
    [Fact]
    public void PpjSourceBoundTextShadowSkewXEditsDirectRunOwnerAndReprojects()
    {
        var program = ShadowSkewXProgram(skewX: -1.25);
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            Assert.Equal(-75_000, shadow.HorizontalSkew!.Value);
        ...
        var projected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/source/text-shadow-skew-x.pptx",
            },
        });
        ...
        Assert.Equal(-1.25, shadowStyle["skewX"]!.GetValue<double>(), precision: 6);
        var skewLeaf = Assert.Single(projectedElement["nativeRef"]!["leaves"]!.AsArray(),
            leaf => leaf!["kind"]!.GetValue<string>() == "textShadowSkewX");
        Assert.Equal(-1.25, skewLeaf!["value"]!.GetValue<double>(), precision: 6);
        ...
        using (var themeStream = new MemoryStream())
        {
            ...
            var themeProjected = Invoke(new CodecRequest
            {
                ...
                PresentationProgram = new PresentationProgramRequest
                {
                    SourceUri = "deck.assets/source/text-shadow-skew-x-theme.pptx",
                },
            });
            ...
            Assert.Equal(-1.25, themeElement["text"]!["paragraphs"]![0]!["runs"]![0]!["style"]!["shadow"]!["skewX"]!.GetValue<double>(), precision: 6);
        ...
        using (var staleStream = new MemoryStream())
        {
            ...
            using (var stalePackage = PresentationDocument.Open(staleStream, true))
            {
                ...
                staleShadow.HorizontalSkew = 125_000;
            }
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/text-shadow-skew-x.pptx",
            },
        });
        ...
        Assert.Equal(1.25, reprojectedShadow["skewX"]!.GetValue<double>(), precision: 6);
        Assert.Equal(1.25, reprojectedElement["nativeRef"]!["leaves"]!.AsArray()
            .Select(leaf => leaf!.AsObject())
            .Single(leaf => leaf["kind"]!.GetValue<string>() == "textShadowSkewX")["value"]!.GetValue<double>(), precision: 6);
        ...
        invalidProgram["pages"]![0]!["elements"]![0]!["nativeRef"]!["leaves"]!.AsArray()
            .Single(leaf => leaf!["kind"]!.GetValue<string>() == "textShadowSkewX")!["value"] = 90;
    ...
    [Fact]
    public void PpjSourceBoundTextShadowSkewXLeavesStayOpaqueForUnsupportedGraphs()
    {
        var authored = CompileSkewXShadow(ShadowSkewXProgram(skewX: -1.25));
        Assert.True(authored.Ok, Diagnostics(authored));
        var source = RemoveEmbeddedPpj(authored.File.ToByteArray());
        ...
        foreach (var (label, mutate) in new[]
        {
            ...
            ("missing-skew", (Action<A.OuterShadow>)(shadow => shadow.HorizontalSkew = null)),
            ("invalid-skew", (Action<A.OuterShadow>)(shadow => shadow.SetAttribute(new OpenXmlAttribute("kx", "", "invalid")))),
        ...
        })
        {
            ...
            var projected = Invoke(new CodecRequest
            {
                ...
                PresentationProgram = new PresentationProgramRequest
                {
                    SourceUri = $"deck.assets/source/text-shadow-skew-x-{label}.pptx",
                },
            });
            if (label == "invalid-skew")
            {
        ...
        var siblingAuthored = CompileSkewXShadow(ShadowSkewXProgram(skewX: -1.25, siblingGlow: true));
        ...
        var siblingProjected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/source/text-shadow-skew-x-sibling.pptx",
            },
        });
    ...
    private static JsonObject ShadowSkewXProgram(double skewX, bool siblingGlow = false)
    {
    ...
    private static CodecResponse CompileSkewXShadow(JsonObject program) => Invoke(new CodecRequest
    {
        ProtocolVersion = CodecProtocol.ProtocolVersion,
        Operation = CodecOperation.CompilePpjToPptx,
        Family = ArtifactFamily.Presentation,
