...
This change applies the same ownership boundary to accent6 tint without
...
- Accent6 transforms other than `tint`, all accent6 transforms, inherited or
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:tint`. The source writer patches `Accent6Color` only when the authored
   transform is the one existing accent6 tint and rejects all other transform
   flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent6Tint` is accepted only with
   `accentTransforms.accent6.tint` in the source-issued capability. This keeps
