...
{
    [Fact]
    public void PpjSourceBoundPictureShape3dSceneCameraPresetLeafEditsAndReprojects()
    {
        ...
        var source = AddPictureShape3dSceneCameraPreset(RemoveEmbeddedPpj(authored.File.ToByteArray()), "orthographicFront");
        var projected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/source/picture-shape-3d-scene-camera-preset.pptx",
            },
        });
        ...
        Assert.Contains("<a:scene3d><a:camera prst=\"perspectiveFront\"", editedXml, StringComparison.Ordinal);
        ...
        using (var package = PresentationDocument.Open(stream, false))
        {
            ...
            Assert.Equal("perspectiveFront", scene.GetFirstChild<A.Camera>()!
                .GetAttributes().Single(attribute => attribute.LocalName == "prst").Value);
            var lightRig = scene.GetFirstChild<A.LightRig>()!;
            Assert.Equal("threePt", lightRig.GetAttributes().Single(attribute => attribute.LocalName == "rig").Value);
            Assert.Equal("t", lightRig.GetAttributes().Single(attribute => attribute.LocalName == "dir").Value);
        ...
        var reprojected = Invoke(new CodecRequest
        {
            ...
            PresentationProgram = new PresentationProgramRequest
            {
                SourceUri = "deck.assets/edited/picture-shape-3d-scene-camera-preset.pptx",
            },
        });
    ...
    private static byte[] AddPictureShape3dSceneCameraPreset(byte[] source, string preset)
    {
