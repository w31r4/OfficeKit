...
- For a trusted imported deck, assigning `presentation.slideSize = { width, height }` is canvas-only: OfficeKit updates `p:sldSz` and removes a stale preset type while preserving every existing slide/layout/master/chart/shape coordinate. Use explicit layout edits and render review; there is no automatic reflow.
...
- Delete only through the typed facade. Imported top-level ordinary shapes, embedded pictures, canonical connectors, bounded tables, charts, and canonical recursive groups expose `deletionCapability`. Shapes/connectors/tables must be relationship-free; pictures/charts own uniquely used SlidePart relationships; groups prove every descendant native ID, relationship reference, and OPC root as one ownership graph. Export re-proves the concrete source, shared media/ChartParts survive, and raw collection mutation is rejected.
