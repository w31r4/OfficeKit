...
This change applies the same ownership boundary to accent4 shade without
...
- Accent4 transforms other than `shade`, transforms on accent4 through accent6,
...
2. **Keep the source owner strict and role-specific.**
   ...
   `a:shade`. The source writer patches `Accent4Color` only when the authored
   transform is the one existing accent4 shade and rejects all other transform
   flags.

3. **Keep capability authority field-qualified.**
   The operation `setThemeAccent4Shade` is accepted only with
   `accentTransforms.accent4.shade` in the source-issued capability. This keeps
